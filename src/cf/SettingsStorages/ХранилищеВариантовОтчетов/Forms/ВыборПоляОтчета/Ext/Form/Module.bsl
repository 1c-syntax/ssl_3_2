...
&НаКлиенте
Процедура Подключаемый_СписокПолейПриНаведении(Элемент, Строка, Поле)
	...
	ТекущиеДанные = ?(Строка = Неопределено, Список.ТекущиеДанные, ЭтотОбъект.ДоступныеПоля.НайтиПоИдентификатору(Строка));
	
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РедактированиеФормулыДоступно = РедактированиеФормулыДоступно(ТекущиеДанные);
	
	Элементы.ИзменитьФормулу.Видимость = РедактированиеФормулыДоступно;
	Элементы.УдалитьФормулу.Видимость  = РедактированиеФормулыДоступно;
...
Процедура СкрытьНеиспользуемыеКоманды()
	...
	Для Каждого Элемент Из КомандыРедактированияФормул Цикл 
		Элемент.Видимость = РедактированиеФормулыДоступно;
	КонецЦикла;
...
Процедура Инициализировать()
	...
	Команды.ИзменитьФормулу.ИспользуемаяТаблица = Элементы["ДоступныеПоля"];
	Команды.УдалитьФормулу.ИспользуемаяТаблица = Элементы["ДоступныеПоля"];
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы["ДоступныеПоля"].ОтображатьКоманднуюПанель = Истина;
		Элементы.Переместить(Элементы.ИзменитьФормулу, Элементы["ДоступныеПоля"].КоманднаяПанель);
		Элементы.Переместить(Элементы.УдалитьФормулу, Элементы["ДоступныеПоля"].КоманднаяПанель);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИнициализироватьСписокДоступныхПолей()
	...
	ПараметрыДобавленияСпискаПолей.ОбработчикиСписка.Вставить("ПриНаведении", "Подключаемый_СписокПолейПриНаведении");
