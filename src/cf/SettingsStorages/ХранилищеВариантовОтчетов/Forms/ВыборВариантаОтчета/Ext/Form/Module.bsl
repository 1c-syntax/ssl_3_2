...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.ДеревоВариантовОтчета.КоманднаяПанель);
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ДеревоВариантовОтчета.КоманднаяПанель,
			"ФормаВыбрать
			|ДеревоВариантовОтчетаОбновить
			|ДеревоВариантовОтчетаИзменитьВариант
			|ПоказыватьЛичныеВариантыОтчетовДругихАвторов");
	КонецЕсли;
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.ДеревоВариантовОтчета.ОтображатьКоманднуюПанель = Ложь;
...
&НаКлиенте
Процедура ВыполнитьОтборПоАвтору()
	...
	Для Каждого ГруппаИлиВариант Из ГруппыИлиВарианты Цикл
		...
		Иначе // Это группа.
			ГруппаИлиВариант.СкрытОтбором = Не ЕстьВключенные;
		КонецЕсли;
	КонецЦикла;
