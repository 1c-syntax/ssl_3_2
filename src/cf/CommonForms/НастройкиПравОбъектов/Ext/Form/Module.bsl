...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВозможныеПраваДляНастройки.ПоТипамСсылок.Получить(ТипСсылкиНаОбъект) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Права доступа для каждого объекта не настраиваются для объектов типа ""%1"".'"),
	...
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель,
		Элементы.ГруппыПрав.КоманднаяПанель);
	
	// Корректировка порядка команд в командной панели.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ГруппыПрав.КоманднаяПанель,
		"ФормаЗаписатьИЗакрыть
		|ФормаЗаписать
		|ФормаПеречитать");
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"ГруппыПравУдалить", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
...
Процедура ГруппыПравПриАктивизацииСтроки(Элемент)
	...
	ОбновитьДоступностьКомандСтроки(Элементы.ГруппыПрав.ТекущиеДанные);
...
&НаКлиенте
Процедура ГруппыПравПриНаведении(Элемент, Строка, Поле)
	
	ТекущиеДанные = ?(Строка = Неопределено, Элемент.ТекущиеДанные,
		Элемент.ДанныеСтроки(Строка));
	
	ОбновитьДоступностьКомандСтроки(ТекущиеДанные);
...
&НаКлиенте
Процедура ФормаПеречитать(Команда)
...
&НаКлиенте
Процедура ГруппыПравСнятьУстановкуПрава(Команда)
...
&НаКлиенте
Процедура ГруппыПравУстановитьЗапретПрава(Команда)
...
&НаКлиенте
Процедура ГруппыПравУстановитьРазрешениеПрава(Команда)
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	Для Каждого ОписаниеПрава Из ОписанияВозможныхПрав Цикл
		...
		Иначе
			...
			Элемент.АвтоШиринаВТаблице            = АвтоШиринаВТаблице.Нет;
	...
	Если ЗначениеЗаполнено(ТекущийВладелецНастройки)
	   И ТекущийВладелецНастройки <> Параметры.СсылкаНаОбъект Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Эти права унаследованы, их можно изменить в форме настройки прав вышестоящей папки ""%1"".'"),
		...
		Если ПроверкаУдаления Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для удаления всех унаследованных прав снимите флажок ""%1"".'"),
...
&НаКлиенте
Процедура ОбновитьДоступностьКомандСтроки(ТекущиеДанные)
	...
	Элементы.ГруппыПравУдалить.Доступность                = ДоступностьКоманд;
	Элементы.ГруппыПравПереместитьВверх.Доступность       = ДоступностьКоманд;
	Элементы.ГруппыПравПереместитьВниз.Доступность        = ДоступностьКоманд;
