...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВывестиПредупреждениеНастройкаВСервисе();
		...
		Элементы.ПоказыватьВСпискеВыбора.Подсказка =
			НСтр("ru = 'При входе в приложение в сервисе список пользователей отключен для безопасности.'");
	...
	ИначеЕсли ИспользоватьВнешнихПользователей Тогда
		Элементы.ПоказыватьВСпискеВыбора.Доступность = Ложь;
		Элементы.ПоказыватьВСпискеВыбора.Подсказка =
			НСтр("ru = 'Разрешен доступ внешним пользователям, поэтому при входе в приложение список пользователей отключен для безопасности.'");
	КонецЕсли;
	...
	Элементы.МинимальнаяДлинаПароля.РастягиватьПоГоризонтали = Истина;
	Элементы.МаксимальныйСрокДействияПароля.РастягиватьПоГоризонтали = Истина;
	Элементы.МинимальныйСрокДействияПароля.РастягиватьПоГоризонтали = Истина;
	Элементы.ЗапретитьПовторениеПароляСредиПоследних.РастягиватьПоГоризонтали = Истина;
	Элементы.ПредупреждатьОбОкончанииСрокаДействияПароля.РастягиватьПоГоризонтали = Истина;
	Элементы.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода.РастягиватьПоГоризонтали = Истина;
	
	Элементы.МинимальнаяДлинаПароля2.РастягиватьПоГоризонтали = Истина;
	Элементы.МаксимальныйСрокДействияПароля2.РастягиватьПоГоризонтали = Истина;
	Элементы.МинимальныйСрокДействияПароля2.РастягиватьПоГоризонтали = Истина;
	Элементы.ЗапретитьПовторениеПароляСредиПоследних2.РастягиватьПоГоризонтали = Истина;
	Элементы.ПредупреждатьОбОкончанииСрокаДействияПароля2.РастягиватьПоГоризонтали = Истина;
	Элементы.ПросрочкаРаботыВПрограммеДоЗапрещенияВхода2.РастягиватьПоГоризонтали = Истина;
	
	Элементы.ПредупреждатьОбОкончанииСрокаРаботыВПрограмме.РастягиватьПоГоризонтали = Истина;
	Элементы.СрокБездействияДоЗавершенияСеанса.РастягиватьПоГоризонтали = Истина;
	Элементы.ПредупреждатьОЗавершенииСеансаПриБездействии.РастягиватьПоГоризонтали = Истина;
	Элементы.ВремяСохраненияПароля.РастягиватьПоГоризонтали = Истина;
	Элементы.КоличествоПопытокВводаПароляДоБлокировки.РастягиватьПоГоризонтали = Истина;
	Элементы.ДлительностьБлокировкиВводаПароля.РастягиватьПоГоризонтали = Истина;
	
	Элементы.МаксимальноеВремяОжиданияСервисаЗапрещенныхПаролей.РастягиватьПоГоризонтали = Истина;
	...
	Если ВыбранноеЗначение = "СкрытоИВключеноДляВсехПользователей" Тогда
		ТекстВопроса =
			НСтр("ru = 'При входе в приложение список выбора пользователей станет полным
			           |(реквизит ""Показывать в списке выбора"" в карточках всех пользователей будет включен и скрыт).'");
	Иначе
		ТекстВопроса =
			НСтр("ru = 'При входе в приложение список выбора пользователей станет пустым
			           |(реквизит ""Показывать в списке выбора"" в карточках всех пользователей будет очищен и скрыт).'");
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ПредлагаемыеЗначенияНастроек Цикл
		...
		Если ЗначениеЗаполнено(Настройки[КлючИЗначение.Ключ]) Тогда
			...
			Если ПредлагаемоеЗначение.Включить Тогда
				ЭтотОбъект[Имена.ИмяЭлементаВключить] = Не ЗначениеЗаполнено(ПредлагаемоеЗначение.Зависимость)
					Или ЗначениеЗаполнено(Настройки[ПредлагаемоеЗначение.Зависимость]);
			КонецЕсли;
		Иначе
			ЭтотОбъект[Имена.ИмяНастройкиВФорме] = ПредлагаемоеЗначение.Значение;
		КонецЕсли;
		ВедущаяНастройка = ?(ЗначениеЗаполнено(ПредлагаемоеЗначение.Зависимость),
			ПредлагаемоеЗначение.Зависимость, КлючИЗначение.Ключ);
		Элементы[Имена.ИмяЭлемента].Доступность = ЗначениеЗаполнено(Настройки[ВедущаяНастройка]);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ВывестиПредупреждениеНастройкаВСервисе()
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Настройки недоступны для изменения'");
	ПараметрыСообщения.Заголовок = ПараметрыСообщения.КраткоеПредставление;
	ПараметрыСообщения.Сообщение = НСтр("ru = 'Для приложений в сервисе настройки устанавливаются администратором сервиса в менеджере сервиса.'");
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект,
		ПараметрыСообщения, Элементы.ПредупреждениеНастройкаВСервисе);
...
Процедура УстановитьНовыйДополнительныйСписокЗапрещенныхПаролей()
	...
	Если Не ЗначениеЗаполнено(АдресНовогоСпискаПаролейВоВременномХранилище) Тогда
		Возврат;
	КонецЕсли;
