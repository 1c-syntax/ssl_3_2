...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ФормаЗаписатьИЗакрыть, Элементы.КомандыПечати.КоманднаяПанель, Элементы.ФормаПоказатьВСписке);
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.КомандыПечати.КоманднаяПанель);
	КонецЕсли;
...
&НаКлиенте
Процедура СохранитьИПерейтиКСписку()
