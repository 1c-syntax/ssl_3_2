...
Процедура ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных(Команда)
	...
	СтрокиДляОбработки = Элементы.СписокСостоянияУзлов.ПолучитьСтрокиДляОбработки();
	
	Если Не СтрокиДляОбработки.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = СписокСостоянияУзлов.НайтиПоИдентификатору(СтрокиДляОбработки[0]);
