...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОтчетТабличныйДокумент.РежимОтображенияПанелиМножественногоВыделения =
		РежимОтображенияПанелиМножественногоВыделенияТабличногоДокумента.НеОтображать;
	...
	Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
		Элементы.ПараметрыСтраницы.Видимость = Ложь;
	...
	Если Не НастройкиОтчета.ВыводитьСуммуВыделенныхЯчеек Или НастройкиОтчета.ОтключитьСтандартноеКонтекстноеМеню Тогда
		...
		Элементы.КоманднаяПанельЕще.РастягиватьПоГоризонтали = Истина;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		ТекстСостоянияОтчета = НСтр("ru = 'Отчет не сформирован'");
	Иначе
		ТекстСостоянияОтчета = НСтр("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.'");
	КонецЕсли;
	
	ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(
		ЭтотОбъект, ТекстСостоянияОтчета, , ВариантОтображенияСостояния.НачальноеСостояние);
...
Процедура ДругиеОтчеты(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ПанельДругихОтчетов", ПараметрыФормы, ЭтотОбъект,
		Истина,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно, ПослеПредупреждения)
	...
	Иначе
		...
		ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
		ОтображениеСостояния.ИмяКоманды = "";
	КонецЕсли;
	Возврат Результат;
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.ИзменитьСоставБыстрыхНастроек.Видимость = (КоличествоДоступныхНастроек.БыстрогоДоступа > 0);
...
Процедура ЗагрузитьВариант(КлючВарианта, ОчиститьСтекНастроек = Истина)
	...
	ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(ЭтотОбъект, 
		НСтр("ru = 'Выбран другой вариант отчета.'"),
...
Процедура НастроитьРаботуВТакси()
	Элементы.ПанельБыстрыхНастроек.ЦветФона = ЦветаСтиля.ЦветФонаПодсказки;
	Элементы.Переместить(Элементы.ИзменитьСоставБыстрыхНастроек, Элементы.ПанельБыстрыхНастроек);
	
	Элементы.ОсновнаяКоманднаяПанель.РастягиватьПоГоризонтали = Ложь;
	Элементы.Переместить(Элементы.ГруппаНастройкиОтчета, Элементы.ОсновнаяКоманднаяПанель, Элементы.ГруппаГлавное);
	Элементы.Переместить(Элементы.КоманднаяПанельЕще, Элементы.КомандыИПоказатели);
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.КоманднаяПанельЕще);
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.КоманднаяПанельЕще,
		"ГруппаГлавноеЕще
		|ГруппаНастройкиОтчетаЕще
		|ГруппаПрограммныйИнтерфейс
		|ГруппаРаботаВТаблицеЕще
		|ГруппаВыводЕще
		|ГруппаСнимкиОтчетов
		|Отчеты
		|НовоеОкно");
	Элементы.СформироватьОтчетЕщеТакси.Видимость = Истина;
	Элементы.ВсеНастройкиЕщеТакси.Видимость = Истина;
	Элементы.ИзменитьСоставБыстрыхНастроекЕщеТакси.Видимость = Истина;
	Элементы.ПечатьЕщеТакси.Видимость = Истина;
	Элементы.ПараметрыСтраницыТакси.Видимость = Истина;
	Элементы.СохранитьОтчетЕщеТакси.Видимость = Истина;
	Элементы.ОтправитьПоПочтеЕщеТакси.Видимость = Истина;
	Элементы.НайтиЕщеТакси.Видимость = Истина;
КонецПроцедуры
