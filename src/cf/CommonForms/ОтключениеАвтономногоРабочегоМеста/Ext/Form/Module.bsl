...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.АвтономноеРабочееМесто) Тогда
		
		ВызватьИсключение НСтр("ru = 'Не задано автономное рабочее место.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьОсновнойСценарий()
	...
	КомандаПрекратитьСинхронизациюДанных = Элементы.ФормаПрекратитьСинхронизациюДанных.Имя;
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("Начало", КомандаПрекратитьСинхронизациюДанных);
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("Ожидание");
	...
	НовыйПереход = ТаблицаПереходовНоваяСтрока("Завершение");
