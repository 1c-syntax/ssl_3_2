...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		ПараметрыСообщения.Идентификатор = "НастройкаВыполненаВЦентральномУзле";
		ПараметрыСообщения.Сообщение  = НСтр("ru = 'Настройка приложений электронной подписи и шифрования выполнена в главном узле.'");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Настройка выполнена в главном узле'");
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщенияПриложенияИТокены);
	...
	СертификатыТолькоДействующие = "Все";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		ПараметрыРазмещения.КоманднаяПанель = Элементы.СертификатыКоманднаяПанель;
...
&НаКлиенте
Процедура СертификатыПриНаведении(Элемент, Строка, Поле)
	// СтандартныеПодсистемы.ПодключаемыеКоманды
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		МодульПодключаемыеКомандыКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодключаемыеКомандыКлиент");
...
Процедура СертификатыОбновитьОтбор(Форма, СостоянияЗаявленияНеВРаботе, ТекущийПользователь = Неопределено)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Форма.Сертификаты,
		"Действует", Истина, , , ОтборПоДействующим И Форма.СертификатыТолькоДействующие = "Действующие");
...
#Если ВебКлиент Тогда
		...
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			...
			ОбработчикПродолжения = Новый ОписаниеОповещения(
				"ОпределитьУстановленныеПрограммыПослеПодключенияРасширения", ЭтотОбъект, Контекст);
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы, ,
				УникальныйИдентификатор, , , ОбработчикПродолжения,
				РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
			Возврат;
		КонецЕсли;
#Иначе
		КраткоеПредставление = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Не установлено расширение'"),
				ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
		Сообщение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			...
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
		ПоказатьВажноеСообщениеУстановкиКриптопровайдера(КраткоеПредставление, Сообщение, Ложь);
...
&НаСервере
Процедура ПоказатьВажноеСообщениеУстановкиКриптопровайдера(КраткоеПредставление, Сообщение, Скрываемый = Истина)
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Информация";
	ПараметрыСообщения.Идентификатор = ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии();
	ПараметрыСообщения.Сообщение = Сообщение;
	ПараметрыСообщения.КраткоеПредставление = КраткоеПредставление;
	ПараметрыСообщения.Скрываемый = Скрываемый;
	
	// Не скрываются сообщения с навигационной ссылкой для выполнения действия.
	Если Не Скрываемый Тогда
		ПараметрыСообщения.Обработчик = "ПроверитьУстановкуПриложенийКриптографии";
	КонецЕсли;
...
#Иначе
		КраткоеПредставление = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Проверка не выполнена'"),
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
		Сообщение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Нажмите здесь</a>, чтобы увидеть все приложения для работы с электронной подписью.'"),
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
		ПоказатьВажноеСообщениеУстановкиКриптопровайдера(КраткоеПредставление, Сообщение, Ложь);
#КонецЕсли

	ИначеЕсли Результат.ВозможенКонфликт Тогда
		ПараметрыСообщения = Новый Структура("КраткоеПредставление, Сообщение");
		ЭлектроннаяПодписьКлиентЛокализация.ПриПроверкеКонфликтаПрограммКриптографии(ЭтотОбъект, Результат, ПараметрыСообщения);
		Если ЗначениеЗаполнено(ПараметрыСообщения.Сообщение) Тогда
			ПоказатьВажноеСообщениеУстановкиКриптопровайдера(ПараметрыСообщения.КраткоеПредставление, ПараметрыСообщения.Сообщение);
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
	КонецЕсли;
	Если Результат.ПроверкаВыполнена Тогда
		...
		ПараметрыСообщения = Новый Структура("КраткоеПредставление, Сообщение");
		ЭлектроннаяПодписьКлиентЛокализация.ПриПроверкеУстановленныхПрограммКриптографии(ЭтотОбъект,
			Результат, ЕстьПроверяемыеПрограммы, ПараметрыСообщения);
		Если ЗначениеЗаполнено(ПараметрыСообщения.Сообщение) Тогда
			ПоказатьВажноеСообщениеУстановкиКриптопровайдера(ПараметрыСообщения.КраткоеПредставление,
				ПараметрыСообщения.Сообщение, Ложь);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьУстановкуПриложенийКриптографии(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
