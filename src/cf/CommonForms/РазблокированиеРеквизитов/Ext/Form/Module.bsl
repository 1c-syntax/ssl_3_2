...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Команды.Видимость = ДобавляемыеЭлементы.Количество() > 5;
	...
	ДобавитьВажноеСообщениеНаФорму();
...
Процедура ПроверитьИспользование(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ПроверитьИспользованиеЗавершение", ЭтотОбъект);
...
Процедура ДобавитьРеквизитыНаФорму(ДобавляемыеЭлементы)
	...
	Для Каждого БлокируемыйРеквизит Из БлокируемыеРеквизиты Цикл
		...
		Если Не ОбщаяНадписьНастроена Тогда
			...
			Иначе
				ЗаголовокНадписи = ЗаголовокНадписиПоУмолчанию();
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьЭлементыНаФорму(ДобавляемыеЭлементы)
	...
	Для Каждого ДобавляемыйЭлемент Из ДобавляемыеЭлементы Цикл
		...
		СОтступом = ЭлементФлажка <> Неопределено И ЗначениеЗаполнено(ЭлементФлажка.РасширеннаяПодсказка.Заголовок);
...
//
&НаКлиенте
Процедура ПроверитьИспользованиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	ПоказатьКоманды = Элементы.Страницы.ТекущаяСтраница = Элементы.Главное;
	Элементы.ФормаРазрешитьРедактирование.Видимость = ПоказатьКоманды;
	Если ПоказатьКоманды Тогда
		ДобавитьВажноеСообщениеНаФорму();
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ТекстСообщенияРазрешениеРедактирования");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьВажноеСообщениеНаФорму()
	
	Если ПустаяСтрока(ЗаголовокНадписи) Тогда
		ЗаголовокНадписи = ЗаголовокНадписиПоУмолчанию();
	КонецЕсли;
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.Сообщение = ЗаголовокНадписи;
	ПараметрыСообщения.Идентификатор = "ТекстСообщенияРазрешениеРедактирования";
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Рекомендуется проверить использование объекта'");
	ПараметрыСообщения.Скрываемый = Ложь;
	
	Если ЗначениеЗаполнено(АдресСсылокНаОбъекты) Тогда
		ПараметрыСообщения.ПараметрыДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
		ПараметрыСообщения.ПараметрыДействия.ИмяКоманды = "ПроверитьИспользование";
		ПараметрыСообщения.ПараметрыДействия.Заголовок = НСтр("ru = 'Проверить использование'");
		ПараметрыСообщения.ПараметрыДействия.Обработчик = "ПроверитьИспользование";
	КонецЕсли;
...
Функция ЗаголовокНадписиПоУмолчанию()
	
	Возврат НСтр("ru = 'Перед разрешением редактирования рекомендуется проверить использование объекта.
