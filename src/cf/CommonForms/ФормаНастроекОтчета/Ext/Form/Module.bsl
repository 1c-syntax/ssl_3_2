...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЦентрМониторинга") Тогда 
		...
		ПредставлениеТекущегоРежима = ?(РасширенныйРежим=1, НСтр("ru = 'Расширенный'"), НСтр("ru = 'Простой'"));
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 режим'"), ПредставлениеТекущегоРежима);
...
Процедура ПриОткрытии(Отказ)
	УстановитьПометкиНаКнопкиРежимов();
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_ПерейтиКРасширеннойНастройкеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки <> "ПерейтиКРасширеннойНастройке" Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПодсказкаЕстьВложенныеОтчетыОбработкаНавигационнойСсылки(Элемент, Адрес, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	Если Адрес <> "ПерейтиКНастройкамВложенныхОтчетов" Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_Список_ВставитьИзБуфераКартинка(Элемент)
	
	ИмяКоманды = СтрЗаменить(Элемент.Имя, "Картинка", "");
	...
	Подключаемый_Список_ВставитьИзБуфера(Команда);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылка = "ПоказатьПояснениеЕстьНесуществующиеПоля" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(, Элементы.ГруппаЕстьНесуществующиеПоля.РасширеннаяПодсказка.Заголовок);
	КонецЕсли;
...
&НаКлиенте
Процедура ВыбранныеПоля_ДобавитьСортировку(Команда)
	ИдентификаторыСтрок = Элементы.ВыбранныеПоля.ПолучитьСтрокиДляОбработки();
...
&НаКлиенте
Процедура Сортировка_УдалитьИзСортировки(Команда)
	ИдентификаторыСтрок = Элементы.Сортировка.ПолучитьСтрокиДляОбработки();
	Если ИдентификаторыСтрок = Неопределено Тогда 
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыбранныеПоля_Сортировка_ВлевоВсеТакси(Команда)
...
&НаКлиенте
Процедура ВыбранныеПоляПриНаведении(Элемент, Строка, Поле)
	
	ТекущиеДанные = ?(Строка = Неопределено, Элементы.ВыбранныеПоля.ТекущиеДанные,
		ВыбранныеПоля.НайтиПоИдентификатору(Строка));
		
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекущиеДанные.ЭтоФормула Тогда
		Элементы.ВыбранныеПоляИзменитьФормулу.Видимость = Истина;
	Иначе
		Элементы.ВыбранныеПоляИзменитьФормулу.Видимость = Ложь;
	КонецЕсли;
...
Процедура Отборы_Разгруппировать(Команда)
	ИдентификаторыСтрок = Элементы.Отборы.ПолучитьСтрокиДляОбработки();
	Если ИдентификаторыСтрок.Количество() <> 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите одну группу.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтборыПриНаведении(Элемент, Строка, Поле)
	
	ТекущиеДанные = ?(Строка = Неопределено, Элементы.Отборы.ТекущиеДанные, Элемент.ДанныеСтроки(Строка));
	
	Если Элементы.Отборы.ВыделенныеСтроки.Количество() > 0 Тогда
		СтрокиДляОбработки = Элементы.Отборы.ПолучитьСтрокиДляОбработки();
		Если СтрокиДляОбработки.Количество() = 1 Тогда
			ТекущиеДанные = Элемент.ДанныеСтроки(СтрокиДляОбработки[0]);
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	УстановитьПараметрыРедактированияЭлементаОтбора(ТекущиеДанные);
	ОтборыПриИзмененииТекущейСтроки(ТекущиеДанные);
...
&НаКлиенте
Процедура СоставГруппировки_ВключитьВсе(Команда)
	ИзменитьИспользование("СоставГруппировки", Истина);
КонецПроцедуры  

&НаКлиенте
Процедура СоставГруппировки_ВыключитьВсе(Команда)
	ИзменитьИспользование("СоставГруппировки", Ложь);
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_ОчиститьНастройкиОтНесуществующихПолей(Команда)
...
&НаКлиенте
Процедура ВключитьПростойРежим(Команда)
	РасширенныйРежим = 0;
	УстановитьПометкиНаКнопкиРежимов();
	ПриИзмененииРежимаНастроек();
КонецПроцедуры
...
&НаСервере
Процедура ВставитьВажныеСообщения()

	// Есть вложенные фильтры
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	ПараметрыВажногоСообщения.Идентификатор = "ЕстьВложенныеОтборы";
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'На странице <b>Структура</b> для группировок отчета установлены фильтры, которые имеют больший приоритет.'"));
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Есть вложенные фильтры'");
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаЕстьВложенныеОтборы);
	
	// Есть вложенные поля или сортировки
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	ПараметрыВажногоСообщения.Идентификатор = "ЕстьВложенныеПоляИлиСортировки";
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'На странице <b>Структура</b> для группировок отчета выбраны поля и сортировки, которые имеют больший приоритет.'"));
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Есть вложенные поля или сортировки'");
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаЕстьВложенныеПоляИлиСортировки);
	
	// Есть вложенное оформление
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	ПараметрыВажногоСообщения.Идентификатор = "ЕстьВложенноеОформление";
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'На странице <b>Структура</b> для группировок отчета задано оформление, которое имеет больший приоритет.'"));
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Есть вложенное оформление'");
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаЕстьВложенноеОформление);
	
	// Есть вложенные отчеты
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	ПараметрыВажногоСообщения.Идентификатор = "ЕстьВложенныеОтчеты";
	Если ЭтоМобильныйКлиент Тогда
		ПараметрыВажногоСообщения.Сообщение = НСтр("ru='Расширенный режим настройки недоступен в мобильном клиенте'");
	Иначе
		ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'У этого отчета сложная структура настроек (имеются вложенные отчеты). <a href=%1>Перейти к настройке вложенных отчетов</a>.'"),"ПерейтиКНастройкамВложенныхОтчетов");
	КонецЕсли;
	ПараметрыВажногоСообщения.Обработчик = "Подключаемый_ПодсказкаЕстьВложенныеОтчетыОбработкаНавигационнойСсылки";
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Есть вложенные отчеты'");
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаЕстьВложенныеОтчеты);
	
	// Есть несуществующие поля
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Предупреждение";
	ПараметрыВажногоСообщения.Идентификатор = "ЕстьНесуществующиеПоля";
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'В настройках используются несуществующие поля. <a href=%1>Подробнее</a>'"),
			"ПоказатьПояснениеЕстьНесуществующиеПоля");
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Есть несуществующие поля'");
	ПараметрыВажногоСообщения.Обработчик = "Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки";
	ПараметрыДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
	ПараметрыДействия.ИмяКоманды = "ОчиститьНастройкиОтНесуществующихПолей";
	ПараметрыДействия.Обработчик = "Подключаемый_ОчиститьНастройкиОтНесуществующихПолей";
	ПараметрыДействия.Заголовок = НСтр("ru = 'Очистить'");
	ПараметрыВажногоСообщения.ПараметрыДействия = ПараметрыДействия;
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаЕстьНесуществующиеПоля);
	
	// Нет пользовательских настроек
	ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	ПараметрыВажногоСообщения.Идентификатор = "НетПользовательскихНастроек";
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='У этого отчета нет ни одной пользовательской настройки.
		|<a href=%1>Перейти к расширенной настройке</a>'"), "ПерейтиКРасширеннойНастройке");
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Нет пользовательских настроек'");
	ПараметрыВажногоСообщения.Обработчик = "Подключаемый_ПерейтиКРасширеннойНастройкеОбработкаНавигационнойСсылки";
	ПараметрыВажногоСообщения.Скрываемый = Ложь;
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.СтраницаНетПользовательскихНастроек);
...
Процедура СдвинутьПолеГруппировки(ВНачало = Истина)
	ИдентификаторыСтрок = Элементы.СоставГруппировки.ПолучитьСтрокиДляОбработки();
...
&НаКлиенте
Процедура УстановитьПараметрыРедактированияЭлементаОтбора(Строка = Неопределено)
...
&НаКлиенте
Процедура ОтборыПриИзмененииТекущейСтроки(Строка)
	...
	Иначе
		...
		Элементы.Отборы_Разгруппировать.Видимость = Строка.ЭтоГруппа;
		Элементы.Отборы_РазгруппироватьТакси.Видимость = Строка.ЭтоГруппа;
		Элементы.Отборы_ТипГруппы.Видимость = Строка.ЭтоГруппа;
		Элементы.Отборы_ТипГруппыТакси.Видимость = Строка.ЭтоГруппа;
...
Процедура УстановитьДоступностьКомандОтборов(ЭтоОтбор = Ложь, ЭтоРаздел = Ложь)
	...
	Элементы.Отборы_УдалитьТакси.Доступность = ЭтоОтбор;
	Элементы.Отборы_Сгруппировать.Доступность = ЭтоОтбор;
	Элементы.Отборы_СгруппироватьТакси.Доступность = ЭтоОтбор;
	Элементы.Отборы_Разгруппировать.Доступность = ЭтоОтбор;
	Элементы.Отборы_РазгруппироватьТакси.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПереместитьВверх.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПереместитьВверхТакси.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПереместитьВниз.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПереместитьВнизТакси.Доступность = ЭтоОтбор;
	
	Элементы.Отборы_ПоказыватьВШапкеОтчета.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьТолькоФлажокВШапкеОтчета.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьВНастройкахОтчета.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьТолькоФлажокВНастройкахОтчета.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_НеПоказывать.Доступность = Не ЭтоРаздел;
	
	Элементы.Отборы_ПоказыватьВШапкеОтчетаТакси.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьТолькоФлажокВШапкеОтчетаТакси.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьВНастройкахОтчетаТакси.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_ПоказыватьТолькоФлажокВНастройкахОтчетаТакси.Доступность = Не ЭтоРаздел;
	Элементы.Отборы_НеПоказыватьТакси.Доступность = Не ЭтоРаздел;

	Элементы.Отборы_ПоказыватьТолькоФлажокВШапкеОтчета.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПоказыватьТолькоФлажокВШапкеОтчетаТакси.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПоказыватьТолькоФлажокВНастройкахОтчета.Доступность = ЭтоОтбор;
	Элементы.Отборы_ПоказыватьТолькоФлажокВНастройкахОтчетаТакси.Доступность = ЭтоОтбор;
КонецПроцедуры

&НаКлиенте
Процедура ОтборыУстановитьРежимОтображения(РежимОтображения)
	
	ИдентификаторСтрок = Элементы.Отборы.ПолучитьСтрокиДляОбработки();
...
Процедура ПослеАктивизацииВыбранныхПолей()
	...
	Элементы.ВыбранныеПоляИзменитьФормулу.Видимость = Строка <> Неопределено
		И Строка.ЭтоФормула;
КонецПроцедуры
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	Если Строка = Неопределено Тогда
		...
		ПараметрыФормы.Вставить("НомерКартинкиРежимаОтображения", 4);
	Иначе
		ПараметрыФормы.Вставить("ИдентификаторКД", Строка.Идентификатор);
		ПараметрыФормы.Вставить("Наименование", Строка.Заголовок);
		ПараметрыФормы.Вставить("НомерКартинкиРежимаОтображения", Строка.КартинкаРежимаОтображения);
	КонецЕсли;
...
Процедура ОформлениеИзменитьЭлементЗавершение(Результат, ИдентификаторСтроки) Экспорт
	...
	Строка.Картинка = -1;
	Строка.КартинкаРежимаОтображения = ?(Результат.НомерКартинкиРежимаОтображения <> 0,
	   Результат.НомерКартинкиРежимаОтображения, КартинкаРежимаОтображенияЭлементаНастройки(ЭлементНастройки));
...
Процедура СдвинутьОформление(ВНачало = Истина)
	ИдентификаторыСтрок = Элементы.Оформление.ПолучитьСтрокиДляОбработки();
	Строки = Оформление.ПолучитьЭлементы();
...
Процедура СтруктураВариантаПриИзмененииТекущейСтроки()
	...
	Элементы.КомандыСтруктурыВарианта_ДобавлениеТакси.Доступность = Элементы.КомандыСтруктурыВарианта_Добавление.Доступность;
	Если Строка <> Неопределено Тогда
...
Процедура УстановитьДоступностьКомандСтруктурыВарианта(ИдентификаторСтроки)
	...
	Элементы.СтруктураВарианта_ДобавитьТакси.Доступность = МожноДобавлятьВложенные;
	Элементы.СтруктураВарианта_Изменить.Доступность  = МожноОткрывать;
	Элементы.СтруктураВарианта_ИзменитьТакси.Доступность = МожноОткрывать;
	Элементы.СтруктураВарианта_ДобавитьТаблицу.Доступность  = МожноДобавлятьТаблицыИДиаграммы;
	Элементы.СтруктураВарианта_ДобавитьТаблицуТакси.Доступность = МожноДобавлятьТаблицыИДиаграммы;
	Элементы.СтруктураВарианта_ДобавитьДиаграмму.Доступность  = МожноДобавлятьТаблицыИДиаграммы;
	Элементы.СтруктураВарианта_ДобавитьДиаграммуТакси.Доступность = МожноДобавлятьТаблицыИДиаграммы;
	Элементы.СтруктураВарианта_Удалить.Доступность  = МожноУдалятьИПеремещать;
	Элементы.СтруктураВарианта_УдалитьТакси.Доступность = МожноУдалятьИПеремещать;
	Элементы.СтруктураВарианта_Сгруппировать.Доступность  = МожноГруппировать;
	Элементы.СтруктураВарианта_СгруппироватьТакси.Доступность = МожноГруппировать;
	...
	Элементы.СтруктураВарианта_ПереместитьВышеИЛевееТакси.Доступность = МожноУдалятьИПеремещать И РодителяМожноПеремещать И МожноДобавлятьВложенные И МожноГруппировать;
	...
	Элементы.СтруктураВарианта_ПереместитьНижеИПравееТакси.Доступность = МожноУдалятьИПеремещать И ЕстьПодчиненные И МожноДобавлятьВложенные И МожноГруппировать;
	Элементы.СтруктураВарианта_ПереместитьВверх.Доступность  = МожноУдалятьИПеремещать И ЕстьСоседние;
	Элементы.СтруктураВарианта_ПереместитьВверхТакси.Доступность = МожноУдалятьИПеремещать И ЕстьСоседние;
	Элементы.СтруктураВарианта_ПереместитьВниз.Доступность  = МожноУдалятьИПеремещать И ЕстьСоседние;
	Элементы.СтруктураВарианта_ПереместитьВнизТакси.Доступность = МожноУдалятьИПеремещать И ЕстьСоседние;
	...
	Элементы.СтруктураВарианта_ПереместитьНаУровеньВышеТакси.Доступность = МожноУдалятьИПеремещать И ЕстьРодитель И РодителяМожноПеремещать;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНиже.Доступность = МожноУдалятьИПеремещать И ЕстьПотенциальныеРодители;
	Элементы.СтруктураВарианта_ПереместитьНаУровеньНижеТакси.Доступность = МожноУдалятьИПеремещать И ЕстьПотенциальныеРодители;
КонецПроцедуры
...
Процедура УдалитьСтроки(Элемент, Отказ)
	...
	ИдентификаторыСтрок = Элемент.ПолучитьСтрокиДляОбработки();
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	ВыделенныеСтроки = МассивСортировать(ТаблицаЭлемент.ПолучитьСтрокиДляОбработки(), НаправлениеСортировки.Возр);
...
&НаКлиенте
Процедура УстановитьПометкиНаКнопкиРежимов()
	Элементы.ФормаВключитьПростойРежим.Пометка = РасширенныйРежим = 0;
	Элементы.ФормаВключитьРасширенныйРежим.Пометка = РасширенныйРежим = 1;
КонецПроцедуры
...
Функция КорневаяСтрокаПоУмолчанию(Знач ИмяКоллекции)
	...
	ИначеЕсли ИмяКоллекции = "Оформление" Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяКоллекции = "СоставГруппировки" Тогда
		КорневаяСтрока = СоставГруппировки;
	КонецЕсли;
...
#Область СвойстваЭлементовСтраницыПоляИСортировки
	...
	Элементы.ВыбранныеПоля_Добавить.Видимость = ОтображатьВыбранныеПоля И ЭтоРасширенныйРежим;
	Элементы.ВыбранныеПоляИзменитьФормулу.Видимость = ОтображатьВыбранныеПоля И ЭтоРасширенныйРежим;
	Элементы.КомандыВыбранныхПолей_ДобавлениеИзменениеТакси.Видимость = ОтображатьВыбранныеПоля И ЭтоРасширенныйРежим;
	Элементы.КомандыВыбранныхПолей_Группы.Видимость = ОтображатьВыбранныеПоля И ЭтоРасширенныйРежим;
	Элементы.КомандыВыбранныхПолей_ГруппыТакси.Видимость = ОтображатьВыбранныеПоля И ЭтоРасширенныйРежим;
	
	Элементы.Сортировка.Видимость = ОтображатьСортировки;
	Элементы.Сортировка_Добавить.Видимость = ОтображатьСортировки И ЭтоРасширенныйРежим;
	Элементы.Сортировка_Удалить.Видимость = ОтображатьСортировки И ЭтоРасширенныйРежим;
	Элементы.Сортировка_ДобавитьТакси.Видимость = ОтображатьСортировки И ЭтоРасширенныйРежим;
	Элементы.Сортировка_УдалитьТакси.Видимость = ОтображатьСортировки И ЭтоРасширенныйРежим;
...
#Область СвойстваЭлементовСтраницыСтруктураВарианта
	...
	Элементы.КомандыСтруктурыВарианта_ДобавлениеТакси.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ИзменениеТакси.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеПоИерархии.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеПоИерархииТакси.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеВнутриРодителя.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеВнутриРодителяТакси.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеВИерархии.Видимость = ЭтоРасширенныйРежим;
	Элементы.КомандыСтруктурыВарианта_ДвижениеВИерархииТакси.Видимость = ЭтоРасширенныйРежим;
...
#Область ОбщиеСвойстваЭлементов
	...
	Элементы.ФормаВключитьПростойРежим.Видимость = НастройкиОтчета.РазрешеноИзменятьВарианты И Не РежимИзмененияЭлементаСтруктурыНастроек;
	Элементы.ФормаВключитьРасширенныйРежим.Видимость = НастройкиОтчета.РазрешеноИзменятьВарианты И Не РежимИзмененияЭлементаСтруктурыНастроек;
	...
	Если ЭтоМобильныйКлиент Тогда
		Элементы.ФормаВключитьПростойРежим.Видимость = Ложь;
		Элементы.ФормаВключитьРасширенныйРежим.Видимость = Ложь;
...
Процедура НастроитьРаботуВТакси()
	Элементы.КомандыПолейИСортировокТакси.Видимость = Истина;
	Элементы.Переместить(Элементы.ВыбранныеПоля_ДобавитьСортировку, Элементы.КомандыПолейИСортировокТакси, Элементы.ВыбранныеПоля_Сортировка_ВлевоВсеТакси);
	Элементы.Переместить(Элементы.Сортировка_УдалитьИзСортировки, Элементы.КомандыПолейИСортировокТакси, Элементы.ВыбранныеПоля_Сортировка_ВлевоВсеТакси);
	
	СоставГруппировкиОсновное = Элементы.Добавить("СоставГруппировкиОсновное", Тип("ГруппаФормы"), Элементы.СоставГруппировки);
	СоставГруппировкиОсновное.Заголовок = НСтр("ru = 'Основное'");
	СоставГруппировкиОсновное.Вид = ВидГруппыФормы.ГруппаКолонок;
	СоставГруппировкиОсновное.Группировка = ГруппировкаКолонок.ВЯчейке;
	Элементы.Переместить(СоставГруппировкиОсновное, Элементы.СоставГруппировки, Элементы.СоставГруппировкиПоле);
	Элементы.Переместить(Элементы.СоставГруппировкиИспользование, СоставГруппировкиОсновное);
	Элементы.Переместить(Элементы.СоставГруппировкиПоле, СоставГруппировкиОсновное);
	
	Элементы.ОтборыГруппаИспользование.Видимость = Ложь;
	Элементы.Переместить(Элементы.ОтборыИспользование, Элементы.ГруппаОтборыПоле, Элементы.ОтборыПоле);
	
	ВыбранныеПоляГруппа = Элементы.Добавить("ВыбранныеПоляГруппа", Тип("ГруппаФормы"), Элементы.ВыбранныеПоля);
	ВыбранныеПоляГруппа.Заголовок = НСтр("ru = 'Основное'");
	ВыбранныеПоляГруппа.Вид = ВидГруппыФормы.ГруппаКолонок;
	ВыбранныеПоляГруппа.Группировка = ГруппировкаКолонок.ВЯчейке;
	Элементы.Переместить(ВыбранныеПоляГруппа, Элементы.ВыбранныеПоля, Элементы.ВыбранныеПоляПоле);
	Элементы.Переместить(Элементы.ВыбранныеПоляИспользование, ВыбранныеПоляГруппа);
	Элементы.Переместить(Элементы.ВыбранныеПоляПоле, ВыбранныеПоляГруппа);
	
	СортировкаГруппа = Элементы.Добавить("СортировкаГруппа", Тип("ГруппаФормы"), Элементы.Сортировка);
	СортировкаГруппа.Заголовок = НСтр("ru = 'Основное'");
	СортировкаГруппа.Вид = ВидГруппыФормы.ГруппаКолонок;
	СортировкаГруппа.Группировка = ГруппировкаКолонок.ВЯчейке;
	Элементы.Переместить(СортировкаГруппа, Элементы.Сортировка, Элементы.СортировкаПоле);
	Элементы.Переместить(Элементы.СортировкаИспользование, СортировкаГруппа);
	Элементы.Переместить(Элементы.СортировкаПоле, СортировкаГруппа);
	
	ЭлементОформления = Элементы.Добавить("ЭлементОформления", Тип("ГруппаФормы"), Элементы.Оформление);
	ЭлементОформления.Заголовок = НСтр("ru = 'Основное'");
	ЭлементОформления.Вид = ВидГруппыФормы.ГруппаКолонок;
	ЭлементОформления.Группировка = ГруппировкаКолонок.ВЯчейке;
	Элементы.Переместить(ЭлементОформления, Элементы.Оформление, Элементы.ОформлениеЗаголовок);
	Элементы.Переместить(Элементы.ОформлениеИспользование, ЭлементОформления);
	Элементы.Переместить(Элементы.ОформлениеЗаголовок, ЭлементОформления);
	
	Элементы.СтруктураВариантаГруппаИспользование.Видимость = Ложь;
	СтруктураВариантаГруппа = Элементы.Добавить("СтруктураВариантаГруппа", Тип("ГруппаФормы"), Элементы.СтруктураВарианта);
	СтруктураВариантаГруппа.Заголовок = НСтр("ru = 'Основное'");
	СтруктураВариантаГруппа.Вид = ВидГруппыФормы.ГруппаКолонок;
	СтруктураВариантаГруппа.Группировка = ГруппировкаКолонок.ВЯчейке;
	Элементы.Переместить(СтруктураВариантаГруппа, Элементы.СтруктураВарианта, Элементы.СтруктураВариантаПредставление);
	Элементы.Переместить(Элементы.СтруктураВариантаИспользование, СтруктураВариантаГруппа);
	Элементы.Переместить(Элементы.СтруктураВариантаПредставление, СтруктураВариантаГруппа);
КонецПроцедуры
...
#Область УсловноеОформлениеТаблицыФормыОтборы
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОтборыИспользованиеЗаглушка.Имя);
...
#Область УсловноеОформлениеТаблицыФормыОформление
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОформлениеИндексКартинкиДоступаЗаглушка.Имя);
...
#Область УсловноеОформлениеТаблицыФормыСтруктураВарианта
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СтруктураВариантаИспользованиеЗаглушка.Имя);
