...
&НаКлиенте
Асинх Процедура ПоместитьВБуферОбмена(Команда)
	
	Если Не СредстваБуфераОбмена.ИспользованиеДоступно() Тогда
		Возврат;
	КонецЕсли;
	
	КопируемоеЗначение = ПолучитьЗначениеТекущейСтроки();
	
	Если Не ЗначениеЗаполнено(КопируемоеЗначение) Тогда
		ПредупреждениеАсинх(НСтр("ru = 'Выберите поле объекта, оператор или функцию.'"));
	КонецЕсли;
	
	ЭлементыБуфераОбмена = Новый Массив;
	
	Если СредстваБуфераОбмена.ПоддерживаетсяФорматДанных(СтандартныйФорматДанныхБуфераОбмена.Текст) Тогда
		ЭлементБуфераОбмена = Новый ЭлементБуфераОбмена(СтандартныйФорматДанныхБуфераОбмена.Текст, КопируемоеЗначение);
		ЭлементыБуфераОбмена.Добавить(ЭлементБуфераОбмена);
	КонецЕсли;
	
	Результат = Ждать СредстваБуфераОбмена.ПоместитьДанныеАсинх(ЭлементыБуфераОбмена);
...
#КонецЕсли
	...
	ФайловаяСистемаКлиент.СохранитьФайл(ОбработчикПослеРазрешений, ПараметрыОткрытия.АдресДокумента,
		ВременныйКаталог + ПараметрыОткрытия.ИмяФайлаДляОткрытия, ПараметрыСохранения);
КонецПроцедуры
...
Функция ПолучитьХешМакета(АдресИлиДанные)
	Хеширование = Новый ХешированиеДанных(ХешФункция.CRC32);
	Если ТипЗнч(АдресИлиДанные) = Тип("Строка") И ЭтоАдресВременногоХранилища(АдресИлиДанные) Тогда
...
&НаКлиенте
Функция ЭтоНовый()
	Возврат Не ЗначениеЗаполнено(Параметры.Ссылка)
		И ПустаяСтрока(ИдентификаторМакета)
		И ПустаяСтрока(Параметры.ПутьКФайлу);
КонецФункции
...
Процедура ПереключитьЯзыкПослеЗагрузкиФайлаПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Сохранить изменения в %1%2?'"), ИмяДокумента, ОписаниеЯзыка);
...
Процедура УдалитьЯзыкМакета(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьЯзыкМакетаПродолжение", ЭтотОбъект, ,
		"ОшибкаПеремещенияУдаленияФайла", ЭтотОбъект);
...
Процедура ПредупредитьОБлокировкеИОткрытьПриложение()
	...
	ПоказатьПредупреждение(ОписаниеОповещения,
		НСтр("ru = 'Завершите работу с файлом в другом приложении.'"), ,
...
Процедура РасширитьСписокПолей()
	...
	КнопкаНастройкиФормата.ОтображениеГиперссылкиЯчейки = ОтображениеГиперссылкиЯчейки.Отображать;
...
Функция СписокОператоров()
	...
	ДеревоОператоров = УправлениеПечатью.СписокОператоров();
	
	Группа = ДеревоОператоров.Строки.Добавить();
	Группа.Идентификатор = "УсловныйВывод";
	Группа.Представление = НСтр("ru = 'Условный вывод'");
	Группа.Порядок = 7;
	
	Оператор = Группа.Строки.Добавить();
	Оператор.Идентификатор = "НачалоОбласти";
	Оператор.Представление = НСтр("ru = 'Начало условия'");
	...
	НСтр("ru = '{v8 %1 [вставьте текст условия]}'"), "Условие");
	
	Оператор = Группа.Строки.Добавить();
	Оператор.Идентификатор = "ОкончаниеОбласти";
	Оператор.Представление = НСтр("ru = 'Окончание условия'");
	Оператор.Картинка = БиблиотекаКартинок.Пустая;
	Оператор.ВыражениеДляВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("ru = '{/v8 %1}'"), "Условие");
	
	Возврат КонструкторФормул.КоллекцияПолей(ДеревоОператоров);
...
Процедура Подключаемый_СписокПолейНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Реквизит = ЭтотОбъект[ИмяСпискаПолей()].НайтиПоИдентификатору(ЗначениеПеретаскивания);
	
	Если Реквизит.Папка Или Реквизит.Таблица И Не СтрНачинаетсяС(Реквизит.ПутьКДанным, "ОбщиеРеквизиты.") Тогда
		Выполнение = Ложь;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьЗначениеТекущейСтроки()
	...
	Если ТекущийЭлемент.Имя = ИмяСпискаОператоров() Тогда
		Возврат СписокОператоровВыражениеДляВставки();
	ИначеЕсли ТекущийЭлемент.Имя = ИмяСпискаПолей() Тогда
		Возврат СписокПолейВыражениеДляВставки();
	КонецЕсли;
...
Процедура Подключаемый_ОператорыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ПараметрыПеретаскивания.Значение = СписокОператоровВыражениеДляВставки();
	Выполнение = ЗначениеЗаполнено(ПараметрыПеретаскивания.Значение);
...
Функция ПолучитьПечатнуюФорму()
	...
	ОфисныеДокументы = УправлениеПечатью.СформироватьОфисныйДокумент(
		Макет, Ссылки, ОбъектыПечати, КодЯзыка, ПараметрыПечати);
...
&НаКлиенте
Функция СписокОператоровВыражениеДляВставки()
	
	Если Элементы[ИмяСпискаОператоров()].ТекущиеДанные = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Оператор = КонструкторФормулКлиент.ВыбранноеПолеВСпискеПолей(ЭтотОбъект, ИмяСпискаОператоров());
	ВыражениеДляВставки = КонструкторФормулКлиент.ВыражениеДляВставки(Оператор);
	
	Если Оператор.ПутьКДанным = "УправлениеПечатью_КоличествоСтрок" Тогда
		ПредставлениеТекущейТаблицы = ПредставлениеТекущейТаблицы();
		Если ПредставлениеТекущейТаблицы = Неопределено Тогда
			ВыражениеДляВставки = "";
		Иначе
			ВыражениеДляВставки = СтрЗаменить(ВыражениеДляВставки, "()", "([" + ПредставлениеТекущейТаблицы + "])");
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Оператор.Родитель)
	   И НЕ ПустаяСтрока(ВыражениеДляВставки)
	   И НЕ ЗначениеЗаполнено(Оператор.ВыражениеДляВставки) Тогда
		...
		Если НЕ ИмяГруппыОператоров = "ЛогическиеОператорыИКонстанты" Тогда
			...
			ВыражениеДляВставки = "[" + ВыражениеДляВставки + "]";
			
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ВыражениеДляВставки;
	
КонецФункции

&НаКлиенте
Функция СписокПолейВыражениеДляВставки()
	
	ВыбранноеПоле = Элементы[ИмяСпискаПолей()].ТекущиеДанные;
	Если ВыбранноеПоле = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	ВыражениеДляВставки = "[" + ВыбранноеПоле.ПредставлениеПутиКДанным + "]";
	
	Если ЗначениеЗаполнено(ВыбранноеПоле.Формат) И ВыбранноеПоле.Формат <> ВыбранноеПоле.ФорматПоУмолчанию Тогда
		ВыражениеДляВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"[Формат(%1, %2)]", ВыражениеДляВставки, """" + ВыбранноеПоле.Формат + """");
	КонецЕсли;
