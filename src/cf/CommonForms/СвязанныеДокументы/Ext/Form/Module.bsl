...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(Элементы.ФормаКоманднаяПанель, Элементы.КоманднаяПанельЕще);
	КонецЕсли;
...
Функция ЗапросПоРеквизитамОбъектов(СписокОбъектов)
	...
	Для каждого ТипОбъекта Из ОбъектыПоТипу Цикл
		...
		ДопРеквизиты = РеквизитыДляПредставления(МетаданныеОбъекта.ПолноеИмя());
...
&НаСервере
Функция РеквизитыДляПредставления(Знач ПолноеИмяОбъектаМетаданных)
