...
&НаКлиенте
Процедура ДобавитьПодписи(Команда)
	...
	Если ТипЗнч(ОписаниеДанных.Объект) <> Тип("ОписаниеОповещения") Тогда
		...
		Исключение
			...
			ДобавитьПодписиЗавершение(Новый Структура("ОписаниеОшибки", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
			Возврат;
		КонецПопытки;
	...
	Иначе
		...
		ПараметрыВыполнения.Вставить("Оповещение", Новый ОписаниеОповещения("ДобавитьПодписиЗавершение", ЭтотОбъект));
	...
	ДобавитьПодписиЗавершение(Новый Структура);
	
КонецПроцедуры

// Продолжение процедуры Добавить.
&НаКлиенте
Процедура ДобавитьПодписиЗавершение(Результат, Контекст = Неопределено) Экспорт
