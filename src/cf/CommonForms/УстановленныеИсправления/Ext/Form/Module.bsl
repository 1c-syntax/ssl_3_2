...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоWindowsКлиент() Тогда
		...
		Если РазделениеВключено Тогда
			ПараметрыСообщения.Сообщение = НСтр("ru = 'Состав исправлений настраивается администратором приложения.'");
		ИначеЕсли Параметры.ПриОбновлении Тогда
			ПараметрыСообщения.Сообщение = НСтр("ru = 'Установленные исправления вступят в силу после перезапуска приложения.'");
		ИначеЕсли ЭтоВебКлиент Или Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
			ПараметрыСообщения.Сообщение = НСтр("ru = 'Изменения, которые будут сделаны, вступят в силу при следующем входе в приложение.'");
		Иначе
			ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'Изменения, которые будут сделаны, вступят в силу при следующем входе в приложение.
				|<a href=""Ссылка"">Поиск и установка обновлений</a> с портала 1С:Обновления программ.'"));
		КонецЕсли;
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Изменение состава исправлений'");
		ПараметрыСообщения.ТипСообщения = "Информация";
		ПараметрыСообщения.Обработчик = "ИнформацияНадписьОбработкаНавигационнойСсылки";
		ПараметрыСообщения.Идентификатор = "УстановленныеИсправления";
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.УстановленныеИсправления.КоманднаяПанель);
	КонецЕсли;
...
&НаКлиенте
Процедура ИнформацияНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура СохранитьИсправления(ТолькоПодключенные = Ложь)
	...
	Иначе
		ВыделенныеСтроки = Элементы.УстановленныеИсправления.ПолучитьСтрокиДляОбработки();
	КонецЕсли;
