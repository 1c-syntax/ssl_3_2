...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭлектроннаяПодпись.ОбщиеНастройки().ПроверятьЭлектронныеПодписиНаСервере
		...
		Иначе
			...
			ПоказатьВажноеСообщениеОшибкиПриПолученииАдресовСписковОтзыва(Ложь);
		КонецЕсли;
...
Процедура Подключаемый_ОбработатьДействиеИнформационногоСообщения(ЭлементИлиКоманда, НавигационнаяСсылка = Неопределено, СтандартнаяОбработка = Неопределено)
	...
	Если НавигационнаяСсылка = "ОшибкаПриПолученииПутиСертификации" Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		...
		Возврат;
	ИначеЕсли НавигационнаяСсылка = "ОшибкаПриПолученииАдресовСписковОтзыва" Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
...
Асинх Процедура ПослеПолученияЦепочкиСертификатов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат.Ошибка) Тогда
		...
		ПоказатьВажноеСообщениеОшибкиПриПолученииПутиСертификации();
	Иначе
		Для Каждого ТекущийСертификат Из Результат.Сертификаты Цикл
			...
			НоваяСтрока.ДанныеСертификата = ТекущийСертификат.ДанныеСертификата;
		КонецЦикла;
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОшибкаПриПолученииЦепочкиСертификатов");
	КонецЕсли;
...
Асинх Процедура ЗаполнитьСпискиОтзыва()
	...
	Исключение
		...
		ПоказатьВажноеСообщениеОшибкиПриПолученииАдресовСписковОтзыва();
	...
	ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОшибкаПриПолученииАдресовСписковОтзыва");
...
&НаСервере
Процедура ПоказатьВажноеСообщениеОшибкиПриПолученииАдресовСписковОтзыва(ВывестиНаФорму = Истина)
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Ошибка";
	ПараметрыСообщения.Идентификатор = "ОшибкаПриПолученииАдресовСписковОтзыва";
	ПараметрыСообщения.Скрываемый = Ложь;
	ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Не удалось получить адреса списков отзыва.</a>'"),
			"ОшибкаПриПолученииАдресовСписковОтзыва");
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Ошибка получения списков отзыва'");
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщенияСпискиОтзыва);
	
	// Если Ложь, то важное сообщение обязательно выводится позднее.
...
&НаСервере
Процедура ПоказатьВажноеСообщениеОшибкиПриПолученииПутиСертификации(ВывестиНаФорму = Истина)
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Ошибка";
	ПараметрыСообщения.Идентификатор = "ОшибкаПриПолученииПутиСертификации";
	ПараметрыСообщения.Скрываемый = Ложь;
	ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Не удалось получить цепочку сертификатов.</a>'"),
			"ОшибкаПриПолученииПутиСертификации");
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Ошибка получения цепочки сертификатов'");
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщенияПутьСертификации);
