...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.СписокРасширений.КоманднаяПанель);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура СписокРасширенийПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	УдалитьРасширения(Элемент.ПолучитьСтрокиДляОбработки());
...
Процедура СохранитьКак(Команда)
	...
	ВыделенныеСтроки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	ЗагрузитьРасширение(Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки());
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Если ИзменилосьСостояниеРасширений Тогда
...
Процедура УстановитьДоступностьКнопокКомандныхПанелей()
	...
	ИначеЕсли Элементы.СтраницыОбновление.ТекущаяСтраница = Элементы.СтраницаСписокРасширений Тогда 
		...
		ВыбранаОднаСтрока = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки().Количество() = 1;
...
Процедура ВключитьБезопасныйРежимЗавершение()
	...
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Истина, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
...
Процедура ВыключитьБезопасныйРежимЗавершение()
	...
	ОшибкиИзмененияРасширений = Новый Массив;
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Ложь, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаСервере
Процедура ВключитьВыключитьБезопасныйРежимДляВыделенныхРасширений(Значение, ОшибкиИзмененияРасширений, СтрокиДляОбработки)
...
Процедура ВключитьПередачуВПодчиненныеУзлыРИБЗавершение()
	...
	ОшибкиИзмененияРасширений = Новый Массив;
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Истина, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
Процедура ВыключитьПередачуВПодчиненныеУзлыРИБЗавершение()
	...
	ОшибкиИзмененияРасширений = Новый Массив;
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Ложь, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
&НаСервере
Процедура ВключитьВыключитьПередачуВПодчиненныеУзлыРИБ(Значение, ОшибкиИзмененияРасширений, СтрокиДляОбработки)
...
Процедура ВключитьПодключениеРасширений()
	...
	ОшибкиИзмененияРасширений = Новый Массив;
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьПодключениеРасширений(Истина, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
Процедура ВыключитьПодключениеРасширений()
	...
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	СинонимыРасширений = СинонимыРасширенийВыделенныхРасширенийДляВыключения(СтрокиДляОбработки);
	...
	Иначе
		ОшибкиИзмененияРасширений = Новый Массив;
		ВключитьВыключитьПодключениеРасширений(Ложь, ОшибкиИзмененияРасширений, СтрокиДляОбработки);
		ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
		ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
	КонецЕсли;
...
&НаСервере
Процедура ВключитьВыключитьПодключениеРасширений(Значение, ОшибкиИзмененияРасширений, СтрокиДляОбработки, ОбрабатыватьРасширенияСДанными = Истина)
...
&НаСервере
Функция СинонимыРасширенийВыделенныхРасширенийДляВыключения(СтрокиДляОбработки)
...
Процедура ВыключитьРасширенияПослеВопроса(Результат, Контекст) Экспорт
	...
	ОшибкиИзмененияРасширений = Новый Массив;
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	ВключитьВыключитьПодключениеРасширений(Ложь, ОшибкиИзмененияРасширений, СтрокиДляОбработки, ОбрабатыватьРасширенияСДанными);
	ПодключитьОбработчикОжидания("СкрытьДлительнуюОперацию", 0.1, Истина);
	ВывестиИнформациюОбОшибкахПеренастройкиРасширений(ОшибкиИзмененияРасширений);
...
Процедура УстановитьОтветственногоПослеВыбора(Ответственный, ДополнительныеПараметры) Экспорт
	...
	СтрокиДляОбработки = Элементы.СписокРасширений.ПолучитьСтрокиДляОбработки();
	УстановитьОтветственногоДляВыделенныхРасширений(Ответственный, СтрокиДляОбработки);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтветственногоДляВыделенныхРасширений(Ответственный, СтрокиДляОбработки)
...
Процедура ВывестиВажноеСообщение()
	...
	Если ЭтоНеразделенныйПользовательВОбласти Тогда
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Чтобы использовать расширения области данных, требуется запустить сеанс с разделителями.
			|Изменения расширений для пользователей области данных вступят в силу при их следующем входе в приложение.'")
	Иначе
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Чтобы продолжить работу с учетом сделанных изменений, необходимо <a href=""Ссылка"">перезапустить</a> приложение.'"));
	КонецЕсли;
	ПараметрыСообщения.Обработчик = "Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки";
	ПараметрыСообщения.Скрываемый = Ложь;
	ПараметрыСообщения.Идентификатор = "ИзменилсяСоставРасширений";
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.СтраницаСписокРасширений, Элементы.СписокРасширений);
