...
Процедура ПредупреждениеОЗацикливанииОписаниеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "1" Тогда
		...
		ОткрытьФорму("РегистрСведений.КонтурСинхронизации.Форма.ЗацикливаниеСинхронизации",,,,,, Новый ОписаниеОповещения("ПослеЗакрытияФормыЗацикливаниеСинхронизации", ЭтотОбъект),
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокПриложений
...
Процедура СписокПриложенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "СписокПриложенийДоступенПереходНаWS" Тогда
		...
		Если ТекущиеДанные.ДоступенПереходНаWS Тогда
			...
			ОткрытьФорму("Обработка.ПомощникСозданияОбменаДанными.Форма.ПереходНаИнтернетСоединение",
				ПараметрыФормы, ЭтотОбъект,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	...
	ИначеЕсли Поле.Имя = "СписокПриложенийТранспортКартинка" Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторТранспорта) Тогда
			...
			ОткрытьФорму(ПолноеИмяФормыНастройки, ПараметрыФормы,,,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
&НаКлиенте
Процедура СценарииУзла(Команда)
...
Процедура ОткрытьФормуНастройкиПараметровСинхронизации(ТекущиеДанные)
	...
	Иначе
		...
		ОткрытьФорму(ИмяОткрываемойФормы, ПараметрыПомощника, ЭтотОбъект, , , , ОповещениеОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура ОткрытьРезультатыСинхронизации(ДанныеТекущейСтроки = Неопределено, ИмяНабораСобытий = "")
	...
	ОткрытьФорму("РегистрСведений.РезультатыОбменаДанными.Форма.ПредупрежденияСинхронизаций", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения,, РежимОтображенияОкнаФормы.ВДиалоговомОкне);
...
Процедура ОткрытьФормуРазбораРезультатовПообъектногоОбмена(ТекущиеДанные)
	...
	ОткрытьФорму("Обработка.РаботаСРезультатамиОбмена.Форма.РазборРезультатовПообъектногоОбмена", ПараметрыОткрытия
	             , ЭтотОбъект, ТекущиеДанные.УзелИнформационнойБазы,,,, РежимБлокировкиПриОткрытииОкнаФормы.НеБлокировать);
...
Процедура ВыполнитьКомандуСПредварительнойПроверкой(КомандаСтрокой)
	...
	ИначеЕсли КомандаСтрокой = "НастроитьСценарииОбменаДанными" Тогда
		...
		ОбменДаннымиКлиент.ОбработкаКомандыСценарииСинхронизаций(ЭтотОбъект);
		
	ИначеЕсли КомандаСтрокой = "СценарииУзла" Тогда	
...
Процедура ОткрытьФормуПомощникаНастройкиНовойСинхронизации(ТекущиеДанные, ДанныеДляЗавершенияНастройки)
	...
	Иначе
		...
		ОткрытьФорму("Обработка.ПомощникСозданияОбменаДанными.Форма.НастройкаСинхронизации",
			ПараметрыПомощника, ЭтотОбъект, КлючУникальностиПомощника, , , ОповещениеОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.НеБлокировать);
...
Процедура ПриЗавершенииПолученияВариантовНастроекОбменаДаннымиСВнешнимиСистемами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Иначе
			...
			ОткрытьФорму(ИмяОткрываемойФормы, ДополнительныеПараметры.ПараметрыПомощника, ЭтотОбъект, , , , ДополнительныеПараметры.ОповещениеОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.НеБлокировать);
...
&НаКлиенте
Процедура ПослеЗакрытияФормыЗацикливаниеСинхронизации(Результат, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	ИспользуетсяИнтерфейсТакси = ИспользуетсяИнтерфейсТакси();
	
	Элементы.ФормаГруппаКомандСинхронизировать.Видимость = ИспользуетсяИнтерфейсТакси;
	Элементы.ФормаГруппаКомандНастройки.Видимость		 = ИспользуетсяИнтерфейсТакси;
	...
	Если МодельСервиса Тогда
		...
		Элементы.СписокПриложенийНастроитьРасписаниеВыполненияОбмена.Видимость = Ложь;
	...
	Если ОтборПоВариантамНастроек.Количество() Тогда
		...
		Для Каждого СтрокаМонитора Из ТаблицаМонитора Цикл
			...
			Иначе
				...
				Если ВариантыНастроекОбмена.Количество() Тогда
...
&НаСервере
Функция ИспользуетсяИнтерфейсТакси()
	
	ТекущийВариантИнтерфейса = КлиентскоеПриложение.ТекущийВариантИнтерфейса();
	
	Возврат ТекущийВариантИнтерфейса = ВариантИнтерфейсаКлиентскогоПриложения.Такси;
