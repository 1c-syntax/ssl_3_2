...
Процедура ПриВыполненииПоискаНаСервере(Знач СтрокаПоиска, Знач НаправлениеПоиска = "ПерваяЧасть")
	...
	ПараметрыПоиска.ВидОтображения = ВидОтображенияПолнотекстовогоПоиска.XML;
...
Процедура ОбновитьФорму(РезультатыПоиска)
	...
	Если Количество = 0 Тогда
		Элементы.Следующие.Видимость  = Ложь;
		Элементы.Предыдущие.Видимость = Ложь;
	Иначе
		Элементы.Следующие.Видимость  = (ПолноеКоличество - ТекущаяПозиция) > Количество;
		Элементы.Предыдущие.Видимость = (ТекущаяПозиция > 0);
	КонецЕсли;
	...
	Если ПустаяСтрока(КодОшибки) Тогда 
		РезультатыПоискаПредставление = НоваяФорматированнаяСтрокаРезультата(РезультатыПоиска);
	Иначе
		РезультатыПоискаПредставление = НоваяФорматированнаяСтрокаОшибки();
	КонецЕсли;
	РезультатыПоискаДокумент.УстановитьФорматированнуюСтроку(РезультатыПоискаПредставление);
	
	Если СостояниеПоиска = "ВыполняетсяОбновлениеИндекса"
	...
	Иначе // ПоискРазрешен
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СостояниеПоиска");
	КонецЕсли;
	...
	Если СостояниеПоиска <> "ПоискРазрешен" Тогда
		...
		ПараметрыСообщения.Идентификатор = "СостояниеПоиска";
		ПараметрыСообщения.Сообщение = СостояниеПоискаПредставление;
		ПараметрыСообщения.КраткоеПредставление = СостояниеПоискаПредставление;
...
//
&НаСервере
Функция НоваяФорматированнаяСтрокаРезультата(РезультатыПоиска)
	
	ШаблонСтраницы = "%Строки%";
	
	ШаблонСтроки = "%НомерСтроки%<a href=""%Ссылка%"">%Представление%</a>
		| %Описание%";
	
	ШаблонНеактивнойСтроки = "%НомерСтроки%<a href=""#"">%Представление%</a>
		| %Описание%";
	
	Строки = "";
	ОписаниеТипа = Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(10, ДопустимаяДлина.Фиксированная));

	НомерСтроки = ТекущаяПозиция;
	Для Каждого СтрокаРезультатаПоиска Из РезультатыПоиска Цикл
		...
		Описание      = СтрокаРезультатаПоиска.Описание;
		...
		Отступ = ОписаниеТипа.ПривестиЗначение("");
		ПредставлениеНомераСтроки = ОписаниеТипа.ПривестиЗначение(Строка(НомерСтроки) + ". ");
		
		Строка = СтрЗаменить(Строка, "%НомерСтроки%",   ПредставлениеНомераСтроки);
		Строка = СтрЗаменить(Строка, "%Представление%", Представление);
		Строка = СтрЗаменить(Строка, "%Описание%",      Описание);
		
		Строки = Строки + Символы.ПС + Символы.ПС + Строка;
		
	КонецЦикла;
	
	Результат = СтрЗаменить(ШаблонСтраницы, "%Строки%", Строки);
	
	Возврат СтроковыеФункции.ФорматированнаяСтрока(СокрЛ(Результат));
	
КонецФункции

&НаСервере
Функция НоваяФорматированнаяСтрокаОшибки()
	
	ШрифтРекомендаций = "ПолнотекстовыйПоискШрифтПунктаРекомендаций";
	ЦветЗаголовка     = "ПолнотекстовыйПоискЦветЗаголовкаРекомендаций";
	ШрифтЗаголовка    = "ПолнотекстовыйПоискШрифтЗаголовкаРекомендаций";
	
	Рекомендации = 
		НСтр("ru = '<span style=""color: %4; font: %5"">Рекомендации:</span>
			|
			|%1%2
			|<span style=""font: %6"">       • Воспользуйтесь поиском по началу слова.</span>
			...
			|
			|<span style=""font: %6"">       • Воспользуйтесь нечетким поиском.</span>
			|         Используйте решетку (#).
			|         Например, Ромашка#2 найдет все документы, содержащие такие слова, которые отличаются от слова Ромашка на одну или две буквы.
			|
			|<a href=""%3"">Полное описание формата поисковых выражений</a>'");
	...
	Если КодОшибки = "НичегоНеНайдено" Тогда 
		...
		Если ИскатьВРазделах И УказаныОбластиПоиска Тогда 
			...
			РекомендацияОбластиПоиска = 
				НСтр("ru = '<span style=""font: %1"">       • Уточните область поиска.</span>
				|         Попробуйте выбрать больше областей поиска или все разделы.'") + Символы.ПС;
			
		КонецЕсли;
		
		РекомендацияТекстЗапроса =
			НСтр("ru = '<span style=""font: %1"">       • Упростите запрос, исключив из него какое-либо слово.</span>'") + Символы.ПС;
	...
	ИначеЕсли КодОшибки = "СлишкомМногоРезультатов" Тогда
		...
		Если Не ИскатьВРазделах Или Не УказаныОбластиПоиска Тогда 
			...
			РекомендацияОбластиПоиска = 
				НСтр("ru = '<span style=""font: %1"">       • Уточните область поиска.</span>
				|         Попробуйте выбрать область поиска, задав точный раздел или список.'") + Символы.ПС;
			
		КонецЕсли;
		
	КонецЕсли;
	
	РекомендацияОбластиПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(РекомендацияОбластиПоиска,
			ШрифтРекомендаций);
	РекомендацияТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(РекомендацияТекстЗапроса,
			ШрифтРекомендаций);
	
	Рекомендации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Рекомендации,
		РекомендацияОбластиПоиска,
		РекомендацияТекстЗапроса,
		ПолучитьНавигационнуюСсылку(Метаданные.ОбщиеКоманды.ОткрытьПолноеОписаниеФорматаПоисковыхВыражений),
		ЦветЗаголовка,
		ШрифтЗаголовка,
		ШрифтРекомендаций);
	
	Возврат СтроковыеФункции.ФорматированнаяСтрока(Рекомендации);
