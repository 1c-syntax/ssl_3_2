...
Функция УстановленныеМетки(Метки)
	...
	Для Каждого Метка Из Метки Цикл
		Если Метка.Значение Тогда
			ОписаниеМетки = Новый Структура("Свойство, Наименование, НомерКартинки");
			ЗаполнитьЗначенияСвойств(ОписаниеМетки, Метка);
			УстановленныеМетки.Добавить(ОписаниеМетки);
		КонецЕсли;
	КонецЦикла;
