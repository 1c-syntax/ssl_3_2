...
&НаКлиенте
Процедура ОписаниеПредупрежденияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "СписокРасширений" Тогда
...
Процедура НастроитьОграниченияПередачиДанных(Команда)
	...
	ОткрытьФорму(ИмяФормыНастройкиУзла, ПараметрыФормы, ЭтотОбъект,,,, Обработчик);
...
Функция РасширенияИзменяющиеСтруктуруДанных()
	...
	Для Каждого Расширение Из РасширенияОбласти Цикл
		...
		Если Расширение.ОбластьДействия = ОбластьДействияРасширенияКонфигурации.ИнформационнаяБаза Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Для Каждого КомандаНавигации Из Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы Цикл
		Если КомандаНавигации <> Элементы.Справка Тогда
...
&НаСервере
Функция ТаблицаПереходовНоваяСтрока(ИмяОсновнойСтраницы, КомандыНавигации = "", ИмяСтраницыДекорации = "")
...
Процедура СценарийСозданияАвтономногоРабочегоМеста()
	...
	ИмяКомандыНазад  = Элементы.КомандаНазад.Имя;
	ИмяКомандыДалее  = Элементы.КомандаДалее.Имя;
	ИмяКомандыГотово = Элементы.КомандаГотово.Имя;
	ИмяКомандыДалееСоздать = Элементы.КомандаДалееСоздать.Имя;
	
	ТаблицаПереходов.Очистить();
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("НаличиеРасширенийИзменяющихСтруктуруДанных", ИмяКомандыГотово);
	НовыйПереход.ИмяОбработчикаПриОткрытии = "Подключаемый_НаличиеРасширенийИзменяющихСтруктуруДанных_ПриОткрытии";
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("Начало", ИмяКомандыДалее);
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("НастройкаВыгрузки", ИмяКомандыНазад + "," + ИмяКомандыДалееСоздать);
	НовыйПереход.ИмяОбработчикаПриПереходеДалее = "Подключаемый_НастройкаВыгрузки_ПриПереходеДалее";
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОжиданиеСозданияНачальногоОбраза");
	...
	НовыйПереход = ТаблицаПереходовНоваяСтрока("Окончание", ИмяКомандыГотово);
	НовыйПереход.ИмяОбработчикаПриОткрытии = "Подключаемый_Окончание_ПриОткрытии";
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОшибкаВыполнения", ИмяКомандыДалее);
	
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ОжиданиеУдаленияНачальногоОбраза");
	НовыйПереход.ДлительнаяОперация = Истина;
	...
	НовыйПереход = ТаблицаПереходовНоваяСтрока("ЗавершениеУдаленияНачальногоОбраза", ИмяКомандыГотово);
	НовыйПереход.ДлительнаяОперация = Истина;
