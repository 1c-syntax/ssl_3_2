...
Процедура УдалениеОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено) Экспорт
	...
	Если БезопасныйРежим Тогда
		...
		КонецЦикла;
	ИначеЕсли ВстроенаБСПВерсии321() Тогда
...
Функция ПолучитьПараметрыОбработчика(ИмяОбласти)
	...
	Для НомерСтроки = ОбластьПараметров.Верх По ОбластьПараметров.Низ Цикл
		...
		Если ТекущаяОбласть.ТекущаяОбласть = Неопределено Тогда
			Параметр = Область.Область(НомерСтроки, 2, НомерСтроки, 2).Текст;
		Иначе
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьФайлаАрхива(ИмяАрхива, ПарольДляСжатияФайлаОбмена,, НСтр("ru = 'Файл обмена данными'"));
...
Функция РаспаковатьZipФайл(ИмяФайлаДляРаспаковки)
	...
	Попытка
		...
		Архиватор = Новый ЧтениеФайлаАрхива(ИмяФайлаДляРаспаковки, ПарольДляРаспаковкиФайлаОбмена);
		
		Если Архиватор.Элементы.Количество() > 0 Тогда
			
			ЭлементАрхива = Архиватор.Элементы.Получить(0);
			
			Архиватор.Извлечь(ЭлементАрхива, КаталогДляРаспаковки, РежимВосстановленияПутейФайлаАрхива.НеВосстанавливать);
...
Функция ВстроенаБСПВерсии321()
	
	Если ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
		ТекущаяВерсияБСП = СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
		
		Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ТекущаяВерсияБСП, "3.2.1.0") > 0;
	Иначе
		Возврат Ложь;
	КонецЕсли;
