...
Процедура ПриОткрытии(Отказ)
	...
	ЭтоЗагрузка = (Элементы.ГлавнаяПанельФормы.ТекущаяСтраница = Элементы.Загрузка);
	Элементы.ФормаВыполнитьЗагрузку.Важность = ?(ЭтоЗагрузка, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	
	ЭтоВыгрузка = (Элементы.ГлавнаяПанельФормы.ТекущаяСтраница = Элементы.Выгрузка);
	Элементы.ФормаВыполнитьВыгрузку.Важность = ?(ПравилаЗагружены И ЭтоВыгрузка, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	...
	ПриИзмененииНазначениеВыгрузки();
	...
	ОбработатьДоступностьЭлементовУправленияТранзакциямиПриВыгрузке();
	
	АвтоматическиЗагружатьДанныеПриИзменении(Элементы.АвтоматическиЗагружатьДанные);
	
	РазвернутьСтрокиДерева(УдаляемыеДанные, Элементы.УдаляемыеДанные, "Пометка");
	
#Если ВебКлиент Тогда
		Элементы.ГруппаВключениеОтладки.Видимость = Ложь;
		Элементы.ГруппаПредупреждениеВебКлиент.Видимость = Истина;
...
#КонецЕсли
	...
	ГлавнаяПанельФормыПриСменеСтраницы(Элементы.ГлавнаяПанельФормы, Элементы.ГлавнаяПанельФормы.ТекущаяСтраница);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Объект.НастройкаАвтоматическойЗагрузкиДанных > 0 Тогда
		
		АвтоматическиЗагружатьДанные = Истина;
		Элементы.ЗадатьВопросПередАвтоматическойЗагрузкойДанных.Доступность = Истина;
		
		ЗадатьВопросПередАвтоматическойЗагрузкойДанных = НЕ (Объект.НастройкаАвтоматическойЗагрузкиДанных > 1);
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	Если ТипЗнч(Настройки) = Тип("Соответствие") Тогда
		...
		Настройки["Объект.НастройкаАвтоматическойЗагрузкиДанных"] = Объект.НастройкаАвтоматическойЗагрузкиДанных;
...
&НаКлиенте
Процедура АвтоматическиЗагружатьДанныеПриИзменении(Элемент)
	
	Объект.НастройкаАвтоматическойЗагрузкиДанных = ?(АвтоматическиЗагружатьДанные, 1, 0);
	
	Если НЕ АвтоматическиЗагружатьДанные Тогда
		ЗадатьВопросПередАвтоматическойЗагрузкойДанных = Истина;
	КонецЕсли;

	Элементы.ЗадатьВопросПередАвтоматическойЗагрузкойДанных.Доступность = АвтоматическиЗагружатьДанные;
	
КонецПроцедуры

&НаКлиенте
Процедура НепосредственноеЧтениеВИБПриемникеПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДекорацияНастройкаСохраненияРезультатаНажатие(Элемент)
	
	Если Объект.НепосредственноеЧтениеВИБПриемнике = Истина Тогда
		
		ОткрытьНастройкуПодключенияКИБ();
		
	Иначе
		
		ОткрытьНастройкуСохраненияФайла();
...
&НаКлиенте
Процедура ЗадатьВопросПередАвтоматическойЗагрузкойДанныхПриИзменении(Элемент)
	
	Объект.НастройкаАвтоматическойЗагрузкиДанных = ?(ЗадатьВопросПередАвтоматическойЗагрузкойДанных, 1, 2);
...
Процедура ГлавнаяПанельФормыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ЭтоЗагрузка = (ТекущаяСтраница.Имя = "Загрузка");
	ЭтоВыгрузка = (ТекущаяСтраница.Имя = "Выгрузка");
	ЭтоУдаление = (ТекущаяСтраница.Имя = "Удаление");
	
	Если ЭтоВыгрузка Тогда
		
		Объект.РежимОбмена = "Выгрузка";
		
	ИначеЕсли ЭтоЗагрузка Тогда
	...
	Элементы.ФормаВыполнитьЗагрузку.Важность = ?(ЭтоЗагрузка, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	Элементы.ФормаВыполнитьВыгрузку.Важность = ?(ПравилаЗагружены И ЭтоВыгрузка, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	Элементы.УдаляемыеДанныеУдалениеУдалить.Важность = ?(ЭтоУдаление, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	
	Элементы.ФлагРежимОтладкиОбработчиков.Доступность = ЭтоЗагрузка;
	Элементы.ФлагРежимОтладкиОбработчиковВыгрузка.Доступность = ЭтоВыгрузка;
...
&НаКлиенте
Процедура ИспользоватьТранзакцииПриВыгрузкеДляПлановОбменаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ФлагРежимОтладкиОбработчиковПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПарольДляРаспаковкиФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ПолеПароляНачалоВыбора(Элемент, СтандартнаяОбработка);
...
Процедура ТестПодключения(Команда)
	...
	Если ВыполнитьПодключениеКИБПриемникуНаСервере() = Истина Тогда
...
Процедура ВыгрузкаУстановитьУзелОбмена(Команда)
	...
	КоллекцииСтрок = Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы();
	Если КоллекцииСтрок.Количество() < 1 Тогда
		
		ТекстПредупреждения = НСтр("ru ='Требуется добавить правила обмена, чтобы заполнить типы выгружаемых данных...'");
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораУзла", ЭтотОбъект);
	
	ИмяОбработки = Лев(ИмяФормы, ПоследнийРазделитель(ИмяФормы));
	ИмяВызываемойФормы = ИмяОбработки + "ВыборУзлаПланаОбмена";
	
	ПараметрыФормы = Новый Структура;
	
	Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	ОткрытьФорму(ИмяВызываемойФормы, ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, Режим);
...
Процедура НаКлиенте(Команда)
	...
	ОбновитьПредставлениеНастройкиСохраненияРезультата();
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И Объект.НепосредственноеЧтениеВИБПриемнике Тогда
...
Процедура СвернутьГруппы(Команда)
	...
	Если ЭтоВыгрузка Тогда
		...
		ИмяТаблицы = "ТаблицаПравилВыгрузки";
		КоллекцияЭлементов = Объект[ИмяТаблицы].ПолучитьЭлементы();
		
	Иначе
		
		ИмяТаблицы = "УдаляемыеДанные";
		КоллекцияЭлементов = УдаляемыеДанные.ПолучитьЭлементы();
		
	КонецЕсли;
	
	СвернутьРазвернутьДерево(КоллекцияЭлементов, ИмяТаблицы, "Свернуть");
...
Процедура РазвернутьГруппы(Команда)
	...
	СвернутьРазвернутьДерево(КоллекцияЭлементов, ИмяТаблицы, "Развернуть");
...
&НаКлиенте
Процедура ОбновитьПредставлениеНастройкиСохраненияРезультата()
	
	Элементы.ТестПодключения.Видимость = Объект.НепосредственноеЧтениеВИБПриемнике;
	
	Если Объект.НепосредственноеЧтениеВИБПриемнике = Истина Тогда
		
		Если ЭтоWindowsКлиент() Тогда
			
			Элементы.ГруппаОС.ТекущаяСтраница = Элементы.ГруппаНастройкаСохраненияРезультата;
			ПредставлениеНастройкиСохраненияРезультатаПоИБ();
			
		Иначе
			
			Элементы.ГруппаОС.ТекущаяСтраница = Элементы.ГруппаLinux;
			
		КонецЕсли;
		
	Иначе
		
		ПредставлениеНастройкиСохраненияРезультатаПоФайлу();
...
&НаКлиенте
Процедура ПредставлениеНастройкиСохраненияРезультатаПоИБ()
	
	ПредельныйРазмерСтроки = 40;
	ОстатокПраво = 32;
	
	Если Объект.ТипИнформационнойБазыДляПодключения = ЛОЖЬ // На сервере
		И НЕ ПустаяСтрока(Объект.ИмяСервераИнформационнойБазыДляПодключения) Тогда
		
		ШаблонПредставления = "Srvr=""%1"";Ref=""%2"";";
		СтрокаПодключения = СтрШаблон(ШаблонПредставления, Объект.ИмяСервераИнформационнойБазыДляПодключения, Объект.ИмяИнформационнойБазыНаСервереДляПодключения);
		
		Если СтрДлина(СтрокаПодключения) > ПредельныйРазмерСтроки Тогда
			
			СтрокаПодключения = "Srvr=..." + Прав(СтрокаПодключения, ОстатокПраво)
			
		КонецЕсли;
		
		ПредставлениеПодключения = СтрШаблон("%1 %2", НСтр("ru ='ИБ'"), СтрокаПодключения);
		
	ИначеЕсли Объект.ТипИнформационнойБазыДляПодключения = ИСТИНА // На локальном ПК
		И НЕ ПустаяСтрока(Объект.КаталогИнформационнойБазыДляПодключения) Тогда
		
		ШаблонПредставления = "File=""%1"";";
		СтрокаПодключения = СтрШаблон(ШаблонПредставления, Объект.КаталогИнформационнойБазыДляПодключения);
		
		Если СтрДлина(СтрокаПодключения) > ПредельныйРазмерСтроки Тогда
			
			СтрокаПодключения = "File=..." + Прав(СтрокаПодключения, ОстатокПраво);
	...
	Иначе
		...
		ПредставлениеПодключения = НСтр("ru ='<Выполните настройку подключения>'");
		
	КонецЕсли;
	
	Элементы.ДекорацияНастройкаСохраненияРезультата.Заголовок = ПредставлениеПодключения;
	
КонецПроцедуры

&НаКлиенте
Процедура ПредставлениеНастройкиСохраненияРезультатаПоФайлу()
	
	ПредельныйРазмерСтроки = 40;
	ОстатокПраво = 32;
	
	ПредставлениеПодключения = ИмяФайлаДанных;
	Если ЭтоКлиент = Истина Тогда
	
		ПредставлениеПодключения = НСтр("ru ='Настройки файла выгрузки данных'");
		
	ИначеЕсли СтрДлина(ПредставлениеПодключения) > ПредельныйРазмерСтроки Тогда
		
		ПредставлениеПодключения = Лев(ПредставлениеПодключения, 5) + "..." + Прав(ПредставлениеПодключения, ОстатокПраво);
		
	ИначеЕсли ПустаяСтрока(ПредставлениеПодключения) Тогда
		
		ПредставлениеПодключения = НСтр("ru ='<Укажите файл выгрузки данных>'");
...
&НаКлиенте
Процедура ОбработатьДоступностьЭлементовУправленияТранзакциямиПриВыгрузке()
	
	Элементы.ГруппаТранзакцияПриВыгрузке.Доступность = Объект.ИспользоватьТранзакцииПриВыгрузкеДляПлановОбмена;
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьНастройкуПодключенияКИБ()
	
	ИмяОбработки = Лев(ИмяФормы, ПоследнийРазделитель(ИмяФормы));
	ИмяВызываемойФормы = ИмяОбработки + "НастройкаПодключенияКПриемнику";
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТипИнформационнойБазыДляПодключения", Объект.ТипИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("ВерсияПлатформыИнформационнойБазыДляПодключения", Объект.ВерсияПлатформыИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("КаталогИнформационнойБазыДляПодключения", Объект.КаталогИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("ИмяСервераИнформационнойБазыДляПодключения", Объект.ИмяСервераИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("ИмяИнформационнойБазыНаСервереДляПодключения", Объект.ИмяИнформационнойБазыНаСервереДляПодключения);
	ПараметрыФормы.Вставить("АутентификацияWindowsИнформационнойБазыДляПодключения", Объект.АутентификацияWindowsИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("ПользовательИнформационнойБазыДляПодключения", Объект.ПользовательИнформационнойБазыДляПодключения);
	ПараметрыФормы.Вставить("ПарольИнформационнойБазыДляПодключения", Объект.ПарольИнформационнойБазыДляПодключения);
	
	Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	ОписаниеОповещения = Новый ОписаниеОповещения("НастройкаПодключенияККорреспондентуЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура НастройкаПодключенияККорреспондентуЗавершение(ПараметрыПодключения, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(Объект, ПараметрыПодключения);
	ОбновитьПредставлениеНастройкиСохраненияРезультата();
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьНастройкуСохраненияФайла()
	
	ИмяОбработки = Лев(ИмяФормы, ПоследнийРазделитель(ИмяФормы));
	ИмяВызываемойФормы = ИмяОбработки + "НастройкаСохраненияФайла";
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЭтоКлиент", ЭтоКлиент);
	ПараметрыФормы.Вставить("ИмяФайлаПравил", ИмяФайлаПравил);
	ПараметрыФормы.Вставить("ИмяФайлаДанных", ИмяФайлаДанных);
	ПараметрыФормы.Вставить("АрхивироватьФайл", Объект.АрхивироватьФайл);
	ПараметрыФормы.Вставить("ПарольДляСжатияФайлаОбмена", Объект.ПарольДляСжатияФайлаОбмена);
	ПараметрыФормы.Вставить("ВыгрузкаДанныхВФайл", ВыгрузкаДанныхВФайл);
	
	Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	ОписаниеОповещения = Новый ОписаниеОповещения("НастройкаСохраненияФайлаЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура НастройкаСохраненияФайлаЗавершение(ПараметрыПодключения, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(Объект, ПараметрыПодключения); // АрхивироватьФайл, ПарольДляСжатияФайлаОбмена
	
	ИмяФайлаПравил = ПараметрыПодключения.ИмяФайлаПравил;
	ИмяФайлаДанных = ПараметрыПодключения.ИмяФайлаДанных;
	ВыгрузкаДанныхВФайл = ПараметрыПодключения.ВыгрузкаДанныхВФайл;
...
// Параметры:
//  ТекСтрока - строка дерева значений.
//  ИмяФлажка - наименование колонки.
// 
&НаКлиенте
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Если ЭтоКлиент Тогда
		...
		ДиалогВыбораФайла = ПодготовленныеПараметрыДиалогаПомещенияФайлов();
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьЗагрузкуИзФормыЗавершение", ЭтотОбъект);
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения,,, АдресФайла, ДиалогВыбораФайла, УникальныйИдентификатор);
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Элементы.ФормаВыполнитьВыгрузку.Важность = ВажностьКнопки.Обычная;
	
	Элементы.НадписьПояснениеВыгрузки.Видимость = Истина;
	Элементы.НастройкаОтладкиВыгрузки.Доступность = Ложь;
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если Не Объект.НепосредственноеЧтениеВИБПриемнике
		И Не ЭтоКлиент
		И ИменаФайловПравилИОбменаСовпадают() Тогда
	...
	Если ЭтоКлиент И Не Объект.НепосредственноеЧтениеВИБПриемнике И Не Объект.ФлагОшибки Тогда
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не Объект.НепосредственноеЧтениеВИБПриемнике Тогда
	...
	Если ЭтоКлиент И Не Объект.НепосредственноеЧтениеВИБПриемнике Тогда
...
Процедура УстановитьДоступностьКомандОтладки()
	...
	ЕстьПравилаВыгрузки = (Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы().Количество() <> 0) И Объект.ФлагРежимОтладкиОбработчиков;
	Элементы.НастройкаОтладкиВыгрузки.Доступность = ЕстьПравилаВыгрузки;
...
&НаКлиенте
Процедура ПриИзмененииНазначениеВыгрузки()
...
Процедура ИзменитьРасположениеФайла()
	...
	Элементы.ДекорацияВыбранныйФайл.Видимость = ЭтоКлиент;
	
	Элементы.ИмяФайлаПравил.Видимость = НЕ ЭтоКлиент;
	Элементы.ИмяФайлаОбмена.Видимость = НЕ ЭтоКлиент;
	Элементы.ИнформацияОФайлеОбмена.Видимость = НЕ ЭтоКлиент;
	Элементы.ГруппаБезопаснаяЗагрузкаИмяФайлаПравил.Видимость = НЕ ЭтоКлиент;
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	КоманднаяПанель.ПодчиненныеЭлементы.РежимОбработки.Заголовок = 
		?(РежимРаботы, НСтр("ru = 'Режим (на клиенте)'"), НСтр("ru = 'Режим (на сервере)'"));
...
Процедура УстановитьПризнакЗагрузкиПравил(Признак)
	...
	Элементы.ФормаВыполнитьВыгрузку.Доступность = ПравилаЗагружены;
	Элементы.ФормаВыполнитьВыгрузку.Важность = ?(ПравилаЗагружены, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
	Элементы.НадписьПояснениеВыгрузки.Видимость = Не ПравилаЗагружены;
...
Процедура ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	ДопустимыеВерсии = Новый Массив(2);
	ДопустимыеВерсии[0] = "8.3";
	ДопустимыеВерсии[1] = "8.5";
	
	Информация = Новый СистемнаяИнформация;
	
	Если Не (ДопустимыеВерсии.Найти(Лев(Информация.ВерсияПриложения, 3)) <> Неопределено
...
Процедура ИзменитьРежимБезопаснаяЗагрузка(Интерактивно = Истина)
	...
	Элементы.ГруппаБезопаснаяЗагрузкаИмяФайлаПравил.Доступность = Объект.БезопаснаяЗагрузка;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОтбораДанных.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.ТаблицаПравилВыгрузки.ЭтоГруппа");
	ЭлементОтбораДанных.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбораДанных.ПравоеЗначение = Истина;
	ЭлементОтбораДанных.Использование  = Истина;
	
	ЭлементОформляемогоПоля = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ЭлементОформляемогоПоля.Поле = Новый ПолеКомпоновкиДанных("СсылкаНаУзелОбмена");
...
&НаКлиенте
Процедура СвернутьРазвернутьДерево(КоллекцияЭлементов, Знач ИмяЭлемента, Знач Действие)
	...
	Для каждого ЭлементКоллекции Из КоллекцияЭлементов Цикл
		...
		Если ВложенныеЭлементыКоллекции.Количество() > 0 Тогда
			...
			СвернутьРазвернутьДерево(ВложенныеЭлементыКоллекции, ИмяЭлемента, Действие);
			
		КонецЕсли;
		
		Если Действие = "Свернуть" Тогда
			
			Элементы[ИмяЭлемента].Свернуть(ЭлементКоллекции.ПолучитьИдентификатор());
			
		ИначеЕсли Действие = "Развернуть" Тогда
			
			Элементы[ИмяЭлемента].Развернуть(ЭлементКоллекции.ПолучитьИдентификатор());
...
&НаКлиенте
Процедура ПослеВыбораУзла(Результат, ДополнительныеЗначения) Экспорт
	...
	УстановитьУзелОбменаУСтрокДереваНаСервере(Результат);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолеПароляНачалоВыбора(Элемент, СтандартнаяОбработка)
