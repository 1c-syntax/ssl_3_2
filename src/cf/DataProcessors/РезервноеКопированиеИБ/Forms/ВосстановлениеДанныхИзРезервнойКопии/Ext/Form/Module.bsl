...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобразитьПредупреждение(ЭтотОбъект, "");
	...
	Если КоличествоСеансовИнформационнойБазы() > 1 Тогда
		...
		ПараметрыСообщения.Сообщение = НСтр("ru = 'В данный момент с приложением работают другие пользователи.
			|Их работа будет завершена автоматически.'");
		ПараметрыСообщения.Идентификатор = "ЕстьАктивныеСеансы";
		
		ПараметрыДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
		ПараметрыДействия.ИмяКоманды = "СписокАктивныхСеансов";
		ПараметрыДействия.Обработчик = "СписокАктивныхСеансов";
		ПараметрыДействия.Заголовок  = НСтр("ru = 'Список активных сеансов'");
		ПараметрыСообщения.ПараметрыДействия = ПараметрыДействия;
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.СтраницаНастройкиЗагрузки, Элементы.ГруппаОшибкаПодключения);
	КонецЕсли;
...
&НаКлиенте
Процедура СписокАктивныхСеансов(Команда)
	СтандартныеПодсистемыКлиент.ОткрытьСписокАктивныхПользователей(, ЭтотОбъект);
КонецПроцедуры
...
Процедура ГотовоПослеПроверкиДоступаКИнформационнойБазе(РезультатПодключения, Контекст) Экспорт
	...
	Если РезультатПодключения.ОшибкаПодключенияКомпоненты Тогда
		ОтобразитьПредупреждение(ЭтотОбъект, "ОшибкаПодключения");
...
#Иначе
	...
	Иначе 
		...
		Попытка
			ЧтениеФайлаАрхива = Новый ЧтениеФайлаАрхива(ИмяФайла);
		Исключение
		...
		Если ЧтениеФайлаАрхива.Элементы.Количество() <> 1 Тогда
		...
		ФайлВАрхиве = ЧтениеФайлаАрхива.Элементы[0];
...
Процедура ОтобразитьПредупреждение(Форма, Предупреждение)
	
	Если Не ЗначениеЗаполнено(Предупреждение) Тогда
		Форма.Элементы.ГруппаОшибкаПодключения.Видимость = Ложь;
	ИначеЕсли Предупреждение = "ОшибкаПодключения" Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(Форма, "ЕстьАктивныеСеансы");
		Форма.Элементы.ГруппаОшибкаПодключения.Видимость = Истина;
	КонецЕсли;
