...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолноправныйПользователь Тогда
		Элементы.ЗапуститьПовторно.Видимость = Ложь;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.ОтложенныеОбработчики.КоманднаяПанель);
	КонецЕсли;
...
Процедура ГиперссылкаОбработчикиОбновленияНажатие(Элемент)
	ОбработчикиОбновления(Неопределено);
КонецПроцедуры

&НаКлиенте
Процедура ОтложенныеОбработчикиИнформацияОПроцедуреОбновленияОткрытие(Элемент, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Элемент.ТекстРедактирования) Тогда
		Текст = Новый ТекстовыйДокумент;
		Текст.УстановитьТекст(Элемент.ТекстРедактирования);
		Текст.Показать(Заголовок + " (" + НСтр("ru = 'Сведения о процедуре обработки'") + ")");
...
&НаКлиенте
Процедура ПрогрессОтложенногоОбновления(Команда)
	ОткрытьФорму("Отчет.ПрогрессОтложенногоОбновления.Форма");
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработчикиОбновления(Команда)
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		ДобавитьОтложенныйОбработчик(Обработчик, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики,
			НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета);
	...
	Элементы.ФормаПроверитьНаличиеИсправлений.Видимость = СведенияОбОбновлении.ОтложенноеОбновлениеЗавершеноУспешно <> Истина
		И ОбновлениеИнформационнойБазыСлужебный.ДоступнаРучнаяПроверкаНаличияИсправления();
...
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики,
	...
	СтрокаСписка.Обработчик    = ?(ЗначениеЗаполнено(СтрокаОбработчика.Комментарий),
		                           СтрокаОбработчика.Комментарий + Символы.ПС + Символы.ПС + СтрокаОбработчика.ИмяОбработчика,
