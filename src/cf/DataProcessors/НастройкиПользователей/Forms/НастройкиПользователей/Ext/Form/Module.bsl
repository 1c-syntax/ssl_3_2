...
Процедура НастроитьФормуДляНеполноправногоПользователя()
	...
	Элементы.ВнешнийВидСкопироватьДругимВнешнийВид.Видимость           = Ложь;
	Элементы.ВнешнийВидКонтекстноеМенюСкопироватьДругимФормы.Видимость = Ложь;
	Элементы.НастройкиОтчетовДеревоСкопироватьДругимОтчеты.Видимость = Ложь;
	Элементы.НастройкиОтчетовДеревоКонтекстноеМенюСкопироватьДругимОтчеты.Видимость = Ложь;
	Элементы.ПрочиеНастройкиСкопироватьДругимПрочее.Видимость = Ложь;
	Элементы.ПрочиеНастройкиКонтекстноеМенюСкопироватьДругимПрочее.Видимость = Ложь;
...
Процедура Очистить(Команда)
	...
	Если ДеревоНастроек.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
...
Процедура ОчиститьНастройкиВыбраннымПользователям(Команда)
	...
	ДеревоНастроек = ТаблицаФормыВыбраннойСтраницыНастроек();
	ВыделенныеСтроки = ДеревоНастроек.ПолучитьСтрокиДляОбработки();
...
Процедура ОткрытьНастройку(Команда)
	...
	ПользователиСлужебныйКлиент.ОткрытьОтчетИлиФорму(ДеревоНастроек,
		ПользовательИнформационнойБазы, ТекущийПользовательИнформационнойБазы, ИмяФормыПерсональныхНастроек);
...
Процедура РассчитатьКоличествоНастроек()
	...
	ВсегоНастроекОтчетов = КоличествоНастроекВДереве(СписокНастроек);
	
	СписокНастроек = ВнешнийВид.ПолучитьЭлементы();
	ВсегоНастроекФорм = КоличествоНастроекВДереве(СписокНастроек);
	
	СписокНастроек = ПрочиеНастройки.ПолучитьЭлементы();
	ВсегоНастроекПрочих = КоличествоНастроекВДереве(СписокНастроек);
...
Процедура ОчиститьЗавершение(Ответ, ДеревоНастроек) Экспорт
	...
	ВыделенныеНастройки = ДеревоНастроек.ПолучитьСтрокиДляОбработки();
	...
	УдалитьНастройкиИЗДереваЗначений(ВыделенныеНастройки);
...
Процедура НастройкиПередУдалениемЗавершение(Ответ, Элемент) Экспорт
	...
	ВыделенныеСтроки = Элемент.ПолучитьСтрокиДляОбработки();
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
		...
		Возврат;
	ИначеЕсли ДеревоНастроек.ПолучитьСтрокиДляОбработки().Количество() = 1 Тогда
...
Функция ВыделенныеЭлементыНастроек()
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		ТекущееДерево = Элементы.НастройкиОтчетовДерево;
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Тогда
		ТекущееДерево = Элементы.ВнешнийВид;
	Иначе
		ТекущееДерево = Элементы.ПрочиеНастройки;
	КонецЕсли;
	
	ВыделенныеСтроки = ТекущееДерево.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0
		И ТекущееДерево.ТекущаяСтрока <> Неопределено Тогда
		ВыделенныеСтроки.Добавить(ТекущееДерево.ТекущаяСтрока);
	КонецЕсли;
...
Функция ВыделенныеНастройки()
	...
	ВыделенныеСтроки = ВыделенныеЭлементыНастроек();
	...
	Для Каждого ВыделенныйЭлемент Из ВыделенныеСтроки Цикл
