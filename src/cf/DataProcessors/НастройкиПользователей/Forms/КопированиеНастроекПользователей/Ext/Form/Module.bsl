...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		ВыбранныеПользователи.Добавить(Параметры.Пользователь);
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.ПереключательКомуКопироватьНастройки.ВидПереключателя = ВидПереключателя.Переключатель;
		Элементы.ПереключательКопируемыеНастройки.ВидПереключателя = ВидПереключателя.Переключатель;
	КонецЕсли;
...
Процедура ПользовательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВыбранныеПользователи.Количество() <> 0 Тогда
		...
		СкрываемыеПользователи.ЗагрузитьЗначения(ВыбранныеПользователи.ВыгрузитьЗначения());
	КонецЕсли;
...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если ПользовательСсылка <> Неопределено Тогда
		...
		Если ВыбранныеПользователи.НайтиПоЗначению(ПользовательСсылка) <> Неопределено Тогда
...
Процедура ПереключательКомуКопироватьНастройкиПриИзменении(Элемент)
	...
	Если ПереключательКомуКопироватьНастройки = "ВыбраннымПользователям" Тогда
		Элементы.ВыбранныеПользователи.Доступность = Истина;
	Иначе
		Элементы.ВыбранныеПользователи.Доступность = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыбранныеПользователи1НачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура Скопировать(Команда)
	...
	Если ВыбранныеПользователи.Количество() = 0 И ПереключательКомуКопироватьНастройки <> "ВсемПользователям" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
...
Процедура СкопироватьНастройки(ИмяКоманды)
	...
	Если ПереключательКомуКопироватьНастройки = "ВыбраннымПользователям" Тогда
		...
		ПояснениеКомуСкопированыНастройки = ПользователиСлужебныйКлиент.ПояснениеПользователи(
			ВыбранныеПользователи.Количество(), ВыбранныеПользователи.ВыгрузитьЗначения()[0]);
	Иначе
...
Процедура СкопироватьВыбранныеНастройки(Отчет)
	...
	Если ПереключательКомуКопироватьНастройки = "ВыбраннымПользователям" Тогда
		Приемники = ВыбранныеПользователи.ВыгрузитьЗначения();
...
Процедура ПроверитьАктивныхПользователей()
	...
	МассивПользователей = ВыбранныеПользователи.ВыгрузитьЗначения();
