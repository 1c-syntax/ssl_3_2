...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказатьПредупреждениеОЗащищенномСоединении(Истина);
...
Процедура АдресСервераПриИзменении(Элемент)
	...
	ПоказатьПредупреждениеОЗащищенномСоединении();
...
&НаСервере
Процедура ПоказатьПредупреждениеОЗащищенномСоединении(ПриСоздании = Ложь)
	
	ИдентификаторСообщения = "ЭкземплярСервераDSS";
	АдресЗаполнен = ЗначениеЗаполнено(Объект.АдресСервера);
	
	Если СтрНачинаетсяС(Объект.АдресСервера, "https://") Или Не АдресЗаполнен Тогда
		
		Если Не ПриСоздании Тогда
			ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, ИдентификаторСообщения);
		КонецЕсли;
		Элементы.ЗащищенноеСоединениеГОСТ.Видимость = АдресЗаполнен;
		
	Иначе
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.ТипСообщения = "Предупреждение";
		ПараметрыСообщения.Идентификатор = ИдентификаторСообщения;
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Рекомендуется использовать защищенное соединение (протокол https).'");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Рекомендуется протокол https'");
		ПараметрыСообщения.Скрываемый = Истина;
		...
		Элементы.ЗащищенноеСоединениеГОСТ.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПослеВыбораРасширенныхНастроек(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено И РезультатВыбора.Выполнено Тогда
