...
Функция ПолучитьСостояниеЗаявления(Программа, ИдентификаторДокументооборота, КлючПоиска = Неопределено) Экспорт
	...
	ФайлНайден = Ложь;
	Попытка
		Архив1 = Новый ЧтениеФайлаАрхива(ИмяФайлаПакетОтвета);
		...
		Для Счетчик = 0 По Архив1.Элементы.Количество() - 1 Цикл
			...
			Если Архив1.Элементы[Счетчик].Расширение = "bin" Тогда
				...
				Архив2 = Новый ЧтениеФайлаАрхива(ВременныйКаталог + Архив1.Элементы[Счетчик].Имя);
	...
	Попытка
		Архив = Новый ЧтениеФайлаАрхива(ИмяФайлаПакетОтвета);
...
Функция ПодготовитьФайлОтправки(МассивФайлов, ВременныйКаталог, ИмяФайлаПакета)
	...
	ЗаписьАрхива = Новый ЗаписьФайлаАрхива(ВременныйКаталог + ИмяФайлаПакета, , , , , УровеньСжатияФайлаАрхива.Максимальный);
