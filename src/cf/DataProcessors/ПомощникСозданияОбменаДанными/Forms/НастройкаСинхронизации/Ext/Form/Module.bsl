...
Процедура ВыборТипаТранспорта(Команда)
	...
	ПараметрыФормы = ИнициализироватьПараметрыФормыВыборТипаТранспорта();
...
&НаКлиенте
Процедура ОтобразитьПодсказку(Команда)
	
	КомандыТиповТранспорта = Новый Массив;
	КомандыТиповТранспорта.Добавить(Элементы.ВыборТипаТранспорта); 
	КомандыТиповТранспорта.Добавить(Элементы.НастроитьПараметрыПодключения);
	КомандыТиповТранспорта.Добавить(Элементы.ПолучитьПодтверждениеПодключения);
	КомандыТиповТранспорта.Добавить(Элементы.НастроитьОбщиеПараметры);
	КомандыТиповТранспорта.Добавить(Элементы.НастроитьПравилаОтправкиИПолучения);
	КомандыТиповТранспорта.Добавить(Элементы.СоздатьНачальныйОбразРИБ);
	КомандыТиповТранспорта.Добавить(Элементы.ВыполнитьСопоставлениеИЗагрузкуДанных);
	КомандыТиповТранспорта.Добавить(Элементы.ПрочитатьПервоначальныеДанные);
	КомандыТиповТранспорта.Добавить(Элементы.СопоставитьПервоначальныеДанныеПомощником);
	КомандыТиповТранспорта.Добавить(Элементы.РассмотретьПроблемыЗагрузкиПервоначальныхДанных);
	КомандыТиповТранспорта.Добавить(Элементы.ПодтвердитьДействияПриЗагрузкеДанных);
	КомандыТиповТранспорта.Добавить(Элементы.ВыполнитьНачальнуюВыгрузкуДанных);
	
	Если Элементы.ОтобразитьПодсказку.Картинка = БиблиотекаКартинок.ОтобразитьПодсказку Тогда
		Элементы.ОтобразитьПодсказку.Картинка = БиблиотекаКартинок.СкрытьПодсказку;
		Элементы.ОтобразитьПодсказку.Заголовок = НСтр("ru = 'Скрыть подсказку'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ВариантОтображенияПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Иначе
		Элементы.ОтобразитьПодсказку.Картинка = БиблиотекаКартинок.ОтобразитьПодсказку;
		Элементы.ОтобразитьПодсказку.Заголовок = НСтр("ru = 'Отобразить подсказку'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ВариантОтображенияПодсказки = ОтображениеПодсказки.Нет;
	КонецЕсли;
	
	Для Каждого КомандаТранспорта Из КомандыТиповТранспорта Цикл
		КомандаТранспорта.ОтображениеПодсказки = ВариантОтображенияПодсказки
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ОписаниеСинхронизации(Команда)
...
&НаКлиенте
Функция ИнициализироватьПараметрыФормыВыборТипаТранспорта()
...
Процедура ОбновитьОтображениеТекущегоСостоянияНастройки()
	...
	Если ПустаяСтрока(ТекущийЭтапНастройки) Тогда
		...
		Для Каждого ЭтапНастройки Из ЭтапыНастройки Цикл
			...
			Элементы[ЭтапНастройки.Группа].ОтображатьКакКарточку = Ложь;
			...
			Если ЭтапНастройки.Основное Тогда
				Элементы[ЭтапНастройки.ДекорацияСтатусаЭтапа].Картинка = БиблиотекаКартинок.ОформлениеФлажок;
			Иначе
				Элементы[ЭтапНастройки.ДекорацияСтатусаЭтапа].Картинка = БиблиотекаКартинок.Пустая;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Для Каждого ЭтапНастройки Из ЭтапыНастройки Цикл
			Если ЭтапНастройки.Название = ТекущийЭтапНастройки Тогда
				Элементы[ЭтапНастройки.Группа].Доступность = Истина;
				Элементы[ЭтапНастройки.Группа].ОтображатьКакКарточку = Истина;
				Элементы[ЭтапНастройки.ДекорацияСтатусаЭтапа].Картинка = БиблиотекаКартинок.СтартБизнесПроцесса; 
				
				ТекущийЭлемент = Элементы[ЭтапНастройки.Группа];
...
Процедура ЗаполнитьТаблицуЭтаповНастройки()
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусВыборТипаТранспорта.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусНастройкаПодключения.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусОбщиеПараметры.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусПодтверждениеПодключения.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусНастройкаПравил.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусНачальныйОбразРИБ.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусСопоставлениеИЗагрузка.Имя);
...
#Область ОбъектнаяКонвертация
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусЧтенияПервоначальныхДанных.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусСопоставленияПервоначальныхДанных.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусРассмотренияПроблемЗагрузки.Имя);
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусПодтвержденияДействийПриЗагрузке.Имя);
...
#КонецОбласти
	...
	ЭлементыФормы.Вставить("ДекорацияСтатусаЭтапа", Элементы.СтатусНачальнаяВыгрузкаДанных.Имя);
