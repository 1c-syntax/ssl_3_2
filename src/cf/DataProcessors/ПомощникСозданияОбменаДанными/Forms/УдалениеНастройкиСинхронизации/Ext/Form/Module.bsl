...
Процедура ПриЗавершенииУдаленияНастройкиСинхронизации()
	...
	Если НастройкаУдалена Тогда
		...
		// Очищаем кэш повторного использования для того, чтобы при получении узлов не получить битые ссылки на удаленные узлы
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если ПолучитьПараметрыКорреспондента Тогда
		НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаПолучениеПараметровКорреспондента");
	...
	НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаНачало", Элементы.ФормаКомандаДалее.Имя);
	...
	НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаОжидание");
	НовыйПереход.ИмяОбработчикаПриОткрытии = "Подключаемый_СтраницаОжидание_ПриОткрытии";
	
	НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаОкончание", Элементы.ФормаКомандаГотово.Имя);
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Для Каждого КомандаПанели Из КомандыПанелиФормы() Цикл
	...
	Если Не ПустаяСтрока(СтрокаПереходаТекущая.КомандыНавигации) Тогда
		...
		Для Каждого КомандаНавигации Из КомандыНавигации Цикл
			...
			Элементы[КомандаНавигации].Доступность = ДоступностьКоманд;
		КонецЦикла;
	КонецЕсли;
...
Функция КомандыПанелиФормы()
	...
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаКомандаДалее);
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаКомандаГотово);
