...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ФормаКомандаПрименить.Отображение                 = ОтображениеКнопки.Картинка;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ФормаКомандаПрименить, Элементы.ПанельВнизу);
	КонецЕсли;
...
Процедура НаселенныйПунктНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.НаселенныйПунктАдреса", ПараметрыФормы, 
		Элемент,,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		КонецЕсли;
		Элементы.АдресНаДату.РасширеннаяПодсказка.Заголовок = ТекстИсторическийАдрес;
	Иначе
		АдресДействуетС = АдресНаДату;
	КонецЕсли;
...
Процедура ПоказатьСписокДомовИлиЗемельныхУчастков(ВключитьОтбор)
	...
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		...
		ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ВыборДома", ПараметрыФормы,
			ЭтотОбъект,,,, ОповещениеОЗакрытие, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	Иначе
...
Процедура ПослеВводаЛогинИПароля(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = Неопределено Тогда
		...
		ОткрытьФорму("Обработка.РасширенныйВводКонтактнойИнформации.Форма.ЗагрузкаАдресногоКлассификатора",
			ДополнительныеПараметры.ПараметрыФормы, ЭтотОбъект,,,, Оповещение,
