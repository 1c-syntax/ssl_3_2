...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЭтоЗагрузкаСообщенияДляСопоставления() Тогда
			...
			ИмяФайлаОбмена = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов, 
...
Процедура ВнешнееСоединениеПередЗагрузкойДанных() Экспорт
	...
	Если ПараметрыАрхива <> Неопределено И ПараметрыАрхива.КоличествоФайлов > 0 Тогда
		...
		НомерПакета = 0;
		КаталогВременногоХранилищаФайлов = ОбменДаннымиСервер.КаталогВременногоХранилищаФайлов();
		ВременныйКаталог = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(КаталогВременногоХранилищаФайлов,
			"{" + Строка(Новый УникальныйИдентификатор) + "}"); //АПК: 441 временный каталог будет удалена в процедуре СобратьИПоместитьСообщениеОбменаВАрхив
