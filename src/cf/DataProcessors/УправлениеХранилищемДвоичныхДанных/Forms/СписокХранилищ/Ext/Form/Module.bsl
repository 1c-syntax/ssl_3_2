...
Процедура ДобавитьХранилищеВТаблицу(ИмяХранилища, АдресХранилища, РежимЧтенияЗаписиХДД, ХранилищеПоУмолчанию, МинимальныйРазмерДанных, ДатаОчисткиХранилища)
	...
	НоваяЗаписьОХранилище = ХранилищаДвоичныхДанных.Добавить();
	НоваяЗаписьОХранилище.ИмяХранилища = ИмяХранилища;	
	НоваяЗаписьОХранилище.АдресХранилища = АдресХранилища;

	Если РежимЧтенияЗаписиХДД = РежимЧтенияЗаписиХранилищаДвоичныхДанных.ТолькоЧтение Тогда
 		НоваяЗаписьОХранилище.РежимИспользования = НСтр("ru = 'Только чтение'", ОбщегоНазначения.КодОсновногоЯзыка());
		НоваяЗаписьОХранилище.МинимальныйРазмерДанных = "";
	Иначе 
		НоваяЗаписьОХранилище.РежимИспользования = НСтр("ru = 'Чтение и запись'", ОбщегоНазначения.КодОсновногоЯзыка());
		НоваяЗаписьОХранилище.МинимальныйРазмерДанных = МинимальныйРазмерДанных;
	КонецЕсли;

	НоваяЗаписьОХранилище.ХранилищеПоУмолчанию = ?(ХранилищеПоУмолчанию, 0, 1);	
	
	Если НоваяЗаписьОХранилище.ДатаОчисткиХранилища <> Неопределено Тогда
		НоваяЗаписьОХранилище.ДатаОчисткиХранилища = ДатаОчисткиХранилища;
	КонецЕсли;
