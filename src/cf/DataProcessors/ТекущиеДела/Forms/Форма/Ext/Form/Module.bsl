	...
	Для Каждого Дело Из ТекущиеДела Цикл
		...
		Если Дело.ЭтоРаздел Тогда
			...
			Если ТекущаяОбщаяГруппа <> ИмяОбщейГруппы Тогда
				...
				ГруппаЗаголовка.ВертикальноеПоложениеПодчиненных = ВертикальноеПоложениеЭлемента.Центр;
				...
				Исключение
					ВызватьИсключение ТекстОшибкиВыводаДела(Дело, ИнформацияОбОшибке());
				КонецПопытки;
			...
			// Создание группы дел.
			ИмяГруппы = "Группа" + Дело.ИдентификаторВладельца; // Включает все элементы дел раздела
			ИмяГруппыДела = "ГруппаДела" + ИмяГруппы; // Содержит ссылки на дела раздела
			Если ТекущаяГруппа <> ИмяГруппы Тогда
				...
				ИмяОтступа = ИмяГруппы + "ОтступГруппы";
				Элемент = Элементы.Добавить(ИмяОтступа, Тип("ДекорацияФормы"), Группа); // РасширениеПоляФормыДляПоляНадписи
				Элемент.Вид = ВидДекорацииФормы.Картинка;
				Элемент.Картинка = БиблиотекаКартинок.Пустая;
				
				ГруппаДела = Группа(ИмяГруппыДела, Группа);
				ГруппаДела.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
				
			КонецЕсли;
			
			ИмяГруппыДело = "ГруппаДело" + Дело.Идентификатор; // Включает только дело и его дочерние дела
			ГруппаДело = Группа(ИмяГруппыДело, ГруппаДела);
			ГруппаДело.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;
			...
			Попытка
				СоздатьДело(Дело, ГруппаДело, ВключенаВидимостьДела);
			Исключение
				ВызватьИсключение ТекстОшибкиВыводаДела(Дело, ИнформацияОбОшибке());
			КонецПопытки;
...
&НаСервере
Функция ТекстОшибкиВыводаДела(Дело, ИнформацияОбОшибке)
	ТекстОшибки = НСтр("ru = 'При выводе дела ""%1"" с идентификатором ""%2"" возникла ошибка:
		|%3'");
...
Процедура СоздатьДело(Дело, Группа, ВключенаВидимостьДела)
	...
	Если Дело.Важное Тогда
		Элемент.Заголовок = Новый ФорматированнаяСтрока(ЗаголовокДела, , ЦветаСтиля.ЦветОтрицательногоЧисла);
	Иначе
...
Процедура СоздатьЗаголовок(Дело, Группа, РазделСвернут)
	...
	Элемент.Шрифт = ШрифтыСтиля.ТекстТретьегоУровня;
	Элемент.РазмерКартинки = РазмерКартинки.ПоРазмеруШрифта;
	
	Если РазделСвернут = Истина Тогда
		Если Дело.ЕстьДела И Дело.Важное Тогда
			Элемент.Картинка = БиблиотекаКартинок.СтрелкаСвернутьГруппу;
			Элемент.ЦветКартинки = ЦветаСтиля.ЦветОтрицательногоЧисла;
		Иначе
			Элемент.Картинка = БиблиотекаКартинок.СтрелкаСвернутьГруппу;
			Элемент.ЦветКартинки = ЦветАвто();
		КонецЕсли;
	Иначе
		Элемент.Картинка = БиблиотекаКартинок.СтрелкаРазвернутьГруппу;
		Элемент.ЦветКартинки = ЦветАвто();
	КонецЕсли;
	...
	Иначе
		Элемент.Шрифт = ШрифтыСтиля.ЗаголовокТретьегоУровня;
	КонецЕсли;
...
Процедура СоздатьДочернееДело(Дело)
	...
	ГруппаРодитель = ЭлементДелоВладелец.Родитель; 
	ЗаголовокПодчиненногоДела = ЗаголовокПодчиненногоДела(ЭлементДелоВладелец.РасширеннаяПодсказка.Заголовок, Дело);
	
	Элемент = Элементы.Добавить(Дело.Идентификатор + Дело.ИдентификаторВладельца, Тип("ДекорацияФормы"), ГруппаРодитель); // РасширениеПоляФормыДляПоляНадписи
	Элемент.Вид = ВидДекорацииФормы.Надпись;
	Элемент.Видимость = ЭлементДелоВладелец.Видимость;
	Элемент.РастягиватьПоВертикали = Ложь;
	Элемент.РастягиватьПоГоризонтали = Истина;
	Элемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
	Элемент.Заголовок = ЗаголовокПодчиненногоДела;
	Элемент.АвтоМаксимальнаяШирина = Ложь;
	Элемент.Шрифт = ШрифтыСтиля.ТекущиеДелаЗаголовокДочернегоДела;
	Элемент.УстановитьДействие("ОбработкаНавигационнойСсылки", "Подключаемый_ОбработкаНажатияНавигационнойСсылки");
	...
	Если Дело.ЕстьДела
		...
		ИдентификаторРаздела = СтрЗаменить(ЭлементДелоВладелец.Родитель.Имя, "ГруппаДело", "");
...
Функция ЗаголовокПодчиненногоДела(ТекущийЗаголовок, Дело)
	...
	Если Дело.Важное Тогда
		ЦветДела = ЦветаСтиля.ЦветОтрицательногоЧисла;
	Иначе
		ЦветДела = ЦветАвто();
	КонецЕсли;
...
Процедура ПереместитьДело(СтрокаДело)
	...
	Если ПеремещаемыйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ГруппаДела = ПеремещаемыйЭлемент.Родитель;
	Элементы.Переместить(ГруппаДела, ГруппаДела.Родитель);
...
Процедура ЗаполнитьСвернутыеГруппы()
	...
	КартинкаСтрелкаВправо = БиблиотекаКартинок.СтрелкаСвернутьГруппу;
	КартинкаСтрелкаВправоКрасная = БиблиотекаКартинок.СтрелкаСвернутьГруппу;
	СвернутыеРазделы = Новый Соответствие;
...
Процедура ПереключитьКартинку(ИмяЭлемента)
	...
	Свернут = Ложь;
	Если Элемент.Картинка = БиблиотекаКартинок.СтрелкаРазвернутьГруппу Тогда
...
Процедура УстановитьКартинкуРазделовСВажнымиДелами()
	...
	Картинка = БиблиотекаКартинок.СтрелкаСвернутьГруппу;
	Для Каждого РазделСВажнымиДелами Из РазделыСВажнымиДелами Цикл
		...
		ЭлементКартинка.Шрифт = ШрифтыСтиля.ТекстТретьегоУровня;
		ЭлементКартинка.РазмерКартинки = РазмерКартинки.ПоРазмеруШрифта;
		ЭлементКартинка.ЦветКартинки = ЦветаСтиля.ЦветОтрицательногоЧисла;
	КонецЦикла;
