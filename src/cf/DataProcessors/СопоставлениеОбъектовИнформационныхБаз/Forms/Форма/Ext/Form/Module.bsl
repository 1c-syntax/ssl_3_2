...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДобавитьСопоставление.Видимость                          = ВыполнятьСопоставлениеДанных;
	Элементы.ОтменитьСопоставление.Видимость                          = ВыполнятьСопоставлениеДанных;
	...
	Для Каждого СтрокаСписка Из Объект.СписокИспользуемыхПолей Цикл
		Если ЗначениеЗаполнено(ПредставлениеКолонок) Тогда
			ПредставлениеКолонок = ПредставлениеКолонок + "," + СтрокаСписка.Значение;
		Иначе
			ПредставлениеКолонок = СтрокаСписка.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Элементы.ТаблицаСопоставленияПредставлениеИсточник.Заголовок = НаименованиеЭтойПрограммы + ":" + ПредставлениеКолонок;
	Элементы.ТаблицаСопоставленияПредставлениеПриемник.Заголовок = НаименованиеВторойПрограммы + ":" + ПредставлениеКолонок;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.СопоставлениеОбъектовИнформационныхБаз.Форма.НастройкаАвтоматическогоСопоставления") Тогда
		...
		ПараметрыФормы.Вставить("НаименованиеЭтойПрограммы",                   НаименованиеЭтойПрограммы);
		ПараметрыФормы.Вставить("НаименованиеВторойПрограммы",                 НаименованиеВторойПрограммы);
...
Процедура ОтменитьСопоставление(Команда)
	...
	ВыделенныеСтроки = Элементы.ТаблицаСопоставления.ПолучитьСтрокиДляОбработки();
	...
	Если ТекущиеДанные.СтатусСопоставления = -1 Или ТекущиеДанные.СтатусСопоставленияДополнительный = 1 Тогда
		
		ПоказатьПредупреждение(, НСтр("ru = 'Объекты не сопоставлены. Отменить сопоставление невозможно.'"), 2);
...
Процедура ОтменитьСопоставлениеДанных(ТекущиеДанные, ЭтоНеутверженнаяСвязь)
	...
	НоваяСтрокаПриемника.ПредставлениеПриемник = "Истина, 00-11122222, Поварешка, 80273472987";
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Возврат ?(Данные.СтатусСопоставления = -1, Данные.ПредставлениеИсточник, Данные.ПредставлениеПриемник);
...
Процедура ПослеСопоставленияОбъектов()
	...
	ИмяПоляИсточника = "ИсточникПолеNN";
	ИмяПоляПриемника = "ПриемникПолеNN";
	
	Для Каждого Строка Из ТаблицаСопоставления Цикл
		Строка.СопоставлениеПоСсылке = ОбъектыСопоставленыПоСсылке(Строка); 
		
		ПредставлениеИсточник = "";
		ПредставлениеПриемник = "";
		
		Для Каждого КолонкаТаблицы Из Объект.СписокИспользуемыхПолей Цикл
			НомерПоля = Объект.СписокИспользуемыхПолей.Индекс(КолонкаТаблицы) + 1;
			...
			Если ЗначениеЗаполнено(Строка.УникальныйИдентификаторИсточника) Тогда
				ТекущееЗначение = Строка[ПолеИсточника];
				
				ТекущееЗначение = ?(ЗначениеЗаполнено(ТекущееЗначение), ТекущееЗначение, "<...>");
				
				Если ЗначениеЗаполнено(ПредставлениеИсточник) Тогда
					ПредставлениеИсточник = ПредставлениеИсточник+ "," + ТекущееЗначение;
				Иначе
					ПредставлениеИсточник = ТекущееЗначение;
				КонецЕсли;	
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Строка.УникальныйИдентификаторПриемника) Тогда
				ТекущееЗначение = Строка[ПолеПриемника];
				
				ТекущееЗначение = ?(ЗначениеЗаполнено(ТекущееЗначение), ТекущееЗначение, "<...>");
				
				Если ЗначениеЗаполнено(ПредставлениеПриемник) Тогда
					ПредставлениеПриемник = ПредставлениеПриемник + "," +ТекущееЗначение;
				Иначе
					ПредставлениеПриемник = ТекущееЗначение;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Строка.ПредставлениеИсточник = ПредставлениеИсточник;
		Строка.ПредставлениеПриемник = ПредставлениеПриемник;
	КонецЦикла;
