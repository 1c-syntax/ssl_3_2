...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.УстановитьПериодРассчитанныхИтоговРасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.УстановитьПериодРассчитанныхИтоговРасширеннаяПодсказка.Заголовок,
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаВыбораПериода") Тогда
		...
		ПараметрыИтогов.Вставить("МассивСтрок",            Элементы.СписокИтогов.ПолучитьСтрокиДляОбработки());
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаПараметровПерестроения") Тогда
		...
		Если КонтекстВыбора = "ПерестроитьАгрегаты" Тогда
			...
			ПараметрыИтогов.Вставить("МассивСтрок",            Элементы.АгрегатыПоРегистрам.ПолучитьСтрокиДляОбработки());
...
Процедура СписокИтоговПриАктивизацииСтроки(Элемент)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.СписокИтогов.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура УстановитьПериодИтогов(Команда)
	...
	Для Каждого Индекс Из Элементы.СписокИтогов.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Только итоги'"));
...
Процедура ПолучитьОптимальныеАгрегатыКлиент()
	...
	Если ПолныеВозможности Тогда
		Если Элементы.АгрегатыПоРегистрам.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
	Иначе
		Если АгрегатыПоРегистрам.Количество() = 0 Тогда
...
Функция ПолучитьОптимальныеАгрегатыСервер()
	...
	Если Успешно > 0 Тогда
		...
		Иначе
			...
			РежимСохранения = РежимСохраненияПутейФайлаАрхива.СохранятьОтносительныеПути;
			РежимОбработки = РежимОбработкиПодкаталоговФайлаАрхива.ОбрабатыватьРекурсивно;
			ЗаписьФайлаАрхива = Новый ЗаписьФайлаАрхива(ПолноеИмяФайла);
			Для Каждого ЭлементСписка Из ФайлыДляАрхивации Цикл
				ЗаписьФайлаАрхива.Добавить(ЭлементСписка.Значение, РежимСохранения, РежимОбработки);
			КонецЦикла;
...
Процедура ОбновитьСписокИтоговНаСервере()
	...
	ИтогиВсего = СписокИтогов.Количество();
...
Процедура ОбновитьАгрегатыПоРегистрамНаСервере()
	...
	АгрегатыВсего = АгрегатыПоРегистрам.Количество();
