...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ОбработчикиОбновления.КоманднаяПанель, "ФормаДобавитьСтроку");
	КонецЕсли;
...
Процедура ЗавершениеУдаленияОписанияОбработчика(Результат, ДополнительныеСвойства) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если Элемент.ПолучитьСтрокиДляОбработки().Количество() > 0 Тогда
			УдаляемыеСтроки = Новый Массив;
			Для Каждого ИдентификаторСтроки Из Элемент.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ДобавитьСтроку(Команда)
	Элементы.ОбработчикиОбновления.ДобавитьСтроку();
КонецПроцедуры
...
Функция ВыделенныеОбработчики()
	...
	Результат = Новый Массив;
	ВыделенныеСтроки = Элементы.ОбработчикиОбновления.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0
		И Элементы.ОбработчикиОбновления.ТекущаяСтрока <> Неопределено Тогда
		ВыделенныеСтроки.Добавить(Элементы.ОбработчикиОбновления.ТекущаяСтрока);
	КонецЕсли;
...
Процедура УстановитьНомерСборкиОбработчикамНаСервере(НовыйНомерСборки)
	...
	Для Каждого ИдСтроки Из Элементы.ОбработчикиОбновления.ПолучитьСтрокиДляОбработки() Цикл
		Обработчик = Объект.ОбработчикиОбновления.НайтиПоИдентификатору(ИдСтроки);
