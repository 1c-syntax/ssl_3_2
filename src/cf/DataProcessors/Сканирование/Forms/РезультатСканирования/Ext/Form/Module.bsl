...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПараметрыЗаданияСканирования <> Неопределено Тогда
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Сканирование завершилось аварийно. При возникновении затруднений обратитесь в службу поддержки фирмы ""1С"".'");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Сканирование завершилось аварийно'");
		ПараметрыСообщения.Идентификатор = "ОшибкаСканирования";
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Обработка.Сканирование.Форма.ОшибкаСканирования") Тогда
		Если РаботаСФайламиСлужебныйКлиент.БылаОшибкаСканирования() Тогда
			ВывестиВажноеСообщение();
		Иначе
			ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОшибкаСканирования");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если Не ПоказыватьДиалогСканера Тогда
		...
		Если СохранятьВPDF Тогда
			...
			Представление = Представление + НСтр("ru = 'Будет сохранен как:'") + " ";
		...
		Представление = Представление + НСтр("ru = 'Будет сохранен как многостраничный:'") + " ";
	...
	Элементы.Картинка.РасширеннаяПодсказка.Заголовок = Представление;
	...
	Если Источник = "TWAIN" И Событие = "ImageAcquired" Тогда
		...
		Если ТаблицаФайлов.Количество() > 1 И Элементы.ТаблицаФайлов.Видимость = Ложь Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				Элементы.ФормаПринятьВсеКакОдинФайл.Важность = ВажностьКнопки.Главная;
			Иначе
