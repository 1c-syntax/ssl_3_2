...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТипОрганизацииСтрока = ?(ЭтоФизическоеЛицо, "ФизическоеЛицо", "Организация");
...
&НаКлиенте
Процедура ТипОрганизацииСтрокаПриИзменении(Элемент)
	
	ВыбранныйЭлемент = Элемент.СписокВыбора.НайтиПоЗначению(ТипОрганизацииСтрока);
	Если ВыбранныйЭлемент = Неопределено Или Не ЭтоФизическоеЛицо И ВыбранныйЭлемент.Значение = "Организация"
...
&НаКлиенте
Процедура ОбновитьСписокПриложений(Команда)
	ПодключитьОбработчикОжидания("ПриИзмененииСоставаИлиНастроекПрограмм", 0.1, Истина);
КонецПроцедуры
...
Процедура ПослеЗаписи()
	...
	ИначеЕсли ПолученныйСертификатУстановлен Тогда
...
Процедура УстановитьВидимостьИДоступность()
	...
	Если СертификатКУстановке Тогда
		Если ДанныеЗаявления.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.ИсполненоРаспискаНеОтправлена
			...
			Элементы.ПодписатьИОтправитьСертификат.Видимость = Не ТолькоПросмотрЗаявления;
			Элементы.УстановитьСертификат.Видимость = Ложь;
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				Элементы.ПодписатьИОтправитьСертификат.Важность = ?(Не ТолькоПросмотрЗаявления,
					ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
			Иначе
				Элементы.ПодписатьИОтправитьСертификат.КнопкаПоУмолчанию = Не ТолькоПросмотрЗаявления;
			КонецЕсли;
			Если Не ТолькоПросмотрЗаявления Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.УстановитьСертификат.Видимость = Не ТолькоПросмотрЗаявления;
			Элементы.ПодписатьИОтправитьСертификат.Видимость = Ложь;
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				Элементы.УстановитьСертификат.Важность = ?(Не ТолькоПросмотрЗаявления,
					ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
			Иначе
				Элементы.УстановитьСертификат.КнопкаПоУмолчанию = Не ТолькоПросмотрЗаявления;
			КонецЕсли;
...
Процедура УстановитьОтображениеРеквизитовЗависимыхОтВидаПродления()
	...
	Элементы.ДекорацияУсловияСоглашения.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Нажимая кнопку <span style=""font: ЗаявлениеНаВыпускСертификатаТекстУсловийСоглашенияПолужирный"">%1</span>, вы принимаете
		|<a href = ""%2"">условия соглашения</a> о присоединении к <a href = ""%3"">Порядку удостоверяющего центра</a>.'"),
			Элементы.ОтправитьЗаявление.Заголовок, "УсловияСоглашения", "ПорядокУдостоверяющегоЦентра");
...
Функция ТекущееЗначениеРеквизита(Реквизит, РеквизитТип, ИмяПараметраФормы = "")
	...
	Если ЗначениеЗаполнено(ИмяПараметраФормы) И Параметры.Свойство(ИмяПараметраФормы)
		И ЗначениеЗаполнено(Параметры[ИмяПараметраФормы]) И Реквизит = Параметры[ИмяПараметраФормы] Тогда
		Возврат Реквизит;
	КонецЕсли;
...
Процедура ПриИзмененииВладельцаСертификатаНаСервере(Загрузка = Ложь)
	...
	Элементы.ТипОрганизацииСтрока2.Доступность = Не СозданиеПоПараметруФизическоеЛицо;
...
Процедура УстановитьОтображениеРеквизитовЗависимыхОтОрганизации()
	...
	Элементы.ТипОрганизацииСтрока.Доступность = Не СозданиеПоПараметруОрганизация;
	...
	Элементы.ГруппаТипОрганизации.Видимость = ПереключениеДоступно;
	...
	Элементы.ГруппаТипОрганизации2.Видимость = ЭтоФизическоеЛицо И ПереключениеДоступно;
...
Процедура УстановитьОтображениеРеквизитовЗависимыхОтВладельца()
	...
	ВладелецВведенВручную = ЗначениеЗаполнено(СтрокаФИО)
		И (Не ЗначениеЗаполнено(Сотрудник) Или ЗначениеЗаполнено(Сотрудник) И Строка(Сотрудник) <> СтрокаФИО);
...
Процедура ОбработатьДокументыНаОтправку(Знач ДокументыНаОтправку)
	...
	Для Каждого Документ Из ДокументыНаОтправку Цикл
		...
		Архив.Добавить(ИмяВременногоФайла, РежимСохраненияПутейФайлаАрхива.НеСохранятьПути);
	КонецЦикла;
	
	ДвоичныеДанныеАрхива = Архив.ПолучитьДвоичныеДанные(, , , МетодСжатияФайлаАрхива.Сжатие, УровеньСжатияФайлаАрхива.Максимальный);
...
Процедура ПроверитьДоступностьОтправки(ЭтоПроверкаПередОтправкой = Ложь)
	...
	Если Не ЭтоФизическоеЛицо И Не ЭтоИндивидуальныйПредприниматель
		...
		ОткрытьФорму("Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.Форма.ИнформацияОДоступностиВыпуска",
			ПараметрыФормы, ЭтотОбъект, Истина,,,ОписаниеОповещения, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура ОтправитьЗаявлениеВБумажномВидеЗавершение(Результат, Контекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки.Текст = УстановитьГиперссылку(ОписаниеОшибки.Текст, "its.1c.ru", "https://its.1c.ru");
...
Процедура ПодтвердитьДанныеЗаявленияПослеСозданияКлючаИЗапросаНаСертификатОбработчикОжидания()
	...
	ОткрытьФорму("Обработка.ЗаявлениеНаВыпускНовогоКвалифицированногоСертификата.Форма.Информация",
		ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Функция ПакетЗаявления(Знач ОписанияФайловПакета, Знач ТипТранзакции)
	...
	ЗаписьАрхива = Новый ЗаписьФайлаАрхива(ИмяФайлаПакета, , , , , УровеньСжатияФайлаАрхива.Максимальный);
...
Функция ТикетАутентификацииНаПорталеПоддержки(ОписаниеОшибки)
	...
	Результат = МодульИнтернетПоддержкаПользователей.ТикетАутентификацииНаПорталеПоддержки("https://regservice.1c.ru");
...
Процедура ОтобразитьПоискПриложений(ИдетПоиск, ЭтоТокен = Ложь)
	...
	Элементы.ОбновитьСписокПриложений.Видимость = Не ИдетПоиск;
		...
		Иначе
			...
			Для каждого ЭлементСписка Из Контекст.СписокИмен Цикл
				...
				Если УникальноеИмя = ЭлементСписка.Представление
					...
					// ViPNet 3.2 (основной каталог контейнеров не указан).
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Сохранение сертификатов во временный каталог отменено.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Или ПолученныеФайлы.Количество() <> 2 Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Сертификаты не были сохранены во временный каталог.'");
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		...
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru = 'Не удалось найти личный сертификат в каталоге ""Личное""
...
Процедура ПослеУстановкиСертификатаНаСервере(Установлен)
	...
	Если Установлен Тогда
		...
		ПолученныйСертификатУстановлен = Истина;
		Если ДоступныНапоминанияПользователя Тогда 
			ЭлектроннаяПодписьСлужебный.ИзменитьОтметкуОНапоминании(
...
Процедура ОтобразитьИнформациюОТарифах()
	...
	Если ТолькоПросмотрЭлементов Тогда
		...
		Иначе
			...
			Элементы.ПредставлениеТарифа.ТолькоПросмотр = Истина;
		КонецЕсли;
...
Функция СсылкаНаДоверенность()
	...
	Возврат "https://ca.1c.ru/doc/doverennost_p3.rtf";
...
Функция СвойстваУЦ(ВариантУдостоверяющегоЦентра)
	...
	Иначе
		...
		СвойстваУЦ.АдресПорядкаУЦ           = НСтр("ru = 'https://ca.1c.ru/reglament.pdf'");
...
Процедура ЗаполнитьСпискиВыбораТипов()
	...
	// Организация
	Элементы.ТипОрганизацииСтрока.СписокВыбора.Добавить("Организация",    НСтр("ru = 'Организация'"));
	Элементы.ТипОрганизацииСтрока.СписокВыбора.Добавить("ФизическоеЛицо", НСтр("ru = 'Физическое лицо'"));
	
	// Владелец сертификата
	Элементы.ТипОрганизацииСтрока2.СписокВыбора.Добавить("Организация",    НСтр("ru = 'Организация'"));
	Элементы.ТипОрганизацииСтрока2.СписокВыбора.Добавить("ФизическоеЛицо", НСтр("ru = 'Физическое лицо'"));
...
Процедура НастроитьРаботуВТакси()
	...
	ШрифтЗаголовковДляТакси = ШрифтыСтиля.ЗаголовокЧетвертогоУровня;
	
	Элементы.ДекорацияПаспорт.Шрифт = ШрифтЗаголовковДляТакси;
	Элементы.ДекорацияСНИЛС.Шрифт = ШрифтЗаголовковДляТакси;
	Элементы.ДекорацияДоверенность.Шрифт = ШрифтЗаголовковДляТакси;
