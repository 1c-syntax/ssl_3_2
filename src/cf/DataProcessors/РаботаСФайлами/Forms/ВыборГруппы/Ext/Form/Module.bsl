...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КоманднаяПанель, "ФормаВыбрать");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	НастроитьВидимостьКнопкиСоздатьГруппу();
КонецПроцедуры
...
&НаКлиенте
Процедура НастроитьВидимостьКнопкиСоздатьГруппу()
	Проверка = Новый Структура("СписокСоздатьГруппу");
	ЗаполнитьЗначенияСвойств(Проверка, Элементы);
	Если Проверка.СписокСоздатьГруппу <> Неопределено Тогда
		Проверка.СписокСоздатьГруппу.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
