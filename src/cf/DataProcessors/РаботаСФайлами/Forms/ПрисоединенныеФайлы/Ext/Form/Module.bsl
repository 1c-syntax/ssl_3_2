...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Не СписокОткрытИзКарточкиФайла И Не Параметры.ОткрытиеИзПанелиНавигации Тогда
...
Процедура Отправить(Команда)
	...
	ПриОтправкеФайловПочтой(ПараметрыОтправки, Элементы.Список.ПолучитьСтрокиДляОбработки(), ВладелецФайла, УникальныйИдентификатор);
	
	РаботаСФайламиСлужебныйКлиент.ОтправитьФайлыПоПочте(
		Элементы.Список.ПолучитьСтрокиДляОбработки(), УникальныйИдентификатор, ПараметрыОтправки);
...
Процедура СравнитьФайлы(Команда)
	...
	ЧислоВыделенныхСтрок = Элементы.Список.ПолучитьСтрокиДляОбработки().Количество();
	
	Если ЧислоВыделенныхСтрок = 2 Тогда
		
		Ссылка1 = Элементы.Список.ПолучитьСтрокиДляОбработки()[0];
		Ссылка2 = Элементы.Список.ПолучитьСтрокиДляОбработки()[1];
...
Процедура ПеренестиВГруппу(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		...
		ПараметрыОткрытия.Вставить("ПеремещаемыеФайлы", ВыделенныеСтроки);
...
Процедура Подписать(Команда)
	...
	Элементы.Список.ПолучитьСтрокиДляОбработки();
	
	РаботаСФайламиКлиент.ПодписатьФайл(Элементы.Список.ПолучитьСтрокиДляОбработки(), УникальныйИдентификатор,
...
Процедура РедактироватьФайлы(ВыбранныеСтрокиИзСпискаФайлов)
	...
	Для Каждого ИдентификаторСтроки Из ВыбранныеСтрокиИзСпискаФайлов Цикл
		...
		ТекущиеДанные = ТекущиеДанные(ИдентификаторСтроки);
		 
		РедактируетДругойПользователь = ТекущиеДанные.ФайлРедактируется
			И Не ТекущиеДанные.ФайлРедактируетТекущийПользователь;
		Если РедактируетДругойПользователь Тогда
			ЗанятыеФайлы.Добавить(ТекущиеДанные.Ссылка);
			Продолжить;
		КонецЕсли;
		
		Если ТекущиеДанные.Зашифрован Или ТекущиеДанные.ПодписанЭП Тогда
			Продолжить;
		КонецЕсли;
	...
	ТекстПредупреждения = ТекстПредупрежденияОЗанятыхФайлах(ЗанятыеФайлы);
	ПоказатьПредупреждение(, ТекстПредупреждения);
	
КонецПроцедуры

&НаКлиенте
Функция ТекстПредупрежденияОЗанятыхФайлах(ЗанятыеФайлы)
	
	КоличествоЗанятых = ЗанятыеФайлы.Количество();
	
	Если КоличествоЗанятых = 1 Тогда
		Возврат СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Файл <b>%1</b> недоступен для редактирования (занят другим пользователем).
				 |Доступен только просмотр.'"), ЗанятыеФайлы[0]);
	ИначеЕсли КоличествоЗанятых = 2 Тогда
		Возврат СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Файлы <b>%1</b> и <b>%2</b> недоступны для редактирования (заняты другими пользователями).
				 |Доступен только просмотр.'"), ЗанятыеФайлы[0], ЗанятыеФайлы[1]);
	Иначе
		КоличествоОстальных = КоличествоЗанятых - 1;
		Возврат СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Файл <b>%1</b> и другие (%2) недоступны для редактирования (заняты другими пользователями).
				 |Доступен только просмотр.'"), ЗанятыеФайлы[0], КоличествоОстальных);
	КонецЕсли;
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ИначеЕсли МассивФайлов.Количество() = 1 Тогда 
		...
		ДанныеСтроки             = ДанныеСтрокиСписка(МассивФайлов[0]);
...
&НаКлиенте
Процедура УстановитьДоступностьФайловыхКоманд(Строка = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт
	
	ТекущиеДанные = ТекущиеДанные(Строка);
...
Функция ДоступныеКоманды(ТекущиеДанныеФайла, ФайлыРедактируютсяВОблачномСервисе, ВозможностьОсвободитьФайл, 
	...
	Если ФайлРедактируется Тогда
		...
		Иначе
			ИменаКоманд["ЗакончитьРедактирование"] = Ложь Или ЭтоВариантИнтерфейса85;
			Если Не ВозможностьОсвободитьФайл Тогда
				ИменаКоманд["Освободить"] = Ложь Или ЭтоВариантИнтерфейса85;
			КонецЕсли;
			ИменаКоманд["Редактировать"] = Ложь Или ЭтоВариантИнтерфейса85;
		КонецЕсли;
		
		ИменаКоманд["Занять"] = Ложь Или ЭтоВариантИнтерфейса85;
...
Процедура ОбновитьПояснениеОблачногоСервиса()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации <> Неопределено  Тогда
			...
			СтатусСинхронизации = СтроковыеФункции.ФорматированнаяСтрока(
...
&НаКлиенте
Функция ТекущиеДанные(Строка = Неопределено)
	Перем Результат;
	Если СтандартныеПодсистемыКлиент.ЭтоЭлементДинамическогоСписка(Элементы.Список) Тогда
		...
		Иначе
			Результат = Элементы.Список.ДанныеСтроки(Строка);
		КонецЕсли;
	КонецЕсли;
	Если Результат <> Неопределено Тогда
		Если Результат.ВладелецФайла <> ВладелецФайла Тогда
			Результат = Неопределено;
		КонецЕсли;
	КонецЕсли;	
	Возврат Результат;
