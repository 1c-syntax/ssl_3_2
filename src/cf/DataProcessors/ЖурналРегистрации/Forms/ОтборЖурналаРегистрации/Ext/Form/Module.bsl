...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ГруппаДаты, ЭтотОбъект, Элементы.ГруппаШапка);
		Элементы.Переместить(Элементы.Комментарий, Элементы.ГруппаДанные);
		Элементы.ГруппаДаты.Объединенная = Истина;
		Элементы.ГруппаДанныеСвойства.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		Элементы.Комментарий.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
		СтандартныеПодсистемыСервер.СброситьШиринуЭлементовГруппы(Элементы.ГруппаОтборДанных);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыОтбора()
	...
	ИначеЕсли ЕстьОтборПоСтатусу Или ЗначениеЗаполнено(Транзакция) Тогда
		Элементы.ГруппаПрочее.ЗаголовокСвернутогоОтображения = Элементы.ГруппаПрочее.Заголовок + " *";
	КонецЕсли;
