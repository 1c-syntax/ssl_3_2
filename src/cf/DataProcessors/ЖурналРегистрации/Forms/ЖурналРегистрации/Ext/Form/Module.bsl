...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		ОтображатьКоманднуюПанель = Ложь;
		Элементы.ЖурналГруппаКомандФормы.Видимость = Истина;
		Элементы.Журнал2ГруппаКомандФормы.Видимость = Истина;
	КонецЕсли;
...
Процедура ЖурналПриАктивизацииПоля(Элемент)
	...
	Элементы.УстановитьОтборПоЗначению2.Доступность
		= ДоступенОтборПоЗначениюВТекущейКолонке;
...
Процедура УстановитьОтборПоЗначению(Команда)
	...
	УстановитьОтборПоЗначениюВТекущейКолонке(КолонкиИсключения, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтключитьОтборПоКолонке(Команда)
	
	КолонкиИсключения = Новый Массив;
	УстановитьОтборПоЗначениюВТекущейКолонке(КолонкиИсключения, Ложь);
...
&НаКлиенте
Процедура УстановитьОтборПоЗначениюВТекущейКолонке(КолонкиИсключения, Установить)
	
	Элемент = ЭлементЖурнал();
	Если Элемент.ТекущийЭлемент = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Выберите колонку. Для этого выделите ячейку таблицы.'"));
		Возврат;
	КонецЕсли;
	...
	ТекущиеДанные = ?(Установить, Элемент.ТекущиеДанные, Null);
	
	Если ЖурналРегистрацииКлиент.УстановитьОтборПоЗначениюВТекущейКолонке(ТекущиеДанные,
