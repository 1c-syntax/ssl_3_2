...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Комментарий) Тогда
		Элементы.Комментарий.МногострочныйРежим = Ложь;
		Элементы.Комментарий.РастягиватьПоВертикали = Ложь;
		Элементы.Комментарий.Высота = 1;
		Элементы.Комментарий.КнопкаОткрытия = Ложь;
	КонецЕсли;
	
	Если СтрЧислоСтрок(ПредставлениеДанных) < 2 Тогда
		Элементы.ПредставлениеДанных.МногострочныйРежим = Ложь;
	КонецЕсли;
	...
	Если Событие = "_$Access$_.Access" Тогда
		...
		ИначеЕсли ДанныеСобытия <> Неопределено Тогда
			СоздатьТаблицуФормы("ТаблицаДанных", "ТаблицаДанных", Новый Структура);
		КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Основное.РастягиватьПоГоризонтали = Истина;
...
Процедура СоздатьТаблицуФормы(Знач ИмяПоляТаблицыФормы, Знач ИмяРеквизитаДанныеФормыКоллекция, Знач ТаблицаЗначений)
	...
	Если ТипЗнч(ТаблицаЗначений) = Тип("Структура")
	 Или ТипЗнч(ТаблицаЗначений) = Тип("ФиксированнаяСтруктура") Тогда
		...
		ДеревоЗначений = ЖурналРегистрации.ДеревоИзДанныхСтруктуры(ТаблицаЗначений, КоличествоСтрок);
	...
	Если КоличествоСтрок < 7 Тогда
		Элементы[ИмяПоляТаблицыФормы].ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ПоСодержимому;
	КонецЕсли;
	...
	Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
		...
		ЭлементРеквизита.АвтоШиринаВТаблице = АвтоШиринаВТаблице.Нет;
	КонецЦикла;
...
Процедура УстановитьВидимостьГруппКромеТаблицы(Видимость)
	...
	Элементы.Основное.РастягиватьПоГоризонтали = Не Видимость;
	Элементы.ГруппаОсновное.Видимость = Видимость;
	Элементы.ГруппаСобытиеКомментарий.Видимость = Видимость;
	Элементы.СтандартныеДанныеСвойства.Видимость = Видимость;
	Элементы.ГруппаПрочее.Видимость = Видимость;
