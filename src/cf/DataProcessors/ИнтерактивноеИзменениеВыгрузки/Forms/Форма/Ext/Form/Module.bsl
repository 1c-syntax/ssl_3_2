...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Настройки.Видимость = ПравоДоступа("СохранениеДанныхПользователя", Метаданные);
	
	СброситьНадписьКоличествТаблицы();
	ОбновитьНадписьОбщегоКоличества();
	
	СоздатьОбновитьКомандыВариантовНастроекОтборов();
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДополнительнаяРегистрация
...
Процедура ДополнительнаяРегистрацияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьНастройкуОтбора(СтандартнаяОбработка);
...
Процедура ДополнительнаяРегистрацияПередУдалением(Элемент, Отказ)      
	...
	Выделенные = Элементы.ДополнительнаяРегистрация.ПолучитьСтрокиДляОбработки();
	...
	Оповещение.ДополнительныеПараметры.Вставить("СтрокиДляОбработки", Выделенные);
...
Процедура ОбновитьКоличествоКлиент(Команда)
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
			Строка.Количество = НСтр("ru='Идет расчет'");
		КонецЦикла;
...
Процедура НастройкаОтбора(Команда)
	...
	ОткрытьНастройкуОтбора(Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьТекущуюНастройку(Команда)
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗавершениеЗакрытияФормыНастроекОтборов", ЭтотОбъект);
	
	ОткрытьФорму(ИмяФормыНастроек, ПараметрыФормыНастроек, Элементы.ДополнительнаяРегистрация,,,, ОповещениеОЗакрытии);
...
Процедура ДополнительнаяРегистрацияПередУдалениемЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого ИдентификаторСтроки Из ДополнительныеПараметры.СтрокиДляОбработки Цикл
...
Процедура ЗагрузитьЗначенияКоличествСервер()
	...
	Если СтрокаДанных = Неопределено Тогда
		ОбновитьНадписьОбщегоКоличества(Неопределено);
	Иначе
		ОбновитьНадписьОбщегоКоличества(СтрокаДанных.РегистрацияДополнительно, СтрокаДанных.РегистрацияАвтоматически + СтрокаДанных.РегистрацияПоСсылке);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьНадписьОбщегоКоличества(Количество = Неопределено, ЗарегистрированоНаУзле = Неопределено) 
	
	ОстановитьРасчетКоличества();
	
	Если Количество = Неопределено Тогда
		ТекстКоличества = НСтр("ru = 'Будет отправлено: <не рассчитано>'");
	Иначе
		ТекстКоличества = НСтр("ru = 'Будет отправлено: %1 + по общим правилам: %2'");
		ТекстКоличества = СтрЗаменить(ТекстКоличества, "%1", Формат(Количество, "ЧН=0"));
		ТекстКоличества = СтрЗаменить(ТекстКоличества, "%2", Формат(ЗарегистрированоНаУзле, "ЧН=0"));
	КонецЕсли;
	
	Элементы.ДекорацияБудетОтправлено.Заголовок = ТекстКоличества;
...
&НаКлиенте
Процедура ОткрытьНастройкуОтбора(СтандартнаяОбработка)
	...
	Если Элементы.ДополнительнаяРегистрация.ТекущаяСтрока = Неопределено Тогда
		ПараметрыФормы.Вставить("ДействиеВыбора", - Объект.ДополнительнаяРегистрация.Индекс(ТекущиеДанные));
	Иначе
...
&НаКлиенте
Процедура ЗавершениеЗакрытияФормыНастроекОтборов(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура СоздатьОбновитьКомандыВариантовНастроекОтборов()
	
	СписокВариантов = ПрочитатьСписокВариантовНастроекСервер();
	
	СоздатьКомандыВариантовНастроекОтборов(СписокВариантов);
	УдалитьКомандыВариантовНастроекОтборов(СписокВариантов);
	
КонецПроцедуры

&НаСервере
Процедура СоздатьКомандыВариантовНастроекОтборов(СписокВариантов)
	
	Для Каждого СтрокаТаблицы Из СписокВариантов Цикл
		Найденные = СписокКомандВариантовНастроекОтборов.НайтиСтроки(Новый Структура("ИмяВарианта", СтрокаТаблицы.Значение));
		
		Если Найденные.Количество() Тогда
			Продолжить;
		КонецЕсли;
		
		ИдентификаторКоманды = СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", "");
		
		ИмяКоманды = "КомандаВариантаНастройкиОтборов_" + ИдентификаторКоманды;
		
		Команда = Команды.Добавить(ИмяКоманды);
		Команда.Действие = "Подключаемый_ЗагрузитьВариантНастройкиОтборов";
		
		Кнопка = Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элементы.ВариантыНастроекОтборов);
		Кнопка.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		Кнопка.ИмяКоманды = ИмяКоманды;
		Кнопка.Заголовок = СтрокаТаблицы.Значение;
		
		НоваяСтрока = СписокКомандВариантовНастроекОтборов.Добавить();
		НоваяСтрока.ИмяКоманды = ИмяКоманды;
		НоваяСтрока.ИмяВарианта = СтрокаТаблицы.Значение;
	КонецЦикла; 
	
КонецПроцедуры

&НаСервере
Процедура УдалитьКомандыВариантовНастроекОтборов(СписокВариантов)
	
	Если СписокКомандВариантовНастроекОтборов.Количество() Тогда		
		КомандыНаУдаление = Новый Массив;
		
		Для Каждого СтрокаТаблицы Из СписокКомандВариантовНастроекОтборов Цикл
			НайденныеСтроки = СписокВариантов.НайтиПоЗначению(СтрокаТаблицы.ИмяВарианта);
			
			Если НайденныеСтроки = Неопределено Тогда
				КомандыНаУдаление.Добавить(СтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
		
		Если КомандыНаУдаление.Количество() Тогда
			Для Каждого ЭлементМассива Из КомандыНаУдаление Цикл
				КомандаФормы = Команды.Найти(ЭлементМассива.ИмяКоманды);
				
				Если КомандаФормы <> Неопределено Тогда
					Команды.Удалить(КомандаФормы);
				КонецЕсли;
				
				ЭлементКоманды = Элементы.Найти(ЭлементМассива.ИмяКоманды);
				
				Если ЭлементКоманды <> Неопределено Тогда
					Элементы.Удалить(ЭлементКоманды);
				КонецЕсли;
				
				СписокКомандВариантовНастроекОтборов.Удалить(ЭлементМассива);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
