...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВывестиВажноеСообщение(РаботаСВнешнимиРесурсамиЗаблокирована);
	...
	ЗаполнитьОтборПоСостоянию(Элементы.ОтборСостояние.СписокВыбора);
	Элементы.ТаблицаРегламентныеЗадания.ОтборСтрок = ОтборСтрокТаблицы();
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.ТаблицаРегламентныеЗадания.КоманднаяПанель);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "РазрешенаРаботаСВнешнимиРесурсами" Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "РаботаСВнешнимиРесурсамиЗапрещена");
...
Процедура ВидОтбораПоПериодуПриИзменении(Элемент)
	...
	Элементы.ГруппаОтборИнтервал.ТолькоПросмотр  = НЕ (ВидОтбораПоПериоду = 4);
	
	Если ВидОтбораПоПериоду = 0 Тогда
		ДатаНачала = '00010101';
		ДатаОкончания = '00010101';
		Элементы.ГруппаОтборИнтервал.Доступность = Ложь;
	ИначеЕсли ВидОтбораПоПериоду = 4 Тогда
		ДатаНачала  = НачалоДня(ТекущаяДатаСеанса);
		ДатаОкончания = ДатаНачала;
		Элементы.ГруппаОтборИнтервал.Доступность = Истина;
	Иначе
...
Процедура РегламентноеЗаданиеДляОтбораОчистка(Элемент, СтандартнаяОбработка)
	...
	РегламентноеЗаданиеДляОтбораИдентификатор = "ВсеЗадания";
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ПолучитьСтрокиДляОбработки().Количество() > 1 Тогда
...
&НаКлиенте
Процедура Подключаемый_РазблокироватьРаботуСВнешнимиРесурсами(Команда)
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	ВыделенныеСтроки = Элементы.ТаблицаРегламентныеЗадания.ПолучитьСтрокиДляОбработки();
	Индекс = 0;
...
Процедура НастроитьРасписание(Команда)
	...
	ВыбранныеСтроки = Элементы.ТаблицаРегламентныеЗадания.ПолучитьСтрокиДляОбработки();
	...
	ИначеЕсли ВыбранныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите одно регламентное задание.'"));
	Иначе
		ТекущиеДанные = ТаблицаРегламентныеЗадания.НайтиПоИдентификатору(ВыбранныеСтроки[0]);
...
Процедура ВключитьРегламентноеЗадание(Команда)
	...
	УстановитьИспользованиеРегламентногоЗадания(Истина, Элементы.ТаблицаРегламентныеЗадания.ПолучитьСтрокиДляОбработки());
...
Процедура ВыключитьРегламентноеЗадание(Команда)
	...
	УстановитьИспользованиеРегламентногоЗадания(Ложь, Элементы.ТаблицаРегламентныеЗадания.ПолучитьСтрокиДляОбработки());
...
Процедура ПоказатьВсеЗадания(Команда)
	НовоеЗначение = Не Элементы.ТаблицаРегламентныеЗаданияПоказатьВсеЗадания.Пометка;
	Элементы.ТаблицаРегламентныеЗаданияПоказатьВсеЗадания.Пометка = НовоеЗначение;
	
	Если НовоеЗначение Тогда
		Элементы.ТаблицаРегламентныеЗадания.ОтборСтрок = Неопределено;
	Иначе
		Элементы.ТаблицаРегламентныеЗадания.ОтборСтрок = ОтборСтрокТаблицы();
	КонецЕсли;
	
	Если ВсегоРегламентных > 0 Тогда
		Если НовоеЗначение Тогда
			ВсегоРегламентных = ВсегоРегламентных + ОтключеноЗаданий;
		Иначе
			ВсегоРегламентных = ВсегоРегламентных - ОтключеноЗаданий;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ВывестиВажноеСообщение(РаботаСВнешнимиРесурсамиЗаблокирована)
	
	Если РаботаСВнешнимиРесурсамиЗаблокирована Тогда
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение  = НСтр("ru = 'Работа со всеми внешними ресурсами (синхронизация данных, отправка почты и т.п.), выполняемая по расписанию, заблокирована для предотвращения конфликтов с основной информационной базой.'");
		ПараметрыСообщения.Идентификатор = "РаботаСВнешнимиРесурсамиЗапрещена";
		
		ПараметрыСообщения.ПараметрыДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
		ПараметрыСообщения.ПараметрыДействия.ИмяКоманды = "РазблокироватьРаботуСВнешнимиРесурсами";
		ПараметрыСообщения.ПараметрыДействия.Заголовок = НСтр("ru = 'Разблокировать'");
		ПараметрыСообщения.ПараметрыДействия.Обработчик = "Подключаемый_РазблокироватьРаботуСВнешнимиРесурсами";
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщения);
		ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиФормы(Знач Настройки)
	...
	Если Настройки.Получить("ОтборСостояние") = Неопределено
		Или Настройки.Получить("ОтборСостояние").Количество() = 0 Тогда
		Настройки.Вставить("ОтборСостояние", Новый СписокЗначений);
		ЗаполнитьОтборПоСостоянию(Настройки["ОтборСостояние"], Ложь);
	КонецЕсли;
	
	Если Настройки.Получить("РегламентноеЗаданиеДляОтбораИдентификатор") = Неопределено Тогда
		Настройки.Вставить("РегламентноеЗаданиеДляОтбораИдентификатор", "ВсеЗадания");
	КонецЕсли;
	...
	Элементы.ГруппаОтборИнтервал.Доступность = (ВидОтбораПоПериоду = 4);
	Элементы.ГруппаОтборИнтервал.ТолькоПросмотр  = НЕ (ВидОтбораПоПериоду = 4);
...
Процедура ОбновитьСписокВыбораРегламентногоЗадания()
	...
	Если Список.Количество() = 0 Тогда
		Список.Добавить("ВсеЗадания", НСтр("ru = 'Все задания'"));
...
Процедура ОбновитьАвтоматическийПериод(Форма, ТекущаяДатаСеанса)
	...
	Если Форма.ВидОтбораПоПериоду = 1 Тогда
		Форма.ДатаНачала  = НачалоДня(ТекущаяДатаСеанса) - 3*3600;
		Форма.ДатаОкончания = НачалоДня(ТекущаяДатаСеанса) + 9*3600;
		
	ИначеЕсли Форма.ВидОтбораПоПериоду = 2 Тогда
		Форма.ДатаНачала  = НачалоДня(ТекущаяДатаСеанса) - 24*3600;
		Форма.ДатаОкончания = КонецДня(Форма.ДатаНачала);
		
	ИначеЕсли Форма.ВидОтбораПоПериоду = 3 Тогда
		Форма.ДатаНачала  = НачалоДня(ТекущаяДатаСеанса);
		Форма.ДатаОкончания = КонецДня(Форма.ДатаНачала);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьИспользованиеРегламентногоЗадания(Включено, ВыделенныеСтроки)
...
Процедура ОбработатьРезультат(ПараметрыЗадания)
	...
	ВсегоРегламентных = ТаблицаРегламентныеЗадания.Количество();
	ОтключеноЗаданий  = ТаблицаРегламентныеЗадания.НайтиСтроки(Новый Структура("Отключено", Истина)).Количество();
	Если ВсегоРегламентных > 0 Тогда
...
Функция ОтборФоновыхЗаданий()
	...
	Если ОтборСостояние.НайтиПоЗначению("Выполняется") <> Неопределено Тогда 
		МассивСостояний.Добавить(СостояниеФоновогоЗадания.Активно);
	КонецЕсли;
	
	Если ОтборСостояние.НайтиПоЗначению("Выполнено") <> Неопределено Тогда
		МассивСостояний.Добавить(СостояниеФоновогоЗадания.Завершено);
	КонецЕсли;
	
	Если ОтборСостояние.НайтиПоЗначению("ЗавершеноСОшибками") <> Неопределено Тогда
		МассивСостояний.Добавить(СостояниеФоновогоЗадания.ЗавершеноАварийно);
	КонецЕсли;
	
	Если ОтборСостояние.НайтиПоЗначению("Отменено") <> Неопределено Тогда
	...
	Если РегламентноеЗаданиеДляОтбораИдентификатор <> "ВсеЗадания" Тогда
...
Процедура ОбновитьТаблицуФоновыхЗаданий(АдресРезультата = Неопределено)
	...
	КонецЕсли;
	ВсегоФоновых = ТаблицаФоновыеЗадания.Количество();
...
&НаКлиентеНаСервереБезКонтекста
Функция ОтборСтрокТаблицы()
	
	ОтборСтрок = Новый Структура;
	ОтборСтрок.Вставить("Отключено", Ложь);
	
	Возврат Новый ФиксированнаяСтруктура(ОтборСтрок);
	
КонецФункции

&НаСервере
Процедура ЗаполнитьОтборПоСостоянию(Список, СписокВыбора = Истина)
	
	Список.Очистить();
	Если СписокВыбора Тогда
		Список.Добавить("Выполнено",          НСтр("ru = 'Выполнено'"));
		Список.Добавить("ЗавершеноСОшибками", НСтр("ru = 'Завершено с ошибками'"));
		Список.Добавить("Отменено",           НСтр("ru = 'Отменено'"));
	КонецЕсли;
	Список.Добавить("Выполняется", НСтр("ru = 'Выполняется'"));
