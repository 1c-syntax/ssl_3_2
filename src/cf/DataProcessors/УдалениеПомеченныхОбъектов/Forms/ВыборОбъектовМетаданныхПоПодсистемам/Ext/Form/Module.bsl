...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.ДеревоРазделовПоиска.КоманднаяПанель);  
		
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ДеревоРазделовПоиска.КоманднаяПанель,
			"КомандаВыбрать");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьЗаголовокОбластиПоиска();
...
Процедура ДеревоРазделовПоискаПометкаПриИзменении(Элемент)
	...
	ЭлементДерева = Элементы.ДеревоРазделовПоиска.ТекущиеДанные;
...
&НаКлиенте
Процедура УстановитьЗаголовокОбластиПоиска()
	
	Если ОбластьПоискаВезде() Тогда
		Заголовок = НСтр("ru='Область поиска (везде)'");
	Иначе
		Заголовок = НСтр("ru='Область поиска (по разделам)'");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция ОбластьПоискаВезде()
	
	ЕстьНевыбранныеЭлементы = Ложь;
	ЕстьВыбранныеЭлементы   = Ложь;
	Для Каждого ВерхнийЭлементДерева Из ДеревоРазделовПоиска.ПолучитьЭлементы() Цикл
		
		Если ВерхнийЭлементДерева.Пометка = 2 Тогда
			Возврат Ложь;
		ИначеЕсли ВерхнийЭлементДерева.Пометка = 0 Тогда
			ЕстьНевыбранныеЭлементы = Истина;
		ИначеЕсли ВерхнийЭлементДерева.Пометка = 1 Тогда
			ЕстьВыбранныеЭлементы = Истина;
		КонецЕсли;
		
		Если ЕстьВыбранныеЭлементы И ЕстьНевыбранныеЭлементы Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Если (ЕстьНевыбранныеЭлементы И Не ЕстьВыбранныеЭлементы)
	 Или (ЕстьВыбранныеЭлементы И Не ЕстьНевыбранныеЭлементы) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Для каждого ОбъектПодсистемы Из СоставПодсистемы Цикл
		...
		Если ОбщегоНазначения.ЭтоСправочник(ОбъектПодсистемы)
			...
			Если ОбъектМетаданныхДоступен(ОбъектПодсистемы) Тогда
				...
				Если НоваяСтрокаОбъект.Пометка Тогда
					ПометитьЭлементыРодителейРекурсивно(НоваяСтрокаОбъект);
				КонецЕсли;
...
Процедура ПометитьЭлементыРодителейРекурсивно(ЭлементДерева)
	...
	Иначе
		Родитель = ЭлементДерева.Родитель; // СтрокаДереваЗначений
	КонецЕсли;
	...
	Иначе
		ЭлементыРодителя = Родитель.Строки; // СтрокаДереваЗначений
	КонецЕсли;
...
Функция СостояниеВложенныхЭлементов(ЭлементДерева)
	...
	Если ТипЗнч(ЭлементДерева) = Тип("ДанныеФормыЭлементДерева") Тогда
		ВложенныеЭлементы = ЭлементДерева.ПолучитьЭлементы();
	Иначе
		ВложенныеЭлементы = ЭлементДерева.Строки; // СтрокаДереваЗначений
	КонецЕсли;
