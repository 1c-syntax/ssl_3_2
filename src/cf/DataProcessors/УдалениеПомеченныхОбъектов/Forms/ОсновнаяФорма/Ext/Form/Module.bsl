...
&НаКлиенте
Процедура СсылкаПодробнееНажатие(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "report" Тогда
...
&НаКлиенте
Процедура МестаИспользованияНеудаленныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если Поле.Имя = Элементы.СвязиНеУдаленныхДействие.Имя Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Элемент", Элемент);
		Оповещение = Новый ОписаниеОповещения("МестаИспользованияНеудаленныхВыборЗавершение",
			ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВыборИзСписка(Оповещение, Поле.СписокВыбора, Элемент);
	Иначе
		ПоказатьОбъектТаблицы(Элемент);
	КонецЕсли;
...
&НаКлиенте
Процедура МестаИспользованияНеудаленныхВыборЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	
	Если ВыбранноеЗначение <> Неопределено Тогда
		СвязиНеУдаленныхДействиеОбработкаВыбора(ДополнительныеПараметры.Элемент,
			ВыбранноеЗначение.Значение, Ложь);
	КонецЕсли;
...
Процедура УстановитьЗаменитьНа(Команда)
	...
	Если ТекущиеДанные.ОсновнаяПричина Тогда
		...
		ОткрытьФорму(ПутьКФорме, ПараметрыФормы, Элементы.МестаИспользованияНеудаленных,,,,,
...
Процедура Настроить(Команда)
	...
	ОткрытьФорму("Обработка.УдалениеПомеченныхОбъектов.Форма.НастройкиУдаленияОбъектов", ПараметрыФормы, ЭтотОбъект, , , ,
		ОповещениеОЗакрытии, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
КонецПроцедуры
...
&НаКлиенте
Процедура УдалитьВажноеСообщение(Команда)
	ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СведенияОРезультатахУдаления");
...
Процедура УстановитьСостояниеВыбораПомеченныхНаУдаление()
	...
	Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Доступность = ДеревоПомеченныхНаУдаление.ПолучитьЭлементы().Количество() > 0;
	...
	Элементы.ВыполнитьДействияИУдалить.Видимость = Ложь;
	Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Видимость = Истина;
...
&НаСервере
Процедура ВывестиИнформационноеСообщение(Состояние, ТекстСообщения = "")
	
	Элементы.СтраницыОтображениеСостояния.ТекущаяСтраница = Элементы.ИнформацияОСостоянии;
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.Идентификатор = "СведенияОРезультатахУдаления";
	ВывестиСообщение = Ложь;
	
	Если Состояние = "УдалениеУспешноЗавершено" Тогда
		ВывестиСообщение = Истина;
		ПараметрыСообщения.ТипСообщения = "Успех";
		ПараметрыСообщения.Идентификатор = "СведенияОРезультатахУдаления";
		ПараметрыСообщения.Сообщение  = НСтр("ru = 'Удаление успешно завершено.'");
		ПараметрыСообщения.ОбработчикЗакрытия = "УдалитьВажноеСообщение";
	ИначеЕсли Состояние = "УдалениеНеТребуется" Тогда
		ВывестиСообщение = Истина;
		ПараметрыСообщения.ТипСообщения = "Информация";
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Удаление не требуется.
			|Нет объектов, помеченных на удаление.'");
		ПараметрыСообщения.ОбработчикЗакрытия = "УдалитьВажноеСообщение";
	ИначеЕсли Состояние = "ЧастичноеУдаление" Тогда
		ВывестиСообщение = Истина;
		ПараметрыСообщения.ТипСообщения = "Информация";
	...
	Если ВывестиСообщение Тогда
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ИнформацияОСостоянии);
...
Процедура УстановитьСостояниеНеуспешноеУдаление()
	...
	Элементы.ВыполнитьДействияИУдалить.Видимость = Истина;
	Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Видимость = Ложь;
	Элементы.СтраницыОтображениеСостояния.Видимость = Ложь;
	Элементы.СтраницыИнформация.ТолькоПросмотр = Ложь;
	Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Доступность = ДеревоПомеченныхНаУдаление.ПолучитьЭлементы().Количество() > 0;
...
Процедура ПриПодтвержденииУдалить(Результат, ДополнительныеПараметры) Экспорт
	...
	Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Доступность = Ложь;
...
Процедура НачатьПоискПомеченных_НастройкаФормы(Форма)
	...
	Форма.Элементы.ДеревоПомеченныхНаУдалениеУдалитьВыбранные.Доступность = Ложь;
...
Процедура ПослеЗавершенияПоискаПомеченных(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполняетсяЗадание = Ложь;
	Если Результат.Статус = "Ошибка" Тогда
		ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(Результат);
		УстановитьСостояниеВыбораПомеченныхНаУдалениеСПанельюСостояния();
	Иначе
		...
		Если ДеревоПомеченныхНаУдаление.ПолучитьЭлементы().Количество() = 0 Тогда
			...
			УстановитьСостояниеВыбораПомеченныхНаУдалениеСПанельюСостояния();
			ВывестиИнформационноеСообщение("УдалениеНеТребуется");
			ОчиститьОчередьЗаданийФормы();
		Иначе
			УстановитьСостояниеВыбораПомеченныхНаУдаление();
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(Знач Результат)
	
	ИнформацияОбОшибкеФоновогоЗадания = Результат.ИнформацияОбОшибке;
	ТекстСообщения = ?(ЭтоОшибкаУстановкиМонопольногоРежима(Результат.ИнформацияОбОшибке),
	...
	ТекстСообщения = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru='%1
		|<a href=""report"">Сформировать отчет об ошибке</a>'"), ТекстСообщения);
	...
	ПараметрыСообщения.Сообщение = ТекстСообщения;
	ПараметрыСообщения.ОбработчикЗакрытия = "УдалитьВажноеСообщение"; 
	ПараметрыСообщения.Обработчик = "СсылкаПодробнееНажатие";
...
Процедура ОбработатьРезультатВыполненияУдаления(СведенияОРезультатахУдаления)
	...
	Если НеУдаленные.ПолучитьЭлементы().Количество() > 0 Тогда
		...
		ВывестиИнформационноеСообщение("ЧастичноеУдаление", ТекстСообщения);
		
	ИначеЕсли СведенияОРезультатахУдаления.УдаленныеКоличество > 0 Тогда 
		
		УстановитьСостояниеВыбораПомеченныхНаУдалениеСПанельюСостояния();
		ВывестиИнформационноеСообщение("УдалениеУспешноЗавершено");
