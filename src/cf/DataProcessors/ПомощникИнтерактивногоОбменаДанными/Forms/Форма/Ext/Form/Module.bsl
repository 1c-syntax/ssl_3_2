...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДополнениеВыгрузкиВариантВыгрузки.Доступность = ДополнитьДанными;
	Элементы.УказатьПериод.Заголовок = Строка(ДополнениеВыгрузки.ПериодОтбораСценарияУзла);
	Элементы.ДополнениеВыгрузкиОтбор.Заголовок = ДополнениеВыгрузки.ПредставлениеОтбораСценарияУзла;
...
&НаКлиенте
Процедура ДополнитьДаннымиПриИзменении(Элемент)
	
	Элементы.ДополнениеВыгрузкиВариантВыгрузки.Доступность = ДополнитьДанными;
	
	Если Не ДополнитьДанными Тогда
		
		ДополнениеВыгрузки.ВариантВыгрузки = 0;
		
	ИначеЕсли ДополнениеВыгрузки.ПараметрыСценарияДополнения.ВариантДополнительно.Использование = Истина Тогда
		
		ДополнениеВыгрузки.ВариантВыгрузки = Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора[0].Значение;
		
	ИначеЕсли ДополнениеВыгрузки.ПараметрыСценарияДополнения.ВариантДополнительно.Использование = Ложь Тогда
		
		ДополнениеВыгрузки.ВариантВыгрузки = Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора[1].Значение;
...
Процедура УказатьПериод(Команда)
	...
	ДиалогПериода.Период = ДополнениеВыгрузки.ПериодОтбораСценарияУзла;
	ДиалогПериода.Показать(Новый ОписаниеОповещения("ЗавершениеВыбораПериода", ЭтотОбъект));
...
&НаКлиенте
Процедура ДеревоИнформацииСтатистикиПриНаведении(Элемент, Строка, Поле)
	
	Если Строка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаДерева = ДеревоИнформацииСтатистики.НайтиПоИдентификатору(Строка);
	
	РодительСтрокиДерева = СтрокаДерева.ПолучитьРодителя();
	
	ЭтоРодитель = РодительСтрокиДерева = Неопределено;
	
	Элементы.ОткрытьФормуСопоставления.Доступность        = Не ЭтоРодитель;
	Элементы.ВыполнитьЗагрузкуДанныхДляСтроки.Доступность = Не ЭтоРодитель;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	ВыделенныеСтроки = Элементы.ДеревоИнформацииСтатистики.ПолучитьСтрокиДляОбработки();
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	ДополнениеВыгрузкиИсторияОтборовДействиеКоманды(Элементы.ДополнениеВыгрузкиИсторияОтборов, 2); 
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнениеВыгрузкиИсторияОтборовВариантДополнения(Команда)
	
	ДополнениеВыгрузкиИсторияОтборовДействиеКоманды(Элементы.ДополнениеВыгрузкиИсторияОтборовВариантДополнения, 3); 
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнениеВыгрузкиОтбор(Команда)
...
&НаКлиенте
Процедура Подключаемый_ЗагрузитьВариантНастройкиОтборов(Команда)
	
	НайденныеСтроки = СписокКомандВариантовНастроекОтборов.НайтиСтроки(Новый Структура("ИмяКоманды", Команда.Имя));
	
	Если Не НайденныеСтроки.Количество() Тогда
		Возврат;
	КонецЕсли;  
	
	ПредставлениеНастройки = НайденныеСтроки[0].ИмяВарианта;
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	ДоступностьКоманд = Не (ЭтоПереходДалее И СтрокаПереходаТекущая.ДлительнаяОперация);
	
	Для Каждого КомандаПанели Из Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы Цикл
		Если КомандаПанели <> Элементы.Справка Тогда
			КомандаПанели.Видимость = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	КомандыНавигации = СтрРазделить(СтрокаПереходаТекущая.ИменаКомандНавигации, ",");
	
	Для Каждого КомандаНавигации Из КомандыНавигации Цикл
		Элементы[КомандаНавигации].Видимость = Истина;
		Элементы[КомандаНавигации].Доступность = Доступность;
	КонецЦикла;
	
	Заголовок = СтрокаПереходаТекущая.ЗаголовокФормы;
...
Процедура ТаблицаПереходовНоваяСтрока(
	...
	НоваяСтрока.ИменаКомандНавигации    = ИменаКомандНавигации;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиВопросНесопоставленные(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(ДополнительныеПараметры.ПолучитьСтрокиДляОбработки());
...
Процедура ОбработатьОшибкуРасхожденияВерсий()
	...
	Для Каждого КомандаНавигации Из Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы Цикл
		Если КомандаНавигации.Важность = ВажностьКнопки.Главная Тогда
			КомандаНавигации.Видимость = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	Элементы.ПанельОсновная.ТекущаяСтраница             = Элементы.СтраницаОшибкаРасхожденияВерсий;
	Элементы.КомандаПродолжитьСинхронизацию.Видимость     = Истина;
...
Процедура ОбновитьДоступностьКомандПереходаИнформацииСтатистики()
	...
	Элементы.КомандаСохранитьИЗакрыть.Доступность = Не ЗакончитьСопоставлениеДанных Или Не ЗагрузитьСообщениеПослеСопоставления;	
	Элементы.КомандаДалее.Доступность = ЗакончитьСопоставлениеДанных И ЗагрузитьСообщениеПослеСопоставления;
...
Процедура ИнициализироватьРеквизитыДополненияВыгрузки()
	...
	Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора.Очистить();
	Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора.Добавить(2, НСтр("ru = 'Произвольными данными'"));
	
	ПредставлениеВариантаДокументами = НСтр("ru = 'Документами'");
	
	Если ПараметрыСценарияДополнения.ВариантДополнительно.Использование Тогда
		
		Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора.Добавить(3, ПредставлениеВариантаДокументами);
		
	ИначеЕсли ПараметрыСценарияДополнения.ВариантВсеДокументы.Использование Тогда
		
		Элементы.ДополнениеВыгрузкиВариантВыгрузки.СписокВыбора.Добавить(1, ПредставлениеВариантаДокументами);
...
Процедура ДополнениеВыгрузкиВариантВыгрузкиУстановитьВидимость()
	...
	Если ДополнениеВыгрузки.ВариантВыгрузки = 0 Тогда
		Элементы.СтраницыОтборы.ТекущаяСтраница = Элементы.СтраницаТиповойВариантДополненияНет;
	ИначеЕсли ДополнениеВыгрузки.ВариантВыгрузки = 1 Тогда
		Элементы.СтраницыОтборы.ТекущаяСтраница = Элементы.СтраницаТиповойВариантДополненияДокументы;
	ИначеЕсли ДополнениеВыгрузки.ВариантВыгрузки = 2 Тогда	
		Элементы.СтраницыОтборы.ТекущаяСтраница = Элементы.СтраницаТиповойВариантДополненияПроизвольный;
	ИначеЕсли ДополнениеВыгрузки.ВариантВыгрузки = 3 Тогда	
		Элементы.СтраницыОтборы.ТекущаяСтраница = Элементы.СтраницаНастраиваемыйВариантДополнения;	
	КонецЕсли;
...
&НаСервере 
Функция ДополнениеВыгрузкиИсторияНастроекСервер(ВариантДополнения = Неопределено)
	
	Возврат ОбменДаннымиСервер.ИнтерактивноеИзменениеВыгрузкиИсторияНастроек(ДополнениеВыгрузки, ВариантДополнения);
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеПолучитьСообщениеОбменаВоВременныйКаталогНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных)
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьАвтоматическоеСопоставлениеДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных)
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьЗагрузкуДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных)
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьВыгрузкуДанныхНаСервере(ПараметрыЗадания, ПолучитьПарольИзСеансовыхДанных)
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	ИмяКомандыСохранитьИЗакрыть       = Элементы.КомандаСохранитьИЗакрыть.Имя;
	ИмяКомандыПродолжитьСинхронизацию = Элементы.КомандаПродолжитьСинхронизацию.Имя;
	ИмяКомандыДалее  				  = Элементы.КомандаДалее.Имя;
	ИмяКомандыГотово 				  = Элементы.КомандаГотово.Имя;
	ИмяКомандыОтмена 				  = Элементы.КомандаОтмена.Имя;
	ИмяКомандыДополнениеВыгрузкиСоставВыгрузки = Элементы.ДополнениеВыгрузкиСоставВыгрузки.Имя;
	...
	Если ОбменМеждуПриложениямиВСервисе Тогда
		...
		Если ПолученоСообщениеДляСопоставленияДанных Тогда
			...
			ТаблицаПереходовНоваяСтрокаДлительнаяОперация("СтраницаОжиданияАнализаДанных", ИмяКомандыОтмена, Истина,
			...
			Если НЕ Параметры.ОбъектнаяКонвертацияXDTO Тогда
				...
				ШаблонЗаголовка = НСтр("ru = 'Сопоставление данных ""%1""'");
				ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НаименованиеКорреспондента);
				
				// Сопоставление данных пользователем.
				ТаблицаПереходовНоваяСтрока("СтраницаИнформацииСтатистики", ИмяКомандыСохранитьИЗакрыть + "," + ИмяКомандыДалее, 
					"Подключаемый_СтраницаИнформацииСтатистики_ПриОткрытии",
					"Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее", ЗаголовокФормы);
				
			КонецЕсли;
			
			// Загрузка данных.
			ТаблицаПереходовНоваяСтрокаДлительнаяОперация(ИмяСтраницыСинхронизацияЗагрузка, ИмяКомандыОтмена, Истина,
		...
		ИначеЕсли ОтправитьДанные Тогда
			...
			Если РежимДополненияВыгрузки Тогда
				...
				КомандыНавигации = ИмяКомандыДалее + "," + ИмяКомандыДополнениеВыгрузкиСоставВыгрузки;
				
				ШаблонЗаголовка = НСтр("ru = 'Данные для отправки ""%1""'");
				ЗаголовокФормыСоставВыгрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НаименованиеКорреспондента);
				
				ТаблицаПереходовНоваяСтрока("СтраницаВопросОСоставеВыгрузки", КомандыНавигации, "Подключаемый_СтраницаВопросОСоставеВыгрузки_ПриОткрытии",, ЗаголовокФормыСоставВыгрузки);
				
			КонецЕсли;
			
			// Выгрузка и загрузка данных.
			ТаблицаПереходовНоваяСтрокаДлительнаяОперация(ИмяСтраницыСинхронизацияВыгрузка, ИмяКомандыОтмена, Истина,
	...
	Иначе
		...
		Если ПолучитьДанные Тогда
			...
			Если НЕ Параметры.ОбъектнаяКонвертацияXDTO Тогда
				...
				// Сопоставление данных пользователем.
				Если ПолученоСообщениеДляСопоставленияДанных Тогда 				
					КомандыНавигации = ИмяКомандыГотово + "," + ИмяКомандыДалее;
					
					ТаблицаПереходовНоваяСтрока("СтраницаИнформацииСтатистики", КомандыНавигации, 
						"Подключаемый_СтраницаИнформацииСтатистики_ПриОткрытии",
						"Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее", ЗаголовокФормы);
				Иначе
					ТаблицаПереходовНоваяСтрока("СтраницаИнформацииСтатистики", ИмяКомандыДалее, 
					"Подключаемый_СтраницаИнформацииСтатистики_ПриОткрытии",
					"Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее", ЗаголовокФормы);
				КонецЕсли;
		...
		Если ОтправитьДанные Тогда
			...
			Если ОбменЧерезВнутреннююПубликацию Тогда
				ТаблицаПереходовНоваяСтрока("СтраницаПроверкаЗадачОбмена", ИмяКомандыДалее, 
			...
			Если РежимДополненияВыгрузки Тогда
				...
				ТаблицаПереходовНоваяСтрокаДлительнаяОперация("СтраницаРегистрацияДанных", ИмяКомандыОтмена, Истина,
	...
	// Итоги.
	ТаблицаПереходовНоваяСтрока("СтраницаОкончанияСопоставления", ИмяКомандыГотово, "Подключаемый_СтраницаОкончанияСопоставления_ПриОткрытии");
...
&НаКлиенте
Процедура ЗавершениеВыбораПериода(Результат, ДополнительныеПараметры) Экспорт 
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДополнениеВыгрузки.ПериодОтбораСценарияУзла = Результат;
	
	Если ЗначениеЗаполнено(Результат) Тогда
		Элементы.УказатьПериод.Заголовок = Строка(Результат);
	Иначе
		Элементы.УказатьПериод.Заголовок = НСтр("ru='Период не выбран'");
	КонецЕсли;
...
&НаКлиенте
Процедура ДополнениеВыгрузкиИсторияОтборовДействиеКоманды(КомандаФормы, ВариантДополнения)
	
	// Организуем выбор из меню-списка, все варианты сохраненных настроек.
	СписокВариантов = ДополнениеВыгрузкиИсторияНастроекСервер(ВариантДополнения);
	...
	ПоказатьВыборИзМеню(ОписаниеОповещения, СписокВариантов, КомандаФормы);
