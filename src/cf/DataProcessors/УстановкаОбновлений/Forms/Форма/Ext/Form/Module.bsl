...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьНезавершенныеОбработчики    = ЗначениеЗаполнено(СтатусНевыполненныхОбработчиков);
	ЕстьПроблемыВеденияУчета        = ЕстьПроблемыСистемныхПроверок();
	ЕстьРасширения                  = ОбновлениеКонфигурации.ПредупреждатьОНаличииРасширений();
	...
	Элементы.ДекорацияПоискИУстановкаОбновлений.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы");
	...
	УстановитьВидимостьБаннеров(Объект.РежимОбновления);
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Результат.Поддерживается
		...
		Элементы.СтраницыОсновное.ТекущаяСтраница = Элементы.СтраницаНедоступно;
...
&НаКлиенте
Процедура ДекорацияПоискИУстановкаОбновленийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура НадписьОтложенныеОбработчикиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "Подробнее" Тогда
		Заголовок = НСтр("ru = 'Дополнительные процедуры обработки данных'");
		ТекстСообщения = НСтр("ru = 'При обновлении приложения на новый исправительный релиз (изменилась 4-я цифра версии, например, 3.1.1.ХХ) можно не дожидаться завершения дополнительных процедур обработки данных и продолжить обновление.
			|Если же выполняется обновление на новый функциональный релиз (изменилась 1-я, 2-я или 3-я цифра номера версии), то рекомендуется дождаться полного завершения процедур обработки данных. В новых функциональных релизах состав данных процедур может измениться и некоторые из них могут быть удалены, из-за чего они не будут выполнены и может произойти потеря некоторых данных.
			|Если какие-то дополнительные процедуры обработки данных завершились с ошибкой, то рекомендуется обратиться в службу технической поддержки и после исправления ошибки выполнить обновление на новый исправительный релиз.'");
		ОбщегоНазначенияКлиент.ПоказатьФормуПроизвольногоСообщения(Заголовок, ТекстСообщения);
	Иначе
...
Процедура НадписьВыявленныеПроблемыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "Подробнее" Тогда
		СтандартнаяОбработка = Ложь;
		Заголовок = НСтр("ru = 'Проблемы ведения учета'");
		ТекстСообщения = СтрЗаменить(НСтр("ru = 'Одна из частых причин различных блокирующих ошибок при обновлении на новую версию приложения - это проблемы ведения учета,
			|такие как, битые ссылки на несуществующие объекты, незаполненные обязательные реквизиты документов и другие.
			|В таком случае рекомендуется устранять их перед началом обновления, для этого перейти к отчету и следовать приведенным в нем рекомендациям.'"), Символы.ПС, " ");
		ОбщегоНазначенияКлиент.ПоказатьФормуПроизвольногоСообщения(Заголовок, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольВеденияУчета") Тогда
		...
		Если Информация.ПредупреждатьОНеобходимостиПовторнойПроверки Тогда
			Если ЗначениеЗаполнено(Информация.ДатаПоследнейПроверки) Тогда
				ПоследняяПроверкаУчета = НСтр("ru = 'Последняя проверка выполнялась %1.'");
				ПоследняяПроверкаУчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПоследняяПроверкаУчета, 
					Формат(Информация.ДатаПоследнейПроверки, "ДЛФ=Д"));
			Иначе
				ПоследняяПроверкаУчета = "";
			КонецЕсли;
			ПроверкаУчетаДавноНеВыполнялась = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередОткрытиемСтраницы(Знач НоваяСтраница = Неопределено)
	...
	УстановитьВидимостьБаннеров(Объект.РежимОбновления, НоваяСтраница.Имя);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьБаннеров(РежимОбновления, ИмяТекущейСтраницы = Неопределено, ИнформацияОСоединениях = Неопределено)
	Если ИмяТекущейСтраницы = Неопределено Тогда
		ТекущаяСтраница = Элементы.СтраницыПомощника.ТекущаяСтраница;
	Иначе
		ТекущаяСтраница = Элементы[ИмяТекущейСтраницы];
	КонецЕсли;
	
	ЭтоВыборРежимаОбновленияФайл   = (ТекущаяСтраница = Элементы.ВыборРежимаОбновленияФайл);
	ЭтоВыборРежимаОбновленияСервер = (ТекущаяСтраница = Элементы.ВыборРежимаОбновленияСервер);
	
	Элементы.ПанельАктивныеПользователи.Видимость       = ЭтоВыборРежимаОбновленияФайл И ЕстьАктивныеСоединения;
	Элементы.ПанельАктивныеПользователиСервер.Видимость = ЭтоВыборРежимаОбновленияСервер И ЕстьАктивныеСоединения И РежимОбновления = 0;
	
	Элементы.ПанельПредупреждений.Видимость = Элементы.ПанельАктивныеПользователи.Видимость
		Или Элементы.ПанельАктивныеПользователиСервер.Видимость;
	
	Если ЕстьНезавершенныеОбработчики Тогда
		СообщениеОбновление = СтрЗаменить(НСтр("ru = 'В приложении есть незавершенные <a href=""ОтложенныеОбработчики"">дополнительные процедуры обработки данных</a>,
			|рекомендуется дождаться их выполнения до обновления. <a href=""Подробнее"">Подробнее</a>.'"), Символы.ПС, " ");
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(СообщениеОбновление);
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Обновление не завершено'");
		ПараметрыСообщения.Идентификатор = "ОбновлениеНеЗавершено";
		ПараметрыСообщения.Обработчик = "НадписьОтложенныеОбработчикиОбработкаНавигационнойСсылки";
		ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОбновлениеНеЗавершено");
	КонецЕсли;
	
	Если ЕстьПроблемыВеденияУчета Тогда
		СообщениеПроверкаУчета = НСтр("ru = 'В приложении выявлены <a href=""Ссылка"">проблемы ведения учета</a>, которые рекомендуется устранить до обновления. <a href=""Подробнее"">Подробнее</a>.'");
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(СообщениеПроверкаУчета);
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Проблемы проверки учета'");
		ПараметрыСообщения.Идентификатор = "ПроблемыВеденияУчета";
		ПараметрыСообщения.Обработчик = "НадписьВыявленныеПроблемыОбработкаНавигационнойСсылки";
		ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПроблемыВеденияУчета");
	КонецЕсли;
	
	Если ЕстьРасширения Тогда
		СообщениеРасширения = НСтр("ru = 'Перед началом обновления убедитесь, что <a href=""Ссылка"">установленные расширения</a> совместимы с новой версией конфигурации.
			|Рекомендуем предварительно выполнить обновление на резервной копии.'");
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(СообщениеРасширения);
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Требуется проверка расширений'");
		ПараметрыСообщения.Идентификатор = "ТребуетсяПроверкаРасширений";
		ПараметрыСообщения.Обработчик = "НадписьЕстьРасширенияОбработкаНавигационнойСсылки";
		ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ТребуетсяПроверкаРасширений");
	КонецЕсли;
	
	Если ЭтоВыборРежимаОбновленияСервер Тогда
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.ТипСообщения = "Информация";
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Перед установкой обновления рекомендуется выполнить резервное копирование информационной базы.'");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Выполните резервное копирование'");
		ПараметрыСообщения.Идентификатор = "РекомендуетсяРезервноеКопирование";
		ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "РекомендуетсяРезервноеКопирование");
	КонецЕсли;
	
	Если ПроверкаУчетаДавноНеВыполнялась Тогда
		СообщениеПроверкаУчета = НСтр("ru = 'Рекомендуется выполнить <a href=""Ссылка"">системные проверки</a> до обновления.'");
		Если ЗначениеЗаполнено(ПоследняяПроверкаУчета) Тогда
			СообщениеПроверкаУчета = СообщениеПроверкаУчета + Символы.ПС + ПоследняяПроверкаУчета;
		КонецЕсли;
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.ТипСообщения = "Информация";
		ПараметрыСообщения.Заголовок = НСтр("ru = 'Проверка ведения учета давно не выполнялась'");
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(СообщениеПроверкаУчета);
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Проверка учета не актуальна'");
		ПараметрыСообщения.Идентификатор = "ПроверкаВеденияУчетаНеАктуальна";
		ПараметрыСообщения.Обработчик = "НадписьИнформацияОбработкаНавигационнойСсылки";
		ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПроверкаВеденияУчетаНеАктуальна");
	КонецЕсли;
...
Процедура ОбновитьИнформациюОНаличииСоединений(ТекущаяСтраница = Неопределено)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборРежимаОбновленияСервер Тогда
		...
		Если НаличиеСоединений Тогда
			ВсеСтраницы = Элементы.ПанельАктивныеПользователиСервер.ПодчиненныеЭлементы;
			Элементы.ПанельАктивныеПользователиСервер.ТекущаяСтраница = ? (СоединенияИнфо.НаличиеCOMСоединений, 
	...
	ЕстьАктивныеСоединения = ?(СоединенияИнфо = Неопределено, Ложь, СоединенияИнфо.НаличиеАктивныхСоединений);
	УстановитьВидимостьБаннеров(Объект.РежимОбновления, ТекущаяСтраница.Имя, СоединенияИнфо);
...
Процедура ПослеВыбораИнтервалаУведомленияПользователей(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонЗаголовка = НСтр("ru = '<a href=""Ссылка"">за %1 минут</a>'");
