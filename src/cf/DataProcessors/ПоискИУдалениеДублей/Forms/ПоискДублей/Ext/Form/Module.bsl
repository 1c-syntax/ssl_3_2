...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШагПоиск.КнопкаОтмена.Видимость = Ложь;
	//
	// 2. Длительный поиск.
...
&НаКлиенте
Процедура ОписаниеСостоянияНайденныхДублейОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылкаФорматированнойСтроки = "СпособУдаления" Тогда
...
&НаКлиенте
Процедура ВсеМестаИспользованияНеобработанных(Команда)
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Или ДанныеСтроки.Основной Тогда
		...
		Элементы.ОписаниеТекущейГруппыНеобработанныхДублей.Заголовок = НСтр("ru = 'Для просмотра причин выберите проблемный элемент-дубль.'");
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		МестаИспользованияНеобработанных.Очистить();
		Элементы.ОписаниеТекущейГруппыНеобработанныхДублей.Заголовок = 
...
&НаКлиенте
Процедура НеобработанныеДублиПриНаведении(Элемент, Строка, Поле)
	
	КомандаВидна = Ложь;
	Если ТипЗнч(Строка) = Тип("Число") Тогда
		СтрокаТаблицы = НеобработанныеДубли.НайтиПоИдентификатору(Строка);
	...
	Элементы.НеобработанныеДублиОткрытьНеобработанныйДубль.Видимость = КомандаВидна;
...
Процедура УстановитьОтборВозможныхДублей(Форма)
	...
	Если Форма.ОтображаемаяСтепеньРодства = 1 Тогда
		...
		Форма.Элементы.ВозможныеДублиПоказатьВсе.Пометка = Истина;
		Форма.Элементы.ВозможныеДублиПоказатьВажные.Пометка = Ложь;
	Иначе
		Форма.Элементы.ВозможныеДубли.ОтборСтрок = Новый ФиксированнаяСтруктура("СтепеньРодства", 2);
		Форма.Элементы.ВозможныеДублиПоказатьВсе.Пометка = Ложь;
		Форма.Элементы.ВозможныеДублиПоказатьВажные.Пометка = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьВажные(Команда)
...
Процедура ПриАктивацииШагаМастера()
	...
	Если ТекущаяСтраница = Элементы.ШагПоискНеВыполнялся Тогда
		...
		Элементы.ПредставлениеПравилПоиска.Подсказка = ТекстПравил;
...
Процедура ПоказатьМестаИспользования(ДеревоИсточник)
	...
	РежимОкна = РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца;
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей()
	...
	КонецЕсли;
	Элементы.ОписаниеСостоянияНайденныхДублей.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(Описание + Символы.ПС + ТекстПодсказки);
...
Функция НайтиИУдалитьДубли()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		ОтображениеСостояния = Элементы.ВыполнениеУдаления.ОтображениеСостояния;
		ОтображениеСостояния.Текст =  НСтр("ru = 'Обработка дублей...'");
		ОтображениеСостояния.ВариантОтображения             = ВариантОтображенияСостояния.НачальноеСостояние;
		ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
		ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.ДлительнаяОперация48;
	...
	Если ЗавершеноБезОшибок Тогда
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				СообщениеДляПользователя = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ПараметрыВажногоСообщения.ТипСообщения  = "Успех";
		ПараметрыВажногоСообщения.Идентификатор = "УспешноеУдалениеДублей";
		ПараметрыВажногоСообщения.Сообщение     = СообщениеДляПользователя;
		ПараметрыВажногоСообщения.Скрываемый    = Ложь;
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ШагУспешноеУдаление);
		
	Иначе
		ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыВажногоСообщения.Идентификатор = "НеУспешноеУдалениеДублей";
		ПараметрыВажногоСообщения.Сообщение     = НСтр("ru = 'Не удалось заменить дубли на оригиналы в некоторых местах использования'");
		ПараметрыВажногоСообщения.Скрываемый    = Ложь;
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.ГруппаИнформацияНеудачныеЗамены);
