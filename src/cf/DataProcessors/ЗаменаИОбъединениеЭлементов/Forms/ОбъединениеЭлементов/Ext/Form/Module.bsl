...
Процедура СформироватьПодсказкуОбъединения()
	...
	Если ЕстьПравоБезвозвратногоУдаления Тогда
		Если ТекущийВариантУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""%3"">помечены на удаление<img src=""СтрелкаВниз""></a> и заменены во всех местах использования на ""%2"" (отмечен стрелкой).'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""%3"">удалены безвозвратно<img src=""СтрелкаВниз""></a> и заменены во всех местах использования на ""%2"" (отмечен стрелкой).'");
		КонецЕсли;
	Иначе
	...
	Элементы.ПодсказкаВыбораОсновногоЭлемента.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ТекстПодсказки, 
		МестаИспользования.Количество()-1, ОсновнойЭлемент, "ПереключениеРежимаУдаления");
