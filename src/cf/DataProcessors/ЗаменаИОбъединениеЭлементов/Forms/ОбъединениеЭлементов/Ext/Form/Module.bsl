...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШагПоиск.КнопкаОтмена.Видимость = Истина;
	...
	Шаг.КнопкаОтмена.Заголовок = НСтр("ru = 'Готово'");
	...
	КомандыДоступны = МестаИспользования.Количество() > 2;
	Элементы.МестаИспользованияУдалить.Доступность                = КомандыДоступны;
	Элементы.МестаИспользованияКонтекстноеМенюУдалить.Доступность = КомандыДоступны;
...
Процедура ПодсказкаВыбораОсновногоЭлементаОбработкаНавигационнойСсылки(Элемент, ЗначениеНавигационнойСсылки, СтандартнаяОбработка)
	...
	Если ЗначениеНавигационнойСсылки = "ПереключениеРежимаУдаления" Тогда
		ОпределитьТекущийВариантУдаления();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Асинх Процедура ОпределитьТекущийВариантУдаления()
	
	ДействиеПослеЗамены = Новый СписокЗначений;
	ДействиеПослеЗамены.Добавить("Пометка",         НСтр("ru = 'Помечены на удаление'"));
	ДействиеПослеЗамены.Добавить("Непосредственно", НСтр("ru = 'Удалены безвозвратно'"));
	
	Ответ = Ждать ВыбратьИзСпискаАсинх(ДействиеПослеЗамены, Элементы.ПодсказкаВыбораОсновногоЭлемента);
	Если Ответ <> Неопределено Тогда
		ТекущийВариантУдаления = Ответ.Значение;
...
Процедура МестаИспользованияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не ПустаяСтрока(ИмяФормыВыбора) Тогда
		...
		КонецЕсли;
		ОткрытьФорму(ИмяФормыВыбора, ПараметрыФормы, Элемент,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность(Форма)
	...
	Элементы.ВсеМестаИспользования.Видимость = СтрСравнить(ТекущийШаг.ИмяСтраницы, "ШагВыборОсновногоЭлемента") = 0;
...
//                              - Число
//                              - ГруппаФормы - страница, на которую необходимо перейти.
//
&НаКлиенте
...
Процедура СформироватьПодсказкуОбъединения()
	...
	Если ЕстьПравоБезвозвратногоУдаления Тогда
		Если ТекущийВариантУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""%3"">помечены на удаление<img src=""%4""></a> и заменены во всех местах использования на ""%2"" (отмечен стрелкой).'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""%3"">удалены безвозвратно<img src=""%4""></a> и заменены во всех местах использования на ""%2"" (отмечен стрелкой).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут помечены на удаление и заменены во всех местах использования на ""%2"" (отмечен стрелкой).'");
	КонецЕсли;
	
	Элементы.ПодсказкаВыбораОсновногоЭлемента.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ТекстПодсказки, 
		МестаИспользования.Количество()-1, ОсновнойЭлемент, "ПереключениеРежимаУдаления", "СтрелкаВниз");
