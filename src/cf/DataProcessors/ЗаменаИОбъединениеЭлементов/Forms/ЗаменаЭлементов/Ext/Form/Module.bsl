...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = "";
...
Асинх Процедура ОпределитьТекущийВариантУдаления()
	...
	Если Количество = 1 Тогда
		ДействиеПослеЗамены.Добавить("Пометка",         НСтр("ru = 'Помечен на удаление'"));
		ДействиеПослеЗамены.Добавить("Непосредственно", НСтр("ru = 'Удален безвозвратно'"));
	Иначе
	...
	Ответ = Ждать ВыбратьИзСпискаАсинх(ДействиеПослеЗамены, Элементы.ДекорацияМенюВыбора);
...
Процедура СформироватьЦелевойЭлементИПодсказку(Контекст)
	...
	Если ТекущиеДанные = Неопределено Или ЗначениеРеквизита(ТекущиеДанные, "ЭтоГруппа", Ложь) Тогда
		Контекст.Элементы.ШагМастераДалее.Доступность = Ложь;
		Возврат;
	КонецЕсли;
	Контекст.ЦелевойЭлемент = ТекущиеДанные.Ссылка;
	Контекст.Элементы.ШагМастераДалее.Доступность = Истина;
	...
	Если Количество = 1 Тогда
		...
		Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
			Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
				ТекстПодсказки = 
					НСтр("ru = 'Выбранный элемент будет заменен на ""%1"" и <a href = ""%2"">помечен на удаление<img src=""СтрелкаВниз""></a>.'");
			Иначе
				ТекстПодсказки = 
					НСтр("ru = 'Выбранный элемент будет заменен на ""%1"" и <a href = ""%2"">удален безвозвратно<img src=""СтрелкаВниз""></a>.'");
			КонецЕсли;
		Иначе
			ТекстПодсказки = НСтр("ru = 'Выбранный элемент будет заменен на ""%1"" и помечен на удаление.'");
		КонецЕсли;
		
		Контекст.Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ТекстПодсказки, 
			Контекст.ЦелевойЭлемент, "ПереключениеРежимаУдаления");
		
	Иначе
		
		Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
			Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
				ТекстПодсказки = НСтр("ru = 'Выбранные элементы (%1) будут заменены на ""%2"" и <a href = ""%3"">помечены на удаление<img src=""СтрелкаВниз""></a>.'");
			Иначе
				ТекстПодсказки = НСтр("ru = 'Выбранные элементы (%1) будут заменены на ""%2"" и <a href = ""%3"">удалены безвозвратно<img src=""СтрелкаВниз""></a>.'");
			КонецЕсли;
		Иначе
			ТекстПодсказки = НСтр("ru = 'Выбранные элементы (%1) будут заменены на ""%2"" и помечены на удаление.'");
		КонецЕсли;
			
		Контекст.Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ТекстПодсказки, 
			Количество, Контекст.ЦелевойЭлемент, "ПереключениеРежимаУдаления");
