	...
	Если Количество = 1 Тогда
		...
		Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
			Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
				ТекстПодсказки = 
					НСтр("ru = 'Выбранный элемент будет заменен на ""%1"" и <a href = ""%2"">помечен на удаление<img src=""%3""></a>.'");
			Иначе
				ТекстПодсказки = 
					НСтр("ru = 'Выбранный элемент будет заменен на ""%1"" и <a href = ""%2"">удален безвозвратно<img src=""%3""></a>.'");
			КонецЕсли;
		Иначе
		...
		Контекст.Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ТекстПодсказки, 
			Контекст.ЦелевойЭлемент, "ПереключениеРежимаУдаления", "СтрелкаВниз");
		
	Иначе
		
		Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
			Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
				ТекстПодсказки = НСтр("ru = 'Выбранные элементы (%1) будут заменены на ""%2"" и <a href = ""%3"">помечены на удаление<img src=""%4""></a>.'");
			Иначе
				ТекстПодсказки = НСтр("ru = 'Выбранные элементы (%1) будут заменены на ""%2"" и <a href = ""%3"">удалены безвозвратно<img src=""$4""></a>.'");
			КонецЕсли;
		Иначе
		...
		Контекст.Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ТекстПодсказки, 
			Количество, Контекст.ЦелевойЭлемент, "ПереключениеРежимаУдаления", "СтрелкаВниз");
