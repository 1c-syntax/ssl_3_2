...
Процедура ПрименитьЗавершение(Ответ, Вариант) Экспорт
	...
	Если ВведеныКорректныеПараметрыАдминистрирования И ЭтоАдминистраторСистемы И Не ЭтоФайловаяБаза
		...
		Исключение
			...
			ВывестиСообщениеОбОшибке(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат;
		КонецПопытки;
...
Функция УстановитьСнятьБлокировку()
	...
	Попытка
		...
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОшибкаБлокировки");
	Исключение
...
Процедура ОбновитьСостояниеБлокировки(Форма)
	...
	Форма.Элементы.ДекорацияСостояние.РасширеннаяПодсказка.Заголовок = ТекстСостояния;
...
Процедура УстановитьНачальныйСтатусЗапретаРаботыПользователей()
	...
	ПараметрыСообщения.Сообщение = НачальныйСтатусЗапретаРаботыПользователей;
	ПараметрыСообщения.Идентификатор = "СтатусЗапретаРаботыПользователей";
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.СтраницаНастройки, Элементы.СообщениеДляПользователей);
...
Процедура ВывестиСообщениеОбОшибке(Сообщение)
	...
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Ошибка работы с блокировкой'");
	ПараметрыСообщения.Сообщение = Сообщение;
	ПараметрыСообщения.Идентификатор = "ОшибкаБлокировки";
