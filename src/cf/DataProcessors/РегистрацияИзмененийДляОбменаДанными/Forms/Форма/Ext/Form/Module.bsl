...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьЭлементыНомераСообщения(ЭтотОбъект);
...
&НаКлиенте
Процедура НомерОтправленногоПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(УзелОбменаСсылка) Тогда
		ЗаписатьНомерСообщения(Элементы.НомерОтправленного.Имя, НомерОтправленного);
		Оповестить("ИзменениеДанныхУзлаОбмена", УзелОбменаСсылка, ЭтотОбъект);
		
		НавигационнаяСсылкаУзла = ПолучитьНавигационнуюСсылку(УзелОбменаСсылка);
		НаименованиеУзла = ПолучитьНаименованиеУзла(УзелОбменаСсылка);
		
		ПоказатьОповещениеПользователя(НСтр("ru='Номер отправленного сообщения в узле изменен.'"),
			НавигационнаяСсылкаУзла,
			НаименованиеУзла,
...
&НаКлиенте
Процедура НомерПринятогоПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(УзелОбменаСсылка) Тогда
		ЗаписатьНомерСообщения(Элементы.НомерПринятого.Имя, НомерПринятого);
		Оповестить("ИзменениеДанныхУзлаОбмена", УзелОбменаСсылка, ЭтотОбъект);
		
		НавигационнаяСсылкаУзла = ПолучитьНавигационнуюСсылку(УзелОбменаСсылка);
		НаименованиеУзла = ПолучитьНаименованиеУзла(УзелОбменаСсылка);
		
		ПоказатьОповещениеПользователя(НСтр("ru='Номер принятого сообщения в узле изменен.'"),
			НавигационнаяСсылкаУзла,
			НаименованиеУзла,
			БиблиотекаКартинок.ДиалогИнформация,
			СтатусОповещенияПользователя.Информация);
...
&НаКлиенте
Процедура РазблокироватьИзменениеНомеровСообщенияНажатие(Элемент)
	
	Если Элементы.РазблокироватьИзменениеНомеровСообщения.Видимость Тогда
		ТекстВопроса = НСтр("ru='Включить возможность изменять номера сообщения?'");
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗавершениеВопросаОРедактировании", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		Элементы.ЗаблокироватьИзменениеНомеровСообщения.Видимость = Ложь;
		Элементы.РазблокироватьИзменениеНомеровСообщения.Видимость = Истина;
...
&НаКлиенте
Процедура ЗаблокироватьИзменениеНомеровСообщенияНажатие(Элемент)
...
Процедура СписокКонстантПриАктивизацииСтроки(Элемент)
	...
	ДоступностьКоманды = Элементы.СписокКонстант.ПолучитьСтрокиДляОбработки().Количество() > 0;
...
Процедура СписокСсылокПриАктивизацииСтроки(Элемент)
	...
	ДоступностьКоманды = Элементы.СписокСсылок.ПолучитьСтрокиДляОбработки().Количество() > 0;
	Элементы.ПоказатьРезультатВыгрузки.Доступность = ДоступностьКоманды;
...
Процедура СписокНаборовЗаписейПриАктивизацииСтроки(Элемент)
	...
	ДоступностьКоманды = Элементы.СписокНаборовЗаписей.ПолучитьСтрокиДляОбработки().Количество() > 0;
	Элементы.ПоказатьРезультатВыгрузки.Доступность = ДоступностьКоманды;
	...
	Если Данные <> Неопределено Тогда
		...
		ОткрытьФорму(ИмяЭтойФормы + "Форма.УзлыРегистрацииОбъекта",
			Новый Структура("ОбъектРегистрации, ТаблицаРегистрации, ОповещатьОбИзменениях", 
				Данные, ТаблицаРегистрации, Истина), ЭтотОбъект,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
	КонецЕсли;
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если ТекущаяСтраница = Элементы.СтраницаКонстанты Тогда 
		...
		Для Каждого Строка Из ЭлементФормы.ПолучитьСтрокиДляОбработки() Цикл
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаНаборЗаписей Тогда
		...
		Для Каждого Элемент Из ЭлементФормы.ПолучитьСтрокиДляОбработки() Цикл
...
Функция ДополнительныеПараметрыРедактированияНомераСообщения(Ссылка)
	...
	Для Каждого СтрокаТаблицы Из Элементы.СписокСсылок.ПолучитьСтрокиДляОбработки() Цикл
		ВыделенныеСтроки.Добавить(Элементы.СписокСсылок.ДанныеСтроки(СтрокаТаблицы).Ссылка);
	КонецЦикла;
	
	Возврат Новый Структура("Ссылка", ВыделенныеСтроки);
...
Процедура ОбработкаВыбораУзлаОбменаСервер()
	...
	Элементы.ГруппаНомераСообщенийУзла.Доступность = ЗначениеЗаполнено(УзелОбменаСсылка);
	
	НомераСообщения = ПрочитатьНомераСообщений();
	
	НомерПринятого	   = НомераСообщения.НомерПринятого;
	НомерОтправленного = НомераСообщения.НомерОтправленного;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	Для Каждого Строка Из Элемент.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ДобавитьРегистрациюВСписокСсылок(ЭтоПодбор = Ложь)
	...
	ОткрытьФорму(ТекИмяФормы, ТекПараметры, Элементы.СписокСсылок,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ДобавитьРегистрациюВСписокОтбор()
	...
	Если ВыделенныеСтроки.Количество() Тогда
		ТекущаяСтрока = ДеревоМетаданных.НайтиПоИдентификатору(ВыделенныеСтроки[0]);
		ПодчиненныеСтроки = ТекущаяСтрока.ПолучитьЭлементы();
		
		Если ПодчиненныеСтроки.Количество() Тогда
			Возврат;
		Иначе 	
			СписокСсылокИмяТаблицы = ТекущаяСтрока.МетаПолноеИмя;
		КонецЕсли;
	КонецЕсли;
	
	ТекИмяФормы = ИмяЭтойФормы + "Форма.ВыборОбъектовОтбором";
	ТекПараметры = Новый Структура("ДействиеВыбора, ИмяТаблицы", Истина, СписокСсылокИмяТаблицы);
...
Процедура УдалитьРегистрациюВСпискеОтбор()
	...
	ТекИмяФормы = ИмяЭтойФормы + "Форма.ВыборОбъектовОтбором";
	ТекПараметры = Новый Структура("ДействиеВыбора, ИмяТаблицы", Ложь, СписокСсылокИмяТаблицы);
...
Функция ПрочитатьНомераСообщений()
	...
	Иначе
		Данные = Новый Структура("НомерОтправленного, НомерПринятого", 0, 0);
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		Элементы.УзелОбменаСсылка.Доступность = Истина;
		Заголовок = НСтр("ru = 'Регистрация изменений для обмена данными'");
	Иначе
		Элементы.УзелОбменаСсылка.Доступность = Ложь;
...
Процедура ЗаполнитьДополнительнуюИнформацию()
	...
	Если НЕ КонфигурацияПоддерживаетБСД() Тогда
		Элементы.ДекорацияРежимВыборочнойРегистрации.Видимость = Ложь;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗавершениеВопросаОРедактировании(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		Элементы.ЗаблокироватьИзменениеНомеровСообщения.Видимость = Истина;
		Элементы.РазблокироватьИзменениеНомеровСообщения.Видимость = Ложь;
...
&НаСервере
Процедура ЗаписатьНомерСообщения(ИмяРеквизита, НомерСообщения)
	
	Данные = Новый Структура(ИмяРеквизита, НомерСообщения);
	ЭтотОбъект().УстановитьПараметрыУзлаОбмена(УзелОбменаСсылка, Данные);
	
КонецПроцедуры  

&НаСервере
Функция ПолучитьНаименованиеУзла(УзелОбмена)
	
	РеквизитыЗапроса = "Наименование";
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьЭлементыНомераСообщения(Форма)
	
	Элементы = Форма.Элементы;
	
	Если Элементы.РазблокироватьИзменениеНомеровСообщения.Видимость Тогда
		Элементы.НомерОтправленного.ТолькоПросмотр = Истина;
		Элементы.НомерПринятого.ТолькоПросмотр = Истина;
	Иначе
