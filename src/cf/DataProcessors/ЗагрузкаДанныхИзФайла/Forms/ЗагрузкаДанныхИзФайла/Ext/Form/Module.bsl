...
Процедура СписокКолонокСопоставленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.ВыборКолонок", ПараметрыФормы,
		ЭтотОбъект, , , , ОписаниеОповещения, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура ТаблицаОтчетПриАктивизации(Элемент) 
	...
	Если ТаблицаОтчет.ТекущаяОбласть = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаСопоставленияДанныхПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекущиеДанные.РезультатСопоставленияСтроки = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусНеоднозначность() Тогда
		
		СтандартнаяОбработка = Ложь;
		ИмяПоле = Элемент.ТекущийЭлемент.Имя;
		ОткрытьФормуРазрешенияНеоднозначности(Элемент.ТекущаяСтрока, ИмяПоле, СтандартнаяОбработка);
...
Процедура ТаблицаСопоставленияДанныхПриАктивизацииЯчейки(Элемент)
	...
	УстановитьДоступностьКонтекстногоМеню(Элемент, Ложь);
	
	Если Не ПроверитьТекущиеДанныеИЭлемент(Элемент) Тогда
		Возврат;
	КонецЕсли;
	
	ПодключитьОбработчикОжидания("ОбновитьОтображениеКнопкиВыбора", 0.1, Истина);
...
Процедура ШагНазад()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаСопоставлениеЗагружаемыхДанных
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ОтчетОЗагрузкеДанных Тогда
		...
		Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаСопоставлениеЗагружаемыхДанных;
...
&НаКлиенте
Процедура ОбновитьОтображениеКнопкиВыбора()
	
	Элемент                  = Элементы.ТаблицаСопоставленияДанных;
	Поле                     = Элемент.ТекущийЭлемент;
	ТекущееОтображениеКнопки = Поле.ОтображениеКнопкиВыбора;
	НовоеОтображениеКнопки   = ОпределитьНовоеОтображениеКнопкиВыбора(Элемент, Поле);
	
	Если НовоеОтображениеКнопки <> ТекущееОтображениеКнопки Тогда
		УстановитьСвойстваПоля(Поле, НовоеОтображениеКнопки);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция ПроверитьТекущиеДанныеИЭлемент(Элемент)
	Возврат Элемент.ТекущиеДанные <> Неопределено
		 И Элемент.ТекущийЭлемент <> Неопределено
		 И ЗначениеЗаполнено(Элемент.ТекущиеДанные.РезультатСопоставленияСтроки);
КонецФункции

&НаКлиенте
Процедура УстановитьДоступностьКонтекстногоМеню(Элемент, Доступность)
	Элементы.ТаблицаСопоставленияДанныхКонтекстноеМенюРазрешитьНеоднозначность.Доступность = Доступность;
КонецПроцедуры

&НаКлиенте
Функция ОпределитьНовоеОтображениеКнопкиВыбора(Элемент, Поле)
	
	НовоеОтображениеКнопки = ОтображениеКнопкиВыбора.Авто;
	
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		НовоеОтображениеКнопки = ПроверитьТабличнуюЧасть(Элемент);
	ИначеЕсли Элемент.ТекущиеДанные.РезультатСопоставленияСтроки = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусНеоднозначность() Тогда
		УстановитьДоступностьКонтекстногоМеню(Элемент, Истина);
		
		ДанныеСтроки = Элемент.ТекущиеДанные;
		
		Если ДанныеСтроки <> Неопределено И ТипЗагрузки <> "ТабличнаяЧасть" 
		 Или (ЗначениеЗаполнено(ДанныеСтроки.ОписаниеОшибки)
		   И ЭтоЯчейкаНеоднозначности(Поле.Имя, ДанныеСтроки.ОписаниеОшибки)) Тогда
			
			НовоеОтображениеКнопки = ОтображениеКнопкиВыбора.ОтображатьВПолеВвода;
			
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат НовоеОтображениеКнопки;
	
КонецФункции

&НаКлиенте
Функция ПроверитьТабличнуюЧасть(Элемент)
	
	ПрефиксТабличнойЧасти = Элемент.Имя + "_" + ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТабличнойЧасти() + "_";
	Если СтрДлина(Элемент.ТекущийЭлемент.Имя) > 3 И СтрНачинаетсяС(Элемент.ТекущийЭлемент.Имя, ПрефиксТабличнойЧасти) Тогда
		ИмяКолонки = Сред(Элемент.ТекущийЭлемент.Имя, СтрДлина(ПрефиксТабличнойЧасти) + 1);
		Если СтрНайти(Элемент.ТекущиеДанные.ОписаниеОшибки, ИмяКолонки) > 0 Тогда
			УстановитьДоступностьКонтекстногоМеню(Элемент, Истина);
			Возврат ОтображениеКнопкиВыбора.ОтображатьВПолеВвода;
		КонецЕсли;
	КонецЕсли;
	Возврат ОтображениеКнопкиВыбора.Авто;
	
КонецФункции

&НаКлиенте
Процедура УстановитьСвойстваПоля(Поле, НовоеОтображениеКнопки)
	
	Если НовоеОтображениеКнопки = ОтображениеКнопкиВыбора.ОтображатьВПолеВвода Тогда
		Поле.КнопкаВыпадающегоСписка = Ложь;
		Поле.КнопкаВыбора   = Истина;
		Поле.ОтображениеКнопкиВыбора = ОтображениеКнопкиВыбора.ОтображатьВПолеВвода;
	Иначе
		Поле.КнопкаВыбора            = Неопределено;
		Поле.КнопкаВыпадающегоСписка = Неопределено;
		Поле.ОтображениеКнопкиВыбора = ОтображениеКнопкиВыбора.Авто;
	КонецЕсли;
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаСопоставлениеЗагружаемыхДанных Тогда
		Элементы.ВставитьВСписок.Видимость = Ложь;
		ПодтверждениеЗакрытияФормы         = Истина;
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Элементы.ОбновлятьСуществующие.Заголовок       = НСтр("ru = 'Сопоставленные ('") + Строка(Статистика.Сопоставленных) + ")";
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.РасширениеФайла",,
			ЭтотОбъект, Истина,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
		...
		Элементы.ВариантЗагрузки.Видимость                  = Ложь;
		Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла;
		Элементы.ВидЗагрузкиДанных.Заголовок = НСтр("ru = 'Выбор справочника для загрузки данных из электронных таблиц, расположенных во внешних файлах 
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Элементы.ОтменитьСопоставление.Видимость        = Ложь;
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными;
		Элементы.СписокКолонокСопоставления.Видимость   = Ложь;
...
Процедура ОткрытьФормуИзмененияБланка()
	...
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.РедактированиеБланка", ПараметрыФормы,
		ЭтотОбъект,,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура СкопироватьОднуКолонкуВШаблонСДанными()
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл 
		...
		Если ЗначениеЗаполнено(Строка) Тогда
			...
			Ячейка.Область(1, 1, 1, 1).Текст = Строка;
...
Процедура СопоставитьЗагружаемыеДанныеНаСервере()
	...
	ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		ТаблицаСопоставления = РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
		...
		ЗначениеВРеквизитФормы(ТаблицаСопоставления, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
	КонецЕсли;
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеСопоставленияНаСервере(АдресРезультата)
	...
	Иначе
		Пояснение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для некоторых загруженных данных в приложении не найдено однозначного соответствия. Для несопоставленных данных будут созданы
			|новые записи, либо они будут пропущены (в зависимости от настройки).
			|При необходимости возможно сопоставить данные вручную, заполнив колонку ""%1"".'"), ПараметрыЗагрузки.ПредставлениеОбъекта);
	КонецЕсли;
	
	Элементы.ЗаголовокСопоставлениеЗагружаемыхДанных.РасширеннаяПодсказка.Заголовок = Пояснение;
...
Процедура ЗаписатьЗагружаемыеДанныеКлиент()
	...
	Элементы.ФормаОткрытьСправочник.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элементы.ФормаОткрытьСправочник.Заголовок,
			ПредставлениеОбъекта);
	
	Элементы.ЗагрузкаДанныхЗавершена.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ЗагрузкаДанныхЗавершена.РасширеннаяПодсказка.Заголовок, ПредставлениеОбъекта);
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.СтатистикаЗагрузки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.СтатистикаЗагрузки.Заголовок, Отчет.Создано, Отчет.Обновлено, Отчет.Пропущено, Отчет.Некорректных);
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Или Статистика.Неоднозначных > 0 Тогда
			...
			Элементы.ГруппаИтогоНеСопоставленных.Видимость     = Ложь;
			Элементы.ГруппаНеНайдено.Видимость                 = Ложь;
			
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, "ПояснениеДляПоискаСсылок", НСтр("ru = 'Далее'"));
		Иначе 
			Элементы.ГруппаНеНайдено.Видимость = Истина;
		КонецЕсли;
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, "ПояснениеДляСопоставленияДанных", НСтр("ru = 'Загрузить данные'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, "ПояснениеДляТабличнойЧасти", НСтр("ru = 'Загрузить данные'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(ОтборНеоднозначность);
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, "ПояснениеДляПрикладнойЗагрузки", НСтр("ru = 'Загрузить данные'"));
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьОформлениеДляСтраницыСопоставления(ВидимостьКнопкиСопоставление, ТипПоясняющегоТекста, ТекстКнопкиДалее)
	
	Элементы.СписокКолонокСопоставления.Видимость = ВидимостьКнопкиСопоставление;
	Элементы.Назад.Видимость = Истина;
	Если ТипПоясняющегоТекста = "ПояснениеДляТабличнойЧасти" Тогда
		Элементы.ЗаголовокСопоставлениеЗагружаемыхДанных.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Для некоторых значений в колонках таблицы не найдены однозначные соответствия в данных приложении (выделены красным цветом).
		|Необходимо сопоставить эти значения с данными в приложении вручную. 
		|В случае если в приложении отсутствуют необходимые данные, то ввести их.'");
	ИначеЕсли ТипПоясняющегоТекста = "ПояснениеДляПоискаСсылок" Тогда
		Элементы.ЗаголовокСопоставлениеЗагружаемыхДанных.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Для некоторых введенных строк не найдено данных в приложении, либо имеется несколько вариантов.
		|Для вставки таких строк следует выбрать соответствующие объекты, щелкнув левой кнопкой мыши в колонке ""Объект"".'");
	ИначеЕсли ТипПоясняющегоТекста = "ПояснениеДляПрикладнойЗагрузки" Тогда
		Элементы.ЗаголовокСопоставлениеЗагружаемыхДанных.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для некоторых загруженных данных в приложении не найдено однозначного соответствия.  Для несопоставленных данных будут созданы
			|новые записи, либо они будут пропущены (в зависимости от настройки). При необходимости возможно сопоставить данные вручную, 
			|заполнив колонку ""%1"".'"), ПараметрыЗагрузки.ПредставлениеОбъекта);
	Иначе
		Элементы.ЗаголовокСопоставлениеЗагружаемыхДанных.РасширеннаяПодсказка.Заголовок = НСтр("ru='Сопоставьте загружаемые данные с элементами в справочнике. Сопоставление возможно произвести несколько раз, выбирая разные колонки для сравнения. 
		|Также возможно сопоставить отдельные строки таблицы, выбрав значение в колонке.
		|
		|В зависимости от настройки, для несопоставленных данных будут созданы новые элементы, либо они будут пропущены, сопоставленные данные также будут обновлены либо не загружены.'");
	КонецЕсли;
...
&НаКлиенте
Функция ЭтоЯчейкаНеоднозначности(ПолеИмя, ОписаниеОшибки)
	...
	Если СтрДлина(ПолеИмя) > 3 И СтрНачинаетсяС(ПолеИмя, ПрефиксНачалаТабличнойЧасти) Тогда
		...
		Если СтрНайти(ОписаниеОшибки, Имя) > 0 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Если Строка.РезультатСопоставленияСтроки = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусНеоднозначность()
		   И СтрДлина(Строка.ОписаниеОшибки) > 0 Тогда
			...
			Если СтрДлина(ПолеИмя) > 3 И СтрНачинаетсяС(ПолеИмя, ПрефиксНачалаТабличнойЧасти) Тогда
				Имя = Сред(ПолеИмя, СтрДлина(ПрефиксНачалаТабличнойЧасти) + 1);
				Если СтрНайти(Строка.ОписаниеОшибки, Имя) > 0 Тогда
					СтандартнаяОбработка = Ложь;
					...
					ОткрытьФорму("Обработка.ЗагрузкаДанныхИзФайла.Форма.РазрешенияНеоднозначностей", ПараметрыФормы,
						ЭтотОбъект, Истина , , , Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработатьЗагружаемыеДанные(СсылкиНаОбъектыОповещения)
	...
	ИмяТаблицыСопоставления = ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления();
	СопоставленныеДанные = РеквизитФормыВЗначение(ИмяТаблицыСопоставления);
	
	Если ТипЗагрузки = "ВнешняяЗагрузка" Тогда
		ЗаписатьСопоставленныеДанныеВнешняяОбработка(СопоставленныеДанные);
		ЗначениеВРеквизитФормы(СопоставленныеДанные, ИмяТаблицыСопоставления);
	Иначе
...
Процедура ОчиститьТаблицу()
	...
	ТаблицаСопоставленияДанныхСервер = РеквизитФормыВЗначение(ИмяТаблицыСопоставления);
	...
	РеквизитыТаблицыСопоставления = ПолучитьРеквизиты(ИмяТаблицыСопоставления);
	МассивПутейРеквизитов = Новый Массив;
	Для каждого РеквизитТаблицы Из РеквизитыТаблицыСопоставления Цикл
		МассивПутейРеквизитов.Добавить(ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления(РеквизитТаблицы.Имя));
	КонецЦикла;
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	ТекстОбъектНеНайден = ?(ТипЗагрузки = "ВставкаИзБуфераОбмена", НСтр("ru = '<Не найден>'"), НСтр("ru = '<Новый>'"));
	
	ПрефиксТаблицыСопоставления = ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления();
	ИмяКолонкиОбъектСопоставления = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиОбъектСопоставления();
	ИмяПолеКомпоновкиДанных = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления("РезультатСопоставленияСтроки");
	...
	ПолеОформления.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1_%2", 
		ПрефиксТаблицыСопоставления, ИмяКолонкиОбъектСопоставления));
	ПолеОформления.Использование = Истина;
	
	ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(
		ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления(ИмяКолонкиОбъектСопоставления));
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	ЭлементОтбора.Использование = Истина;
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаПалитры.Серый);
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		...
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ПолеОформления.Использование = Истина;
			ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(ИмяПолеКомпоновкиДанных);
			ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
			ЭлементОтбора.ПравоеЗначение = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусНеоднозначность();
			ЭлементОтбора.Использование  = Истина;
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаПалитры.Красный);
	...
	Иначе
		...
		ПолеОформления.Поле = Новый ПолеКомпоновкиДанных(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1_%2",
			ПрефиксТаблицыСопоставления, ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиОбъектСопоставления()));
		ПолеОформления.Использование = Истина;
		ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных(ИмяПолеКомпоновкиДанных);
		ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура ПоместитьДанныеВТаблицуСопоставления(АдресЗагруженныхДанных, АдресКопииТабличнойЧасти, СписокНеоднозначностей)
	...
	ВременнаяТЗ = РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления()); // см. Обработка.ЗагрузкаДанныхИзФайла.Форма.ЗагрузкаДанныхИзФайла.ТаблицаСопоставленияДанных
	Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Если Неоднозначности.Количество() > 0 Тогда 
			...
			Для каждого Неоднозначность Из Неоднозначности Цикл
				...
				ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(
					ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления("ОписаниеОшибки")); 
...
Функция АдресВХранилищеТаблицыСопоставления()
	Таблица = РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Иначе
		...
		ТекстПодсказкиВариантаЗагрузки = ТекстПодсказки;
	КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ЗначениеЗаполнено(СтруктураОбъектаСопоставления) Тогда 
		...
		КонецЕсли;
		ВременнаяТЗ.Колонки.Добавить(ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиОбъектСопоставления(), СтруктураОбъектаСопоставления.ОписаниеТипаОбъектаСопоставления, ЗаголовокКолонки);
	КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, ОписаниеТипаОбъектаСопоставления , ДобавитьИдентификатор, 
		...
		Если ДобавитьИдентификатор Тогда 
			...
			МассивРеквизитов.Добавить(Новый РеквизитФормы("Идентификатор", ТипЧисло, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), "Идентификатор"));
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОписаниеТипаОбъектаСопоставления) Тогда 
			МассивРеквизитов.Добавить(Новый РеквизитФормы(ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиОбъектСопоставления(),
				ОписаниеТипаОбъектаСопоставления, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), ИмяОбъектаСопоставления));
		КонецЕсли;
		
		Если ДобавитьРезультатСопоставленияСтроки Тогда
			МассивРеквизитов.Добавить(Новый РеквизитФормы("РезультатСопоставленияСтроки", ТипСтрока, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), "Результат"));
		КонецЕсли;
		Если ДобавитьОписаниеОшибки Тогда 
			МассивРеквизитов.Добавить(Новый РеквизитФормы("ОписаниеОшибки", ТипСтрока, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), "Причина"));
		КонецЕсли;

	Если ДобавитьСписокНеоднозначностей Тогда 
		ТипСЗ = Новый ОписаниеТипов("СписокЗначений");
		МассивРеквизитов.Добавить(Новый РеквизитФормы("СписокНеоднозначностей", ТипСЗ, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), "СписокНеоднозначностей"));
	КонецЕсли;
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахАвто(ОписаниеТипаОбъектаСопоставления)
	...
	ВременнаяТЗ = РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
	...
	Для каждого Колонка Из ИнформацияПоКолонкам Цикл
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы(Колонка.ИмяКолонки, Колонка.ТипКолонки, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), Колонка.ПредставлениеКолонки));
	КонецЦикла;
	
	ИзменитьРеквизиты(МассивРеквизитов);
	
	ЗначениеВРеквизитФормы(ВременнаяТЗ, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления()); 
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		...
		НовыйЭлемент.ПутьКДанным        = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления(Колонка.Имя);
		...
		КонецЕсли;
		Если Колонка.Имя = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиОбъектСопоставления() Тогда
			НовыйЭлемент.ФиксацияВТаблице = ФиксацияВТаблице.Лево;
			НовыйЭлемент.ЦветФона         = ЦветаПалитры.ПервыйБрендовый;
			НовыйЭлемент.КартинкаШапки    = Картинка;
			НовыйЭлемент.ТолькоПросмотр   = Ложь;
			НовыйЭлемент.ПодсказкаВыпадающегоСписка = НСтр("ru='Выберите элемент справочника, к которому относятся загружаемые данные'");
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонках()
	...
	Для каждого Колонка Из ИнформацияПоКолонкам Цикл 
		Если ВременнаяТЗ.Колонки.Найти(Колонка.ИмяКолонки) = Неопределено Тогда
			...
			МассивРеквизитов.Добавить(Новый РеквизитФормы(Колонка.ИмяКолонки, Колонка.ТипКолонки, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), ПредставлениеКолонки));
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахДляТЧ() 
	...
	Для каждого Колонка Из РеквизитыТЧ Цикл
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы(ПрефиксТабличнойЧасти + Колонка.Имя, ТипРеквизита, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), Колонка.Представление()));
		
	КонецЦикла;
	
	Для каждого Колонка Из ИнформацияПоКолонкам Цикл
		ВременнаяТЗ.Колонки.Добавить("ФЛ_" + Колонка.ИмяКолонки, ТипСтрока, Колонка.ПредставлениеКолонки);
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ФЛ_" + Колонка.ИмяКолонки, ТипСтрока, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления(), Колонка.ПредставлениеКолонки));
	КонецЦикла;
		...
		Если СтрНачинаетсяС(Колонка.Имя, ПрефиксТабличнойЧасти) Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда 
					...
					НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
					НовыйЭлемент.ПутьКДанным = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления(КолонкаУровень2.Имя);
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				...
				Для каждого КолонкаГруппа Из КолонкиГруппировки Цикл
					...
					Если Колонка2 <> Неопределено Тогда 
						...
						НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
						НовыйЭлемент.ПутьКДанным = ЗагрузкаДанныхИзФайлаКлиентСервер.ИмяКолонкиТаблицыСопоставления(Колонка2.Имя);
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("ШаблонСДанными", ШаблонСДанными);
	ПараметрыПроцедуры.Вставить("ТаблицаСопоставления", РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления()));
	ПараметрыПроцедуры.Вставить("ИнформацияПоКолонкам", РеквизитФормыВЗначение("ИнформацияПоКолонкам"));
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("СопоставленныеДанные", РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления()));
...
Процедура ЗаполнитьТаблицуСопоставленияИзВременногоХранилища(АдресВоВременномХранилище, СсылкиНаОбъектыОповещения)
	...
	ЗначениеВРеквизитФормы(СопоставленныеДанные, ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	СопоставленныеДанные        = РеквизитФормыВЗначение(ЗагрузкаДанныхИзФайлаКлиентСервер.ПрефиксТаблицыСопоставления());
	ТаблицаИнформацияПоКолонкам = РеквизитФормыВЗначение("ИнформацияПоКолонкам");
...
Процедура ПослеСозданияОтчета(Результат, ДополнительныеРезультаты) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ПерейтиКСтранице(Элементы.СтраницаСопоставлениеЗагружаемыхДанных);
		СтандартныеПодсистемыКлиент.ВывестиИнформациюОбОшибке(
			Результат.ИнформацияОбОшибке);
		Возврат;
	КонецЕсли;
	...
	ПодтверждениеЗакрытияФормы = Истина;
	Элементы.ФормаОткрытьСправочник.Видимость = Истина;
