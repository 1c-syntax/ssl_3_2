...
&НаКлиенте
Процедура ТаблицаСопоставленияДанныхПриНаведении(Элемент, Строка, Поле) 
	
	Если Поле = Неопределено Или ТипЗагрузки <> "ТабличнаяЧасть" Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДанныеСтроки <> Неопределено 
		И ДанныеСтроки.РезультатСопоставленияСтроки = ЗагрузкаДанныхИзФайлаКлиентСервер.СтатусНеоднозначность() Тогда
		
		Если ТипЗагрузки <> "ТабличнаяЧасть" 
...
Процедура ТаблицаСопоставленияДанныхПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено
	   ...
	   И ЗначениеЗаполнено(Элемент.ТекущиеДанные.РезультатСопоставленияСтроки) Тогда
...
Функция НезаполненныеОбязательныеКолонки()
	...
	Для НомерКолонки = 1 По Шапка.ШиринаТаблицы Цикл 
		ОбластьЯчейки = Шапка.Область(1, НомерКолонки, 1, НомерКолонки);
		ИмяКолонки = СокрЛП(ОбластьЯчейки.Текст);
