...
Процедура ОпределитьПозицииКолонокВМакете(ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Для НомерКолонки = 1 По ОбластьЗаголовка.ШиринаТаблицы Цикл
		Ячейка=ШаблонСДанными.Область(1, НомерКолонки, 1, НомерКолонки);
...
Функция ОбластьЗаголовкаШаблонаТаблицы(Шаблон)
	...
	Для НомерКолонки = 1 По Шаблон.ШиринаТаблицы Цикл
		Ячейка = Шаблон.Область(2, НомерКолонки, 2, НомерКолонки);
...
Процедура СоздатьИнформациюПоКолонкамНаОснованиеШаблона(ОбластьЗаголовокТаблицы, ПараметрыЗагрузкиИзФайла, ИнформацияПоКолонкам, ИменаДобавляемыхКолонок = Неопределено)
	...
	Если ВысотаШапки = 2 Тогда
		...
		Пока НомерКолонки <= ОбластьЗаголовокТаблицы.ШиринаТаблицы Цикл
			...
			Ячейка = ОбластьЗаголовокТаблицы.Область(1, НомерКолонки, 1, НомерКолонки);
			Группа = Ячейка.Текст;
	...
	Для НомерКолонки = 1 По ОбластьЗаголовокТаблицы.ШиринаТаблицы Цикл
		Ячейка = ОбластьЗаголовокТаблицы.Область(ВысотаШапки, НомерКолонки, ВысотаШапки, НомерКолонки);
...
Процедура ЗаполнитьЯчейкуЗаголовкаМакета(Ячейка, Текст, Ширина, Подсказка, ОбязательноеПоле, Имя = "")
	...
	Ячейка.Текст = Текст;
	Ячейка.Имя = Имя;
	Ячейка.ПараметрРасшифровки = Имя;
	Ячейка.ЦветФона =  ЦветаСтиля.ЦветФонаШапкиОтчета;
	Ячейка.ШиринаКолонки = Ширина;
	Ячейка.Примечание.Текст = Подсказка;
	Если ОбязательноеПоле Тогда 
		Ячейка.Шрифт = ШрифтыСтиля.ВажнаяНадписьШрифт;
	Иначе
		Ячейка.Шрифт = Неопределено;
	КонецЕсли;
	...
	Для Индекс= 1 По ДанныеИзФайла.Количество() - 1 Цикл
		...
		Для НомерКолонки =1 По ШаблонСДанными.ШиринаТаблицы Цикл
			...
			Если Колонка <> Неопределено И Колонка.Видимость = Ложь Тогда
				Продолжить;
			КонецЕсли;
			Ячейка = ШаблонСДанными.Область(2, НомерКолонки, 2, НомерКолонки);
			Если КолонкаВТаблице <> Неопределено Тогда 
				Ячейка.Текст = СтрокаТЗ[КолонкаВТаблице];
				Ячейка.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Обрезать;
			Иначе
				Ячейка.Текст = "";
			КонецЕсли;
...
Процедура ИнициализироватьКолонки(ИнформацияПоКолонкам, ШаблонСДанными, ВысотаШапки = 1)
	...
	Для НомерКолонки = 1 По ШаблонСДанными.ШиринаТаблицы Цикл
		ЯчейкаЗаголовок = ШаблонСДанными.Область(ВысотаШапки, НомерКолонки, ВысотаШапки, НомерКолонки);
...
Процедура СформироватьОтчетОЗагрузкеФон(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	Для НомерСтроки = 1 По СопоставленныеДанные.Количество() Цикл
		...
		ЯчейкаОбласть = ТаблицаОтчет.ПолучитьОбласть(НомерСтроки + 1, 1, НомерСтроки + 1, 1);
		Ячейка = ЯчейкаОбласть.Область(1, 1, 1, 1);
		Ячейка.Текст = ПредставлениеСтатусаЗагрузки(Строка.РезультатСопоставленияСтроки);
		Ячейка.Расшифровка = Строка.ОбъектСопоставления;
		Ячейка.Примечание.Текст = Строка.ОписаниеОшибки;
		Если Строка.РезультатСопоставленияСтроки = "Создан" Тогда 
			Ячейка.ЦветТекста = ЦветаСтиля.РезультатУспехЦвет;
			КоличествоСозданных = КоличествоСозданных + 1;
		ИначеЕсли Строка.РезультатСопоставленияСтроки = "Обновлен" Тогда
			Ячейка.ЦветТекста = ЦветаСтиля.ПоясняющийТекст;
			КоличествоОбновленных = КоличествоОбновленных + 1;
		Иначе
			Ячейка.ЦветТекста = ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет;
		...
		ТаблицаОтчет.Вывести(ЯчейкаОбласть);
		
		Для Индекс = 1 По ИнформацияПоКолонкам.Количество() Цикл 
			ЯчейкаОбласть = ТаблицаОтчет.ПолучитьОбласть(НомерСтроки + 1, Индекс + 1, НомерСтроки + 1, Индекс + 1);
			Ячейка = ЯчейкаОбласть.Область(1, 1, 1, 1);
			...
			Если НайденныеКолонки.Количество() > 0 Тогда 
				...
				Ячейка.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Обрезать;
			КонецЕсли;
			ТаблицаОтчет.Присоединить(ЯчейкаОбласть);
...
Процедура СформироватьМакетОтчета(ТаблицаОтчет, ШаблонСДанными)
	...
	Ячейка = ШаблонСДанными.Область(1, 1, 1, 1);
	...
	ТаблицаОтчет.ВставитьОбласть(Ячейка, ТаблицаОтчет.Область("C1"), ТипСмещенияТабличногоДокумента.ПоГоризонтали);
