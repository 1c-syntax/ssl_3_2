...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Валюта Из Объект.СписокВалют Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Период.ДатаНачала = Объект.НачалоПериодаЗагрузки;
	Период.ДатаОкончания = Объект.ОкончаниеПериодаЗагрузки;
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	
	ВыделитьВсеКурсыВалют();
...
Процедура ПериодПриИзменении(Элемент)
	Объект.НачалоПериодаЗагрузки = Период.ДатаНачала;
	Объект.ОкончаниеПериодаЗагрузки = Период.ДатаОкончания;
КонецПроцедуры
...
&НаКлиенте
Процедура СписокВалютПриИзмененииСоставаВыделенныхСтрок(Элемент)
...
Процедура УстановитьДоступностьЭлементов()
	...
	ЕстьВыбранныеВалюты = Элементы.СписокВалют.ВыделенныеСтроки.Количество() > 0;
...
&НаСервере
Функция ВыполнитьЗагрузкуКурсов(Знач ВыделенныеСтроки)
	...
	ВыбранныеВалюты = Новый ТаблицаЗначений;
	ВыбранныеВалюты.Колонки.Добавить("КодВалюты", ОбщегоНазначения.ОписаниеТипаСтрока(3));
	ВыбранныеВалюты.Колонки.Добавить("Валюта", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		СтрокаСписка = Объект.СписокВалют.НайтиПоИдентификатору(ИдентификаторСтроки);
		СтрокаВалюты = ВыбранныеВалюты.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаВалюты, СтрокаСписка);
	КонецЦикла;
	...
	ПараметрыЗагрузки.Вставить("СписокВалют", ОбщегоНазначения.ТаблицаЗначенийВМассив(ВыбранныеВалюты));
...
Процедура ПродолжитьЗагрузкуКурсовВалют()
	...
	ОперацияЗагрузкиКурсов = ВыполнитьЗагрузкуКурсов(Элементы.СписокВалют.ВыделенныеСтроки);
...
&НаКлиенте
Процедура ВыделитьВсеКурсыВалют()
	
	ВыделенныеСтроки = Элементы.СписокВалют.ВыделенныеСтроки;
