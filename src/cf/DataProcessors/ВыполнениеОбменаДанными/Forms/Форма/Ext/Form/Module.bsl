...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Если Элементы.ВыполнениеОбменаДанными.ТекущаяСтраница <> Элементы.ЗавершениеОбмена Тогда
		Для Каждого ИмяКомандыПанелиФормы Из КомандыПанелиФормы() Цикл
			Элементы[ИмяКомандыПанелиФормы].Видимость = Ложь;
		КонецЦикла; 
		
		Если Не ПустаяСтрока(СтрокаПереходаТекущая.КомандыНавигации) Тогда
			КомандыНавигации = СтрРазделить(СтрокаПереходаТекущая.КомандыНавигации, ",", Ложь);
...
Функция ТаблицаПереходовНоваяСтрока(ИмяОсновнойСтраницы,
	...
	НоваяСтрока.КомандыНавигации = КомандыНавигации;
...
Процедура ОбработатьОшибкуРасхожденияВерсий()
	...
	НастроитьВидимостьКомандПанелиФормы(Элементы.ПродолжитьСинхронизацию.Имя);
	...
	ИначеЕсли ЕстьОшибки Тогда
		...
		Если ТребуетсяОбновление Или ТребуетсяУстановкаОбновления() Тогда
			...
			Если РольДоступнаПолныеПрава Тогда				
				НастроитьВидимостьКомандПанелиФормы(Элементы.ФормаУстановитьОбновление.Имя);
			КонецЕсли;
		...
		ИначеЕсли ТребуетсяПерезагрузка() Тогда
			...
			НастроитьВидимостьКомандПанелиФормы(Элементы.ФормаПерезапуститьПрограмму.Имя);
...
&НаСервере
Функция ЗапуститьФоновоеЗаданиеВыполнитьЗапускОбменаДаннымиНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если ОбменЧерезВнутреннююПубликацию Тогда
		КомандыНавигации = Элементы.ФормаОтменитьОчередьИПродолжить.Имя;
		ТаблицаПереходовНоваяСтрока("СтраницаПроверкаЗадачОбмена", КомандыНавигации, "Подключаемый_СтраницаПроверкаЗадачОбмена_ПриОткрытии");	
	КонецЕсли;
	...
	Иначе
		...
		Если ОбменМеждуПриложениямиВСервисе Или ОбменЧерезВнутреннююПубликацию Тогда
			КомандыНавигацииОжиданиеСинхронизации = ?(Не ФоновоеЗаданиеИспользоватьПрогресс И ОбменЧерезВнутреннююПубликацию, Элементы.ФормаЗавершитьСинхронизацию.Имя, "");
			// Получение и отправка.
			ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияВыгрузка, КомандыНавигацииОжиданиеСинхронизации, "Подключаемый_ВыгрузкаДанных_ПриОткрытии", Истина, "Подключаемый_ВыгрузкаДанных_ОбработкаДлительнойОперации");
...
&НаКлиенте
Функция КомандыПанелиФормы()

	ЭлементыКомандНавигации = Новый Массив;
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаПродолжитьСинхронизацию.Имя);
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаУстановитьОбновление.Имя);
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаПерезапуститьПрограмму.Имя);
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаОтменитьОчередьИПродолжить.Имя);
	ЭлементыКомандНавигации.Добавить(Элементы.ФормаЗавершитьСинхронизацию.Имя);
	
	Возврат ЭлементыКомандНавигации;
	
КонецФункции

&НаКлиенте
Процедура НастроитьВидимостьКомандПанелиФормы(ИмяКоманды)
