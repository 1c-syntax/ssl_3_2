...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ПолучателиСкрытыхКопий.Количество() > 0 Тогда
		ПоказатьПолеВводаСкрытаяКопия();
	КонецЕсли;
...
&НаКлиенте
Процедура КомуДобавлениеМножественныхЗначений(Элемент, Значения, СтандартнаяОбработка)
	
	ПриДобавленииМножественныхЗначенийАдресатов(Объект.ПолучателиПисьма, Значения, СтандартнаяОбработка)
	
КонецПроцедуры

&НаКлиенте
Процедура КопияДобавлениеМножественныхЗначений(Элемент, Значения, СтандартнаяОбработка)
	
	ПриДобавленииМножественныхЗначенийАдресатов(Объект.ПолучателиКопий, Значения, СтандартнаяОбработка)
	
КонецПроцедуры

&НаКлиенте
Процедура СкрытаяКопияДобавлениеМножественныхЗначений(Элемент, Значения, СтандартнаяОбработка)
	
	ПриДобавленииМножественныхЗначенийАдресатов(Объект.ПолучателиСкрытыхКопий, Значения, СтандартнаяОбработка)
...
Процедура АдресатыАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	СписокВыбора = НайтиКонтакты(Текст);
...
&НаКлиенте
Процедура АдресатыНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ОчиститьСообщения();
	
	РежимРедактирования = Объект.СтатусПисьма <> ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено");
	ГруппаПодбора = Элемент.Имя;
	РедактироватьСписокПолучателей(РежимРедактирования, ГруппаПодбора);
	
КонецПроцедуры

&НаКлиенте
Процедура АдресатыОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Получатель = Новый Структура("Адрес, Псевдоним", ВыбранноеЗначение.Адрес, ВыбранноеЗначение.Представление);
	НовыйПолучатель = ДобавитьПолучателя(Объект.ПолучателиПисьма, Получатель);
	НовыйПолучатель.Контакт = ВыбранноеЗначение.Контакт;
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Если Объект.ПолучателиПисьма.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Укажите хотя бы одного получателя письма.'"), , "Объект.ПолучателиПисьма");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьПолеВводаСкрытаяКопия(Команда = Неопределено)
...
Процедура УстановитьУсловноеОформление()
	...
	ПоляСАдресатами = Новый Соответствие;
	ПоляСАдресатами.Вставить(Элементы.Кому.Имя, "Объект" + "." + Метаданные.Документы.ЭлектронноеПисьмоИсходящее.ТабличныеЧасти.ПолучателиПисьма.Имя);
	ПоляСАдресатами.Вставить(Элементы.Копия.Имя, "Объект" + "." + Метаданные.Документы.ЭлектронноеПисьмоИсходящее.ТабличныеЧасти.ПолучателиКопий.Имя);
	ПоляСАдресатами.Вставить(Элементы.СкрытаяКопия.Имя, "Объект" + "." + Метаданные.Документы.ЭлектронноеПисьмоИсходящее.ТабличныеЧасти.ПолучателиСкрытыхКопий.Имя);
	
	Для Каждого ПолеСАдресатами Из ПоляСАдресатами Цикл
		ИмяЭлемента = ПолеСАдресатами.Ключ;
		ИмяРеквизита = ПолеСАдресатами.Значение;
		
		Элемент = УсловноеОформление.Элементы.Добавить();

		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяЭлемента + ".МножественноеЗначение");

		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяРеквизита + ".АдресЗаполненКорректно");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Функция ПроверитьЗаполнениеАдресов()
	...
	ПроверитьАдреса("ПолучателиПисьма", Результат);
	ПроверитьАдреса("ПолучателиКопий", Результат);
	ПроверитьАдреса("ПолучателиСкрытыхКопий", Результат);
...
Процедура СохранитьВложениеИзВременногоХранилища(АдресВоВременномХранилище, ИмяФайла)
	...
	ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, АдресВоВременномХранилище, ИмяФайла, ПараметрыСохранения);
...
Процедура ОбработатьПереданныеПараметры(ПереданныеПараметры)
	...
	Если ПереданныеПараметры.Свойство("Получатель") И ПереданныеПараметры.Получатель <> Неопределено Тогда
		...
		Если ТипЗнч(ПереданныеПараметры.Получатель) = Тип("Строка") И НЕ ПустаяСтрока(ПереданныеПараметры.Получатель) Тогда
			ЗаполнитьАдресатовИзСтроки(ПереданныеПараметры.Получатель, Объект.ПолучателиПисьма);
		ИначеЕсли ТипЗнч(ПереданныеПараметры.Получатель) = Тип("СписокЗначений") Тогда
			ЗаполнитьАдресатовИзСпискаЗначений(ПереданныеПараметры.Получатель, Объект.ПолучателиПисьма);
		ИначеЕсли ТипЗнч(ПереданныеПараметры.Получатель) = Тип("Массив") Тогда
			ЗаполнитьАдресатовИзМассива(ПереданныеПараметры.Получатель);
		КонецЕсли;
...
Процедура РедактироватьСписокПолучателей(ДляПодбора, ГруппаПодбора = "")
	...
	СоответствиеТабличныхЧастей.Вставить("Копия", Объект.ПолучателиКопий);
	СоответствиеТабличныхЧастей.Вставить("СкрытаяКопия", Объект.ПолучателиСкрытыхКопий);
...
Процедура ЗаполнитьПодобранныхПолучателей(СоответствиеТабличныхЧастей, Результат)
	...
	Для Каждого Элемент Из Результат Цикл
		...
		НоваяСтрока = ДобавитьПолучателя(ТабличнаяЧасть, Новый Структура("Адрес, Псевдоним", Элемент.Адрес, Элемент.Представление));
...
Процедура ЗаполнитьУточненныеКонтакты(Результат)
	...
	Для каждого ЭлементМассива Из Результат Цикл
		...
		ЗаполнитьЗначениеАдресаВСтрокеТаблицыАдресатов(СтрокаПолучатели);
	КонецЦикла;
...
Функция ДобавитьПолучателя(СписокПолучателей, Получатель)
	...
	НовыйПолучатель.Значение = ПредставлениеАдреса(НовыйПолучатель.Адрес, НовыйПолучатель.Представление);
...
&НаСервере
Процедура ЗаполнитьАдресатовИзМассива(Знач Адресаты)
	...
	Для Каждого Адресат Из Адресаты Цикл
		...
		Если ВариантОтправки = "Копия" Тогда
			СписокАдресатов = Объект.ПолучателиКопий;
		ИначеЕсли ВариантОтправки = "СкрытаяКопия" Тогда
			СписокАдресатов = Объект.ПолучателиСкрытыхКопий;
		ИначеЕсли ВариантОтправки = "ОбратныйАдрес" Тогда
			СписокАдресатов = Объект.ПолучателиОтвета;
		Иначе
			СписокАдресатов = Объект.ПолучателиПисьма;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Адресат.Адрес) Тогда
			Для Каждого Адрес Из СтрРазделить(Адресат.Адрес, ";", Ложь) Цикл
				Получатель = ДобавитьПолучателя(СписокАдресатов, Новый Структура("Адрес, Псевдоним", Адресат.Адрес, Адресат.Представление));
				Получатель.Контакт = Адресат.ИсточникКонтактнойИнформации;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ПредставлениеАдреса(Адрес, ИмяАдресата)
	...
	Возврат ВзаимодействияКлиентСервер.ПолучитьПредставлениеАдресата(ИмяАдресата, Адрес, "");
...
Процедура ПроверитьАдреса(ИмяРеквизитаСАдресами, Результат)
	...
	КоллекцияАдресов = Объект[ИмяРеквизитаСАдресами];
	
	Для Индекс = 0 По КоллекцияАдресов.Количество() - 1 Цикл
		Получатель = КоллекцияАдресов[Индекс];
		Если ЗначениеЗаполнено(Получатель.ОписаниеОшибки) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				Получатель.ОписаниеОшибки,, "Объект" + "." + ИмяРеквизитаСАдресами + "[" + Индекс + "].Значение");
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ПриДобавленииМножественныхЗначенийАдресатов(КоллекцияПолучателей, Значения, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Для Каждого Значение Из Значения Цикл
		Если ТипЗнч(Значение) = Тип("Структура") Тогда
			Получатель = Новый Структура("Адрес, Псевдоним", Значение.Адрес, Значение.Представление);
			НовыйПолучатель = ДобавитьПолучателя(КоллекцияПолучателей, Получатель);
			НовыйПолучатель.Контакт = Значение.Контакт;
		Иначе
			Для Каждого Получатель Из ОбщегоНазначенияКлиентСервер.АдресаЭлектроннойПочтыИзСтроки(Значение) Цикл
				ДобавитьПолучателя(КоллекцияПолучателей, Получатель);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ПриЧтенииАдресатов()
	
	СпискиАдресатов = Новый Массив;
	СпискиАдресатов.Добавить(Объект.ПолучателиПисьма);
	СпискиАдресатов.Добавить(Объект.ПолучателиКопий);
	СпискиАдресатов.Добавить(Объект.ПолучателиСкрытыхКопий);
	
	Для Каждого СписокАдресатов Из СпискиАдресатов Цикл
		Для Каждого СтрокаТаблицы Из СписокАдресатов Цикл
			ЗаполнитьЗначениеАдресаВСтрокеТаблицыАдресатов(СтрокаТаблицы);
		КонецЦикла;
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьЗначениеАдресаВСтрокеТаблицыАдресатов(СтрокаТаблицы)
	СтрокаТаблицы.АдресЗаполненКорректно = ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(СтрокаТаблицы.Адрес);
	СтрокаТаблицы.Значение = ПредставлениеАдреса(СтрокаТаблицы.Адрес, СтрокаТаблицы.Представление);
КонецПроцедуры
