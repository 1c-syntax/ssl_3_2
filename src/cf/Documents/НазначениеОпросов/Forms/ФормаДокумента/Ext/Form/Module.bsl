...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		СтандартныеПодсистемыСервер.СброситьШиринуЭлементовГруппы(Элементы.ГруппаДаты);
	КонецЕсли;
...
&НаКлиенте
Процедура ТипРеспондентовОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	
	Если ВыбранноеЗначение = ТипРеспондентов Тогда
	...
	Если Объект.Респонденты.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("ru = 'Для продолжения необходимо очистить список респондентов.'");
		...
		Обработчик = Новый ОписаниеОповещения("ТипРеспондентовОбработкаВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ТипРеспондентовОбработкаВыбораЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ТипРеспондентов = ДополнительныеПараметры.ВыбранноеЗначение;
		Модифицированность = Истина;
		ОбработатьИзменениеТипаРеспондента();
		УправлениеДоступностью();
	КонецЕсли;
