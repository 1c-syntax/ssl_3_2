...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.ГруппаНачала.Видимость    = Ложь;
	...
	Иначе
		...
		Элементы.ГруппаНачала.Видимость           = Истина;
		Элементы.ГруппаОкончание.Видимость        = Истина;
		Элементы.ГруппаНачалоИОкончание.Видимость = Ложь;
		Элементы.ГруппаАвторДатаРегистрации.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	КонецЕсли;
...
Процедура ДатаВремяОкончанияПриИзменении(Элемент)
	Если Объект.ДатаОкончания < Объект.ДатаНачала Тогда
		...
		Если НачалоДня(Объект.ДатаОкончания) < НачалоДня(Объект.ДатаНачала) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Дата окончания не может быть меньше даты начала.'"), , "ДатаВремяНачала");
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Время окончания не может быть меньше времени начала.'"), , "ДатаВремяОкончания");
		КонецЕсли;
