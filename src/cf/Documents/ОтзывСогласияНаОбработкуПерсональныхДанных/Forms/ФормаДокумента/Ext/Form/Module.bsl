...
Процедура УстановитьСведенияДействующегоСогласия()
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Или Не ЗначениеЗаполнено(Объект.Субъект) Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОписаниеДействующегоСогласия");
		Возврат;
	КонецЕсли;
	...
	Если Согласие = Неопределено Тогда
		...
		СведенияТекст = НСтр("ru = 'Не обнаружено действующего согласия на обработку персональных данных субъекта.'");
	Иначе
	...
	ПараметрыСообщения.Сообщение = СведенияТекст;
	ПараметрыСообщения.Идентификатор = "ОписаниеДействующегоСогласия";
	ПараметрыСообщения.Скрываемый = Ложь;
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.СведенияОСогласииГруппа);
