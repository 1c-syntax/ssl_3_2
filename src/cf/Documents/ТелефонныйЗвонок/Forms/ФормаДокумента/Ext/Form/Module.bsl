...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказатьПредупреждениеНезаполненногоКонтакта(ЭтотОбъект, Объект.АбонентКонтакт);
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.ГруппаАбонент.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	КонецЕсли;
...
Процедура ПоказатьПредупреждениеНезаполненногоКонтакта(Форма, АбонентКонтакт)
	
	Форма.Элементы.ГруппаДекорацияКонтактыНеЗаполнены.Видимость = Не ЗначениеЗаполнено(АбонентКонтакт);
