...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СкрытьПоказатьДеревоРазделовШапка.Заголовок = ЗаголовокКомандаРазделы;
...
&НаКлиенте
Процедура Подключаемый_КомментарийОткрытие(Элемент, СтандартнаяОбработка)
	
	Если АнкетированиеКлиент.ЕстьРодительскаяГруппаТолькоПросмотр(Элемент) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементовФормы()
	...
	Если Параметры.ТолькоФормаЗаполнения Тогда
		...
		Элементы.ФормаГруппаОсновныеКомандыФормы.Видимость      = Ложь;
		...
		Иначе
			...
			Если СтрокаТаблицы.ТребуетсяКомментарий Тогда
				...
				Элементы[ИмяВопроса + "_Комментарий"].УстановитьДействие("НачалоВыбора",
					"Подключаемый_КомментарийОткрытие");
			КонецЕсли;
