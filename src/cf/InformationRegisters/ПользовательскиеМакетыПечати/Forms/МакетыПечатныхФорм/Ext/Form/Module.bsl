...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ГруппировкаПоВидамОбъектов Тогда
	...
	Если ЕстьДополнительныеЯзыки Тогда
		ВывестиИнформациюОДополнительныхЯзыках();
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(Элементы.Макеты.КоманднаяПанель, КоманднаяПанель);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ДополнительнаяИнформацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ГруппировкаПоВидамОбъектовПриИзменении(Элемент)
	...
	УстановитьОтборМакетов();
	СохранитьНастройкиСпособГруппировкиОбъектов(ГруппировкаПоВидамОбъектов);
...
Процедура ПереключитьИспользованиеВыбранныхМакетов(ИспользуетсяИзмененный)
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Макеты.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура УстановитьДоступностьКнопокКоманднойПанели()
	...
	ВыбраноНесколькоМакетов = Элементы.Макеты.ПолучитьСтрокиДляОбработки().Количество() > 1;
...
Процедура ЗаполнитьСписокМакетовПоРазделам()
	...
	ДеревоМакетов = РеквизитФормыВЗначение("Макеты");
	ДеревоМакетов.Строки.Очистить();
	ВывестиКоллекцию(ДеревоМакетов, Метаданные.Подсистемы);
	ЗаполнитьРазделПрочихПечатныхФорм(ДеревоМакетов);
	
	СортироватьДеревоМакетов(ДеревоМакетов);
	ЗначениеВРеквизитФормы(ДеревоМакетов, "Макеты");
...
Процедура ЗаполнитьСписокМакетовПоОбъектам()
	...
	ДеревоМакетов = РеквизитФормыВЗначение("Макеты");
	ДеревоМакетов.Строки.Очистить();
	ДеревоОбъектовМетаданныхЗаполнить(ДеревоМакетов);
	ЗаполнитьРазделПрочихПечатныхФорм(ДеревоМакетов);
...
&НаСервере
Процедура СортироватьДеревоМакетов(ДеревоМакетов)
	Для Каждого ВеткаПодчиненная Из ДеревоМакетов.Строки Цикл
		ВеткаПодчиненная.Строки.Сортировать("ЭтоПодраздел УБЫВ, СтрокаПоиска", Истина);
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьРазделПрочихПечатныхФорм(Ветка)
	...
	ГруппаПрочие = Ветка.Строки.Добавить();
	...
	Для Каждого Владелец Из ВладельцыМакетов Цикл
		ГруппаМакетов = ГруппаПрочие.Строки.Добавить();
...
Процедура ВывестиКоллекцию(Знач Ветка, Знач КоллекцияОбъектовМетаданных)
	...
	ТипВетки = ТипЗнч(Ветка);
	ЭтоУзелНаКлиенте = ТипВетки = Тип("ДанныеФормыЭлементДерева");
	ЭтоУзел = ЭтоУзелНаКлиенте Или ТипВетки = Тип("СтрокаДереваЗначений");
	Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
		Если ЭтоУзел И ПолноеИмя = Ветка.Идентификатор Тогда
			Продолжить;
		КонецЕсли;

		Если Не ЭтоПодсистема(ПолноеИмя) И Не ОбщегоНазначения.ЭтоЖурналДокументов(ОбъектМетаданных)
			И ОбъектыСКомандамиПечати.НайтиПоЗначению(ПолноеИмя) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭлементыВетки = ?(ЭтоУзелНаКлиенте, Ветка.ПолучитьЭлементы(), Ветка.Строки);
		НоваяВетка = ЭлементыВетки.Добавить();
		
		НоваяВетка.Представление = ОбъектМетаданных.Представление();
		НоваяВетка.КартинкаГруппы = КартинкаВИнтерфейсе(ОбъектМетаданных);
		НоваяВетка.Идентификатор = ПолноеИмя;
		...
		Если ЭтоПодсистема(ПолноеИмя) Тогда
			...
			ЭлементыНовойВетки = ?(ЭтоУзелНаКлиенте, НоваяВетка.ПолучитьЭлементы(), НоваяВетка.Строки);
			Если НЕ ЗначениеЗаполнено(ЭлементыНовойВетки) Тогда
				Индекс = ЭлементыВетки.Индекс(НоваяВетка);
				ЭлементыВетки.Удалить(Индекс);
			КонецЕсли;
		Иначе
...
&НаСервереБезКонтекста
Функция ЭтоПодсистема(ПолноеИмя)
	Результат = СтрНачинаетсяС(ПолноеИмя, "Подсистема");
	Возврат Результат;
КонецФункции
...
Функция ОбъектМетаданныхДоступен(ОбъектМетаданных)
	...
	Если Не ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных)
		...
		И Не ЭтоПодсистема(ОбъектМетаданных.ПолноеИмя()) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВывестиМакеты(ОписаниеОбъекта)
	...
	Если ТипЗнч(Ветка) = Тип("СтрокаДереваЗначений") Тогда
		КоллекцияЭлементов = Ветка.Строки;
	Иначе
...
Процедура ЗагрузитьНастройкиСпособГруппировкиОбъектов()
	...
	ГруппировкаПоВидамОбъектов = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкаОткрытияМакетовСпособГруппировкиОбъектов",
		"ГруппировкаПоВидамОбъектов", Ложь);
...
&НаСервере
Процедура ДеревоОбъектовМетаданныхЗаполнить(ДеревоМакетов)
	...
	Для Каждого ОбъектМетаданных Из ОбъектыМетаданных Цикл
		...
		Если КоллекцииВыбираемыхОбъектовМетаданных.НайтиПоЗначению(ОбъектМетаданных.Имя) <> Неопределено Тогда
			...
			ДобавитьЭлементДереваОбъектовМетаданных(ДеревоМакетов, ОбъектМетаданных);
...
&НаСервере
Процедура ДобавитьЭлементДереваОбъектовМетаданных(ДеревоМакетов, ПараметрыЭлемента)
	
	НоваяВетка = ДеревоМакетов.Строки.Добавить();
...
&НаСервереБезКонтекста
Процедура СохранитьНастройкиСпособГруппировкиОбъектов(Знач ГруппировкаПоВидамОбъектов)
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		"НастройкаОткрытияМакетовСпособГруппировкиОбъектов",
		"ГруппировкаПоВидамОбъектов",
		ГруппировкаПоВидамОбъектов);
	
КонецПроцедуры

&НаСервере
Процедура ВывестиИнформациюОДополнительныхЯзыках()
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Информация";
	ПараметрыСообщения.Идентификатор = "ДоступныДополнительныеЯзыки";
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Доступны языки'");
	ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Список доступных языков печатных форм расширен <a href=""ПерейтиКЯзыкамПечатныхФорм"">дополнительными языками</a>.
		|Для вывода печатной формы на дополнительном языке требуется выполнить перевод ее макета на этот язык.
		|Для перевода макета необходимо перейти к его редактированию, переключить язык и внести необходимые изменения.'"));
	ПараметрыСообщения.Обработчик = "Подключаемый_ДополнительнаяИнформацияОбработкаНавигационнойСсылки";
