...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормаНастройкаРежимаОтладки.Доступность = (ИсточникПравил = "ЗагруженныеИзФайла");
	Элементы.ИмяФайлаПравил.Доступность = Не ИсточникПравил = "ТиповыеИзКонфигурации";
...
Процедура ИсточникПравилПриИзменении(Элемент)
	...
	Элементы.ФормаНастройкаРежимаОтладки.Доступность = (ИсточникПравил = "ЗагруженныеИзФайла");
	Элементы.ИмяФайлаПравил.Доступность = (ИсточникПравил = "ЗагруженныеИзФайла");
...
&НаКлиенте
Процедура ИмяФайлаПравилНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
Процедура НастройкаРежимаОтладки(Команда)
	...
	ПараметрыФормы.Вставить("РежимОтладки", Запись.РежимОтладки);
	ПараметрыФормы.Вставить("РежимОтладкиВыгрузки", Запись.РежимОтладкиВыгрузки);
	ПараметрыФормы.Вставить("ИмяФайлаОбработкиДляОтладкиВыгрузки", Запись.ИмяФайлаОбработкиДляОтладкиВыгрузки);
	ПараметрыФормы.Вставить("РежимОтладкиЗагрузки", Запись.РежимОтладкиЗагрузки);
	ПараметрыФормы.Вставить("ИмяФайлаОбработкиДляОтладкиЗагрузки", Запись.ИмяФайлаОбработкиДляОтладкиЗагрузки);
	ПараметрыФормы.Вставить("РежимПротоколированияОбменаДанными", Запись.РежимПротоколированияОбменаДанными);
	ПараметрыФормы.Вставить("ИмяФайлаПротоколаОбмена", Запись.ИмяФайлаПротоколаОбмена);  
	ПараметрыФормы.Вставить("НеОстанавливатьПоОшибке", Запись.НеОстанавливатьПоОшибке);
...
Процедура ОбновитьСписокВыбораМакетаПравил()
	...
	Если ПустаяСтрока(Запись.ИмяПланаОбмена) Тогда
		...
		Элементы.ИнформацияОПравилах.Заголовок = НСтр("ru = 'Правила конвертации'");
		
	Иначе
		
		Элементы.ИнформацияОПравилах.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Элементы.ИнформацияОПравилах.Заголовок, Метаданные.ПланыОбмена[Запись.ИмяПланаОбмена].Синоним);				
