...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДеревоВерсийПерейтиНаВерсию.Видимость = ПереходНаВерсиюРазрешен;
	Элементы.ФормаПоказатьТехническиеСведенияОбИзмененииОбъекта.Видимость = ПереходНаВерсиюРазрешен;
...
Процедура СформироватьОтчетПоИзменениям(Команда)
	...
	ВыделенныеСтроки = Элементы.ДеревоВерсий.ПолучитьСтрокиДляОбработки();
...
&НаКлиенте
Процедура ПоказатьТехническиеСведенияОбИзмененииОбъекта(Команда)
...
Процедура УстановитьДоступность()
	...
	СтрокиДляОбработки = Элементы.ДеревоВерсий.ПолучитьСтрокиДляОбработки();
	ВсегоВыбраноВерсий = ?(СтрокиДляОбработки = Неопределено, 0, СтрокиДляОбработки.Количество());
	ВыбранаОднаВерсия = ВсегоВыбраноВерсий = 1;
	
	Элементы.ДеревоВерсийОткрытьВерсию.Доступность = ВыбранаОднаВерсия;
	Элементы.ДеревоВерсийПерейтиНаВерсию.Доступность = ВыбранаОднаВерсия;
	Элементы.ДеревоВерсийСравнитьВерсии.Доступность = ВсегоВыбраноВерсий > 0;
...
Процедура ОбновитьСписокВерсий()
	...
	Если ЕстьВерсии Тогда
		...
		Элементы.ДеревоВерсий.Видимость = Истина;
	...
	Иначе
		...
		СообщениеНетВерсий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Предыдущие версии отсутствуют: ""%1"".'"), Строка(Ссылка));
		Элементы.ДеревоВерсий.Видимость = Ложь;
		
		ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыВажногоСообщения.ТипСообщения = "Информация";
		ПараметрыВажногоСообщения.Сообщение = СообщениеНетВерсий;
		ПараметрыВажногоСообщения.КраткоеПредставление = СообщениеНетВерсий;
