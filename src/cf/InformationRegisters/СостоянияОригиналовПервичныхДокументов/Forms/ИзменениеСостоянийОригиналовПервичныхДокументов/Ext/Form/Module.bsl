...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетОригиналовПервичныхДокументов.ВывестиНаФормуКомандыСостоянияОригинала(ЭтотОбъект, Элементы.КомплектПечатныхФорм, СостоянияОригиналов);
	...
	Если ЗначениеЗаполнено(Параметры.ДокументСсылка) И Не Параметры.ДокументСсылка.Пустая() Тогда
		...
		ТекстСообщения = НСтр("ru='Состояние оригинала документа будет установлено по печатным формам.'");
	ИначеЕсли КомплектПечатныхФорм.Количество() = 0 Тогда
		ЗаполнитьВсеПечатныеФормы();
		ФильтрПечатныхФорм = "Все";
		ТекстСообщения = НСтр("ru='Состояние оригинала документа будет установлено по печатным формам.'");
	Иначе
		ТекстСообщения = НСтр("ru = 'Состояние оригинала документа установлено по печатным формам.'");
	...
	Если НастройкиПодсистемы.ОтображатьПодсказкуВФормеИзмененияСостояний Тогда
		...
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Установка состояния оригинала'");
		ПараметрыСообщения.Идентификатор = "СостояниеОригинала";
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.КомплектПечатныхФорм.КоманднаяПанель);
		Элементы.Переместить(Элементы.Применить, Элементы.ГруппаКнопокНиз);
	КонецЕсли;
...
Процедура УстановитьСостояниеОригинала(ИмяКоманды)
	...
	Для Каждого СтрокаСписка Из Элементы.КомплектПечатныхФорм.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ДобавитьВручную(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьДобавление", ЭтотОбъект);
	ОткрытьФорму("РегистрСведений.СостоянияОригиналовПервичныхДокументов.Форма.ДобавитьСвоюПечатнуюФорму", , , , , , ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПродолжитьДобавление(ПечатнаяФормаВручную, ДополнительныеПараметры) Экспорт
