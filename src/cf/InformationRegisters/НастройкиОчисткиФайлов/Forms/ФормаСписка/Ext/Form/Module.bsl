...
#Область ОбработчикиСобытийФPормы
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Настройки очистки файлов'");
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель, Элементы.ДеревоОбъектовМетаданных.КоманднаяПанель);
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
	КонецЕсли;
...
&НаКлиенте
Процедура АвтоматическаяОчисткаНенужныхФайловПриИзменении(Элемент)
	АвтоматическиОчищатьНенужныеФайлы = (АвтоматическаяОчисткаНенужныхФайлов <> "НеИспользуется");
	Если ИспользоватьТома Тогда
	...
	Если АвтоматическаяОчисткаНенужныхФайлов <> "НеИспользуется" И Не РазделениеВключено Тогда
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Иначе
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура АвтоматическаяОчисткаНенужныхФайловРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ДеревоОбъектовМетаданныхДействиеОчистка(Элемент, СтандартнаяОбработка)
	...
	УстановитьДействиеДляВыбранныхОбъектов(
		...
		Элементы.ДеревоОбъектовМетаданных.ПолучитьСтрокиДляОбработки());
	
КонецПроцедуры

&НаКлиенте
Процедура ДеревоОбъектовМетаданныхПриНаведении(Элемент, Строка, Поле)
...
Функция МожноОчищатьВерсии()
	Если Элементы.ДеревоОбъектовМетаданных.ПолучитьСтрокиДляОбработки().Количество() = 1 Тогда
...
&НаКлиенте
Процедура ДобавитьНастройкуОчистки(Команда)
	Элементы.ДеревоОбъектовМетаданных.ДобавитьСтроку();
КонецПроцедуры
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		ВыделенныеСтроки = Элементы.ДеревоОбъектовМетаданных.ПолучитьСтрокиДляОбработки();
...
Процедура УстановитьВидимостьКомандыОчистить()
	Элементы.Очистить.Видимость = ПустаяСтрока(ТекущееФоновоеЗадание);
	Элементы.ИнформацияОФоновомЗадании.Видимость = Не ПустаяСтрока(ТекущееФоновоеЗадание);
КонецПроцедуры
...
Процедура НастроитьРасписаниеЗавершение(Расписание, ДополнительныеПараметры) Экспорт
	...
	Элементы.АвтоматическаяОчисткаНенужныхФайлов.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(Строка(Расписание), , , , "Ссылка");
КонецПроцедуры

&НаСервере
Процедура УстановитьПериодОчисткиДляВыбранныхОбъектов(ПериодОчистки, ВыделенныеСтроки)
...
&НаСервере
Процедура УстановитьДействиеДляВыбранныхОбъектов(Знач Действие, ВыделенныеСтроки)
...
Процедура УстановитьУсловноеОформление()
	...
	Иначе
		Элемент.Оформление.УстановитьЗначениеПараметра("Шрифт", Новый Шрифт(ШрифтыСтиля.ТекстТретьегоУровня, , , Истина));
	КонецЕсли;
...
Процедура ДобавитьНастройкуОчисткиФайлов()
	...
	ОткрытьФорму(ПутьФормыВыбора(СтрокаДерева.ВладелецФайла), ПараметрыФормыВыбора,
		Элементы.ДеревоОбъектовМетаданных,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура НастроитьРежимыОчисткиФайлов()
	...
	Элементы.АвтоматическаяОчисткаНенужныхФайлов.СписокВыбора.Очистить();
	Элементы.АвтоматическаяОчисткаНенужныхФайлов.СписокВыбора.Добавить("НеИспользуется", НСтр("ru = 'Не используется'"));
	Если ИспользоватьТома Тогда
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.СписокВыбора.Добавить("УдаленныеИНенужныеФайлы", НСтр("ru = 'Удаленные и ненужные файлы'"));
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.СписокВыбора.Добавить("ТолькоУдаленныеФайлы", НСтр("ru = 'Только удаленные файлы'"));
	Иначе
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.СписокВыбора.Добавить("Используется", НСтр("ru = 'Используется'"));
	КонецЕсли;
	
	Если Не АвтоматическиОчищатьНенужныеФайлы Или РежимОчисткиФайлов = Перечисления.РежимыОчисткиФайлов.НеОчищать Тогда
		АвтоматическаяОчисткаНенужныхФайлов = "НеИспользуется";
	ИначеЕсли Не ИспользоватьТома Тогда
		АвтоматическаяОчисткаНенужныхФайлов = "Используется";
	ИначеЕсли РежимОчисткиФайлов = Перечисления.РежимыОчисткиФайлов.ОчищатьУдаленныеИНенужные Тогда
		АвтоматическаяОчисткаНенужныхФайлов = "УдаленныеИНенужныеФайлы";
	Иначе
		АвтоматическаяОчисткаНенужныхФайлов = "ТолькоУдаленныеФайлы";
	КонецЕсли;
	
	Элементы.АвтоматическаяОчисткаНенужныхФайлов.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(Строка(ТекущееРасписание()), , , , "Ссылка");
	Если АвтоматическаяОчисткаНенужныхФайлов <> "НеИспользуется" И Не РазделениеВключено Тогда
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Иначе
		Элементы.АвтоматическаяОчисткаНенужныхФайлов.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
...
Процедура РежимОчисткиФайловПриИзмененииСервер()
	Если АвтоматическаяОчисткаНенужныхФайлов = "НеИспользуется" Тогда
		РежимОчисткиФайлов = Перечисления.РежимыОчисткиФайлов.НеОчищать;
	ИначеЕсли АвтоматическаяОчисткаНенужныхФайлов = "УдаленныеИНенужныеФайлы" Тогда
		РежимОчисткиФайлов = Перечисления.РежимыОчисткиФайлов.ОчищатьУдаленныеИНенужные;
	Иначе
		РежимОчисткиФайлов = Перечисления.РежимыОчисткиФайлов.ОчищатьУдаленные;
	КонецЕсли;
