...
Процедура Сохранить(Команда)
	...
	СтрокаСписка = Элементы.АрхивСообщенийОбменов.ТекущиеДанные;
...
Процедура ПроверитьДоступностьПапки(Команда)
	...
	Существует = ПроверитьДоступностьПапкиНаСервере();
	Если Существует Тогда
		
		ТекстСообщения = НСтр("ru = 'Каталог доступен'");
		
	Иначе
		
		ТекстСообщения = НСтр("ru = 'Каталог не доступен'");
...
&НаСервере
Функция ПроверитьДоступностьПапкиНаСервере()
	
	КаталогФайла = Новый Файл(Запись.ПолныйПуть);
	Возврат КаталогФайла.Существует();
