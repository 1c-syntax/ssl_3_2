...
&НаКлиенте
Перем ПользователиПродолжитьДобавление, ВыбранныйПользователь,
	ПользователиТекущаяСтрокаПриАктивизации, ДатыЗапретаПоследняяСтрокаПриНаведении,
	ПользователиВРежимеРедактированияСтроки, ДатыЗапретаВРежимеРедактированияСтроки;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.ДатыЗапретаЗагрузкиДанных
	 ...
	 Или Параметры.ДатыЗапретаЗагрузкиДанных
	   И Не ЕстьПравоРедактированияЗапретаЗагрузкиДанных Тогда
		...
		Элементы.СпособУказанияДатыЗапрета.ТолькоПросмотр = Истина;
		Элементы.СпособУказанияОбщаяДата.ТолькоПросмотр = Истина;
		Элементы.ДатыЗапрета.ТолькоПросмотр = Истина;
		Элементы.ДатыЗапретаОписаниеДатыЗапретаПредставление.ОтображениеГиперссылкиЯчейки =
			ОтображениеГиперссылкиЯчейки.НеОтображать;
		Элементы.ДатыЗапретаПодобрать.Доступность = Ложь;
		Элементы.ДатыЗапретаДобавить.Доступность = Ложь;
		Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Доступность = Ложь;
		Элементы.ДатыЗапретаИзменить.Доступность = Ложь;
		Элементы.ДатыЗапретаКонтекстноеМенюИзменить.Доступность = Ложь;
	КонецЕсли;
	...
	Если БезРазделовИОбъектов Тогда
		УказаныРазныеНастройки = ЗначениеЗаполнено(ТекущийСпособУказанияДатыЗапрета(
			"*", ЕдинственныйРаздел, ЗначениеДляВсехПользователей, НачалоДня));
		Элементы.ГруппаСпособУказанияДатыЗапрета.Видимость = УказаныРазныеНастройки;
	...
	Иначе
		Список.Удалить(Список.НайтиПоЗначению("ПоОбъектам"));
	КонецЕсли;
	СписокВыбора = Элементы.ПользователиСпособУказанияДатыЗапрета.СписокВыбора;
	...
	Элементы.КоманднаяПанельТакси.Видимость = Истина;
	
	// Перемещение команд формы в командную панель для интерфейса Такси.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(КоманднаяПанель,
		Элементы.КоманднаяПанельТакси);
	
	// Корректировка порядка команд в командной панели таблицы пользователей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Пользователи.КоманднаяПанель,
		"ПользователиПодобрать");
	
	// Корректировка порядка команд в командной панели таблицы дат запрета.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ДатыЗапрета.КоманднаяПанель,
		"ДатыЗапретаПодобрать
		|ДатыЗапретаДобавить
		|ДатыЗапретаИзменить
		|ДатыЗапретаУдалить");
	
	// Корректировка порядка команд в контекстном меню таблицы дат запрета.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.ДатыЗапрета.КонтекстноеМеню,
		"ДатыЗапретаКонтекстноеМенюДобавить
		|ДатыЗапретаКонтекстноеМенюИзменить
		|ДатыЗапретаКонтекстноеМенюУдалить");
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("РегистрСведений.ДатыЗапретаИзменения.Форма.РедактированиеДатыЗапрета") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			ВыделенныеСтроки = ВыделенныеСтрокиСТекущейСтрокой(Элементы.ДатыЗапрета);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(Источник) = ВРег("ИспользоватьДатыЗапретаЗагрузки")
	 Или ВРег(Источник) = ВРег("ИспользоватьДатыЗапретаИзменения") Тогда
		...
		ПодключитьОбработчикОжидания("ПриИзмененииИспользованияДатЗапретаОбработчикОжидания", 0.1, Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура СпособУказанияДатыЗапретаПриИзменении(Элемент)
	
	Если СпособУказанияДатыЗапрета = ВыбранныйСпособУказанияДатыЗапрета Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПользователиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПодключитьОбработчикОжидания("ПользователиИзменитьСтрокуОбработчикОжидания", 0.1, Истина);
...
Процедура ПользователиПриАктивизацииСтроки(Элемент)
	...
	Если ПользователиТекущаяСтрокаПриАктивизации = Элементы.Пользователи.ТекущаяСтрока Тогда
		Возврат;
	КонецЕсли;
	ПользователиТекущаяСтрокаПриАктивизации = Элементы.Пользователи.ТекущаяСтрока;
...
&НаКлиенте
Процедура ПользователиПриАктивизацииПоля(Элемент)
	
	ПользователиОбновитьТекущееПоле(Элемент.ТекущиеДанные);
...
Процедура ПользователиПередНачаломИзменения(Элемент, Отказ)
	...
	ПользователиОбновитьТекущееПоле(ТекущиеДанные);
...
Процедура ПользователиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПользователиВРежимеРедактированияСтроки = Истина;
	
	ПользователиОбновитьДоступностьКнопокПолейСтроки(ТекущиеДанные);
	
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
		ТекущиеДанные.НомерКартинки = -1;
		ТекущиеДанные.СпособУказанияДатыЗапрета = "ОбщаяДата";
		ПодключитьОбработчикОжидания("ВыбратьПодобратьПользователейОбработчикОжидания", 0.1, Истина);
	КонецЕсли;
...
Процедура ПользователиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ПользователиВРежимеРедактированияСтроки = Неопределено;
...
&НаКлиенте
Процедура ПользователиПриНаведении(Элемент, Строка, Поле)
	
	ТекущиеДанные = ?(Строка = Неопределено, Элемент.ТекущиеДанные,
...
#Область ОбработчикиСобытийЭлементаСпособУказанияДатыЗапретаТаблицыФормыПользователи

&НаКлиенте
Процедура ПользователиСпособУказанияДатыЗапретаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.Пользователи.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено
	 Или Не ЗначениеЗаполнено(ТекущиеДанные.ПолноеПредставление) Тогда
...
&НаКлиенте
Процедура ПользователиСпособУказанияДатыЗапретаОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ПользователиСпособУказанияДатыЗапретаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	СпособУказанияДатыЗапретаОбработкаВыбора();
...
Процедура ДатыЗапретаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПодключитьОбработчикОжидания("ДатыЗапретаИзменитьСтрокуОбработчикОжидания", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ДатыЗапретаПриАктивизацииПоля(Элемент)
	
	ДатыЗапретаОбновитьТекущееПоле(Элемент.ТекущиеДанные);
		...
		Если ПоказыватьРазделыТекущегоПользователя Тогда
			...
			Если Не ТекущиеДанные.ЭтоРаздел
			   И Родитель <> Неопределено Тогда
				...
				ПодключитьОбработчикОжидания("ДатыЗапретаДобавитьСтрокуОбработчикОжидания", 0.1, Истина);
			КонецЕсли;
...
Процедура ДатыЗапретаПередНачаломИзменения(Элемент, Отказ)
	...
	ДатыЗапретаОбновитьТекущееПоле(ТекущиеДанные, ОткрытьФормуРедактированияДатыЗапрета);
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
		...
		ОбновитьЗначенияПрочитанныхСвойств(ТекущиеДанные,
			ПрочитанныеСвойства, Элементы.Пользователи.ТекущиеДанные);
	КонецЕсли;
...
Процедура ДатыЗапретаПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекущиеДанные = Элементы.ДатыЗапрета.ТекущиеДанные;
	УдалениеВыделенныхСтрок = Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() > 1
		Или Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() > 0
		  И ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85();
	
	Если УдалениеВыделенныхСтрок Тогда
		ТекстВопроса = НСтр("ru = 'Отключить даты запрета для выделенных строк?'");
	...
	ИначеЕсли ТекущиеДанные.ЭтоРаздел Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?'"),
					ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить дату запрета для раздела ""%1""?'"),
					ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
	...
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отключить дату запрета для объекта ""%1""?'"),
	...
	Если УдалениеВыделенныхСтрок Тогда
		Оповещение = Новый ОписаниеОповещения("ДатыЗапретаПередУдалениемЗавершение",
			ЭтотОбъект, Новый ФиксированныйМассив(Элементы.ДатыЗапрета.ВыделенныеСтроки));
	...
	Если ТекущиеДанные.ЭтоРаздел Тогда
		...
		Если ДатаЗапретаУстановлена(ТекущиеДанные, ТекущийПользователь) Или ЭлементыРаздела.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
				// Удаление даты запрета для раздела и всех объектов раздела.
				Оповещение = Новый ОписаниеОповещения("ДатыЗапретаПередУдалениемРаздела",
					ЭтотОбъект, ТекущиеДанные);
			Иначе
				// Удаление общей даты для всех разделов.
				Оповещение = Новый ОписаниеОповещения("ДатыЗапретаПередУдалениемЗавершение", ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элементы.ДатыЗапрета.ТекущаяСтрока));
			КонецЕсли;
			ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстСообщения = НСтр("ru = 'В этой строке дата запрета уже отключена.'");
	...
	// Удаление строки с объектом без даты (запись не существует).
...
Процедура ДатыЗапретаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ДатыЗапретаВРежимеРедактированияСтроки = Истина;
	ДатыЗапретаОбновитьДоступностьКнопокПолейСтроки(ТекущиеДанные);
	
	Если ЗначениеЗаполнено(ТекущиеДанные.ПолноеПредставление) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ТекущиеДанные.ЭтоРаздел Тогда
		УстановитьСвойство(ТекущиеДанные.Раздел, ТекущийРаздел(, Истина));
	КонецЕсли;
	Если ЭтоВсеПользователи(ТекущийПользователь) Или Не ТекущиеДанные.ЭтоРаздел Тогда
		УстановитьСвойство(ТекущиеДанные.ОписаниеДатыЗапрета, "ПроизвольнаяДата");
	КонецЕсли;
	УстановитьПредставлениеОписанияДатыЗапрета(ТекущиеДанные);
	ПодключитьОбработчикОжидания("ВыбратьОбъектыОбработчикОжидания", 0.1, Истина);
...
Процедура ДатыЗапретаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ДатыЗапретаВРежимеРедактированияСтроки = Неопределено;
...
&НаКлиенте
Процедура ДатыЗапретаПриНаведении(Элемент, Строка, Поле)
	
	Если Строка <> Неопределено Тогда
		ДатыЗапретаПоследняяСтрокаПриНаведении = Строка;
	КонецЕсли;
...
&НаКлиенте
Процедура ДатыЗапретаПолноеПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено
	   И Не ТекущиеДанные.ЭтоРаздел
	   И ЗначениеЗаполнено(ТекущиеДанные.Объект) Тогда
...
Процедура ДатыЗапретаПолноеПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.Объект <> ВыбранноеЗначение Тогда
		...
		Иначе
			ОбновитьЗначенияПрочитанныхСвойств(ТекущиеДанные,
				ЗначенияСвойств, Элементы.Пользователи.ТекущиеДанные);
...
&НаКлиенте
Процедура ФормаОбновить(Команда)
	
	ОбновитьНаСервере();
	ПодключитьОбработчикОжидания("РазвернутьДанныеПользователяОбработчикОжидания", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ФормаПоказатьОтчет(Команда)
...
&НаКлиенте
Процедура ПользователиПодобрать(Команда)
...
&НаКлиенте
Процедура ДатыЗапретаПодобрать(Команда)
...
Процедура ДатыЗапретаИзменить(Команда)
	...
	Если Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() > 1
	 Или Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() > 0
	...
	Если Элементы.ДатыЗапрета.ТекущаяСтрока = Неопределено Тогда
		Если ДатыЗапретаПоследняяСтрокаПриНаведении = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Элементы.ДатыЗапрета.ТекущаяСтрока = ДатыЗапретаПоследняяСтрокаПриНаведении;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	// Оформление пустого комментария в строке для всех пользователей.
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	// Оформление значения ВыбранныйСпособУказанияДатыЗапрета.
	Для Каждого ЭлементСписка Из Элементы.СпособУказанияДатыЗапрета.СписокВыбора Цикл
		Элемент = УсловноеОформление.Элементы.Добавить();
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ЭлементСписка.Представление);
		
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПользователиСпособУказанияДатыЗапрета.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиДатЗапрета.СпособУказанияДатыЗапрета");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = ЭлементСписка.Значение;
	КонецЦикла;
	
	// Отметка незаполненного объекта (раздел всегда заполнен).
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	// Оформление пустой даты (когда запись не существует).
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	...
	// Оформление пустой даты (когда запись существует).
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ДатыЗапретаДатаЗапрета.Имя);
	...
	// Оформление неустановленного значения даты запрета (когда запись не существует).
	Элемент = УсловноеОформление.Элементы.Добавить();
...
&НаКлиенте
Функция ВыделенныеСтрокиСТекущейСтрокой(ТаблицаФормы)
	
	ВыделенныеСтроки = ТаблицаФормы.ВыделенныеСтроки;
	
	Если ВыделенныеСтроки.Количество() = 0
	   И ТаблицаФормы.ТекущаяСтрока <> Неопределено Тогда
		
		ВыделенныеСтроки = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТаблицаФормы.ТекущаяСтрока);
	КонецЕсли;
...
Процедура ПрочитатьПользователей()
	...
	Запрос.УстановитьПараметр("ЕдинственныйРазделЗаполнен", ЗначениеЗаполнено(ЕдинственныйРаздел));
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	-1 КАК НомерКартинки,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА МАКСИМУМ(НЕ(ДатыЗапретаИзменения.Раздел = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
	|							И ДатыЗапретаИзменения.Объект = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка))) <> ИСТИНА
	|				ТОГДА ""ОбщаяДата""
	|			КОГДА МАКСИМУМ(ДатыЗапретаИзменения.Раздел <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
	|						И ДатыЗапретаИзменения.Объект <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
	|						И ДатыЗапретаИзменения.Объект <> ДатыЗапретаИзменения.Раздел) <> ИСТИНА
	|				ТОГДА ""ПоРазделам""
	|			КОГДА МАКСИМУМ(ДатыЗапретаИзменения.Раздел <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
	|							И ДатыЗапретаИзменения.Раздел <> &ЕдинственныйРаздел) <> ИСТИНА
	|					И &ЕдинственныйРазделЗаполнен
	|				ТОГДА ""ПоОбъектам""
	|			ИНАЧЕ ""ПоРазделамИОбъектам""
	|		КОНЕЦ КАК СТРОКА(150)) КАК СпособУказанияДатыЗапрета
	|ИЗ
	...
	Если ОписаниеВсехПользователей = Неопределено Тогда
		...
		ОписаниеВсехПользователей.СпособУказанияДатыЗапрета = "ОбщаяДата";
	КонецЕсли;
...
Процедура ПрочитатьДанныеПользователя(Форма, ТекстОшибки, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	Если ТекущийСпособУказания = Неопределено Тогда
		...
		Если Форма.СпособУказанияДатыЗапрета <> ТекущийСпособУказания Тогда
			...
			СпособУказанияДатыЗапретаПослеИзменения(Форма);
		КонецЕсли;
	КонецЕсли;
	...
	Если Форма.СпособУказанияДатыЗапрета = "ОбщаяДата" Тогда
		...
		Форма.ВыбранРасширенныйРежим = Ложь;
...
&НаКлиенте
Процедура РазвернутьДанныеПользователяОбработчикОжидания()
...
&НаКлиенте
Процедура ПриИзмененииИспользованияДатЗапретаОбработчикОжидания()
...
Процедура ПриИзмененииИспользованияДатЗапретаНаСервере()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных
	   И Не Константы.ИспользоватьДатыЗапретаЗагрузки.Получить()
	 Или Не Параметры.ДатыЗапретаЗагрузкиДанных
	   И Не Константы.ИспользоватьДатыЗапретаИзменения.Получить() Тогда
	
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Идентификатор = "ДатыЗапретаОтключены";
		ПараметрыСообщения.ТипСообщения = "Предупреждение";
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Даты запрета отключены'");
		ПараметрыСообщения.Заголовок = ПараметрыСообщения.КраткоеПредставление;
		ПараметрыСообщения.Сообщение = ?(Параметры.ДатыЗапретаЗагрузкиДанных,
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ДатыЗапретаОтключены");
	КонецЕсли;
...
&НаКлиенте
Процедура СпособУказанияДатыЗапретаОбработкаВыбора()
	
	ВыбранноеЗначение = ВыбранныйСпособУказанияДатыЗапрета;
	ВыбранныйСпособУказанияДатыЗапрета = СпособУказанияДатыЗапрета;
	...
	СпособУказанияДатыЗапретаПослеИзменения(ЭтотОбъект);
	ТекстОшибки = "";
...
&НаКлиентеНаСервереБезКонтекста
Процедура СпособУказанияДатыЗапретаПослеИзменения(Форма, ДанныеСтроки = Неопределено, СпособУказанияДатыЗапрета = Неопределено)
	
	Элементы = Форма.Элементы;
	Форма.ВыбранныйСпособУказанияДатыЗапрета = Форма.СпособУказанияДатыЗапрета;
	...
	Если ДанныеСтроки = Неопределено Тогда
		...
		Если ТекущаяСтрока = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ДанныеСтроки = Форма.ПользователиДатЗапрета.НайтиПоИдентификатору(ТекущаяСтрока);
		Если ДанныеСтроки = Неопределено Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если СпособУказанияДатыЗапрета = Неопределено Тогда
		СпособУказанияДатыЗапрета = Форма.СпособУказанияДатыЗапрета;
	КонецЕсли;
	
	ДанныеСтроки.СпособУказанияДатыЗапрета = СпособУказанияДатыЗапрета;
	
КонецПроцедуры


&НаКлиенте
Процедура ПользователиДобавитьСтрокуОбработчикОжидания()
...
&НаКлиенте
Процедура ПользователиИзменитьСтрокуОбработчикОжидания()
...
&НаКлиенте
Процедура ПользователиОбновитьТекущееПоле(ТекущиеДанные)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ПолноеПредставление) Тогда
		УстановитьСвойство(Элементы.Пользователи.ТекущийЭлемент, Элементы.ПользователиПолноеПредставление);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПользователиОбновитьДоступностьКнопокПолейСтроки(ДанныеСтроки)
	
	Если ДанныеСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КомментарийДоступен = ЗначениеЗаполнено(ДанныеСтроки.Представление)
		И (ТипЗнч(ДанныеСтроки.Пользователь) <> Тип("ПеречислениеСсылка.ВидыНазначенияДатЗапрета")
		   Или Не ДанныеСтроки.БезДатыЗапрета);
	
	Если Элементы.ПользователиКомментарий.РедактированиеТекста <> КомментарийДоступен Тогда
		Элементы.ПользователиКомментарий.РедактированиеТекста = КомментарийДоступен;
	КонецЕсли;
...
Процедура ОбновитьДанныеПользователя()
	...
	Если ТекущиеДанные = Неопределено
	 Или Не ЗначениеЗаполнено(ТекущиеДанные.Представление)
	 Или ПользователиПродолжитьДобавление = Истина Тогда
	...
	Контекст = Новый Структура("НовыйПользователь, ТекущийСпособУказания", НовыйПользователь);
	...
	Если ТекущийПользователь <> Неопределено И СпособУказанияЗначениеВСписке <> Неопределено Тогда
		...
		Если ТекущийСпособУказания <> СпособУказанияЗначениеВСписке.Значение 
			...
			Контекст.ТекущийСпособУказания = ТекущийСпособУказания;
	...
	ОбновитьДанныеПользователяЗавершение(Неопределено, Контекст);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьДанныеПользователяЗавершение(Ответ, Контекст) Экспорт
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Если Ответ = КодВозвратаДиалога.Нет Тогда
				...
				ПодключитьОбработчикОжидания(
					"ПользователиВосстановитьТекущуюСтрокуОбработчикОжидания", 0.1, Истина);
				Возврат;
			Иначе
				СпособУказанияДатыЗапретаПослеИзменения(ЭтотОбъект, НайденныеСтроки[0], Контекст.ТекущийСпособУказания);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ТекущийПользователь = Контекст.НовыйПользователь;
	
	// Чтение данных текущего пользователя.
	Если Контекст.НовыйПользователь = Неопределено Тогда
	...
	Если ПользователиПродолжитьДобавление = Истина Тогда
		ПодключитьОбработчикОжидания("ПользователиДобавитьСтрокуОбработчикОжидания", 0.1, Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПользователиВосстановитьТекущуюСтрокуОбработчикОжидания()
...
Процедура ПользователиОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Для Каждого Значение Из ВыбранноеЗначение Цикл
		...
		Если ПользователиДатЗапрета.НайтиСтроки(Отбор).Количество() = 0 Тогда
			...
			ОписаниеПользователя.СпособУказанияДатыЗапрета = "ОбщаяДата";
...
&НаКлиенте
Процедура ДатыЗапретаДобавитьСтрокуОбработчикОжидания()
	
	Элементы.ДатыЗапрета.ДобавитьСтроку();
	
КонецПроцедуры

&НаКлиенте
Процедура ДатыЗапретаИзменитьСтрокуОбработчикОжидания()
...
&НаКлиенте
Процедура ДатыЗапретаОбновитьТекущееПоле(ТекущиеДанные, ОткрытьФормуРедактированияДатыЗапрета = Ложь)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
		...
		УстановитьСвойство(Элементы.ДатыЗапрета.ТекущийЭлемент, Элементы.ДатыЗапретаПолноеПредставление);
		
	ИначеЕсли Элементы.ДатыЗапрета.ТекущийЭлемент = Элементы.ДатыЗапретаОписаниеДатыЗапретаПредставление
	      Или ТекущиеДанные.ОписаниеДатыЗапрета <> "ПроизвольнаяДата"
	        И ТекущиеДанные.ОписаниеДатыЗапрета <> "" Тогда
		
		ОткрытьФормуРедактированияДатыЗапрета = Истина;
	Иначе
		УстановитьСвойство(Элементы.ДатыЗапрета.ТекущийЭлемент, Элементы.ДатыЗапретаДатаЗапрета);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДатыЗапретаОбновитьДоступностьКнопокПолейСтроки(ДанныеСтроки)
	
	Если ДанныеСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЗначениеВыбрано = ЗначениеЗаполнено(ДанныеСтроки.Представление);
	Если Элементы.ДатыЗапретаПолноеПредставление.КнопкаВыбора <> (Не ЗначениеВыбрано) Тогда
		Элементы.ДатыЗапретаПолноеПредставление.КнопкаВыбора = Не ЗначениеВыбрано;
	КонецЕсли;
	
	Если Элементы.ДатыЗапретаДатаЗапрета.КнопкаВыбора <> ЗначениеВыбрано Тогда
		Элементы.ДатыЗапретаДатаЗапрета.КнопкаВыбора = ЗначениеВыбрано;
	КонецЕсли;
	Если Элементы.ДатыЗапретаДатаЗапрета.КнопкаОчистки <> ЗначениеВыбрано Тогда
		Элементы.ДатыЗапретаДатаЗапрета.КнопкаОчистки = ЗначениеВыбрано;
	КонецЕсли;
	Если Элементы.ДатыЗапретаДатаЗапрета.РедактированиеТекста <> ЗначениеВыбрано Тогда
		Элементы.ДатыЗапретаДатаЗапрета.РедактированиеТекста = ЗначениеВыбрано;
	КонецЕсли;
...
Процедура ДатыЗапретаПередУдалениемЗавершение(Ответ, ВыделенныеСтроки) Экспорт
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		// @skip-check query-in-loop - По-объектная запись данных
		УдалитьЗаписьПользователя(АдресУстановленныхБлокировок,
			ТекущиеДанные.Раздел, ТекущиеДанные.Объект, ТекущийПользователь);
...
Процедура ДатыЗапретаПриУдалении(ТекущиеДанные)
	...
	УдаляетсяТекущаяСтрока = Элементы.ДатыЗапрета.ТекущиеДанные = ТекущиеДанные;
	...
	Если Не УдаляетсяТекущаяСтрока Тогда
		Возврат;
	КонецЕсли;
...
Функция ДатыЗапретаПользователя(Знач Форма)
	...
	Иначе
		ПрочитанныеДатыЗапрета = ПрочитатьДанныеПользователяБезРазделов(
			Форма.Пользователь, Форма.ЕдинственныйРаздел, Форма.НачалоДня);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПрочитатьДанныеПользователяБезРазделов(Знач Пользователь, Знач ЕдинственныйРаздел, Знач НачалоДня)
	...
	Попытка
		...
		|	ВЫБОР
		|		КОГДА ОбщаяДата.ДатаЗапрета ЕСТЬ NULL
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК БезДатыЗапрета,
	...
	Пока Индекс >= 0 Цикл
		...
		Если Строка.ЗаписьСуществует Тогда
			Строка.ДатаЗапрета = ДатыЗапретаИзмененияСлужебный.ДатаЗапретаПоОписанию(
				Строка.ОписаниеДатыЗапрета, Строка.ДатаЗапрета, НачалоДня);
...
Процедура ОбновитьЗначенияПрочитанныхСвойств(ТекущиеЗначенияСвойств, ПрочитанныеСвойства, КомментарийТекущиеДанные = Ложь)
	...
	Если ПрочитанныеСвойства.Комментарий <> Неопределено Тогда
		Если КомментарийТекущиеДанные = Ложь Тогда
			УстановитьСвойство(ТекущиеЗначенияСвойств.Комментарий, ПрочитанныеСвойства.Комментарий);
			
		ИначеЕсли КомментарийТекущиеДанные <> Неопределено Тогда
			УстановитьСвойство(КомментарийТекущиеДанные.Комментарий, ПрочитанныеСвойства.Комментарий);
		КонецЕсли;
	КонецЕсли;
	
	УстановитьСвойство(ТекущиеЗначенияСвойств.ЗаписьСуществует,
		ПрочитанныеСвойства.ДатаЗапрета <> Неопределено);
	
	УстановитьСвойство(ТекущиеЗначенияСвойств.ДатаЗапрета,
		ПрочитанныеСвойства.ДатаЗапрета);
	
	УстановитьСвойство(ТекущиеЗначенияСвойств.ОписаниеДатыЗапрета,
		ПрочитанныеСвойства.ОписаниеДатыЗапрета);
	
	УстановитьСвойство(ТекущиеЗначенияСвойств.КоличествоДнейРазрешения,
		ПрочитанныеСвойства.КоличествоДнейРазрешения);
...
Процедура ЗаписатьОписаниеИДатуЗапрета(ТекущиеДанные = Неопределено)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекущиеДанные = Элементы.ДатыЗапрета.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.ДатаЗапрета)
			   И ТекущиеДанные.ОписаниеДатыЗапрета = "" Тогда
				
				ТекущиеДанные.ОписаниеДатыЗапрета = "ПроизвольнаяДата";
				
			ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.ДатаЗапрета)
			        И ТекущиеДанные.ОписаниеДатыЗапрета = "ПроизвольнаяДата" Тогда
...
&НаКлиенте
Процедура ВыбратьПодобратьПользователейОбработчикОжидания()
	
	Если ПользователиВРежимеРедактированияСтроки = Истина Тогда
...
&НаКлиенте
Процедура ВыбратьОбъектыОбработчикОжидания()
	
	Если ДатыЗапретаВРежимеРедактированияСтроки = Истина Тогда
...
Процедура УстановитьВидимость()
	...
	НастройкаДляВсехПользователей = УстановкаДатыЗапрета = "ДляВсехПользователей";
	
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если НастройкаДляВсехПользователей Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета загрузки данных прошлых периодов из других приложений для всех информационных баз.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета загрузки данных прошлых периодов для выбранных информационных баз.'");
		КонецЕсли;
	Иначе
		Если НастройкаДляВсехПользователей Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета ввода и редактирования данных прошлых периодов для всех пользователей.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru = 'Даты запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.'");
		КонецЕсли;
	КонецЕсли;
	...
	УстановитьСвойство(Элементы.УстановкаДатыЗапрета.РасширеннаяПодсказка.Заголовок,
		СтроковыеФункции.ФорматированнаяСтрока(РасширеннаяПодсказка));
	
	УстановитьСвойство(Элементы.УстановкаПоПользователям.Видимость, Не НастройкаДляВсехПользователей);
	Если НастройкаДляВсехПользователей Тогда
		Элементы.ДанныеПользователя.ТекущаяСтраница = Элементы.СтраницаПользовательВыбран;
	КонецЕсли;
	УстановитьСвойство(Элементы.СпособУказанияДатыЗапрета.Видимость, НастройкаДляВсехПользователей
		И Не (СпособУказанияДатыЗапрета = "ОбщаяДата" И Не ВыбранРасширенныйРежим));
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() = 0
	   И ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85()
	 Или Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() < 2
	   И Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		
		ТекущаяСтрока = Элементы.ДатыЗапрета.ТекущаяСтрока;
		Если ТекущаяСтрока = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ДанныеСтроки = Элементы.ДатыЗапрета.ДанныеСтроки(ТекущаяСтрока);
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Представление) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'В выбранной строке не заполнен объект.'"));
			Возврат;
		КонецЕсли;
		ВыделенныеСтроки = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущаяСтрока);
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.ДатыЗапрета.ВыделенныеСтроки Цикл
		ДанныеСтроки = Элементы.ДатыЗапрета.ДанныеСтроки(ВыделеннаяСтрока);
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Представление) Тогда
	...
	Если Не ЕстьЗаполненные Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Выделите хотя бы одну строку, у которой заполнен объект.'"));
		Возврат;
	КонецЕсли;
	
	Если ЕстьНезаполненные Тогда
		ТекстВопроса =
			НСтр("ru = 'У некоторых строк не заполнен объект, для них нельзя изменить дату запрета.
			           |
			           |Снять выделение со строк с незаполненным объектом?'");
		Оповещение = Новый ОписаниеОповещения(
			"РедактироватьДатуЗапретаВФормеПродолжение", ЭтотОбъект);
		Ответы = Новый СписокЗначений;
		Ответы.Добавить("СнятьЛишнееВыделение", НСтр("ru = 'Снять лишнее выделение'"));
		Ответы.Добавить("Отмена", НСтр("ru = 'Отмена'"));
		ПоказатьВопрос(Оповещение, ТекстВопроса, Ответы);
	Иначе
		РедактироватьДатуЗапретаВФормеЗавершение(Элементы.ДатыЗапрета.ВыделенныеСтроки)
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура РедактироватьДатуЗапретаВФормеПродолжение(Ответ, Контекст) Экспорт
	
	Если Ответ <> "СнятьЛишнееВыделение" Тогда
		Возврат;
	КонецЕсли;
	
	// Упорядочение выделенных строк по порядку.
	СписокСтрок = Новый СписокЗначений;
	СписокСтрок.ЗагрузитьЗначения(Элементы.ДатыЗапрета.ВыделенныеСтроки);
	КорневыеЭлементы = ДатыЗапрета.ПолучитьЭлементы();
	Для Каждого ЭлементСписка Из СписокСтрок Цикл
		СтрокаТаблицы = ДатыЗапрета.НайтиПоИдентификатору(ЭлементСписка.Значение);
		СтрокаРодителя = СтрокаТаблицы.ПолучитьРодителя();
		Если СтрокаРодителя <> Неопределено Тогда
			ТекущиеЭлементы = СтрокаРодителя.ПолучитьЭлементы();
			ЭлементСписка.Представление =
				Формат(КорневыеЭлементы.Индекс(СтрокаРодителя),
					"ЧЦ=10; ЧН=0000000000; ЧВН=; ЧГ=");
		Иначе
			ТекущиеЭлементы = КорневыеЭлементы;
		КонецЕсли;
		ЭлементСписка.Представление = ЭлементСписка.Представление
			+ Формат(ТекущиеЭлементы.Индекс(СтрокаТаблицы),
				"ЧЦ=10; ЧН=0000000000; ЧВН=; ЧГ=");
	КонецЦикла;
	СписокСтрок.СортироватьПоПредставлению();
	ВыделенныеСтроки = СписокСтрок.ВыгрузитьЗначения();
	
	// Отмена выделения строк с незаполненными объектами.
	НоваяТекущаяСтрока = Элементы.ДатыЗапрета.ТекущаяСтрока;
	...
	Пока Индекс >= 0 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Представление) Тогда
			...
			Элементы.ДатыЗапрета.ВыделенныеСтроки.Удалить(
				Элементы.ДатыЗапрета.ВыделенныеСтроки.Найти(ВыделеннаяСтрока));
			Если НоваяТекущаяСтрока = ВыделеннаяСтрока Тогда
				Если Индекс < ВыделенныеСтроки.Количество() Тогда
					НоваяТекущаяСтрока = ВыделенныеСтроки[Индекс];
				ИначеЕсли Индекс > 0 Тогда
					НоваяТекущаяСтрока = ВыделенныеСтроки[Индекс - 1];
				Иначе
	...
	Если Элементы.ДатыЗапрета.ТекущаяСтрока <> НоваяТекущаяСтрока Тогда
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			ВыделенныеСтроки = Новый ФиксированныйМассив(Элементы.ДатыЗапрета.ВыделенныеСтроки);
		КонецЕсли;
		Элементы.ДатыЗапрета.ТекущаяСтрока = НоваяТекущаяСтрока;
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			Элементы.ДатыЗапрета.ВыделенныеСтроки.Очистить();
			Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
				Элементы.ДатыЗапрета.ВыделенныеСтроки.Добавить(ВыделеннаяСтрока);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		// @skip-check query-in-loop - В этом вызове ветка с запросом не выполняется
		ПрочитанныеСвойства = ЗаблокироватьЗаписьПользователяНаСервере(АдресУстановленныхБлокировок,
			ТекущийРаздел(ДанныеСтроки), ДанныеСтроки.Объект, ТекущийПользователь);
		
		ОбновитьЗначенияПрочитанныхСвойств(ДанныеСтроки,
			ПрочитанныеСвойства, Элементы.Пользователи.ТекущиеДанные);
	КонецЦикла;
...
Процедура УстановитьПредставлениеОписанияДатыЗапрета(Знач Данные)
	...
	Если Не ЗначениеЗаполнено(Данные.ДатаЗапрета) И Не Данные.ЗаписьСуществует Тогда
		УстановитьСвойство(Данные.ОписаниеДатыЗапретаПредставление, НетДатыЗапретаПредставление(Данные));
	...
	КонецЕсли;
	УстановитьСвойство(Данные.ОписаниеДатыЗапретаПредставление, Представление);
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		...
		НСтр("ru = 'Для установки дат запрета по отдельным объектам выберите один из разделов ниже и повторите действие.'"));
