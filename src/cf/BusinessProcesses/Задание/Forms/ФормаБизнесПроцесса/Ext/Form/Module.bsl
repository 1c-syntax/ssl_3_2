...
Процедура ИнициализироватьФорму()
	...
	БизнесПроцессыИЗадачиСервер.УстановитьФорматДаты(Элементы.СрокПроверки);
...
Процедура ВывестиВажноеСообщение()
	...
	ПараметрыСообщения.Сообщение            = СтроковыеФункции.ФорматированнаяСтрока(СообщениеОСтатусеЗадания(ЭтотОбъект));
	ПараметрыСообщения.Идентификатор        = "СообщениеОСтатусеЗадания";
	ПараметрыСообщения.Скрываемый           = Ложь;
	ПараметрыСообщения.Обработчик           = "ИнфоНадписьЗаголовокОбработкаНавигационнойСсылки";
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаСостояние);
...
Процедура УстановитьДоступностьПроверяющего(Форма)
	...
	Форма.Элементы.Проверяющий.Доступность = ДоступностьПоля;
	Форма.Элементы.СрокПроверки.Доступность = ДоступностьПоля;
...
Процедура УстановитьСвойстваЭлементовФормы(Форма)
	...
	Форма.Элементы.ГлавнаяЗадача.Видимость = ЗначениеЗаполнено(Форма.Объект.ГлавнаяЗадача);
	Форма.Элементы.Предмет.Видимость = ЗначениеЗаполнено(Форма.Объект.Предмет);
	...
	Иначе
		...
		Форма.Элементы.ФормаЗаписатьИЗакрыть.Видимость           = Не Форма.Объект.Завершен;
