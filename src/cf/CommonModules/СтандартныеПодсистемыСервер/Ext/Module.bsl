...
//              и АдминистрированиеКластера.ПараметрыАдминистрированияИнформационнойБазыКластера.
...
#Область ПоддержкаИнтерфейсаТаксиПослеАдаптацииФормКИнтерфейсу85

// Позволяет переместить элементы из одной командной панели в другую.
// Например, из командной панели формы в командную панель списка или наоборот.
//
// Параметры:
//  Источник - ГруппаФормы
//  Приемник - ГруппаФормы
//  Исключения - Строка - строка имен элементов через перевод строки, которые нужно пропустить, например,
//                "ФормаВыбрать
...
//
Процедура ПереместитьЭлементыВГруппу(Источник, Приемник, Исключения = "") Экспорт
	
	Элементы = ВсеЭлементыФормы(Источник);
	Индекс = Источник.ПодчиненныеЭлементы.Количество();
	Место = Неопределено;
	СписокИсключений = СтрРазделить(Исключения, Символы.ПС);
	
	Пока Индекс > 0 Цикл
		Индекс = Индекс - 1;
		Элемент = Источник.ПодчиненныеЭлементы[Индекс];
		Если СписокИсключений.Найти(Элемент.Имя) = Неопределено Тогда
			Элементы.Переместить(Элемент, Приемник, Место);
			Место = Элемент;
		КонецЕсли;
	КонецЦикла;
...
// Позволяет скорректировать порядок одних элементов относительно других.
// Кроме того, позволяет скрыть видимость элементов дублей в интерфейсе Такси, если созданы.
//
// Параметры:
//  Группа - ГруппаФормы
//  ОписаниеЭлементов - Строка - многострочная строка со строками в следующих вариантах
//      "ИмяПеремещаемогоЭлемента" - вставить после предыдущего (в первой строке вставить в начало),
//      "ИмяПеремещаемогоЭлемента>ИмяЭлементаПередКоторымВставить" - вставить перед указанным элементом,
//      "ИмяПеремещаемогоЭлемента>." - вставить в конец,
//      "ИмяПеремещаемогоЭлемента<ИмяЭлементаПослеКоторогоВставить" - вставить после указанного элемента.
//    Вместо "ИмяПеремещаемогоЭлемента" можно указать два имени через двоеточие
//      "ИмяПеремещаемогоЭлемента:ЭлементКоторомуОтключитьВидимость", например,
//      отключить видимость дубля элемента в интерфейсе Такси.
//    Вместо "ИмяЭлементаПередКоторымВставить" или "ИмяЭлементаПослеКоторогоВставить"
//      можно указать несколько имен через слеш, например,
//      "ИмяЭлементаПередКоторымВставить1/ИмяЭлементаПередКоторымВставить2",
//      тогда если первый элемент не найден, будет использоваться второй.
//    Если имя элемента не найдено, то элемент пропускается (как будто не указан).
//
// Пример:
//	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КоманднаяПанель,
//		"СписокВключитьПометку
//		|СписокОтключитьПометку
//		|СписокВыбрать:ФормаВыбрать
//		|ФормаИмпортФайлов>ФормаГлобальныеКоманды
//		|ФормаИмпортПапки
//		|СписокУстановитьПароль<СписокИзменить/СписокСкопировать
//		|СписокНазначитьГруппы
//		|ФормаИзменитьФорму>.
//		|ФормаСправка"
//	Первый элемент СписокВключитьПометку без уточнения куда вставить и на первой строке,
//		поэтому вставляется в начало.
//	Второй элемент СписокОтключитьПометку тоже без уточнения куда вставить, но не на первой строке,
//		поэтому идет за предыдущим СписокВключитьПометку.
//	Третий элемент СписокВыбрать тоже без уточнения куда вставить,
//		поэтому идет за предыдущим СписокОтключитьПометку,
//		но у элемента ФормаВыбрать отключается видимость (если найден).
//	Четвертый элемент ФормаИмпортФайлов с уточнением вставить перед элементом ФормаГлобальныеКоманды,
//		поэтому, если элемент ФормаГлобальныеКоманды будет найден,
//		тогда элемент ФормаИмпортФайлов вставится перед ним,
//		в противном случае вставится за предыдущим элементом СписокВыбрать.
//	Пятый элемент ФормаИмпортПапки без уточнения куда вставить,
//		поэтому идет за предыдущим ФормаИмпортФайлов.
//	Шестой элемент СписокУстановитьПароль с уточнением вставить после элемента СписокИзменить,
//		поэтому вставится после СписокИзменить, если элемент СписокИзменить будет найден,
//		в противном случае вставится после СписокСкопировать, если элемент СписокСкопировать будет найден,
//		в противном случае вставится за предыдущим элементом ФормаИмпортПапки.
//	Седьмой элемент СписокНазначитьГруппы без уточнения куда вставить,
//		поэтому идет за предыдущим СписокУстановитьПароль.
//	Восьмой элемент ФормаИзменитьФорму с уточнением вставить в конец,
//		поэтому будет перемещен в конец (без вариантов).
//	Девятый элемент ФормаСправка без уточнения куда вставить,
//		поэтому идет за предыдущим ФормаИзменитьФорму.
//
Процедура ПереместитьЭлементыВГруппе(Группа, ОписаниеЭлементов) Экспорт
	
	Элементы = ВсеЭлементыФормы(Группа);
	ПоследнееМесто = ?(Группа.ПодчиненныеЭлементы.Количество() > 0,
		Группа.ПодчиненныеЭлементы[0], Неопределено);
	
	Для Каждого ОписаниеЭлемента Из СтрРазделить(ОписаниеЭлементов, Символы.ПС) Цикл
		Если Не ЗначениеЗаполнено(ОписаниеЭлемента) Тогда
			Продолжить;
		КонецЕсли;
		ВставитьПосле = СтрНайти(ОписаниеЭлемента, "<") > 0;
		ЧастиОписания = СтрРазделить(ОписаниеЭлемента, ?(ВставитьПосле, "<", ">"));
		Места = ?(ЧастиОписания.Количество() > 1, ЧастиОписания[1], "");
		Если Места = "." Тогда
			ПоследнееМесто = Неопределено;
		Иначе
			ИменаЭлементовМеста = СтрРазделить(Места, "/");
			Для Каждого ИмяЭлемента Из ИменаЭлементовМеста Цикл
				Элемент = Элементы.Найти(ИмяЭлемента);
				Если Элемент <> Неопределено Тогда
					ПоследнееМесто = Элемент;
					Если ВставитьПосле Тогда
						УстановитьСледующееМесто(ПоследнееМесто, Группа);
					КонецЕсли;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Имена = СтрРазделить(ЧастиОписания[0], ":");
		Элемент85 = Элементы.Найти(Имена[0]);
		Если Имена.Количество() > 1 Тогда
			ЭлементТакси = Элементы.Найти(Имена[1]);
			Если ЭлементТакси <> Неопределено Тогда
				ЭлементТакси.Видимость = Ложь;
			КонецЕсли;
		КонецЕсли;
		Если Элемент85 = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если Элемент85 = ПоследнееМесто Тогда
			УстановитьСледующееМесто(ПоследнееМесто, Группа);
		Иначе
			Элементы.Переместить(Элемент85, Группа, ПоследнееМесто);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

// Копирует в указанную группу недостающие элементы с указанными именами,
// например, кнопки из командной панели в контекстное меню.
//
// Параметры:
//  Группа - ГруппаФормы
//  ОписаниеЭлементов - Строка - многострочная строка формата
//   "ИмяКопируемогоЭлемента*ИмяЭлементаКопии
//   |..."
//
// Пример:
//	СтандартныеПодсистемыСервер.СкопироватьНедостающиеКнопки(Элементы.ПользователиИБ.КонтекстноеМеню,
//		"ПерейтиКПользователю*КонтекстноеМенюПерейтиКПользователю
//		|Сопоставить*КонтекстноеМенюСопоставить
//		|СопоставитьСНовым*КонтекстноеМенюСопоставитьСНовым
//		|ОтменитьСопоставление*КонтекстноеМенюОтменитьСопоставление");
//
Процедура СкопироватьНедостающиеКнопки(Группа, ОписаниеЭлементов) Экспорт
	
	Элементы = ВсеЭлементыФормы(Группа);
	
	Для Каждого ОписаниеЭлемента Из СтрРазделить(ОписаниеЭлементов, Символы.ПС) Цикл
		Имена = СтрРазделить(ОписаниеЭлемента, "*");
		КопируемыйЭлемент = Элементы.Найти(Имена[0]);
		НовыйЭлемент = Элементы.Найти(Имена[1]);
		Если КопируемыйЭлемент = Неопределено
		 Или НовыйЭлемент <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		НовыйЭлемент = Элементы.Добавить(Имена[1], ТипЗнч(КопируемыйЭлемент), Группа);
		ЗаполнитьЗначенияСвойств(НовыйЭлемент, КопируемыйЭлемент);
	КонецЦикла;
	
КонецПроцедуры

// Сбрасывает ширину всех подчиненных элементов группы формы в значение "0".
// Применяется в интерфейсе "Такси" для обеспечения обратной совместимости при переходе на интерфейс "8.5".
//
// Параметры:
//  ГруппаФормы - ГруппаФормы - группа формы, для подчиненных элементов которой необходимо сбросить ширину.
//
Процедура СброситьШиринуЭлементовГруппы(ГруппаФормы) Экспорт
	
	ПодчиненныеЭлементы = ГруппаФормы.ПодчиненныеЭлементы;
	
	Для Каждого ПодчиненныйЭлемент Из ПодчиненныеЭлементы Цикл
		
		СвойстваЭлемента = Новый Структура("Ширина", Неопределено);
		ЗаполнитьЗначенияСвойств(СвойстваЭлемента, ПодчиненныйЭлемент);
		
		Если СвойстваЭлемента.Ширина = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			ПодчиненныеЭлементы, ПодчиненныйЭлемент.Имя, "Ширина", 0);
...
// Возвращает заголовок приложения по умолчанию, полученный из значения
// константы ЗаголовокСистемы, свойства метаданных ПодробнаяИнформация,
// префикса "КОПИЯ" (если требуется) и представления области данных в модели сервиса.
//
// Возвращаемое значение:
//  Строка - заголовок приложения по умолчанию.
//
Функция ЗаголовокПриложенияПоУмолчанию() Экспорт
	
	ЗаголовокСистемы = Константы.ЗаголовокСистемы.Получить();
	ПодробнаяИнформация = Метаданные.ПодробнаяИнформация;
	
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1/%2",
			НСтр("ru = 'Не установлены разделители'"),
			ПодробнаяИнформация);
	ИначеЕсли ОбщегоНазначения.РазделениеВключено()
		...
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"%1/%2",
			МодульРаботаВМоделиСервиса.ПредставлениеОбластиДанных(),
			ПодробнаяИнформация);
	ИначеЕсли ЗначениеЗаполнено(ЗаголовокСистемы) Тогда
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1/%2",
				ЗаголовокСистемы,
				ПодробнаяИнформация);
	Иначе
		ЗаголовокПриложения = ПодробнаяИнформация;
	КонецЕсли;
...
// См. ПриОтправкеДанныхПодчиненномуСобытие
//
// Параметры:
//  ЭлементДанных            - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному.ЭлементДанных
//  ОтправкаЭлемента         - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному.ОтправкаЭлемента
//  СозданиеНачальногоОбраза - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному.СозданиеНачальногоОбраза
//  Получатель               - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному.Источник
//
...
// См. ПриОтправкеДанныхГлавномуСобытие
//
// Параметры:
//  ЭлементДанных    - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхГлавному.ЭлементДанных
//  ОтправкаЭлемента - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхГлавному.ОтправкаЭлемента
//  Получатель       - см. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхГлавному.Источник
//
Процедура ПриОтправкеДанныхГлавному(ЭлементДанных, ОтправкаЭлемента, Знач Получатель) Экспорт
...
// См. ПриПолученииДанныхОтПодчиненногоСобытие
//
// Параметры:
//  ЭлементДанных     - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного.ЭлементДанных
//  ПолучениеЭлемента - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного.ПолучениеЭлемента
//  ОтправкаНазад     - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного.ОтправкаНазад
//  Отправитель       - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного.Источник
//
...
// См. ПриПолученииДанныхОтГлавногоСобытие
//
// Параметры:
//  ЭлементДанных     - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного.ЭлементДанных
//  ПолучениеЭлемента - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного.ПолучениеЭлемента
//  ОтправкаНазад     - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного.ОтправкаНазад
//  Отправитель       - см. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного.Источник
//
...
Функция ЭтоИнформационнаяБазаВМоделиСервиса() Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБиблиотеки, "2.1.1.39") > 0 Тогда
...
// См. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхПодчиненному.
...
// См. ОбщегоНазначенияПереопределяемый.ПриОтправкеДанныхГлавному.
...
// См. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтПодчиненного.
...
// См. ОбщегоНазначенияПереопределяемый.ПриПолученииДанныхОтГлавного.
...
//  Отказ - Булево
//  ПолучениеИзГлавногоУзла - Булево
//
Процедура ПослеПолученияДанных(Отправитель, Отказ, ПолучениеИзГлавногоУзла = Ложь) Экспорт
	
	Если Отправитель.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Отправитель.Метаданные().РаспределеннаяИнформационнаяБаза Тогда
		Возврат;
	КонецЕсли;
	
	Если Отправитель.ЭтоНовый()
		Или Отправитель.НомерПринятого = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отправитель.Ссылка, "НомерПринятого") Тогда
		Возврат;
	КонецЕсли;
	
	ПолучениеИзГлавногоУзла = (ПланыОбмена.ГлавныйУзел() = Отправитель.Ссылка);
	ИнтеграцияПодсистемБСП.ПослеПолученияДанных(Отправитель, Отказ, ПолучениеИзГлавногоУзла);
...
Функция МинимальнаяВерсияПлатформыДляЗапуска() Экспорт
	...
	Возврат "8.5.1.960"; // Недопустимо менять в исправительных релизах.
...
Функция МинимальнаяВерсияПлатформыДляИспользования() Экспорт
	...
	Версии.Добавить("8.5.1", "8.5.1.960");
...
// Для процедуры ПереместитьЭлементыВГруппе.
Процедура УстановитьСледующееМесто(ПоследнееМесто, Группа)
	
	Если ПоследнееМесто = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Индекс = Группа.ПодчиненныеЭлементы.Индекс(ПоследнееМесто);
	Если Индекс + 1 < Группа.ПодчиненныеЭлементы.Количество() Тогда
		ПоследнееМесто = Группа.ПодчиненныеЭлементы[Индекс + 1];
	Иначе
		ПоследнееМесто = Неопределено;
	КонецЕсли;
...
// Для процедур ПереместитьЭлементыВГруппу, ПереместитьЭлементыВГруппе, ДобавитьНедостающиеКоманды.
Функция ВсеЭлементыФормы(Элемент)
...
// Возвращает элементы стиля конфигурации (цвета и шрифты).
//
// Возвращаемое значение:
//  Структура:
//   * Ключ - Строка - имя элемента стиля.
//   * Значение - Цвет, Шрифт - значение элемента стиля.
//
Функция ЭлементыСтиля() Экспорт
	...
	Для Каждого ЭлементСтиля Из Метаданные.ЭлементыСтиля Цикл
		Если ТипЗнч(ЭлементСтиля.Значение) <> Тип("Цвет")
		   И ТипЗнч(ЭлементСтиля.Значение) <> Тип("Шрифт") Тогда
			Продолжить;
		КонецЕсли;
		НаборЭлементовСтиля.Вставить(ЭлементСтиля.Имя, ЭлементСтиля.Значение);
	КонецЦикла;
	
	СтандартныеИменаЦветовСтиля = СтандартныеИменаЦветовСтиля();
	ЭлементыЦветовСтиля = Новый Структура(СтандартныеИменаЦветовСтиля);
	ЗаполнитьЗначенияСвойств(ЭлементыЦветовСтиля, ЦветаСтиля);
	
	Для Каждого ЭлементСтиля Из ЭлементыЦветовСтиля Цикл
		НаборЭлементовСтиля.Вставить(ЭлементСтиля.Ключ, ЭлементСтиля.Значение);
	КонецЦикла;
	
	СтандартныеИменаШрифтовСтиля = СтандартныеИменаШрифтовСтиля();
	ЭлементыШрифтовСтиля = Новый Структура(СтандартныеИменаШрифтовСтиля);
	ЗаполнитьЗначенияСвойств(ЭлементыШрифтовСтиля, ШрифтыСтиля);
	
	Для Каждого ЭлементСтиля Из ЭлементыШрифтовСтиля Цикл
		НаборЭлементовСтиля.Вставить(ЭлементСтиля.Ключ, ЭлементСтиля.Значение);
	КонецЦикла;
	
	НаборЭлементовСтиля.Вставить("ВторойБрендовый", ЦветаПалитры.ВторойБрендовый);
	НаборЭлементовСтиля.Вставить("Голубой",         ЦветаПалитры.Голубой);
	НаборЭлементовСтиля.Вставить("Желтый",          ЦветаПалитры.Желтый);
	НаборЭлементовСтиля.Вставить("Зеленый",         ЦветаПалитры.Зеленый);
	НаборЭлементовСтиля.Вставить("Красный",         ЦветаПалитры.Красный);
	НаборЭлементовСтиля.Вставить("Оранжевый",       ЦветаПалитры.Оранжевый);
	НаборЭлементовСтиля.Вставить("ПервыйБрендовый", ЦветаПалитры.ПервыйБрендовый);
	НаборЭлементовСтиля.Вставить("Розовый",         ЦветаПалитры.Розовый);
	НаборЭлементовСтиля.Вставить("Серый",           ЦветаПалитры.Серый);
	НаборЭлементовСтиля.Вставить("Синий",           ЦветаПалитры.Синий);
	НаборЭлементовСтиля.Вставить("Сиреневый",       ЦветаПалитры.Сиреневый);
	НаборЭлементовСтиля.Вставить("Фиолетовый",      ЦветаПалитры.Фиолетовый);
	
	Для Каждого ЦветПалитры Из Метаданные.ЦветаПалитры Цикл
		НаборЭлементовСтиля.Вставить(ЦветПалитры.Имя, ЦветПалитры.Цвет);
	КонецЦикла;
...
// Для функции ЭлементыСтиля.
Функция СтандартныеИменаЦветовСтиля()
	
	Возврат
	"АльтернативныйЦветФонаПоля,
	|ЦветАктивности,
	|ЦветАкцента,
	|ЦветВажного,
	|ЦветДополнительнойНавигации,
	|ЦветЛинииОтчета,
	|ЦветНавигации,
	|ЦветОсобогоТекста,
	|ЦветОтрицательногоЧисла,
	|ЦветРамки,
	|ЦветРамкиКнопки,
...
// Для функции ЭлементыСтиля.
Функция СтандартныеИменаШрифтовСтиля()
	
	Возврат
	"ЗаголовокВторогоУровня,
	|ЗаголовокПервогоУровня,
	|ЗаголовокПятогоУровня,
	|ЗаголовокТретьегоУровня,
	|ЗаголовокЧетвертогоУровня,
	|КрупныйШрифтТекста,
	|МелкийШрифтТекста,
	|ОбычныйШрифтТекста,
	|ОченьКрупныйШрифтТекста,
	|ПодзаголовокВторогоУровня,
	|ПодзаголовокПервогоУровня,
	|ТекстВторогоУровня,
	|ТекстПервогоУровня,
	|ТекстТретьегоУровня,
	|ШрифтТекста";
...
//  Структура:
//   * Ключ - Строка - имя элемента стиля.
//   * Значение - Цвет, Шрифт, Рамка - значение элемента стиля.
//              - ХранилищеЗначения - для толстого клиента обычного приложения
//                   значение элемента стиля преобразуется в сериализуемое значение.
//
Функция НаборЭлементовСтиля()
	...
	Для Каждого ЭлементСтиля Из ЭлементыСтиля Цикл
		НаборЭлементовСтиля.Вставить(ЭлементСтиля.Ключ, Новый ХранилищеЗначения(ЭлементСтиля.Значение));
	КонецЦикла;
