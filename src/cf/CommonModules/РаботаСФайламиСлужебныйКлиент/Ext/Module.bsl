...
Процедура ДействиеПриОткрытииФайлаВРабочемКаталоге(ОбработчикРезультата, ИмяФайлаСПутем, ДанныеФайла)
		   ...
		   И Параметры.РазмерВХранилищеФайлов <> Параметры.РазмерВРабочемКаталоге Тогда
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru = 'Размер файла в рабочем каталоге на компьютере и его копии в приложении отличается.
				           ...
				           |
				           |Открыть существующий без обновления или
				           |взять файл из приложения и заменить им существующий на компьютере?'"));
		Иначе
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru = 'Файл в приложении, отмеченный как занятый для редактирования, имеет более позднюю дату изменения (новее), чем его копия в рабочем каталоге на компьютере.
				           |
				           |Взять файл из приложения и заменить им существующий на компьютере или открыть существующий?'"));
		Иначе
			ВернутьРезультат(ОбработчикРезультата, "ВзятьИзХранилищаИОткрыть");
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Иначе
			...
			Параметры.Вставить(
				"Сообщение",
				НСтр("ru = 'Копия файла в рабочем каталоге на компьютере имеет более позднюю дату изменения (новее), чем в приложении.
				           |Возможно, эта копия была отредактирована.
				           |
				           |Открыть существующий файл на компьютере или заменить его на файл из приложения c потерей изменений и открыть?'"));
		КонецЕсли;
	КонецЕсли;
	
	ОткрытьФорму("ОбщаяФорма.ВыборДействияПриОбнаруженииОтличийФайла",
		Параметры, , , , , ОбработчикРезультата, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ОсвободитьФайлы(СписокФайлов) Экспорт
	...
	ВыбранныеФайлы = СписокФайлов.ПолучитьСтрокиДляОбработки();
...
// Обработчик оповещения, вызываемый из см. ДействиеСФайлом для выполнения операций,
		...
		Если ФайлВРабочемКаталоге Тогда
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Дата изменения файла ""%1"" в рабочем каталоге на компьютере более поздняя (новее), чем в приложении.
				...
				ОткрытьФорму("Обработка.РаботаСФайлами.Форма.РежимСозданияФайлаДляСохранитьКак",
					ПараметрыОткрытияФормы, , , , , Обработчик, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьКакСРасширениемПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВыборСохраненияШифрованногоФайла", , , , , ,
			Обработчик, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ВопросПриИмпортеФайлов",
			Параметры, , , , , Обработчик, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьНаОсновеШаблона(ПараметрыВыполнения) Экспорт
	...
	РежимОткрытия = РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс;
...
Процедура ВыбратьРежимИРедактироватьФайл(ОбработчикРезультата, ДанныеФайла, ДоступностьКомандыРедактировать)
	...
	Если Не ЗначениеЗаполнено(ДанныеФайла.Редактирует)
		...
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.КакОткрытьФайл",,,,,,
			Обработчик, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
		Возврат;
	КонецЕсли;
...
#Если Не ВебКлиент Тогда	
	...
	ЗаписьZIP = Новый ЗаписьФайлаАрхива(Контекст.ИмяФайла);
