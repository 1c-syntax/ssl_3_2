...
Процедура ПриСозданииНаСервере_ФормаСписка(Форма, Список, Расположение = Неопределено) Экспорт
	...
	РеквизитСписокСостояние.Гиперссылка = Истина;
...
Процедура ВывестиНаФормуКомандыСостоянияОригинала(Форма, Список, СостоянияОригиналов) Экспорт
	...
	ЭтоКомандыСписка = ТипЗнч(Список) = Тип("ТаблицаФормы");
	
	Элементы = Форма.Элементы;
	Родитель = ?(ЭтоКомандыСписка, Список.КоманднаяПанель, Форма.КоманднаяПанель);
	Картинка = ?(ЭтоКомандыСписка, БиблиотекаКартинок.УстановитьСостояниеОригиналаПервичногоДокумента,
	...
	Для Каждого Состояние Из СостоянияОригиналов Цикл
		...
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			...
			Если ЭтоКомандыСписка Тогда
				НастроитьРасположениеКнопокИКоманд(Команда, КнопкаУстановитьСостояние, Список);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если ТипЗнч(Список) = Тип("Булево") Тогда
		...
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			...
			Если ЭтоКомандыСписка Тогда
				НастроитьРасположениеКнопокИКоманд(КомандаФормы, КнопкаНастроитьСостояния, Список);
			КонецЕсли;
		КонецЕсли;
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено И Не ТипЗнч(Список) = Тип("Булево") Тогда
		...
		Если ЭтоКомандыСписка Тогда
			НастроитьРасположениеКнопокИКоманд(КомандаФормы, НоваяКнопка, Список);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Для Каждого Состояние Из СостоянияОригиналов Цикл
		...
		Команда.ИспользованиеТекущейСтроки = "Использует";
...
Процедура НастроитьРасположениеКнопокИКоманд(КомандаФормы, НоваяКнопка, Список)
	
	КомандаФормы.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.Использует;
	КомандаФормы.ИспользованиеВыделенныхСтрок = ИспользованиеВыделенныхСтрок.Использует;
	КомандаФормы.ИспользуемаяТаблица = Список;
