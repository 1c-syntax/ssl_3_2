...
Процедура ПоказатьВыборФорматаВложений(ОписаниеОповещения, НастройкиФормата = Неопределено, Владелец = Неопределено) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.ВыборФорматаВложений", ПараметрыФормы, , , , ,
		ОписаниеОповещения, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
КонецПроцедуры

// Открывает форму произвольного сообщения пользователю с заданными параметрами.
//
// Параметры:
//  Заголовок      - Строка - заголовок формы сообщения
//  ТекстСообщения - Строка, ФорматированнаяСтрока
//  Обработчик     - ОписаниеОповещения - обработчик действия в окне сообщения с параметрами:
//     * ИдентификаторДействия   - Строка - идентификатор ссылки форматированной строки или кнопки.
//     * ДополнительныеПараметры - Произвольный - параметры, которые были указаны в обработчике оповещения.
//  Кнопки - СписокЗначений:
//     * Значение      - Строка - имя кнопки.
//     * Представление - Строка - заголовок кнопки.
//     * Пометка       - Булево - Истина, если кнопку надо сделать главной.
//
Процедура ПоказатьФормуПроизвольногоСообщения(Заголовок, ТекстСообщения, Обработчик = Неопределено, Кнопки = Неопределено) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Заголовок", Заголовок);
	Параметры.Вставить("ТекстСообщения", ТекстСообщения);
	Параметры.Вставить("Кнопки", Кнопки);
	ОткрытьФорму("ОбщаяФорма.ФормаПроизвольногоСообщения", Параметры, , , , , Обработчик);
...
#Область РаботаСВажнымиСообщениями

// Обновляет добавленный ранее статус на форме.
//
// Параметры:
//   Форма         - ФормаКлиентскогоПриложения
//   Идентификатор - Строка - идентификатор обновляемого статуса
//   Наименование  - Строка - представление статуса. Рекомендуемая длина до 15 символов.
//   Картинка      - Картинка - картинка статуса.
//
Процедура ОбновитьСтатус(Форма, Идентификатор, Наименование, Картинка = Неопределено) Экспорт
	...
	Если ПроверкаСтатусы.БФ_ДобавляемыеСтатусы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтатусНайден = Ложь;
	Для Каждого Элемент Из Форма.БФ_ДобавляемыеСтатусы Цикл
		Если Элемент.Значение.Идентификатор = Идентификатор Тогда
			Элемент.Значение.Наименование = Наименование;
			Если ЗначениеЗаполнено(Картинка) Тогда
				Элемент.Значение.Картинка = Картинка;
			КонецЕсли;
	...
	ПрефиксСтатуса = ОбщегоНазначенияКлиентСервер.ПрефиксСтатуса();
	ПрефиксГруппыСтатуса = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыСтатуса();
	
	ИмяЭлементаНаименование    = ПрефиксСтатуса + Идентификатор;
	ИмяЭлементаНаименованиеВсе = ПрефиксСтатуса + ПрефиксВсе + Идентификатор;
	
	Элемент = Форма.Элементы.Найти(ИмяЭлементаНаименование);
	Если Элемент <> Неопределено Тогда
		Элемент.Заголовок = Наименование;
	КонецЕсли;
	
	Элемент = Форма.Элементы.Найти(ИмяЭлементаНаименованиеВсе);
	Если Элемент <> Неопределено Тогда
		Элемент.Заголовок = Наименование;
	КонецЕсли;
	...
	ТипКартинка = "_" + "КартинкаСтатуса";
	
	ИмяКартинки    = ПрефиксГруппыСтатуса + Идентификатор + ТипКартинка;
	ИмяКартинкиВсе = ПрефиксВсе + ПрефиксГруппыСтатуса + Идентификатор + ТипКартинка;
	
	Элемент = Форма.Элементы.Найти(ИмяКартинки);
	Если Элемент <> Неопределено Тогда
		Элемент.Картинка = Картинка;
	КонецЕсли;
	
	Элемент = Форма.Элементы.Найти(ИмяКартинкиВсе);
	Если Элемент <> Неопределено Тогда
		Элемент.Картинка = Картинка;
	КонецЕсли;
...
// Стандартный обработчик действия в важном сообщении.
//
// Параметры:
//   Форма   - ФормаКлиентскогоПриложения - форма, в которой обрабатывается действие.
//   Элемент - ПолеФормы
//           - КомандаФормы - элемент, нажатие которого необходимо обработать.
//   НавигационнаяСсылка  - Строка - идентификатор ссылки, если выполнялся переход из форматированной строки.
//   СтандартнаяОбработка - Булево - возвращаемый параметр, устанавливается в значение Ложь.
//
Процедура ОбработатьДействиеИнформационногоСообщения(Форма, Элемент, НавигационнаяСсылка, СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	НавигационнаяСсылкаПреобразованная = СтандартныеПодсистемыКлиент.ПреобразоватьСтрокуВДопустимоеНаименованиеКолонки(НавигационнаяСсылка);
	
	Если ФайловаяСистемаСлужебныйКлиент.ЭтоДопустимаяСсылка(НавигационнаяСсылка)
		И Не Форма.БФ_ОбработчикиКомандВажногоСообщения.Свойство(НавигационнаяСсылкаПреобразованная) Тогда
	...
	ЭтоСтатус  = Ложь;
	Если СтрНачинаетсяС(ИмяКоманды, ОбщегоНазначенияКлиентСервер.ПрефиксСтатуса()) Тогда
		// Обработка статуса.
		ИмяКоманды = СтрЗаменить(ИмяКоманды, ОбщегоНазначенияКлиентСервер.ПрефиксСтатуса(), "");
		Родитель = Элемент.Родитель;
		ЭтоСтатус = Истина;
	КонецЕсли;
	
	ЭтоПереходПоСсылке = Ложь;
	Если ЗначениеЗаполнено(НавигационнаяСсылка) Тогда
		ЭтоПереходПоСсылке = Истина;
		ИмяКоманды = НавигационнаяСсылкаПреобразованная;
	КонецЕсли;
	
	ИмяКоманды = СтрЗаменить(ИмяКоманды, ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе(), "");
	
	ПрефиксКомандыЗакрытияВажногоСообщения = ОбщегоНазначенияКлиентСервер.ПрефиксКомандСкрытияВажногоСообщения();
	Если СтрНачинаетсяС(ИмяКоманды, ПрефиксКомандыЗакрытияВажногоСообщения)
		И Не Форма.БФ_ОбработчикиКомандВажногоСообщения.Свойство(ИмяКоманды) Тогда
		// Стандартная обработка закрытия баннера, выполняется только при отсутствии обработчика закрытия.
		ИмяСкрываемойГруппы = СтрЗаменить(ИмяКоманды, ПрефиксКомандыЗакрытияВажногоСообщения, "");
		ИмяГруппы = ИмяСкрываемойГруппы;
		ИмяГруппыПоказатьВсе = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе() + ИмяСкрываемойГруппы;
		
		Если Форма.Элементы.Найти(ИмяГруппы) <> Неопределено Тогда
			Форма.Элементы[ИмяГруппы].Видимость = Ложь;
		КонецЕсли;
		
		Если Форма.Элементы.Найти(ИмяГруппыПоказатьВсе) <> Неопределено Тогда
			ГруппаПоказатьВсе = Форма.Элементы[ИмяГруппыПоказатьВсе];
			ГруппаПоказатьВсе.Видимость = Ложь;
			
			Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				РодительПоказатьВсе = ГруппаПоказатьВсе.Родитель;
				ОбщаяВидимость = Ложь;
				Для Каждого ГруппаПоказатьВсе Из РодительПоказатьВсе.ПодчиненныеЭлементы Цикл
					ОбщаяВидимость = ОбщаяВидимость Или ГруппаПоказатьВсе.Видимость;
				КонецЦикла;
				РодительПоказатьВсе.Видимость = ОбщаяВидимость;
			КонецЕсли;
		КонецЕсли;
		
		Форма.БФ_ВсегоВажныхСообщений = Форма.БФ_ВсегоВажныхСообщений - 1;
		Возврат;
	КонецЕсли;
	
	Идентификатор = ИмяКоманды;
	Если ЭтоСтатус Тогда
		Обработчик = Форма.БФ_ОбработчикиСтатусов[Идентификатор];
		ОбработчикЧастями = СтрРазделить(Обработчик, ".");
		
		ДополнительныйПараметр = Новый Структура;
		ДополнительныйПараметр.Вставить("Идентификатор", Идентификатор);
		ДополнительныйПараметр.Вставить("Родитель", Родитель);
	Иначе
		ПараметрыОбработчика = Форма.БФ_ОбработчикиКомандВажногоСообщения[ИмяКоманды];
		Если ТипЗнч(ПараметрыОбработчика) = Тип("Строка") Тогда
			ОбработчикЧастями = СтрРазделить(ПараметрыОбработчика, ".");
		Иначе
			ОбработчикЧастями = СтрРазделить(ПараметрыОбработчика.Обработчик, ".");
			Идентификатор = ПараметрыОбработчика.Идентификатор;
		КонецЕсли;
		Если ЭтоПереходПоСсылке Тогда
			ДополнительныйПараметр = НавигационнаяСсылка;
		Иначе
			ДополнительныйПараметр = Идентификатор;
		КонецЕсли;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения(ОбработчикЧастями[1], ОбщийМодуль(ОбработчикЧастями[0]), ДополнительныйПараметр);
	ВыполнитьОбработкуОповещения(ОписаниеОповещения, Форма);
...
//
// Вызывается перед клиентским кодом, который использует менеджер COM-соединений (V85.COMConnector)
