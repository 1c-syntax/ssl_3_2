...
Процедура ВыгрузкаОбъектаВыборки(КомпонентыОбмена, Объект, ПравилоОбработки = Неопределено) Экспорт
	...
	Если ОбъектСсылочногоТипа = Истина Тогда
		...
		Если КомпонентыОбмена.ВыгруженныеОбъекты.Найти(Объект.Ссылка) <> Неопределено Тогда
			
			Возврат;
			
		КонецЕсли;
		
		КомпонентыОбмена.ВыгруженныеОбъекты.Добавить(Объект.Ссылка);
		...
		Если Не ЭтоПовторноеОткрытие Тогда
			...
			Если ЭтоОбменЧерезПланОбмена Тогда
				...
				Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
					...
					КомпонентыОбмена.ПриоритетыКонвертации = КонвертацияОбъектноСервер.ПриоритетыКонвертацииВерсииФормата(КомпонентыОбмена.XMLСхема);
					КомпонентыОбмена.ВерсияФорматаОбмена = ФорматОбмена.Версия;
					КомпонентыОбмена.МенеджерОбмена = МенеджерОбменаВерсииФормата(КомпонентыОбмена.ВерсияФорматаОбмена, КомпонентыОбмена.УзелКорреспондента);
...
Процедура СнятьПометкуУдаленияСПредопределенногоЭлемента(Объект, ТипОбъекта, КомпонентыОбмена) Экспорт
	...
	Если ПометкаПредопределенный.ПометкаУдаления
		...
		ЗаписьПротокола            = ЗаписьПротоколаОбмена(80);
		ЗаписьПротокола.ТипОбъекта = ТипОбъекта;
		ЗаписьПротокола.Объект     = Строка(Объект);
		...
		ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 80, ЗаписьПротокола, Ложь);
...
Процедура ВыгрузитьОбъектыПоСсылке(КомпонентыОбмена, СсылкиИзОбъекта)
	...
	Если КомпонентыОбмена.ФильтрВыборкиИзменений <> Неопределено Тогда
		...
		ВыгрузитьОбъектыПоСсылкеПоФильтруВыборки(КомпонентыОбмена, СсылкиИзОбъекта);
		
	Иначе
		
		ВыгрузитьОбъектыПоСсылкеБезФильтраВыборки(КомпонентыОбмена, СсылкиИзОбъекта);
...
Процедура ВыгрузитьОбъектыПоСсылкеПоФильтруВыборки(КомпонентыОбмена, СсылкиИзОбъекта)
...
Процедура ВыгрузитьОбъектыПоСсылкеБезФильтраВыборки(КомпонентыОбмена, СсылкиИзОбъекта)
