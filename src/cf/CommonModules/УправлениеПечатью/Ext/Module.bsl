...
Функция КоллекцияПолейОператоры() Экспорт
...
Функция УпаковатьВАрхив(СписокФайлов) Экспорт
	...
	ЗаписьАрхива = Новый ЗаписьФайлаАрхива(ПотокВПамяти);
...
Функция МакетыРазличаются(Знач ИсходныйМакет, ИзмененныйМакет)
...
Функция ТекстЗапроса(ТипыИзменяемыхОбъектов, ОграничитьВыборку = Ложь)
	...
	Для Каждого ОбъектМетаданных Из ОбъектыМетаданных Цикл
		...
		ТекстЗапроса = "ВЫБРАТЬ " + ?(ОграничитьВыборку, "ПЕРВЫЕ 1001 ", "") 
			+ ТекстЗапроса + Символы.ПС + "
			|ИЗ
			|	"+ ОбъектМетаданных.ПолноеИмя() + " КАК " + ПсевдонимТаблицы; //@query-part
...
Процедура ОбработатьКомандыОтправки(Форма) Экспорт
	...
	Если ПодменюОтправить.ПодчиненныеЭлементы.Количество() = 1 Тогда
		Элемент = ПодменюОтправить.ПодчиненныеЭлементы[0];
		Если ТипЗнч(Элемент) = Тип("КнопкаФормы") Тогда
			Элементы.Переместить(Элемент, ПодменюОтправить.Родитель, ПодменюОтправить);
		КонецЕсли;
	КонецЕсли;
...
Функция ОбъектСОбрабатываемымиПредопределеннымиЗначениями(ОбъектПроверки)
	...
	Если ТипЗнч(ОбъектПроверки) = Тип("ОбъектМетаданных") Тогда
		...
		Результат =
			(ОбщегоНазначения.ЭтоСправочник(ОбъектПроверки)
	...
	Иначе
		...
		Результат =
			(Справочники.ТипВсеСсылки().СодержитТип(ОбъектПроверки)
