...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ПоложениеЗаголовка",         ПоложениеЗаголовкаЭлементаФормы.Авто);
...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Для каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		Если ИспользоватьНеограниченнуюСтроку Тогда
			...
			ОписаниеСвойства.ВыводитьВВидеГиперссылки = Ложь;
		...
		ЭтоСсылочныйРеквизит = Не СтроковыйРеквизит И ОписаниеСвойства.ВыводитьВВидеГиперссылки;
		Если СтроковыйРеквизит
			И Не ИспользоватьНеограниченнуюСтроку
			И ОписаниеСвойства.ВыводитьВВидеГиперссылки
			Или ЭтоСсылочныйРеквизит Тогда
			ФорматированнаяСтрока                           = Новый ОписаниеТипов("ФорматированнаяСтрока");
			ОписаниеСвойства.СтрокаСсылочногоТипа           = Не ЭтоСсылочныйРеквизит;
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		Если ОписаниеСвойства.СтрокаСсылочногоТипа Или ОписаниеСвойства.ВыводитьВВидеГиперссылки Тогда
			Если ОписаниеСвойства.СтрокаСсылочногоТипа И ЗначениеЗаполнено(ОписаниеСвойства.Значение) Тогда
		...
		Если ОписаниеСвойства.ВыводитьВВидеГиперссылки Тогда
			...
			ГруппаГиперссылки.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;
			...
			Если Не ОписаниеСвойства.СтрокаСсылочногоТипа Тогда
				// Добавления поля пустого ссылочного значения.
				Элемент = Форма.Элементы.Добавить(ОписаниеСвойства.ИмяСсылочногоРеквизитаЗначение, Тип("ПолеФормы"), ГруппаГиперссылки); // РасширениеПоляФормыДляПоляНадписи, РасширениеПоляФормыДляПоляВвода
				...
				Элемент.Видимость = Не ЗначениеЗаполнено(ОписаниеСвойства.Значение);
			КонецЕсли;
			...
			Элемент.Гиперссылка = Не ОписаниеСвойства.СтрокаСсылочногоТипа;
			Если Не ОписаниеСвойства.СтрокаСсылочногоТипа И Не ЗначениеЗаполнено(ОписаниеСвойства.Значение) Тогда
		...
		Иначе
			...
			ИначеЕсли ОписаниеСвойства.ВыводитьВВидеГиперссылки Тогда
				Элемент.Вид = ВидПоляФормы.ПолеНадписи;
			Иначе
				Элемент.Вид = ВидПоляФормы.ПолеВвода;
			...
			Элемент.РастягиватьПоВертикали = Ложь;
			Элемент.ПоложениеЗаголовка     = ПоложениеЗаголовкаЭлементаФормы[Форма.ПараметрыСвойств.ПоложениеЗаголовка];
		КонецЕсли;
...
Процедура УдалитьСтарыеРеквизитыИЭлементы(Форма) Экспорт
	...
	Для каждого ОписаниеСвойства Из Форма.Свойства_ОписаниеДополнительныхРеквизитов Цикл
		...
		ЭтоСсылочныйРеквизит = Не ОписаниеСвойства.СтрокаСсылочногоТипа И ОписаниеСвойства.ВыводитьВВидеГиперссылки;
		Если ОписаниеСвойства.СтрокаСсылочногоТипа
			Или ЭтоСсылочныйРеквизит Тогда
...
Процедура ЗаполнитьМеткиОбъекта(Форма, Объект = Неопределено, ПроизвольныйОбъект = Ложь) Экспорт
	...
	ДобавленныеМетки = Форма.Свойства_УстановленныеМетки;
	ДобавленныеМетки.Очистить();
	Для Каждого Метка Из Метки Цикл
		НоваяСтрока = ДобавленныеМетки.Добавить();
		НоваяСтрока.Свойство = Метка.Ссылка;
		НоваяСтрока.Наименование = Метка.Наименование;
		НоваяСтрока.НомерКартинки = Перечисления.ЦветаСвойств.Индекс(Метка.ЦветСвойства) + 1;
	КонецЦикла;
	
	Если Форма.Элементы.Найти("Свойства_УстановленныеМетки") = Неопределено Тогда
		Элемент = Форма.Элементы.Добавить("Свойства_УстановленныеМетки", Тип("ПолеФормы"), ГруппаМетки);
		Элемент.Заголовок                                      = НСтр("ru = 'Метки'");
		Элемент.ПутьКДанным                                    = "Свойства_УстановленныеМетки";
		Элемент.ПутьКДаннымЗначенияМножественногоЗначения      = "Свойства_УстановленныеМетки.Свойство";
		Элемент.КартинкаМножественныхЗначений                  = БиблиотекаКартинок.ПиктограммыСвойств;
		Элемент.ПутьКДаннымКартинкиМножественногоЗначения      = "Свойства_УстановленныеМетки.НомерКартинки";
		Элемент.ПутьКДаннымПредставленияМножественногоЗначения = "Свойства_УстановленныеМетки.Наименование";
		Элемент.РасширенноеРедактированиеМножественныхЗначений = Ложь;
		Элемент.ОтображатьФлажкиВВыпадающемСпискеПриВводеМножественныхЗначений = Ложь;
		Если ПроизвольныйОбъект Тогда
		...
		КонецЕсли;
		Элемент.УстановитьДействие("НачалоВыбора", "Подключаемый_СвойстваМеткиНачалоВыбора");
	КонецЕсли;
...
//    * ВидОбъектов                          - Строка - полное имя объекта метаданных для отображения легенды меток в
//                                                      форме списка.
//
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			// Таблица с добавленными метками.
			ТаблицаМеток = "Свойства_УстановленныеМетки";
			
			Реквизиты.Добавить(Новый РеквизитФормы(
				ТаблицаМеток, Новый ОписаниеТипов("ТаблицаЗначений")));
			
			Реквизиты.Добавить(Новый РеквизитФормы(
				"Наименование", Новый ОписаниеТипов("Строка"), ТаблицаМеток));
			
			Реквизиты.Добавить(Новый РеквизитФормы(
				"Свойство", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения"), ТаблицаМеток));
			
			Реквизиты.Добавить(Новый РеквизитФормы(
				"НомерКартинки", Новый ОписаниеТипов("Число"), ТаблицаМеток));
	...
	Если Контекст.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево Тогда
		Форма.ПараметрыСвойств.Вставить("ПоложениеЗаголовка", "Left");
	ИначеЕсли Контекст.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх Тогда
		Форма.ПараметрыСвойств.Вставить("ПоложениеЗаголовка", "Top");
	Иначе
		Форма.ПараметрыСвойств.Вставить("ПоложениеЗаголовка", "Auto");
	КонецЕсли;
...
Процедура ЗаполнитьЛегендуМеток(Форма, Объект)
	...
	Для Каждого Метка Из Метки Цикл
		...
		НовыйЭлемент.РасширеннаяПодсказка.Заголовок = Метка.Наименование;
		НовыйЭлемент.РасширеннаяПодсказка.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
