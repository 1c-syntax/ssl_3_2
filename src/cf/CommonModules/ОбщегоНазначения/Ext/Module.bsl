...
Функция ОбщиеПараметрыБазовойФункциональности(ВернутьЗакешированноеЗначение = Истина) Экспорт
	...
	Результат.Вставить("ГруппироватьСтатусыИВажныеСообщенияГоризонтально", Ложь);
	...
	ИмяУстаревшегоСвойства = "МинимальноНеобходимаяВерсияПлатформы";
	Результат.Вставить(ИмяУстаревшегоСвойства, Результат.МинимальнаяВерсияПлатформы);
	ИмяУстаревшегоСвойства = "РаботаВПрограммеЗапрещена";
	Результат.Вставить(ИмяУстаревшегоСвойства, Ложь);
...
#Область РаботаСоСтатусамиИВажнымиСообщениями

// Добавляет важное сообщение для привлечения внимания пользователя с размещением в шапке формы.
// Данная процедура только дополняет список сообщений, которые необходимо показать
// пользователю и не выводит его непосредственно на форму.
//
// При повторном добавлении сообщения с уже существующим идентификатором предыдущее будет удалено.
//
// Вывод важных сообщений на форму осуществляется при помощи процедуры ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму,
// подробнее про использование см. в описании к ней.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения
//   Параметры - см. ОбщегоНазначения.ПараметрыВажногоСообщения
//
Процедура ДобавитьВажноеСообщение(Форма, Параметры) Экспорт
	
	НезаполненныеПараметры = Новый Массив;
	Если Не ЗначениеЗаполнено(Параметры.ТипСообщения) Тогда
		НезаполненныеПараметры.Добавить("ТипСообщения");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Параметры.Сообщение) Тогда
		НезаполненныеПараметры.Добавить("Сообщение");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Параметры.КраткоеПредставление)
		И Не ЗначениеЗаполнено(Параметры.Родитель) Тогда
		НезаполненныеПараметры.Добавить("КраткоеПредставление");
	КонецЕсли;
	
	Если НезаполненныеПараметры.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Некорректный вызов процедуры добавления важного сообщения. Не заполнены обязательные параметры:
			|%1'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НезаполненныеПараметры, ", "));
	КонецЕсли;
	
	Проверка = Новый Структура("БФ_ДобавляемыеВажныеСообщения, БФ_КонтрольнаяСумма", "Проверка", "Проверка");
	ЗаполнитьЗначенияСвойств(Проверка, Форма);
	Если Проверка.БФ_ДобавляемыеВажныеСообщения = "Проверка" Тогда
		Реквизиты = Новый Массив;
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ДобавляемыеВажныеСообщения", Новый ОписаниеТипов("СписокЗначений")));
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ОбработчикиКомандВажногоСообщения", Новый ОписаниеТипов()));
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ВсегоВажныхСообщений", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный))));
		Если Проверка.БФ_КонтрольнаяСумма = "Проверка" Тогда
			Реквизиты.Добавить(Новый РеквизитФормы("БФ_КонтрольнаяСумма", Новый ОписаниеТипов("Строка")));
		КонецЕсли;
		
		Форма.ИзменитьРеквизиты(Реквизиты);
		
		Форма.БФ_ОбработчикиКомандВажногоСообщения = Новый Структура;
	КонецЕсли;
	
	Если ТипЗнч(Параметры.Сообщение) = Тип("ФорматированнаяСтрока") И ЗначениеЗаполнено(Параметры.Обработчик) Тогда
		ИмяОбработчикаЧастями = СтрРазделить(Параметры.Обработчик, ".");
		// Если в качестве обработчика передана процедура формы, то сохранять параметры не требуется,
		// т.к. процедура будет обработчиком элемента формы и ссылка будет передана штатно платформой.
		Если ИмяОбработчикаЧастями.Количество() > 1 Тогда
			Строка = ЗначениеВСтрокуВнутр(Параметры.Сообщение);
			РегулярноеВыражение = "<link\s+([^>]+)>";
			РезультатыПоиска = СтрНайтиВсеПоРегулярномуВыражению(Строка, РегулярноеВыражение);
			Для Каждого РезультатПоиска Из РезультатыПоиска Цикл
				Ссылка = СокрЛП(СтрЗаменить(СтрЗаменить(РезультатПоиска.Значение, "<link", ""), ">", ""));
				Ссылка = СтандартныеПодсистемыСервер.ПреобразоватьСтрокуВДопустимоеНаименованиеКолонки(Ссылка);
				Форма.БФ_ОбработчикиКомандВажногоСообщения.Вставить(Ссылка, Параметры.Обработчик);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	
	Если Параметры.ПараметрыДействия <> Неопределено Тогда
		Если ТипЗнч(Параметры.ПараметрыДействия) <> Тип("Массив") Тогда
			ПараметрыДействияВажногоСообщения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Параметры.ПараметрыДействия);
		Иначе
			ПараметрыДействияВажногоСообщения = Параметры.ПараметрыДействия;
		КонецЕсли;
		
		Для Каждого ОписаниеДействия Из ПараметрыДействияВажногоСообщения Цикл
			Форма.БФ_ОбработчикиКомандВажногоСообщения.Вставить(ОписаниеДействия.ИмяКоманды, ОписаниеДействия.Обработчик);
		КонецЦикла;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Параметры.Идентификатор) Тогда
		Для Каждого Элемент Из Форма.БФ_ДобавляемыеВажныеСообщения Цикл
			Если Элемент.Значение.Идентификатор = Параметры.Идентификатор Тогда
				Элемент.Значение = Параметры;
				Возврат;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Форма.БФ_ДобавляемыеВажныеСообщения.Добавить(Параметры);
	
КонецПроцедуры

// Добавляет контекстное важное сообщение для привлечения внимания пользователя с размещением в указанном месте на форме.
// Важно: Контекстно в группу можно вставить только одно сообщение.
//
// Данная процедура только дополняет список сообщений, которые необходимо показать
// пользователю и не выводит его непосредственно на форму.
//
...
//   Параметры - см. ОбщегоНазначения.ПараметрыВажногоСообщения
//   Родитель  - Строка
//             - ГруппаФормы - имя группы, в которой разместить сообщение.
//   Элемент   - Строка
//             - ДекорацияФормы
//             - ГруппаФормы
//             - КнопкаФормы
//             - ТаблицаФормы
//             - ПолеФормы - элемент, перед которым должно быть вставлено важное сообщение.
//
Процедура ВставитьВажноеСообщение(Форма, Параметры, Родитель, Элемент = Неопределено) Экспорт
	Если ТипЗнч(Родитель) = Тип("Строка") Тогда
		Параметры.Родитель = Родитель;
	ИначеЕсли Родитель <> Неопределено Тогда
		Параметры.Родитель = Родитель.Имя;
	КонецЕсли;
	
	Если ТипЗнч(Элемент) = Тип("Строка") Тогда
		Параметры.Элемент = Элемент;
	ИначеЕсли Элемент <> Неопределено Тогда
		Параметры.Элемент = Элемент.Имя;
	КонецЕсли;
	
	ДобавитьВажноеСообщение(Форма, Параметры);
КонецПроцедуры

// Коллекция параметров для настройки важного сообщения.
//
// Возвращаемое значение:
//  Структура:
//     * ТипСообщения - Строка - возможны значения Ошибка, Предупреждение, Информация, Успех. Заполнять обязательно.
//     * Идентификатор - Строка - идентификатор сообщения, который будет использоваться для
//          обновления информации или удаления сообщения при перечитывании данных формы.
//          Необходимо заполнять, если требуется обновлять или удалять важное сообщение при работе пользователя
//          или если добавление сообщения может выполняться не только в событии ПриСозданииНаСервере, в остальных
//          случаях необязательно.
//     * Заголовок  - Строка - заголовок важного сообщения.
//     * Сообщение  - Строка, ФорматированнаяСтрока - содержимое сообщения, заполнять обязательно.
//     * КраткоеПредставление - Строка - краткое и понятное описание сути проблемы.
//          Использование обобщающих формулировок вида "Найдены проблемы" не допускается, вместо
//          этого правильно будет написать, например, "Проблемы проверки учета".
//          Рекомендуемая длина до 30 символов, максимум - 38.
//          Обязательно заполнять для сообщений, выводимых в шапке формы процедурой ДобавитьВажноеСообщение.
//     * Обработчик - Строка - нужно указывать, если передана форматированная строка. Варианты:
//                             1) Процедура модуля формы, состав параметров как у события ОбработкаНавигационнойСсылки.
//                             2) Процедура клиентского общего модуля с параметрами:
//                                   # Форма - ФормаКлиентскогоПриложения
//                                   # Ссылка - Строка - ссылка из форматированной строки.
//     * ОбработчикЗакрытия - обработчик нажатия на кнопку закрытия сообщения. Для работы требуется заполнить свойство Идентификатор.
//                            Важно: Закрытие сообщения в необходимо делать самостоятельно вызовом процедуры ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение.
//                            Возможные варианты:
//                             1) Процедура модуля формы, состав параметров как у обработчика команды.
//                             2) Процедура клиентского общего модуля с параметрами:
//                                   # Форма - ФормаКлиентскогоПриложения
//                                   # Идентификатор - Строка - идентификатор важного сообщения.
//     * Скрываемый - Булево - можно ли скрыть важное сообщение нажатием на "х". По умолчанию - Истина.
//     * ПараметрыДействия - см. ОбщегоНазначения.ПараметрыДействияВажногоСообщения
//                         - Массив из см. ОбщегоНазначения.ПараметрыДействияВажногоСообщения
//
Функция ПараметрыВажногоСообщения() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("ТипСообщения",         "Предупреждение");
	Параметры.Вставить("Скрываемый",           Истина);
	Параметры.Вставить("Идентификатор",        "");
	Параметры.Вставить("Заголовок",            "");
	Параметры.Вставить("Сообщение",            "");
	Параметры.Вставить("КраткоеПредставление", "");
	Параметры.Вставить("Обработчик",           "");
	Параметры.Вставить("ОбработчикЗакрытия",   "");
	Параметры.Вставить("ПараметрыДействия",    Неопределено);
	Параметры.Вставить("Родитель",             Неопределено); // Для внутреннего использования.
	Параметры.Вставить("Элемент",              Неопределено); // Для внутреннего использования.
	
	Возврат Параметры;
	
КонецФункции

// Коллекция параметров для настройки команд, выводимых в важном сообщении.
//
// Возвращаемое значение:
//  Структура:
//     * ИмяКоманды - Строка - имя команды. Рекомендуется подбирать уникальное имя, которое не пересечется с другими командами.
//     * Обработчик - Строка - процедура-обработчик команды формы, возможные варианты:
//                             1) Процедура модуля формы, состав параметров как у обработчика команды.
//                             2) Процедура клиентского общего модуля с параметрами:
//                                   # Форма - ФормаКлиентскогоПриложения
//                                   # Ссылка - Строка - значение из свойства ИмяКоманды.
//     * Заголовок  - Строка
//
Функция ПараметрыДействияВажногоСообщения() Экспорт
	
	ПараметрыДействия = Новый Структура;
	ПараметрыДействия.Вставить("ИмяКоманды", "");
	ПараметрыДействия.Вставить("Обработчик", "");
	ПараметрыДействия.Вставить("Заголовок",  "");
	Возврат ПараметрыДействия;
	
КонецФункции

// Добавляет статус на форму.
// Например, для отображения текущего статуса заказа покупателя.
//
// При повторном добавлении статуса с уже существующим идентификатором предыдущий будет удален.
//
// Вывод статуса на форму осуществляется при помощи процедуры ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму,
// подробнее про использование см. в описании к ней.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения
//   Параметры - см. ОбщегоНазначения.ПараметрыСтатуса
//
Процедура ДобавитьСтатус(Форма, Параметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Параметры.Идентификатор) Тогда
		НезаполненныеПараметры.Добавить("Идентификатор");
	КонецЕсли;
	
	Если НезаполненныеПараметры.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Некорректный вызов процедуры добавления статуса. Не заполнены обязательные параметры:
			|%1'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НезаполненныеПараметры, ", "));
	КонецЕсли;
	
	Проверка = Новый Структура("БФ_ДобавляемыеСтатусы, БФ_КонтрольнаяСумма", "Проверка", "Проверка");
	ЗаполнитьЗначенияСвойств(Проверка, Форма);
	Если Проверка.БФ_ДобавляемыеСтатусы = "Проверка" Тогда
		Реквизиты = Новый Массив;
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ДобавляемыеСтатусы", Новый ОписаниеТипов("СписокЗначений")));
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ОбработчикиСтатусов", Новый ОписаниеТипов()));
		Реквизиты.Добавить(Новый РеквизитФормы("БФ_ВсегоСтатусов", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный))));
		Если Проверка.БФ_КонтрольнаяСумма = "Проверка" Тогда
			Реквизиты.Добавить(Новый РеквизитФормы("БФ_КонтрольнаяСумма", Новый ОписаниеТипов("Строка")));
		КонецЕсли;
		...
		Форма.БФ_ОбработчикиСтатусов = Новый Структура;
	КонецЕсли;
	
	Для Каждого Элемент Из Форма.БФ_ДобавляемыеСтатусы Цикл
		Если Элемент.Значение.Идентификатор = Параметры.Идентификатор Тогда
			Элемент.Значение = Параметры;
			Возврат;
		КонецЕсли;
	...
	Форма.БФ_ДобавляемыеСтатусы.Добавить(Параметры);
	
КонецПроцедуры

// Коллекция параметров для настройки выводимого на форму статуса.
//
// Возвращаемое значение:
//   Структура:
//      * Наименование  - Строка, ФорматированнаяСтрока - представление статуса. Рекомендуемая длина до 15 символов.
//                               Заполнять обязательно. Форматированную строку допустимо использовать только для
//                               оформления цвета статуса, добавление гиперссылок не поддерживается. Пример
//                               использования: СтроковыеФункции.ФорматированнаяСтрока(
//                                   НСтр("ru = '<span style=""color: ЦветТекстаФормы"">Состояние:</span> зарегистрирован'"));
//      * Идентификатор - Строка - идентификатор статуса, должен удовлетворять требованиям, предъявляемым
//                                 к именованию переменных встроенного языка. Заполнять обязательно.
//      * Картинка      - Картинка - картинка статуса.
//      * Порядок       - Число, Неопределено  - порядок вывода статуса на форме. Статусы будут выводиться по
//                                               возрастанию значения. По умолчанию - Неопределено, в порядке добавления
//                                               на форму.
//      * Обработчик    - Строка - обработчик нажатия на гиперссылку статуса.
//                                 Если не заполнен, то статус выводится обычной надписью. Варианты:
//                                 1) Процедура модуля формы, состав параметров как у события Нажатие у элемента ДекорацияФормы.
//                                 2) Процедура клиентского общего модуля с параметрами:
//                                     # Форма         - ФормаКлиентскогоПриложения
//                                     # Параметры - Структура:
//                                        * Идентификатор - Строка - идентификатор статуса.
//                                        * Родитель      - ГруппаФормы - элемент формы, к которому привязывается
//                                                                        расшифровка статуса.
//
Функция ПараметрыСтатуса() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Наименование", "");
	Параметры.Вставить("Идентификатор", Неопределено);
	Параметры.Вставить("Картинка", Неопределено);
	Параметры.Вставить("Порядок", Неопределено);
	Параметры.Вставить("Обработчик", "");
...
// Выводит на форму статусы и важные сообщения для привлечения внимания пользователя.
//
// Внешний вид важных сообщений зависит от их количества:
//   - Если было добавлено только одно сообщение, то оно отображается целиком и
//     дополнительно выделяется цветом в зависимости от значения свойства ТипСообщения.
//   - Если сообщений добавлено больше одного, то они выводятся одной строкой в виде
//     кратких представлений с отображением подробностей по нажатии на них.
//
// Для удаления ранее выведенного на форму статуса или сообщения можно воспользоваться процедурой
// см. ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение.
//
// Использование:
//  1) Вызывать необходимо разово, в конце события ПриСозданииНаСервере, при необходимости можно
//     добавить дополнительные вызовы для обновления состояния статусов и важных сообщений после некоторых действий пользователя.
//
//  2)Для корректной работы так же необходимо добавить подключаемый обработчик события формы:
//
//    &НаКлиенте
...
//   Форма - ФормаКлиентскогоПриложения
//
Процедура ВывестиСтатусыИВажныеСообщенияНаФорму(Форма) Экспорт
	
	Проверка = Новый Массив;
	ПроверкаВажныеСообщения = Новый Структура;
	ПроверкаВажныеСообщения.Вставить("БФ_ДобавляемыеВажныеСообщения");
	ЗаполнитьЗначенияСвойств(ПроверкаВажныеСообщения, Форма);
	ЕстьВажныеСообщения = (ПроверкаВажныеСообщения.БФ_ДобавляемыеВажныеСообщения <> Неопределено
		И Форма.БФ_ДобавляемыеВажныеСообщения.Количество() <> 0);
	
	Если ЕстьВажныеСообщения Тогда
		Проверка.Добавить(Форма.БФ_ДобавляемыеВажныеСообщения);
	КонецЕсли;
	
	ПроверкаСтатусы = Новый Структура;
	ПроверкаСтатусы.Вставить("БФ_ДобавляемыеСтатусы");
	ЗаполнитьЗначенияСвойств(ПроверкаСтатусы, Форма);
	ЕстьСтатусы = (ПроверкаСтатусы.БФ_ДобавляемыеСтатусы <> Неопределено
		И Форма.БФ_ДобавляемыеСтатусы.Количество() <> 0);
	
	Если ЕстьСтатусы Тогда
		Проверка.Добавить(Форма.БФ_ДобавляемыеСтатусы);
	КонецЕсли;
	
	Если Не ЕстьВажныеСообщения И Не ЕстьСтатусы Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Форма.БФ_КонтрольнаяСумма)
		Или Форма.БФ_КонтрольнаяСумма <> КонтрольнаяСуммаСтрокой(Проверка) Тогда
		Форма.БФ_КонтрольнаяСумма = КонтрольнаяСуммаСтрокой(Проверка);
	ИначеЕсли Форма.БФ_КонтрольнаяСумма = КонтрольнаяСуммаСтрокой(Проверка) Тогда
		Возврат;
	КонецЕсли;
	
	РодительскаяГруппа = Форма.Элементы.Найти("БФ_ГруппаВажныеСообщенияИСтатусы");
	Если РодительскаяГруппа <> Неопределено Тогда
		Форма.Элементы.Удалить(РодительскаяГруппа);
	КонецЕсли;
	...
	Иначе
		ПервыйЭлемент = ПодчиненныеЭлементы[0];
	КонецЕсли;
	
	ОбщиеПараметры = ОбщиеПараметрыБазовойФункциональности();
	
	РодительскаяГруппа = Форма.Элементы.Вставить("БФ_ГруппаВажныеСообщенияИСтатусы", Тип("ГруппаФормы"), Форма, ПервыйЭлемент);
	РодительскаяГруппа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	РодительскаяГруппа.ОтображатьЗаголовок = Ложь;
	РодительскаяГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85()
		И Не ОбщиеПараметры.ГруппироватьСтатусыИВажныеСообщенияГоризонтально Тогда
		РодительскаяГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	КонецЕсли;
	
	// Вывод статусов.
	Если ЕстьСтатусы Тогда
		ВывестиСтатусыНаФорму(Форма, РодительскаяГруппа);
	КонецЕсли;
	
	Если Не ЕстьВажныеСообщения Тогда
		Возврат;
	КонецЕсли;
	
	// Группа для важных сообщений.
	ГруппаРазмещенияВажныхСообщений = Форма.Элементы.Найти("БФ_ГруппаСообщенияШапки");
	Если ГруппаРазмещенияВажныхСообщений <> Неопределено Тогда
		// Повторный вывод сообщения.
		Форма.Элементы.Удалить(ГруппаРазмещенияВажныхСообщений);
	КонецЕсли;
	
	ГруппаРазмещенияВажныхСообщений = НоваяГруппаВажногоСообщения(Форма, "БФ_ГруппаСообщенияШапки", РодительскаяГруппа);
	ГруппаРазмещенияВажныхСообщений.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	ВсегоВажныхСообщений = Форма.БФ_ДобавляемыеВажныеСообщения.Количество();
	
	// Вывод контекстных важных сообщений.
	ВывестиКонтекстноеВажноеСообщение(Форма, ВсегоВажныхСообщений);
	
	Если ВсегоВажныхСообщений = 0 Тогда
		Возврат;
	КонецЕсли;
	
	// Вывод одиночного сообщения.
	Если ВсегоВажныхСообщений = 1 Тогда
		ВывестиОдиночноеВажноеСообщение(Форма, ГруппаРазмещенияВажныхСообщений);
		Возврат;
	КонецЕсли;
	
	СгруппированныеСообщения = НоваяТаблицаСгруппированныхСообщений();
	СгруппироватьСообщения(СгруппированныеСообщения, Форма.БФ_ДобавляемыеВажныеСообщения);
	
	ГруппаРазмещенияВажныхСообщений.Группировка = ГруппировкаПодчиненныхЭлементовФормы.АвтоматическиУчитыватьВидЭкрана;
	ГруппаРазмещенияВажныхСообщений.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Авто;
	ГруппаРазмещенияВажныхСообщений.РастягиватьПоГоризонтали = Истина;
	ГруппаРазмещенияВажныхСообщений.ОтображатьКакКарточку = Истина;
	
	// Вывод агрегированного сообщения, когда важных сообщений больше одного.
	ВывестиАгрегированноеВажноеСообщение(Форма, ГруппаРазмещенияВажныхСообщений, СгруппированныеСообщения, ЕстьСтатусы);
	
	// Добавление группы со списком всех важных сообщений.
	ВывестиГруппуПоказатьВсеВажныеСообщения(Форма, СгруппированныеСообщения, ГруппаРазмещенияВажныхСообщений);
	
	Форма.БФ_ВсегоВажныхСообщений = СгруппированныеСообщения.Количество();
...
#Область ВспомогательныеПроцедурыВажныхСообщений

Процедура ВывестиКонтекстноеВажноеСообщение(Форма, ВсегоВажныхСообщений)
	
	Для Каждого ЭлементСписка Из Форма.БФ_ДобавляемыеВажныеСообщения Цикл
		ПараметрыСообщения = ЭлементСписка.Значение;
		Если Не ЗначениеЗаполнено(ПараметрыСообщения.Родитель) Тогда
			Продолжить;
		КонецЕсли;
		ВсегоВажныхСообщений = ВсегоВажныхСообщений - 1;
		
		РодительскаяГруппа = Форма.Элементы[ПараметрыСообщения.Родитель];
		
		Если ЗначениеЗаполнено(ПараметрыСообщения.Идентификатор) Тогда
			ИмяГруппы = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения() + ПараметрыСообщения.Идентификатор;
		Иначе
			ИмяГруппы = ПараметрыСообщения.Родитель + "_" + "БФ_ГруппаКонтекстноеСообщение";
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПараметрыСообщения.Элемент) Тогда
			Элемент = Форма.Элементы[ПараметрыСообщения.Элемент];
		КонецЕсли;
		ГруппаРазмещенияКонтекстногоСообщения = Форма.Элементы.Вставить(ИмяГруппы, Тип("ГруппаФормы"), РодительскаяГруппа, Элемент);
		ГруппаРазмещенияКонтекстногоСообщения.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаРазмещенияКонтекстногоСообщения.ОтображатьЗаголовок = Ложь;
		ГруппаРазмещенияКонтекстногоСообщения.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		ГруппаРазмещенияКонтекстногоСообщения.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		
		ВывестиПростоеВажноеСообщение(Форма, ПараметрыСообщения, ГруппаРазмещенияКонтекстногоСообщения);
	КонецЦикла;
	
КонецПроцедуры

Процедура ВывестиОдиночноеВажноеСообщение(Форма, ГруппаРазмещенияВажныхСообщений)
	
	Для Каждого ЭлементСписка Из Форма.БФ_ДобавляемыеВажныеСообщения Цикл
		Если ЗначениеЗаполнено(ЭлементСписка.Значение.Родитель) Тогда
			// Пропускаем контекстные сообщения.
			Продолжить;
		КонецЕсли;
		ПараметрыСообщения = ЭлементСписка.Значение;
		Прервать;
	...
	Если ЗначениеЗаполнено(ПараметрыСообщения.Идентификатор) Тогда
		ИмяГруппы = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения() + ПараметрыСообщения.Идентификатор;
	Иначе
		ИмяГруппы = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения() + ГруппаРазмещенияВажныхСообщений.Имя + "_" + ПараметрыСообщения.ТипСообщения;
	КонецЕсли;
	
	ГруппаРазмещенияСообщения = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаРазмещенияВажныхСообщений);
	ВывестиПростоеВажноеСообщение(Форма, ПараметрыСообщения, ГруппаРазмещенияСообщения);
	
КонецПроцедуры

Процедура ВывестиАгрегированноеВажноеСообщение(Форма, ГруппаРазмещенияВажныхСообщений, СгруппированныеСообщения, ЕстьСтатусы)
	
	ЦветаВажныхСообщений    = ЦветаВажныхСообщений();
	КартинкиВажныхСообщений = КартинкиВажныхСообщений();
	
	ВыведеноСообщений = 0;
	ЛимитСообщений = 5;
	Если ЕстьСтатусы Тогда
		ВсегоСтатусов = Форма.БФ_ДобавляемыеСтатусы.Количество();
		Если ВсегоСтатусов >= 3 Тогда
			ЛимитСообщений = 2;
		Иначе
			ЛимитСообщений = ЛимитСообщений - ВсегоСтатусов;
		КонецЕсли;
	КонецЕсли;
	
	Для Каждого СтрокаСообщение Из СгруппированныеСообщения Цикл
		
		Если ЗначениеЗаполнено(СтрокаСообщение.ПараметрыСообщения.Идентификатор) Тогда
			ИмяГруппы = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения() + СтрокаСообщение.ПараметрыСообщения.Идентификатор;
		Иначе
			ИмяГруппы = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения() + СгруппированныеСообщения.Индекс(СтрокаСообщение) + "_" + СтрокаСообщение.ТипСообщения;
		КонецЕсли;
		
		ГруппаВажноеСообщение = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаРазмещенияВажныхСообщений);
		ГруппаВажноеСообщение.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		
		ДекорацияКартинка  = Форма.Элементы.Добавить(ИмяГруппы + "_" + "КартинкаСообщения", Тип("ДекорацияФормы"), ГруппаВажноеСообщение);
		ДекорацияКартинка.Вид          = ВидДекорацииФормы.Картинка;
		ДекорацияКартинка.Картинка     = КартинкиВажныхСообщений[СтрокаСообщение.ТипСообщения];
		ДекорацияКартинка.ЦветКартинки = ЦветаВажныхСообщений[СтрокаСообщение.ТипСообщения];
		ДекорацияКартинка.Шрифт        = ШрифтВажныхСообщений();
		ДекорацияКартинка.РазмерКартинки = РазмерКартинки.ПоРазмеруШрифта;
		
		ИмяГруппы = ИмяГруппы + "_" + "Заголовок";
		ГруппаЗаголовок = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаВажноеСообщение);
		ГруппаЗаголовок.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
		ГруппаЗаголовок.Отображение = ОтображениеОбычнойГруппы.Нет;
		ГруппаЗаголовок.ОтображатьЗаголовок = Истина;
		КраткоеПредставление = СтрокаСообщение.ПараметрыСообщения.КраткоеПредставление;
		Если СтрДлина(КраткоеПредставление) > 38 Тогда
			КраткоеПредставление = Лев(КраткоеПредставление, 35) + "...";
		КонецЕсли;
		ГруппаЗаголовок.Заголовок = КраткоеПредставление;
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			ГруппаЗаголовок.ЦветТекстаЗаголовка = ЦветаПалитры.Синий;
		КонецЕсли;
		
		ИмяГруппы = ИмяГруппы + "_" + "Сообщение";
		ГруппаСообщение = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаЗаголовок);
		ГруппаСообщение.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаСообщение.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		ГруппаСообщение.Ширина = 40;
		ГруппаСообщение.ОтображатьКакКарточку = Истина;
		ГруппаСообщение.РастягиватьПоГоризонтали = Ложь;
		ГруппаСообщение.ЦветФона = ЦветаВажныхСообщений[СтрокаСообщение.ТипСообщения];
		
		ИмяГруппы = ИмяГруппы + "_" + "КонтентСообщения";
		ГруппаКонтентСообщения = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаСообщение);
		ГруппаКонтентСообщения.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаКонтентСообщения.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		ГруппаКонтентСообщения.РастягиватьПоГоризонтали = Истина;
		
		ВывестиКонтентСообщения(Форма, ГруппаКонтентСообщения, СтрокаСообщение.ПараметрыСообщения);
		
		ДобавитьКнопкуСкрытияСообщения(Форма, ГруппаСообщение, ГруппаВажноеСообщение, СтрокаСообщение.ПараметрыСообщения);
		
		ВыведеноСообщений = ВыведеноСообщений + 1;
		
		Если ВыведеноСообщений = ЛимитСообщений Тогда
			Прервать;
		КонецЕсли;
...
Процедура ВывестиГруппуПоказатьВсеВажныеСообщения(Форма, СгруппированныеСообщения, ГруппаРазмещенияВажныхСообщений)
	
	ЦветаВажныхСообщений = ЦветаВажныхСообщений();
	ИмяГруппы = ГруппаРазмещенияВажныхСообщений.Имя + "_" + "ПоказатьВсе";
	ГруппаПоказатьВсе = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаРазмещенияВажныхСообщений);
	ГруппаПоказатьВсе.Заголовок = НСтр("ru = 'Показать все'");
	ГруппаПоказатьВсе.ОтображатьЗаголовок = Истина;
	ГруппаПоказатьВсе.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаПоказатьВсе.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
	ГруппаПоказатьВсе.Отображение = ОтображениеОбычнойГруппы.Нет;
	ГруппаПоказатьВсе.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Право;
	ГруппаПоказатьВсе.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	ГруппаПоказатьВсе.ПутьКДаннымЗаголовка = "БФ_ВсегоВажныхСообщений";
	
	Для Каждого СтрокаСообщение Из СгруппированныеСообщения Цикл
		Если ЗначениеЗаполнено(СтрокаСообщение.ПараметрыСообщения.Идентификатор) Тогда
			ИмяГруппыСообщения = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе()
			+ ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения()
			+ СтрокаСообщение.ПараметрыСообщения.Идентификатор;
		Иначе
			ИмяГруппыСообщения = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе()
			+ ОбщегоНазначенияКлиентСервер.ПрефиксГруппыВажногоСообщения()
			+ СгруппированныеСообщения.Индекс(СтрокаСообщение)
			+ "_" + СтрокаСообщение.ТипСообщения;
		КонецЕсли;
		
		ГруппаСообщение = НоваяГруппаВажногоСообщения(Форма, ИмяГруппыСообщения, ГруппаПоказатьВсе);
		ГруппаСообщение.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаСообщение.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		ГруппаСообщение.Ширина = 40;
		ГруппаСообщение.ОтображатьКакКарточку = Истина;
		...
		ИмяГруппыКонтент = ИмяГруппыСообщения + "_" + "КонтентСообщения";
		ГруппаКонтентСообщения = НоваяГруппаВажногоСообщения(Форма, ИмяГруппыКонтент, ГруппаСообщение);
		ГруппаКонтентСообщения.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаКонтентСообщения.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		ГруппаКонтентСообщения.РастягиватьПоГоризонтали = Истина;
		
		ВывестиКонтентСообщения(Форма, ГруппаКонтентСообщения, СтрокаСообщение.ПараметрыСообщения, Истина);
		ДобавитьКнопкуСкрытияСообщения(Форма, ГруппаСообщение, ГруппаСообщение, СтрокаСообщение.ПараметрыСообщения);
	КонецЦикла;

КонецПроцедуры

Процедура ВывестиСтатусыНаФорму(Форма, РодительскаяГруппа)
	
	// Упорядочивание статусов.
	ТаблицаСтатусов = Новый ТаблицаЗначений;
	ТаблицаСтатусов.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число"));
	ТаблицаСтатусов.Колонки.Добавить("ОписаниеСтатуса");
	Для Каждого ЭлементСписка Из Форма.БФ_ДобавляемыеСтатусы Цикл
		ПараметрыСтатуса = ЭлементСписка.Значение;
		
		Строка = ТаблицаСтатусов.Добавить();
		Если ПараметрыСтатуса.Порядок = Неопределено Тогда
			Строка.Порядок = 9999;
		Иначе
			Строка.Порядок = ПараметрыСтатуса.Порядок;
		КонецЕсли;
		Строка.ОписаниеСтатуса = ПараметрыСтатуса;
	КонецЦикла;
	ТаблицаСтатусов.Сортировать("Порядок Возр");
	
	Форма.БФ_ВсегоСтатусов = Форма.БФ_ДобавляемыеСтатусы.Количество();
	// Группа для статусов.
	ГруппаРазмещенияСтатусов = Форма.Элементы.Найти("БФ_ГруппаСтатусы");
	Если ГруппаРазмещенияСтатусов <> Неопределено Тогда
		// Повторный вывод сообщения.
		Форма.Элементы.Удалить(ГруппаРазмещенияСтатусов);
	КонецЕсли;
	ГруппаРазмещенияСтатусов = НоваяГруппаВажногоСообщения(Форма, "БФ_ГруппаСтатусы", РодительскаяГруппа);
	ГруппаРазмещенияСтатусов.ОтображатьКакКарточку = Истина;
	
	ВыводитьПоказатьВсе = Форма.БФ_ВсегоСтатусов > 6;
	
	Если ВыводитьПоказатьВсе Тогда
		ИмяГруппы = ГруппаРазмещенияСтатусов.Имя + "_" + "ПоказатьВсе";
		ГруппаПоказатьВсе = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаРазмещенияСтатусов);
		ГруппаПоказатьВсе.Заголовок = НСтр("ru = 'Показать все'");
		ГруппаПоказатьВсе.ОтображатьЗаголовок = Истина;
		ГруппаПоказатьВсе.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		ГруппаПоказатьВсе.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
		...
		ГруппаПоказатьВсе.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Право;
		ГруппаПоказатьВсе.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
		ГруппаПоказатьВсе.ПутьКДаннымЗаголовка = "БФ_ВсегоСтатусов";
	КонецЕсли;
	
	Для Каждого Строка Из ТаблицаСтатусов Цикл
		ВывестиСтатус(Форма, Строка.ОписаниеСтатуса, ГруппаРазмещенияСтатусов);
		
		Если ВыводитьПоказатьВсе Тогда
			ПрефиксВсе = ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе();
			ВывестиСтатус(Форма, Строка.ОписаниеСтатуса, ГруппаПоказатьВсе, ПрефиксВсе);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ВыводитьПоказатьВсе Тогда
		Форма.Элементы.Переместить(ГруппаПоказатьВсе, ГруппаРазмещенияСтатусов);
	КонецЕсли;
	
КонецПроцедуры

Процедура ВывестиСтатус(Форма, ПараметрыСтатуса, ГруппаРазмещенияСтатусов, ПрефиксВсе = "")
	
	ИмяГруппы = ПрефиксВсе + ОбщегоНазначенияКлиентСервер.ПрефиксГруппыСтатуса() + ПараметрыСтатуса.Идентификатор;
	ГруппаСтатус = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаРазмещенияСтатусов);
	ГруппаСтатус.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	Если ЗначениеЗаполнено(ПрефиксВсе) Тогда
		ГруппаСтатус.Ширина = 30;
		ГруппаСтатус.РастягиватьПоГоризонтали = Ложь;
		ГруппаСтатус.ОтображатьКакКарточку    = Истина;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыСтатуса.Картинка) Тогда
		ДекорацияКартинка  = Форма.Элементы.Добавить(ГруппаСтатус.Имя + "_" + "КартинкаСтатуса", Тип("ДекорацияФормы"), ГруппаСтатус);
		ДекорацияКартинка.Вид      = ВидДекорацииФормы.Картинка;
		ДекорацияКартинка.Картинка = ПараметрыСтатуса.Картинка;
		ДекорацияКартинка.Шрифт    = ШрифтВажныхСообщений();
		ДекорацияКартинка.РазмерКартинки = РазмерКартинки.ПоРазмеруШрифта;
	КонецЕсли;
	
	ДекорацияНадпись = Форма.Элементы.Добавить(ОбщегоНазначенияКлиентСервер.ПрефиксСтатуса() + ПрефиксВсе + ПараметрыСтатуса.Идентификатор, Тип("ДекорацияФормы"), ГруппаСтатус);
	ДекорацияНадпись.Вид       = ВидДекорацииФормы.Надпись;
	ДекорацияНадпись.Заголовок = ПараметрыСтатуса.Наименование;
	ДекорацияНадпись.Шрифт     = ШрифтВажныхСообщений();
	Если ЗначениеЗаполнено(ПараметрыСтатуса.Обработчик) Тогда
		ДекорацияНадпись.Гиперссылка = Истина;
		
		Если СтрРазделить(ПараметрыСтатуса.Обработчик, ".").Количество() = 1 Тогда
			ДекорацияНадпись.УстановитьДействие("Нажатие", ПараметрыСтатуса.Обработчик);
		Иначе
			ДекорацияНадпись.УстановитьДействие("Нажатие", "Подключаемый_ОбработатьДействиеИнформационногоСообщения");
		КонецЕсли;
		
		Форма.БФ_ОбработчикиСтатусов.Вставить(ПараметрыСтатуса.Идентификатор, ПараметрыСтатуса.Обработчик);
	КонецЕсли;

КонецПроцедуры

Процедура ВывестиКонтентСообщения(Форма, ГруппаКонтент, ПараметрыСообщения, ПоказатьВсе = Ложь)
	
	ЕстьЗаголовок    = ЗначениеЗаполнено(ПараметрыСообщения.Заголовок);
	ЕстьДействие     = (ПараметрыСообщения.ПараметрыДействия <> Неопределено);
	
	Если ЕстьЗаголовок Тогда
		ДекорацияЗаголовок = Форма.Элементы.Добавить(ГруппаКонтент.Имя + "_" + "ЗаголовокВажногоСообщения", Тип("ДекорацияФормы"), ГруппаКонтент);
		ДекорацияЗаголовок.Вид       = ВидДекорацииФормы.Надпись;
		ДекорацияЗаголовок.Заголовок = ПараметрыСообщения.Заголовок;
		ДекорацияЗаголовок.АвтоМаксимальнаяШирина = Ложь;
		УстановитьШрифтЗаголовкаВажныхСообщений(ДекорацияЗаголовок.Шрифт);
	КонецЕсли;
	
	ДекорацияНадпись = Форма.Элементы.Добавить(ГруппаКонтент.Имя + "_" + "НадписьВажногоСообщения", Тип("ДекорацияФормы"), ГруппаКонтент);
	ДекорацияНадпись.Вид       = ВидДекорацииФормы.Надпись;
	ДекорацияНадпись.Заголовок = ПараметрыСообщения.Сообщение;
	ДекорацияНадпись.Шрифт     = ШрифтВажныхСообщений();
	ДекорацияНадпись.АвтоМаксимальнаяШирина = Ложь;
	
	Если ТипЗнч(ПараметрыСообщения.Сообщение) = Тип("ФорматированнаяСтрока") Тогда
		Если ЗначениеЗаполнено(ПараметрыСообщения.Обработчик) И СтрРазделить(ПараметрыСообщения.Обработчик, ".").Количество() = 1 Тогда
			ДекорацияНадпись.УстановитьДействие("ОбработкаНавигационнойСсылки", ПараметрыСообщения.Обработчик);
		Иначе
			ДекорацияНадпись.УстановитьДействие("ОбработкаНавигационнойСсылки", "Подключаемый_ОбработатьДействиеИнформационногоСообщения");
		КонецЕсли;
	КонецЕсли;
	
	Если ЕстьДействие Тогда
		ПараметрыДействия = ПараметрыСообщения.ПараметрыДействия;
		Если ТипЗнч(ПараметрыДействия) <> Тип("Массив") Тогда
			ПараметрыДействия = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыДействия);
		КонецЕсли;
		
		ИмяГруппы = ГруппаКонтент.Имя + "_" + "КомандыДействия";
		ГруппаКомандыДействия = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, ГруппаКонтент);
		
		Для Каждого ОписаниеДействия Из ПараметрыДействия Цикл
			ИмяКоманды = ?(ПоказатьВсе, ОбщегоНазначенияКлиентСервер.ПрефиксГруппыПоказатьВсе(), "") + ОписаниеДействия.ИмяКоманды;
			КомандаДействие = Форма.Команды.Найти(ИмяКоманды);
			Если КомандаДействие = Неопределено Тогда
				КомандаДействие = Форма.Команды.Добавить(ИмяКоманды);
			КонецЕсли;
			КомандаДействие.Заголовок = ОписаниеДействия.Заголовок;
			Если СтрРазделить(ОписаниеДействия.Обработчик, ".").Количество() = 1 Тогда
				КомандаДействие.Действие = ОписаниеДействия.Обработчик;
			Иначе
				КомандаДействие.Действие = "Подключаемый_ОбработатьДействиеИнформационногоСообщения";
			КонецЕсли;
			
			ИмяКнопки = ГруппаКомандыДействия.Имя + "_" + ОписаниеДействия.ИмяКоманды;
			КнопкаДействие = Форма.Элементы.Добавить(ИмяКнопки, Тип("КнопкаФормы"), ГруппаКомандыДействия);
			КнопкаДействие.Вид        = ВидКнопкиФормы.Гиперссылка;
			КнопкаДействие.Заголовок  = ОписаниеДействия.Заголовок;
			КнопкаДействие.ИмяКоманды = КомандаДействие.Имя;
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьКнопкуСкрытияСообщения(Форма, ГруппаРазмещения, СкрываемаяГруппа, Параметры)
	
	Если Не Параметры.Скрываемый Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Параметры.ОбработчикЗакрытия)
		И Не ЗначениеЗаполнено(Параметры.Идентификатор) Тогда
		ТекстИсключения = НСтр("ru = 'Для важного сообщения с заполненным свойством ""ОбработчикЗакрытия""
			|должно быть заполнено свойство ""Идентификатор"".
			|
			|Текст проблемного сообщения:
			|%1'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, Параметры.Сообщение);
	КонецЕсли;
	
	ИмяКоманды = ОбщегоНазначенияКлиентСервер.ПрефиксКомандСкрытияВажногоСообщения() + СкрываемаяГруппа.Имя;
	КомандаЗакрытия = Форма.Команды.Найти(ИмяКоманды);
	Если КомандаЗакрытия = Неопределено Тогда
		КомандаЗакрытия = Форма.Команды.Добавить(ИмяКоманды);
	КонецЕсли;
	КомандаЗакрытия.Заголовок = НСтр("ru = 'Скрыть сообщение'");
	КомандаЗакрытия.Подсказка = НСтр("ru = 'Скрыть сообщение'");
	КомандаЗакрытия.Картинка  = БиблиотекаКартинок.ВажноеСообщениеЗакрыть;
	Если ЗначениеЗаполнено(Параметры.ОбработчикЗакрытия) И СтрРазделить(Параметры.ОбработчикЗакрытия, ".").Количество() = 1 Тогда
		КомандаЗакрытия.Действие = Параметры.ОбработчикЗакрытия;
	Иначе
		КомандаЗакрытия.Действие = "Подключаемый_ОбработатьДействиеИнформационногоСообщения";
	КонецЕсли;
	
	КнопкаЗакрытие = Форма.Элементы.Добавить(СкрываемаяГруппа.Имя + "_" + "КнопкаЗакрыть", Тип("КнопкаФормы"), ГруппаРазмещения);
	КнопкаЗакрытие.Вид         = ВидКнопкиФормы.Гиперссылка;
	КнопкаЗакрытие.ИмяКоманды  = КомандаЗакрытия.Имя;
	КнопкаЗакрытие.Отображение = ОтображениеКнопки.Картинка;
	
	Если ЗначениеЗаполнено(Параметры.ОбработчикЗакрытия) Тогда
		ПараметрыКоманды = Новый Структура();
		ПараметрыКоманды.Вставить("Обработчик", Параметры.ОбработчикЗакрытия);
		ПараметрыКоманды.Вставить("Идентификатор", Параметры.Идентификатор);
		Форма.БФ_ОбработчикиКомандВажногоСообщения.Вставить(ИмяКоманды, ПараметрыКоманды);
	КонецЕсли;
...
//   Соответствие из КлючИЗначение:
//      * Ключ     - Строка
//      * Значение - Цвет - цвет из палитры.
//
Функция ЦветаВажныхСообщений()
	
	Соответствие = Новый Соответствие;
	Соответствие.Вставить("Успех",          ЦветаПалитры.Зеленый);
	Соответствие.Вставить("Информация",     ЦветаПалитры.Голубой);
	Соответствие.Вставить("Предупреждение", ЦветаПалитры.Желтый);
	Соответствие.Вставить("Ошибка",         ЦветаПалитры.Красный);
...
//      * Значение - Картинка
//
Функция КартинкиВажныхСообщений()
	
	Соответствие = Новый Соответствие;
	Соответствие.Вставить("Успех",          БиблиотекаКартинок.ВажноеСообщениеУспешно);
	Соответствие.Вставить("Информация",     БиблиотекаКартинок.ВажноеСообщениеИнформация);
	Соответствие.Вставить("Предупреждение", БиблиотекаКартинок.ВажноеСообщениеПредупреждение);
	Соответствие.Вставить("Ошибка",         БиблиотекаКартинок.ВажноеСообщениеОшибка);
	
	Возврат Соответствие;
	
КонецФункции

Процедура УстановитьШрифтЗаголовкаВажныхСообщений(Шрифт)
	
	Шрифт = ШрифтыСтиля.ТекстВторогоУровня;
...
//  Шрифт
//
Функция ШрифтВажныхСообщений()
	
	Возврат ШрифтыСтиля.ТекстТретьегоУровня;
...
//   ГруппаФормы
//
Функция НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, Родитель)
...
//      * ТипСообщения       - Строка
//      * Количество         - Число
//      * ПараметрыСообщения - Массив из см. ОбщегоНазначения.ПараметрыВажногоСообщения
//
Функция НоваяТаблицаСгруппированныхСообщений()
	
	СгруппированныеСообщения = Новый ТаблицаЗначений;
	СгруппированныеСообщения.Колонки.Добавить("ТипСообщения", Новый ОписаниеТипов("Строка"));
	СгруппированныеСообщения.Колонки.Добавить("Уровень",    Новый ОписаниеТипов("Число"));
	СгруппированныеСообщения.Колонки.Добавить("ПараметрыСообщения", Новый ОписаниеТипов("Структура"));
	
	Возврат СгруппированныеСообщения;
	
КонецФункции

// Параметры:
//   СгруппированныеСообщения - см. ОбщегоНазначения.НоваяТаблицаСгруппированныхСообщений
//   ДобавляемыеСообщения     - см. ОбщегоНазначения.ПараметрыВажногоСообщения
//
Процедура СгруппироватьСообщения(СгруппированныеСообщения, ДобавляемыеСообщения)
	
	Для Каждого ДобавляемоеСообщение Из ДобавляемыеСообщения.ВыгрузитьЗначения() Цикл
		Если ЗначениеЗаполнено(ДобавляемоеСообщение.Родитель) Тогда
			Продолжить;
		КонецЕсли;
		
		Строка = СгруппированныеСообщения.Добавить();
		Строка.ТипСообщения = ДобавляемоеСообщение.ТипСообщения;
		Если ДобавляемоеСообщение.ТипСообщения = "Ошибка" Тогда
			Строка.Уровень = 0;
		ИначеЕсли ДобавляемоеСообщение.ТипСообщения = "Предупреждение" Тогда
			Строка.Уровень = 1;
		ИначеЕсли ДобавляемоеСообщение.ТипСообщения = "Информация" Тогда
			Строка.Уровень = 2;
		Иначе
			Строка.Уровень = 3;
		КонецЕсли;
		
		Строка.ПараметрыСообщения = ДобавляемоеСообщение;
	КонецЦикла;
	
	СгруппированныеСообщения.Сортировать("Уровень Возр");
	
КонецПроцедуры

Процедура ВывестиПростоеВажноеСообщение(Форма, ПараметрыСообщения, Родитель)
	
	ЦветаВажныхСообщений    = ЦветаВажныхСообщений();
	КартинкиВажныхСообщений = КартинкиВажныхСообщений();
	
	Родитель.ЦветФона  = ЦветаВажныхСообщений[ПараметрыСообщения.ТипСообщения];
	
	ДекорацияКартинка  = Форма.Элементы.Добавить(Родитель.Имя + "_" + "КартинкаСообщения", Тип("ДекорацияФормы"), Родитель);
	ДекорацияКартинка.Вид      = ВидДекорацииФормы.Картинка;
	ДекорацияКартинка.Картинка = КартинкиВажныхСообщений[ПараметрыСообщения.ТипСообщения];
	ДекорацияКартинка.ЦветКартинки = ЦветаВажныхСообщений[ПараметрыСообщения.ТипСообщения];
	Если ЗначениеЗаполнено(ПараметрыСообщения.Заголовок) Тогда
		УстановитьШрифтЗаголовкаВажныхСообщений(ДекорацияКартинка.Шрифт);
	Иначе
		ДекорацияКартинка.Шрифт = ШрифтВажныхСообщений();
		ДекорацияКартинка.РазмерКартинки = РазмерКартинки.ПоРазмеруШрифта;
	КонецЕсли;
	
	ИмяГруппы = Родитель.Имя + "_" + "БФ_ГруппаВажноеСообщение";
	ГруппаВажноеСообщение = НоваяГруппаВажногоСообщения(Форма, ИмяГруппы, Родитель);
	ГруппаВажноеСообщение.РастягиватьПоГоризонтали = Истина;
	ГруппаВажноеСообщение.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаВажноеСообщение.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	ВывестиКонтентСообщения(Форма, ГруппаВажноеСообщение, ПараметрыСообщения);
	
	ДобавитьКнопкуСкрытияСообщения(Форма, Родитель, Родитель, ПараметрыСообщения);
