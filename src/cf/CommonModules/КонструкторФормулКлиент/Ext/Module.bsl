...
Процедура СписокПолейНачалоПеретаскивания(Форма, Элемент, ПараметрыПеретаскивания, Выполнение) Экспорт
	...
	Реквизит = Форма[ИмяСпискаПолей].НайтиПоИдентификатору(ПараметрыПеретаскивания.Значение[0]);
	...
	Если НастройкиСпискаПолей.СкобкиПредставлений Тогда
		ПараметрыПеретаскивания.Значение[0] = "[" + Реквизит.ПредставлениеПутиКДанным + "]";
	Иначе
		ПараметрыПеретаскивания.Значение[0] = Реквизит.ПредставлениеПутиКДанным;
	КонецЕсли;
	...
	Если ДлинаСтрокиОтбора >= ПодключенныйСписокПолей.КоличествоСимволовДляНачалаПоиска Тогда
		...
		Если ДлительнаяОперация <> Неопределено Тогда 
			...
			Иначе
				...
				Форма.Элементы[ИмяСтрокиПоиска].Картинка = БиблиотекаКартинок.ДлительнаяОперация16;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗавершениеПоискаВСпискеПолей(Результат, ПараметрыЗавершения) Экспорт
	...
	Форма.Элементы[ИмяСтрокиПоиска].Картинка = БиблиотекаКартинок.Найти;
