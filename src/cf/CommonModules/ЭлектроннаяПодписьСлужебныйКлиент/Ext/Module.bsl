...
Процедура ОткрытьФормуСертификата(ПараметрыФормы, ФормаВладелец = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.Сертификат", ПараметрыФормы, ФормаВладелец,,,, ОповещениеОЗавершении, ?(ФормаВладелец
		= Неопределено, Неопределено, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца));
	...
	Если Не Подключено Тогда
		...
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			...
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы,
				, , , , Контекст.ОбработкаПродолжения, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
			Возврат;
		КонецЕсли;
	КонецЕсли;
#КонецЕсли
...
Процедура ОткрытьФормуОповещенияОНеобходимостиЗаменыСертификата(Параметры) Экспорт
	...
	ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.ОповещениеОбОкончанииСрокаДействия",
			Параметры,,,,,,РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
...
#Если ВебКлиент Тогда
	...
	Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
		...
		ПараметрыФормы.Вставить("ИмяТребуемойКомпоненты", "РасширениеРаботыСКриптографией");
		ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы,
			, , , , Обработчик, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
		Возврат;
	КонецЕсли;
#КонецЕсли
...
Асинх Процедура ДобавитьСертификатПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		ОписанияПрограмм = ЭлектроннаяПодписьСлужебныйКлиентСервер.МенеджерКриптографииОписанияПрограмм(
			Неопределено, Контекст.ОшибкаНаКлиенте.Ошибки, ОписанияПрограмм, ПрограммыАвто);	
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПодключенияКомпоненты(ОбъектКомпоненты, ПараметрыУстановкиСертификата) Экспорт
	...
	Если ПараметрыУстановкиСертификата.ВариантыУстановки = Неопределено
		...
		ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.УстановкаСертификата",
			...
			ОповещениеОЗакрытииФормы, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
		Возврат;
	КонецЕсли;
