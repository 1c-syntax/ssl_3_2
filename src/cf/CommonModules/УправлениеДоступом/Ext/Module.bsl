...
Процедура ПриСозданииФормыЗначенияДоступа(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Параметры = ПараметрыПриСозданииФормыЗначенияДоступа();
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("УправлениеДоступом.ПриСозданииФормыЗначенияДоступа",
...
// Добавляет пользователя в группу доступа Администраторы или удаляет из нее.
// Служебный пользователь не будет добавлен в группу доступа Администраторы
// при вызове Добавить = Истина, но будет удален из нее при вызове Добавить = Ложь.
// Запись группы доступа Администраторы не выполняется, если фактические изменения не требуются.
//
// Параметры:
//  Пользователь - СправочникСсылка.Пользователи - пользователь, которого нужно
//    добавить или удалить из группы доступа Администраторы.
//  
//  Добавить - Булево - если Истина, тогда добавить пользователя
//    в группу доступа Администраторы, иначе удалить из нее.
//  
//  Уточнение - Строка - пояснение условия изменения участников
//    группы доступа Администратора для журнала регистрации.
//
// Пример:
//	УстановитьПривилегированныйРежим(Истина);
//	УправлениеДоступом.ИзменитьСоставАдминистраторов(ПользовательОбластиДанных, ДоступРазрешен,
//		НСтр("ru = 'Обработка сообщения удаленного администрирования из менеджера сервиса.'"));
//	УстановитьПривилегированныйРежим(Ложь);
//
Процедура ИзменитьСоставАдминистраторов(Пользователь, Добавить, Уточнение) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("УправлениеДоступом.ИзменитьСоставАдминистраторов",
		"Пользователь", Пользователь, Тип("СправочникСсылка.Пользователи"));
	
	Справочники.ГруппыДоступа.ИзменитьСоставАдминистраторов(Пользователь, Добавить, Уточнение);
...
//
// Возвращаемое значение:
//  Булево - Истина, если для конфигурации указан упрощенный интерфейс.
//
...
// Возвращает описание поставляемых профилей групп доступа, поддерживаемых текущей версией конфигурации.
// Доступен вызов в неразделенном режиме при условии, что это поддерживается в прикладном коде,
// встроенном в процедуры ПриЗаполненииВидовДоступа и ПриЗаполненииПоставляемыхПрофилейГруппДоступа
// общего модуля УправлениеДоступомПереопределяемый.
// Не учитывает динамическое изменение метаданных, например, расширений или конфигурации базы
// в результате которого описание поставляемых профилей может изменится в новом сеансе.
//
// Возвращаемое значение:
//  Массив из Структура:
//   * ИдентификаторПоставляемыхДанных - УникальныйИдентификатор - смотри свойство Идентификатор
//       в функции НовоеОписаниеПрофиляГруппДоступа.
//
//   * Наименование - Строка - смотри одноименное свойство в функции НовоеОписаниеПрофиляГруппДоступа.
//
//   * ЭтоПрофильАдминистратор - Булево - это стандартный поставляемый профиль Администратор.
//
//   * ДаетПраваНаВходВПриложение - Булево - в описании поставляемого профиля
//       есть одна из ролей БазовыеПраваБСП/БазовыеПраваВнешнихПользователейБСП
//       и есть одна из ролей ЗапускТонкогоКлиента/ЗапускВебКлиента/ЗапускМобильногоКлиента/ЗапускТолстогоКлиента.
//
Функция ПоставляемыеПрофилиГруппДоступа() Экспорт
	
	Кэш = УправлениеДоступомСлужебныйПовтИсп.ОписаниеПоставляемыхПрофилейСеанса(Ложь);
	ОписаниеПрофилей = Кэш.ПоставляемыеПрофилиСеанса.ОписанияПрофилейМассив;
	
	Результат = Новый Массив;
	Для Каждого ОписаниеПрофиля Из ОписаниеПрофилей Цикл
		Если ОписаниеПрофиля.ЭтоГруппа Тогда
			Продолжить;
		КонецЕсли;
		Свойства = Новый Структура;
		Свойства.Вставить("ИдентификаторПоставляемыхДанных",
			Новый УникальныйИдентификатор(ОписаниеПрофиля.Идентификатор));
		Свойства.Вставить("Наименование",            ОписаниеПрофиля.Наименование);
		Свойства.Вставить("ЭтоПрофильАдминистратор", ОписаниеПрофиля.Имя = "Администратор");
		Свойства.Вставить("ДаетПраваНаВходВПриложение",
			  (    ОписаниеПрофиля.Роли.Найти("БазовыеПраваБСП") <> Неопределено
			   Или ОписаниеПрофиля.Роли.Найти("БазовыеПраваВнешнихПользователейБСП") <> Неопределено)
			И (    ОписаниеПрофиля.Роли.Найти("ЗапускТонкогоКлиента") <> Неопределено
			   Или ОписаниеПрофиля.Роли.Найти("ЗапускВебКлиента") <> Неопределено
			   Или ОписаниеПрофиля.Роли.Найти("ЗапускМобильногоКлиента") <> Неопределено
			   Или ОписаниеПрофиля.Роли.Найти("ЗапускТолстогоКлиента") <> Неопределено));
...
// Возвращает уже имеющуюся группу доступа указанного профиля или создает ее в обычном интерфейсе
// настройки прав доступа, либо возвращает ссылку на поставляемый профиль в упрощенном интерфейсе.
//
// Параметры:
//  ИдентификаторПоставляемыхДанных - УникальныйИдентификатор - смотри свойство Идентификатор
//    в функции НовоеОписаниеПрофиляГруппДоступа, а также одноименный реквизит справочника.
//
// Возвращаемое значение:
//  СправочникСсылка.ГруппыДоступа - в обычном интерфейсе настройки прав доступа возвращается
//    ссылка на первую группа доступа с указанным профилем по порядку идентификатора ссылки.
//  СправочникСсылка.ПрофилиГруппДоступа - в упрощенном интерфейсе настройки прав доступа возвращается
//    ссылка на поставляемый профиль группа доступа.
//
Функция ГруппаДоступаПоставляемогоПрофиля(ИдентификаторПоставляемыхДанных) Экспорт
	
	Профиль = Справочники.ПрофилиГруппДоступа.ПоставляемыйПрофильПоИдентификатору(
		ИдентификаторПоставляемыхДанных, Истина, Истина);
	...
	Если Профиль = ПрофильАдминистратор() Тогда
		...
		|	Справочник.ГруппыДоступа КАК ГруппыДоступа
		|ГДЕ
		|	ГруппыДоступа.Ссылка = &ГруппаДоступаАдминистраторы";
	Иначе
		...
		|	И НЕ(ГруппыДоступа.Пользователь <> НЕОПРЕДЕЛЕНО
	...
	Попытка
		...
		Иначе
			СвойстваПрофиля = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Профиль,
...
// Для процедуры ВключитьОтключитьПрофильПользователя.
...
// Для процедуры ЗаполнитьВидыИЗначенияДоступаНовойГруппыДоступа и функции ГруппаДоступаПоставляемогоПрофиля.
