...
// Метод длительной операции, выполняемой в функции см. РаботаСФайламиСлужебныйКлиент.ДействиеСФайлом.
//	Получает данные для дальнейшего открытия. Использует метод см. РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия.
//
// Параметры:
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	МодульЭлектроннаяПодписьСлужебный = ОбщегоНазначения.ОбщийМодуль("ЭлектроннаяПодписьСлужебный");
	МодульЭлектроннаяПодписьСлужебный.ОформитьСписокПодписей(Форма, "ЭлектронныеПодписи", "ЭлектронныеПодписиКомуВыданСертификат");
...
Процедура СоздатьФайловыйНачальныйОбразНаСервере(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если Параметры.ЕстьФайлыВТомах Тогда
			ZipФайл = Новый ЗаписьФайлаАрхива();
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	Если ЭтоСлужебныйФайл Тогда
		...
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Служебный файл доступен только для просмотра.'");
		ПараметрыСообщения.Идентификатор = "ПризнакСлужебногоФайла";
		ОбщегоНазначения.ДобавитьВажноеСообщение(Контекст, ПараметрыСообщения);
	КонецЕсли;
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект, Контекст)
	...
	Для каждого Элемент Из Контекст.Элементы Цикл
		Если ТипЗнч(Элемент) = Тип("ПолеФормы")
			...
			Если Элемент.Вид = ВидПоляФормы.ПолеФлажка Или Элемент.Вид = ВидПоляФормы.ПолеКартинки Или Элемент.Вид = ВидПоляФормы.ПолеНадписи Тогда
...
Процедура ЗаполнитьСписокШифрования(Контекст, Знач Источник = Неопределено) Экспорт
	...
	Контекст.ВсегоМогутРасшифровать = Контекст.СертификатыШифрования.Количество();
...
Процедура ЗаполнитьСписокПодписей(Контекст, Знач Источник = Неопределено) Экспорт
	...
	Контекст.ВсегоПодписей = Контекст.ЭлектронныеПодписи.Количество();
