...
Функция ЗаписатьИЗакрытьВыполнить(Форма, ВыполнитьЗадачу = Ложь, ПараметрыОповещения = Неопределено) Экспорт
	...
	ОчиститьСообщения();
	Возврат ЗаписатьЗадачу(Форма, ВыполнитьЗадачу, ПараметрыОповещения, Истина);
...
//   Булево   - Истина, если запись прошла успешно.
//
Функция ЗаписатьЗадачу(Форма, ВыполнитьЗадачу = Ложь, ПараметрыОповещения = Неопределено, ЗакрытьФорму = Ложь) Экспорт
...
Процедура ОткрытьБизнесПроцесс(Список) Экспорт
	...
	СтрокиДляОбработки = Список.ПолучитьСтрокиДляОбработки();
	Если СтрокиДляОбработки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока = СтрокиДляОбработки.Получить(0);
	
	Если ТипЗнч(ТекущаяСтрока) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Команда не может быть выполнена для указанного объекта.'"));
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Список.ДанныеСтроки(ТекущаяСтрока);
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.БизнесПроцесс = Неопределено Тогда
...
Функция ПризнакиПринятьКИсполнению(Список, ПринятаКИсполнению) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПризнакОтменитьПринятиеКИсполнению", Не ПринятаКИсполнению);
	Результат.Вставить("ПризнакПринятьКИсполнению",          ПринятаКИсполнению);
	
	Если Список.ВыделенныеСтроки.Количество() > 0 Тогда
		
		Результат.ПризнакПринятьКИсполнению          = Ложь;
		Результат.ПризнакОтменитьПринятиеКИсполнению = Ложь;
		
		Для Каждого СтрокаДляОбработки Из Список.ПолучитьСтрокиДляОбработки() Цикл
			
			ТекущиеДанныеСтроки = Список.ДанныеСтроки(СтрокаДляОбработки);
			Если ТекущиеДанныеСтроки.Свойство("Выполнена")
			   И ТекущиеДанныеСтроки.Выполнена 
			   И Не ТекущиеДанныеСтроки.Свойство("ПринятаКИсполнению") Тогда
				Продолжить;
			КонецЕсли;
			
			Если ТекущиеДанныеСтроки.Свойство("ПринятаКИсполнению") И ТекущиеДанныеСтроки.ПринятаКИсполнению Тогда
				Результат.ПризнакОтменитьПринятиеКИсполнению = Истина;
			Иначе
				Результат.ПризнакПринятьКИсполнению          = Истина;
			КонецЕсли;
			
			Если Результат.ПризнакПринятьКИсполнению И Результат.ПризнакОтменитьПринятиеКИсполнению Тогда
				Прервать;
			КонецЕсли;
...
Процедура СписокБизнесПроцессовПометкаУдаленияЗавершение(Результат, Список) Экспорт
	...
	БизнесПроцессСсылка = БизнесПроцессыИЗадачиВызовСервера.ПометитьНаУдалениеБизнесПроцессы(СтрокиДляОбработки);
	Список.Обновить();
