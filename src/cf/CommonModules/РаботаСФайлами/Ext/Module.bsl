...
Процедура СоздатьПолеФайла(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыПоляФайла)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		СвойстваКнопкиЗагрузка.Вставить("Заголовок", НСтр("ru = 'Добавить с устройства...'"));
	Иначе
		СвойстваКнопкиЗагрузка.Вставить("Заголовок", НСтр("ru = 'Добавить с компьютера...'"));
	КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
		...
		ПодменюДобавления.Заголовок   = НСтр("ru = 'Добавить'");
		ПодменюДобавления.Отображение = ОтображениеКнопки.КартинкаИТекст;
		...
		ПодменюПросмотра = Форма.Элементы.Добавить("ПодменюПросмотраФайла" + НомерЭлемента,
			Тип("ГруппаФормы"), ПанельКоманд);
		
		ПодменюПросмотра.Вид         = ВидГруппыФормы.Подменю;
		ПодменюПросмотра.Заголовок   = НСтр("ru = 'Просмотреть'");
		ПодменюПросмотра.Отображение = ОтображениеКнопки.Текст;
		
		ГруппаПодменюПросмотра = Форма.Элементы.Добавить("ГруппаПросмотраФайла" + НомерЭлемента,
			Тип("ГруппаФормы"), ПодменюПросмотра); // ГруппаФормы
		ГруппаПодменюПросмотра.Вид = ВидГруппыФормы.ГруппаКнопок;
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы Тогда
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			...
			Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
				КнопкаЗагрузкиИзПодменю.Заголовок = НСтр("ru = 'С устройства...'");
			Иначе
				КнопкаЗагрузкиИзПодменю.Заголовок = НСтр("ru = 'С компьютера...'");
			КонецЕсли;
		...
		Если Не ДобавляемыйЭлемент.ТолькоОдинФайл Тогда
			...
			Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
				Кнопка = ДобавитьКнопкуНаФорму(Форма, ИмяКомандыСПрефиксом + НомерЭлемента,
					ГруппаПодменю, СоздатьПоШаблону.Имя);
				Кнопка.Заголовок = НСтр("ru = 'По шаблону...'");
			КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ПросматриватьФайл Тогда
		...
		ПросмотретьФайл.Заголовок = НСтр("ru = 'Оригинал файла'");
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			ДобавитьКнопкуНаФорму(Форма, РаботаСФайламиКлиентСервер.ИмяКомандыПросмотретьФайл() + НомерЭлемента, ГруппаПодменюПросмотра, ПросмотретьФайл.Имя);
		КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ОчищатьФайл
		...
		ОчиститьФайл.Картинка  = БиблиотекаКартинок.Удалить;
		...
		ОчиститьФайл.Отображение = ОтображениеКнопки.КартинкаИТекст;
	...
	Если ДобавляемыйЭлемент.РедактироватьФайл = "ВФорме" Тогда
		...
		Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
			Кнопка = ДобавитьКнопкуНаФорму(Форма, РаботаСФайламиКлиентСервер.ИмяКомандыРедактироватьФайл() + НомерЭлемента, ГруппаПодменюПросмотра, РедактироватьФайл.Имя);
			Кнопка.Заголовок = НСтр("ru = 'Карточку файла'");
		КонецЕсли;
