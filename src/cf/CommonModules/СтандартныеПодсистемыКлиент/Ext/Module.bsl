...
//    * Картинка                      - Картинка     - картинка, выводимая в окне вопроса (по умолчанию ДиалогВопрос).
//                                    - Неопределено - скрыть картинку.
...
Процедура ПоказатьРедакторТабличногоДокумента(Знач ТабличныйДокумент, Знач ПараметрыФормы = Неопределено, 
	...
	Свойства = Новый Структура("РежимБлокировкиПриОткрытииОкна, РежимОткрытияОкна", Неопределено);
	ЗаполнитьЗначенияСвойств(Свойства, ПараметрыФормы);
	РежимБлокировкиОкна = Неопределено;
	Если Свойства.РежимБлокировкиПриОткрытииОкна <> Неопределено Тогда
		РежимБлокировкиОкна = Свойства.РежимБлокировкиПриОткрытииОкна;
	ИначеЕсли Свойства.РежимОткрытияОкна <> Неопределено Тогда
		РежимБлокировкиОкна = Свойства.РежимОткрытияОкна;
	КонецЕсли;
	ПараметрыФормы.Удалить("РежимБлокировкиПриОткрытииОкна");
	ПараметрыФормы.Удалить("РежимОткрытияОкна");
	
	ОткрытьФорму("ОбщаяФорма.РедактированиеТабличногоДокумента", ПараметрыФормы, Владелец,,,,
		ОписаниеОповещенияОЗакрытии, РежимБлокировкиОкна);
...
//      * РежимОткрытияОкна - РежимБлокировкиПриОткрытииОкнаФормы - режим открытия форм Форма или ПрикладнаяФормаПредупреждения.
// 
...
Процедура ПоказатьПодробнуюИнформацию(Обработчик, Текст, Заголовок = Неопределено) Экспорт
	...
	НастройкиДиалога = ПараметрыВопросаПользователю();
	НастройкиДиалога.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	НастройкиДиалога.Картинка = Неопределено;
	НастройкиДиалога.Заголовок = Заголовок;
	
	Если НастройкиДиалога.Заголовок = Неопределено Тогда
		НастройкиДиалога.Заголовок = НСтр("ru = 'Подробнее'");
	КонецЕсли;
	
	ПоказатьВопросПользователю(Обработчик,
		Текст, Новый СписокЗначений, НастройкиДиалога);
...
Процедура ПередНачаломРаботыСистемы2(Параметры) Экспорт
	...
	Если ПараметрыКлиента.Свойство("ИспользуетсяНедопустимаяВерсияПлатформы") Тогда
		Параметры.ИнтерактивнаяОбработка = Новый ОписаниеОповещения(
			"ПредупредитьОНедопустимойВерсииПлатформы", ЭтотОбъект);
	ИначеЕсли ПараметрыКлиента.Свойство("ПоказатьНерекомендуемуюВерсиюПлатформы") Тогда
		Параметры.ИнтерактивнаяОбработка = Новый ОписаниеОповещения(
