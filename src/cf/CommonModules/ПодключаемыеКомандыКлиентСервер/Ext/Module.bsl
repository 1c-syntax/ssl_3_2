...
Процедура ОбновитьКоманды(Форма, Знач Источник = Неопределено) Экспорт
	...
	Если Источник = Неопределено Тогда
		ИмяТекущегоИсточника = Форма.ПараметрыПодключаемыхКоманд.ИмяТекущегоИсточника;
		Источник = ?(ЗначениеЗаполнено(ИмяТекущегоИсточника), Форма.Элементы[ИмяТекущегоИсточника], Неопределено);
	КонецЕсли;
...
//   * Ссылка - ЛюбаяСсылка
// 
Функция ВыбранныеОбъекты(Источник, ПараметрыПодключаемыхКоманд)
	
	ВыбранныеОбъекты = Новый Массив; // Массив из ДанныеФормыСтруктура, ДанныеФормыЭлементКоллекции
	
	Если ПараметрыПодключаемыхКоманд.НаведеннаяСтрока <> Неопределено Тогда
		ДанныеНаведеннойСтроки = Источник.ДанныеСтроки(ПараметрыПодключаемыхКоманд.НаведеннаяСтрока);
		ВыбранныеОбъекты.Добавить(ДанныеНаведеннойСтроки);
	ИначеЕсли ТипЗнч(Источник) = Тип("ТаблицаФормы") Тогда
		ВыделенныеСтроки = Источник.ПолучитьСтрокиДляОбработки();
...
Процедура ОбновитьКомандыИсточника(Знач Форма, Знач Источник, Знач ИмяИсточника = "")
	...
	Если ТипЗнч(Источник) = Тип("ТаблицаФормы") Тогда
		ДоступностьКоманд = ?(ПараметрыКлиента.НаведеннаяСтрока <> Неопределено, Истина, (Источник.ТекущаяСтрока <> Неопределено));
	Иначе
...
#КонецЕсли
	...
	Форма.ПараметрыПодключаемыхКоманд.ИмяТекущегоИсточника = Неопределено;
	...
	ВыбранныеОбъекты = ВыбранныеОбъекты(Источник, ПараметрыКлиента);
	
	Форма.ПараметрыПодключаемыхКоманд.НаведеннаяСтрока = Неопределено;
