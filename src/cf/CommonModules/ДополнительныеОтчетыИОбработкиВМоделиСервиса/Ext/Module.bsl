	...
	Попытка
		...
		Если ПоставляемаяОбработкаСуществует Тогда
			...
			МодульСообщенияВМоделиСервиса.ОтправитьСообщение(Сообщение,
				МодульРаботаВМоделиСервиса.КонечнаяТочкаМенеджераСервиса(),  Истина);
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Если Не ПустаяСтрока(ТекстИсключения) Тогда
		...
		МодульСообщенияВМоделиСервиса.ОтправитьСообщение(Сообщение, МодульРаботаВМоделиСервиса.КонечнаяТочкаМенеджераСервиса());
	КонецЕсли;	
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		...
		МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
		МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров,
			Метаданные.Константы.НезависимоеИспользованиеДополнительныхОтчетовИОбработокВМоделиСервиса.Имя);
		МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(ТаблицаПараметров,
			Метаданные.Константы.ИспользованиеКаталогаДополнительныхОтчетовИОбработокВМоделиСервиса.Имя);
		
		// Константы были переименованы. Менеджер сервиса ожидает параметры со старыми именами - 
		// для совместимости добавляются параметры со старыми именами констант
		Для Каждого КлючИЗначение Из ПереименованияКонстант() Цикл
			ПрежнееИмя = КлючИЗначение.Ключ;
			АктуальноеИмя = КлючИЗначение.Значение;
			СтрокаПараметра = МодульРаботаВМоделиСервиса.ДобавитьКонстантуВТаблицуПараметровИБ(
				ТаблицаПараметров, АктуальноеИмя);
			СтрокаПараметра.Имя = ПрежнееИмя;
		КонецЦикла;
...
Процедура ПриУстановкеЗначенийПараметровИБ(Знач ЗначенияПараметров) Экспорт
	...
	// Обрабатываем установку только переименованных констант, поскольку значения актуальных констант
	// будут установлены автоматически
	Для Каждого КлючИЗначение Из ПереименованияКонстант() Цикл
		ПрежнееИмя = КлючИЗначение.Ключ;
		АктуальноеИмя = КлючИЗначение.Значение;
		НовоеЗначение = Неопределено;
		Если ЗначенияПараметров.Свойство(ПрежнееИмя, НовоеЗначение) Тогда
			УстановитьПривилегированныйРежим(Истина);
			Если НовоеЗначение <> Константы[АктуальноеИмя].Получить() Тогда
				Константы[АктуальноеИмя].Установить(НовоеЗначение);
			КонецЕсли;
			УстановитьПривилегированныйРежим(Ложь);
			ЗначенияПараметров.Удалить(ПрежнееИмя);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриПолученииЗначенийПараметровИБ(ИменаПараметров, ЗначенияПараметров) Экспорт
	...
	// Обрабатываем получение только переименованных констант, поскольку значения актуальных констант
	// будут получены автоматически
	Для Каждого КлючИЗначение Из ПереименованияКонстант() Цикл
		ПрежнееИмя = КлючИЗначение.Ключ;
		АктуальноеИмя = КлючИЗначение.Значение;
		Если ИменаПараметров.Свойство(ПрежнееИмя) Тогда
			УстановитьПривилегированныйРежим(Истина);
			ЗначениеКонстанты = Константы[АктуальноеИмя].Получить();
			УстановитьПривилегированныйРежим(Ложь);
			ЗначенияПараметров.Вставить(ПрежнееИмя, ЗначениеКонстанты);
			ИменаПараметров.Удалить(ПрежнееИмя);
		КонецЕсли;
	КонецЦикла;
...
Функция ПереименованияКонстант()
	
	Переименования = Новый Соответствие;
	Переименования.Вставить("РазрешитьВыполнениеДополнительныхОтчетовИОбработокРегламентнымиЗаданиямиВМоделиСервиса",
		Метаданные.Константы.РазрешитьВыполнениеДООРегламентнымиЗаданиямиВМоделиСервиса.Имя);
	Переименования.Вставить("МинимальныйИнтервалРегламентныхЗаданийДополнительныхОтчетовИОбработокВМоделиСервиса",
		Метаданные.Константы.МинимальныйИнтервалРегламентныхЗаданийДОИОВМоделиСервиса.Имя);
	Переименования.Вставить("ИспользоватьПрофилиБезопасностиДляДОиО",
		Метаданные.Константы.ИспользоватьПрофилиБезопасностиДляДОО.Имя);
