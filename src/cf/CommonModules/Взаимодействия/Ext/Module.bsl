...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	ЭтоФормаВыбораКонтакта = (Форма.ИмяФормы = "ОбщаяФорма.ВыборКонтакта");
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		ПутьКДаннымЗаголовкаСтраницы = "Страница_" + ОписаниеКонтакта.Имя + "_Выбранные";
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ПутьКДаннымЗаголовкаСтраницы, Новый ОписаниеТипов("Число")));
	КонецЦикла;
	
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДобавленныеТаблицыКонтактов", Новый ОписаниеТипов("СписокЗначений")));
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если НЕ ОписаниеКонтакта.ЕстьВладелец Тогда
			...
			ПутьКДаннымЗаголовка = ЭлементСтраница.Имя + "_Выбранные";
			...
			ЭлементСтраница.Группировка          = ГруппировкаПодчиненныхЭлементовФормы.АвтоматическиУчитыватьВидЭкрана;
			ЭлементСтраница.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Двойной;
			ЭлементСтраница.ПутьКДаннымЗаголовка = ПутьКДаннымЗаголовка;
		КонецЕсли;
		...
		Если ЭтоФормаВыбораКонтакта Тогда
		...
		КонецЕсли;
		Форма.ДобавленныеТаблицыКонтактов.Добавить(ЭлементТаблица.Имя);
		
		ГруппаКолонок = Форма.Элементы.Добавить("ГруппаКолонок_" + ОписаниеКонтакта.Имя, Тип("ГруппаФормы"), ЭлементТаблица);
		ГруппаКолонок.Вид = ВидГруппыФормы.ГруппаКолонок;
		ГруппаКолонок.Группировка = ГруппировкаКолонок.ВЯчейке;
		Если Не ЭтоФормаВыбораКонтакта Тогда
			КолонкаВыбран = Форма.Элементы.Добавить(
				"Колонка_" + ОписаниеКонтакта.Имя + "_Выбран", Тип("ПолеФормы"), ГруппаКолонок);
			КолонкаВыбран.Вид = ВидПоляФормы.ПолеФлажка;
			КолонкаВыбран.ПутьКДанным = "Список_" + ОписаниеКонтакта.Имя + ".Выбран";
			КолонкаВыбран.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		КонецЕсли;
		
		КолонкаСсылка = Форма.Элементы.Добавить(
			"Колонка_" + ОписаниеКонтакта.Имя + "_Ссылка", Тип("ПолеФормы"), ГруппаКолонок);
		...
		// Добавление контекстных команд выбора контактов.
		Если Не ЭтоФормаВыбораКонтакта Тогда
			КомандаВыбрать = Форма.Команды.Добавить(ВзаимодействияКлиентСервер.ПрефиксКомандыВыбрать() + ОписаниеКонтакта.Имя);
			КомандаВыбрать.Заголовок = НСтр("ru = 'Выбрать'");
			КомандаВыбрать.Подсказка = НСтр("ru = 'Выбрать'");
			КомандаВыбрать.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.Использует;
			КомандаВыбрать.ИспользуемаяТаблица = ЭлементТаблица;
			КомандаВыбрать.Картинка = БиблиотекаКартинок.ОформлениеФлажок;
			КомандаВыбрать.Действие = "ВыбратьСтрокиСписка";
			
			КнопкаВыбрать = Форма.Элементы.Добавить("КнопкаСписка_" + ОписаниеКонтакта.Имя + "_Выбрать", Тип("КнопкаФормы"), ЭлементТаблица.КоманднаяПанель);
			КнопкаВыбрать.ИмяКоманды = КомандаВыбрать.Имя;
			КнопкаВыбрать.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
			КнопкаВыбрать.ВажностьПриОтображении = ВажностьПриОтображении.Высокая;
			
			КомандаИсключить = Форма.Команды.Добавить(ВзаимодействияКлиентСервер.ПрефиксКомандыИсключить() + ОписаниеКонтакта.Имя);
			КомандаИсключить.Заголовок = НСтр("ru = 'Исключить'");
			КомандаИсключить.Подсказка = НСтр("ru = 'Исключить'");
			КомандаИсключить.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.Использует;
			КомандаИсключить.ИспользуемаяТаблица = ЭлементТаблица;
			КомандаИсключить.Картинка = БиблиотекаКартинок.ОформлениеКрест;
			КомандаИсключить.Действие = "ИсключитьСтрокиСписка";
			
			КнопкаИсключить = Форма.Элементы.Добавить("КнопкаСписка_" + ОписаниеКонтакта.Имя + "_Исключить", Тип("КнопкаФормы"), ЭлементТаблица.КоманднаяПанель);
			КнопкаИсключить.ИмяКоманды = КомандаИсключить.Имя;
			КнопкаИсключить.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
			КнопкаИсключить.ВажностьПриОтображении = ВажностьПриОтображении.Высокая;
		КонецЕсли;
...
Функция ДобавленныеТаблицыКонтактов(Форма) Экспорт
	
	Проверка = Новый Структура("ДобавленныеТаблицыКонтактов");
	ЗаполнитьЗначенияСвойств(Проверка, Форма);
	
	Если Проверка.ДобавленныеТаблицыКонтактов = Неопределено Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	Возврат Проверка.ДобавленныеТаблицыКонтактов.ВыгрузитьЗначения();
...
Функция УстановитьТекстЗапросаКонтакта(Список, ОписаниеКонтакта)
	...
	Если ЕстьВидДляСписка Тогда
		...
		|	СправочникКонтакт.Ссылка В (&Выбранные) КАК Выбран,
		...
		|	И (СправочникКонтакт.Ссылка В (&Выбранные) ИЛИ НЕ &Проверка)";
	Иначе
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СправочникКонтакт.Ссылка,
	...
	КонецЕсли;
	Список.Параметры.УстановитьЗначениеПараметра("Проверка", Ложь);
	Список.Параметры.УстановитьЗначениеПараметра("Выбранные", Новый СписокЗначений);
