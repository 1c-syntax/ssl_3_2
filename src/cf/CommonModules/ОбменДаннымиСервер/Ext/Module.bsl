...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	ПропуститьЗагрузкуПриоритетныхДанных();
	
	ВключитьОтключитьРежимЗагрузки(Истина);
	
	Попытка
		
		ВключитьСинхронизациюДанныхПриНеобходимости();
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
			
			ВыполнитьЗагрузкуПараметровГлавногоУзла(Отказ);
			
		КонецЕсли;
		
	Исключение
		ОбработатьОшибкуЗагрузки();
		ВызватьИсключение;
	КонецПопытки;
	
	ВключитьОтключитьРежимЗагрузки(Ложь);
	
	ПроверитьИОбработатьОтказ(Отказ);
...
Функция СинхронизацияДанныхРазрешена(Знач Пользователь = Неопределено) Экспорт
	...
	РолиПрофиля = РолиПрофиляДоступаСинхронизацияДанныхСДругимиПрограммами();
...
Процедура ПроверитьВозможностьВыполненияОбменов(ПриложениеВИнтернете = Ложь) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.РазделениеВключено()
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
			...
			Если НЕ МодульРаботаВМоделиСервиса.ОбластьДанныхИспользуетсяПользователями(НомерОбластиДанных) Тогда
...
Функция ПараметрыОбменаДаннымиЧерезФайлИлиСтроку() Экспорт
	...
	СтруктураПараметров.Вставить("ПроверенаВозможностьВыполненияОбменов", Ложь);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	ПроверенаВозможностьВыполненияОбменов = ПараметрыОбмена.Свойство("ПроверенаВозможностьВыполненияОбменов")
		И ПараметрыОбмена["ПроверенаВозможностьВыполненияОбменов"];
	
	Если Не ПроверенаВозможностьВыполненияОбменов Тогда
...
Процедура ПередВыполнениемОбменов(УзелИнформационнойБазы, Отказ)
	...
	Если ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		УстановленаПометка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелИнформационнойБазы, "ПометкаУдаления");
		Если УстановленаПометка = Истина Тогда
	...
	Если Отказ Тогда
		...
		ТекстИсключения = НСтр("ru = 'Синхронизация невозможна, потому настройка синхронизации помечена удаленной.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ИмяСобытия = СобытиеЖурналаРегистрацииОбменДанными();
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , , ТекстИсключения);
...
Функция ПолучитьИспользуемыеПланыОбмена() Экспорт
	...
	Для Каждого ИмяПланаОбмена Из ПланыОбменаБСП Цикл
		...
		+ Символы.ПС + "ГДЕ"
		+ Символы.Таб + "НЕ ПланОбмена.ЭтотУзел"; // @Query-part-4
...
//  КлючОбрабатывающегоЗадания - Строка - идентификатор (УИД) задания.
// 
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено, КлючОбрабатывающегоЗадания = "") Экспорт
	...
	ТекстЗапроса =
		...
		|		И НЕ НастройкиВыполненияОбменаНастройкиОбмена.Ссылка.ПометкаУдаления
...
//
Функция ТекстЗапросаПланыОбменаДляМонитора(ПараметрыЗапроса = Неопределено, ИсключитьПланыОбменаАвтономнойРаботы = Истина)
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Попытка
		Архиватор = Новый ЧтениеФайлаАрхива(ПолноеИмяФайлаАрхива, ПарольАрхива);
		Архиватор.ИзвлечьВсе(ПутьРаспаковкиФайлов, РежимВосстановленияПутейФайлаАрхива.НеВосстанавливать);
	Исключение
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Попытка
		...
		Архиватор = Новый ЗаписьФайлаАрхива(ПолноеИмяФайлаАрхива, ПарольАрхива);
	...
	Попытка
		...
		Архиватор.Добавить(МаскаУпаковкиФайлов, РежимСохраненияПутейФайлаАрхива.НеСохранятьПути);
		Архиватор.Записать();
...
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц, который содержит указатель на 
//		временную таблицу ИмяТаблицы.
// 
// Возвращаемое значение:
//  Число - количество записей в таблице базы данных.
//
...
Функция ИнтерактивноеИзменениеВыгрузкиОписаниеДетальногоОтбора(Знач ДополнениеВыгрузки) Экспорт
	...
	Если ДополнениеВыгрузки.ДополнительнаяРегистрация.Количество() Тогда
		Строка = ДополнениеВыгрузки.ДополнительнаяРегистрация[0];
		
		Текст = Текст + Символы.ПС + Строка.Представление + ": " + ПредставлениеОтбораДополненияВыгрузки(Строка.Период, Строка.Отбор);
		
		Если ДополнениеВыгрузки.ДополнительнаяРегистрация.Количество() > 1 Тогда
			Текст = Текст + " " + СтрШаблон(НСтр("ru = 'и (еще %1)'"), Строка(ДополнениеВыгрузки.ДополнительнаяРегистрация.Количество() - 1));
		КонецЕсли;
	КонецЕсли;
...
//
Функция ИнтерактивноеИзменениеВыгрузкиИсторияНастроек(Знач ДополнениеВыгрузки, Знач ВариантДополнения = Неопределено) Экспорт

	ОбработкаДополнения = Обработки.ИнтерактивноеИзменениеВыгрузки.Создать();
	
	Если ВариантДополнения = Неопределено Тогда
		ФильтрВарианта = ИнтерактивноеИзменениеВыгрузкиФильтрВарианта(ДополнениеВыгрузки);
	Иначе
		ФильтрВарианта = Новый Массив;
		ФильтрВарианта.Добавить(ВариантДополнения);
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыЗацикливания(Форма)
	...
	Если УзелЗацикленКонтрольВкл Тогда	
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ФормаОбщаяКомандаПредупреждениеЦикличностиНастроекОбменов",
...
Функция ПропуститьЗагрузкуПриоритетныхДанных()
...
Процедура ВключитьСинхронизациюДанныхПриНеобходимости()
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Или 
		ПолучитьИспользуемыеПланыОбмена().Количество() Тогда
		
		ВключитьСинхронизациюДанных();
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ВключитьСинхронизациюДанных()
...
Процедура ВыполнитьЗагрузкуПараметровГлавногоУзла(Отказ)
	...
	ИдентификаторТранспорта = ТранспортСообщенийОбмена.ТранспортПоУмолчанию(УзелИнформационнойБазы);
	ОбновитьПравилаРегистрацииДляУзла(УзелИнформационнойБазы);
...
Процедура ОбновитьПравилаРегистрацииДляУзла(УзелИнформационнойБазы)
...
Процедура ОбработатьОшибкуЗагрузки()
...
Процедура ВключитьОтключитьРежимЗагрузки(ПризнакРежимаЗагрузки)
	
	УстановитьПривилегированныйРежим(Истина);
	УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", ПризнакРежимаЗагрузки);
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

Процедура ПроверитьИОбработатьОтказ(Отказ)
