...
// Возвращает Истина, если используется вариант интерфейса клиентского приложения "Версия8_5".
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоВариантИнтерфейса85() Экспорт
#Если МобильныйКлиент Тогда
	Возврат Истина;
#КонецЕсли
	
	Возврат КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5;
КонецФункции
...
// Удаляет важное сообщение на форме, если оно выведено на форму.
// 
//
// Параметры:
...
//
//
Процедура УдалитьСтатусИлиВажноеСообщение(Форма, Идентификатор) Экспорт
	
	ИмяГруппы = ПрефиксГруппыВажногоСообщения() + Идентификатор;
	ИмяГруппыВПоказатьВсе = ПрефиксГруппыПоказатьВсе() + ИмяГруппы;
	
	УдалитьГруппуСтатусаИлиВажногоСообщения(Форма, ИмяГруппы, ИмяГруппыВПоказатьВсе, Идентификатор);
	
	ИмяГруппыСтатуса = ПрефиксГруппыСтатуса() + Идентификатор;
	ИмяГруппыСтатусаВПоказатьВсе = ПрефиксГруппыПоказатьВсе() + ИмяГруппыСтатуса;
	
	УдалитьГруппуСтатусаИлиВажногоСообщения(Форма, ИмяГруппыСтатуса, ИмяГруппыСтатусаВПоказатьВсе, Идентификатор, Истина);
...
Функция РаспределитьСуммуПропорциональноКоэффициентам(Знач РаспределяемаяСумма, Знач Коэффициенты, Знач Точность = 2) Экспорт
	...
	Если СуммарнаяПогрешность > 1 Тогда
		...
		Счетчик = 1;
		Пока ЗначениеЗаполнено(СуммарнаяПогрешность) Цикл
			ИндексЭлементаСМаксимальнымВесом = ИндексыВПорядкеВеса[Счетчик - 1];
			ПогашаемаяЧасть = ?(СуммарнаяПогрешность > 1, 1, СуммарнаяПогрешность);
			Результат[ИндексЭлементаСМаксимальнымВесом] = Результат[ИндексЭлементаСМаксимальнымВесом] + ПогашаемаяЧасть;
			СуммарнаяПогрешность = СуммарнаяПогрешность - ПогашаемаяЧасть;
			
			Счетчик = Счетчик + 1;
			Если Счетчик = Размерность Тогда
				Счетчик = 1;
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьСостояниеПоляТабличногоДокумента(ПолеТабличногоДокумента, Состояние = "НеИспользовать") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ИначеЕсли ВРег(Состояние) = "ФОРМИРОВАНИЕОТЧЕТА" Тогда  
			ОтображениеСостояния.Видимость                      = Истина;
			ОтображениеСостояния.ВариантОтображения             = ВариантОтображенияСостояния.НачальноеСостояние;
			ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
#Иначе
	...
	Если ВидСоединения = "КриптоПро" Тогда
		Возврат Новый ЗащищенноеСоединениеКриптоПро(СертификатКлиента,
			СертификатыУдостоверяющихЦентров, РежимПроверкиОтзываTLSСертификатаСервера.Строгий);
	КонецЕсли;
	
	Возврат Новый ЗащищенноеСоединениеOpenSSL(СертификатКлиента,
		СертификатыУдостоверяющихЦентров, РежимПроверкиОтзываTLSСертификатаСервера.Строгий);
#КонецЕсли
...
Функция ПрефиксГруппыВажногоСообщения() Экспорт
	
	Возврат "БФ_ГруппаВажноеСообщение" + "_";
	
КонецФункции

Функция ПрефиксГруппыСтатуса() Экспорт
	
	Возврат "БФ_ГруппаСтатус" + "_";
	
КонецФункции

Функция ПрефиксГруппыПоказатьВсе() Экспорт
	
	Возврат "ПоказатьВсе" + "_";
	
КонецФункции

Функция ПрефиксКомандСкрытияВажногоСообщения() Экспорт
	
	Возврат "КомандаЗакрытьВажноеСообщение" + "_";
	
КонецФункции

Функция ПрефиксСтатуса() Экспорт
	Возврат "БФ_Статус" + "_";
КонецФункции

Процедура УдалитьГруппуСтатусаИлиВажногоСообщения(Форма, ИмяГруппы, ИмяГруппыВПоказатьВсе, Идентификатор, ЭтоСтатус = Ложь)
	...
	Группа = Форма.Элементы.Найти(ИмяГруппыВПоказатьВсе);
	Если Группа <> Неопределено Тогда
		Группа.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтоСтатус Тогда
		ИмяКоллекции = "БФ_ДобавляемыеВажныеСообщения";
		ИмяСчетчика  = "БФ_ВсегоВажныхСообщений";
	Иначе
		ИмяКоллекции = "БФ_ДобавляемыеСтатусы";
		ИмяСчетчика  = "БФ_ВсегоСтатусов";
	КонецЕсли;
	
	Проверка = Новый Структура;
	Проверка.Вставить(ИмяКоллекции);
	ЗаполнитьЗначенияСвойств(Проверка, Форма);
	
	Если Проверка[ИмяКоллекции] = Неопределено
		Или Форма[ИмяКоллекции].Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Элемент Из Форма[ИмяКоллекции] Цикл
		Если Элемент.Значение.Идентификатор = Идентификатор Тогда
			Форма[ИмяКоллекции].Удалить(Элемент);
			
			Форма[ИмяСчетчика] = Форма[ИмяСчетчика] - 1;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
//  Массив из Число
//
Функция ИндексыВПорядкеВеса(Массив)
	// Создаем массив - изначально просто индексы
	Результат = Новый Массив;
	Для Индекс = 0 По Массив.ВГраница() Цикл
		Результат.Добавить(Индекс);
	КонецЦикла;
    
    // Сортируем результат по значениям из входящего массива (пузырьком - один проход внешний)
	Для Индекс1 = 0 По Результат.ВГраница() - 1 Цикл
		Для Индекс2 = 0 По Результат.ВГраница() - Индекс1 - 1 Цикл
			Если Массив[Результат[Индекс2]] < Массив[Результат[Индекс2 + 1]] Тогда
				Смещаемый = Результат[Индекс2];
				Результат[Индекс2] = Результат[Индекс2 + 1];
				Результат[Индекс2 + 1] = Смещаемый;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Возврат Результат;
