...
//	- РежимОткрытияОкна - РежимБлокировкиПриОткрытииОкнаФормы - нужный тип режима открытия формы
//
...
Процедура ДобавлениеПрограммыЭлектроннойПодписиПослеВыбора(РезультатВыполнения, ВходящийКонтекст) Экспорт
	...
	Если РезультатВыполнения.Выполнено 
		...
		Если РезультатВыполнения.Результат = "ОблачнаяПодпись" Тогда
			...
			ПараметрыФормы.Вставить("РежимОткрытияОкна",
				РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
			ОткрытьСведенияУчетнойЗаписи(Неопределено,
				Неопределено, ВходящийКонтекст.ПараметрыОперации);
		Иначе
