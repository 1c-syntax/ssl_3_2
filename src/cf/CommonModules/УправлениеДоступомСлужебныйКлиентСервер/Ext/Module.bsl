...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа) Экспорт
	
	Если ОписаниеВидаДоступа = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ПредставлениеКоличества = Формат(КоличествоЗначений, "ЧН=0; ЧГ=");
	
	ПредставлениеВсеЗапрещены = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Разрешены: %1'"), ПредставлениеКоличества);
	ПредставлениеВсеРазрешены = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Запрещены: %1'"), ПредставлениеКоличества);
	
	Индекс = ?(Форма.ЭтоПрофильГруппДоступа, 2, 0);
	
	Форма.ПредставленияВсеРазрешены[Индекс].Представление = ПредставлениеВсеЗапрещены;
	Форма.ПредставленияВсеРазрешены[Индекс + 1].Представление = ПредставлениеВсеРазрешены;
	
	СписокВыбора = Форма.Элементы.ВидыДоступаВсеРазрешеныПредставление.СписокВыбора; // СписокЗначений
	УстановитьЗначение(СписокВыбора[Индекс].Значение, ПредставлениеВсеЗапрещены);
	УстановитьЗначение(СписокВыбора[Индекс + 1].Значение, ПредставлениеВсеРазрешены);
	
	Если Форма.ЭтоПрофильГруппДоступа Тогда
		УстановитьЗначение(СписокВыбора[Индекс].Представление, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Разрешены: %1 (настройка в профиле)'"), ПредставлениеКоличества));
		УстановитьЗначение(СписокВыбора[Индекс + 1].Представление, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрещены: %1 (настройка в профиле)'"), ПредставлениеКоличества));
	КонецЕсли;
	
	Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
		Если Форма.ЭтоПрофильГруппДоступа И Не ОписаниеВидаДоступа.Предустановленный Тогда
			Имя = "ВначалеВсеРазрешены";
			Форма.Элементы.ТекстИнформацииОбычногоВидаДоступа.Заголовок =
				НСтр("ru = '""Запрещены: 0"" - начальная настройка в новой группе доступа.
				           |Список запрещенных значений настраивается в группах доступа.'");
		Иначе
			Имя = "ВсеРазрешены";
		КонецЕсли;
	Иначе
		Если Форма.ЭтоПрофильГруппДоступа И Не ОписаниеВидаДоступа.Предустановленный Тогда
			...
			Форма.Элементы.ТекстИнформацииОбычногоВидаДоступа.Заголовок =
				НСтр("ru = '""Разрешены: 0"" - начальная настройка в новой группе доступа.
				           |Список разрешенных значений настраивается в группах доступа.'");
		Иначе
			Имя = "ВсеЗапрещены";
		КонецЕсли;
	КонецЕсли;
	
	УстановитьЗначение(ОписаниеВидаДоступа.ВсеРазрешеныПредставление,
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма, ОбработкаНаКлиенте = Истина) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЗначенияРедактируются Тогда
			...
			Если ОтборСтрок = Неопределено
			 Или Отбор.Свойство("ГруппаДоступа") И ОтборСтрок.ГруппаДоступа <> Отбор.ГруппаДоступа
			 Или ОтборСтрок.ВидДоступа <> Отбор.ВидДоступа Тогда
	...
	ОбновитьПросмотрЗначенийДоступа(Форма);
...
// Параметры:
//  Форма - см. ПараметрыФормыРедактированияРазрешенныхЗначений
//  ЗначенияРедактируются - Булево
//
Процедура ОбновитьПросмотрЗначенийДоступа(Форма, ЗначенияРедактируются = Истина) Экспорт
	
	Элементы = Форма.Элементы;
	
	ТолькоПросмотр = Не ЗначенияРедактируются Или Элементы.ВидыДоступа.ТолькоПросмотр;
	Элементы.ЗначенияДоступа.ТолькоПросмотр = ТолькоПросмотр;
	Элементы.ЗначенияДоступаПодобрать.Доступность = Не ТолькоПросмотр;
...
// Для процедуры ЗаполнитьВсеРазрешеныПредставление.
Процедура УстановитьЗначение(СтароеЗначение, НовоеЗначение)
