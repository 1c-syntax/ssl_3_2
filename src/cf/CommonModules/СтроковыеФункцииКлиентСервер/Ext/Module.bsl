			...
			Если ИменаТэгов.Получить(ВРег(ИмяТега)) = Истина Тогда
				...
				Если ВРег(ИмяТега) = "B" Тогда
					Если ТекущийШрифт = Неопределено Тогда
						ТекущийШрифт = Новый Шрифт;
					КонецЕсли;
					// АПК:1345-выкл - поддержка форматирования </b>Жирный текст</b>.
					ТекущийШрифт = Новый Шрифт(ТекущийШрифт,,, Истина);
					// АПК:1345-вкл
				КонецЕсли;
...
Процедура УстановитьСтилиПоЗначениюАтрибута(Знач ОписаниеСтиля, ЭлементыСтиля, ТекущийФон, ТекущийЦвет, ТекущийШрифт)
	...
	Для каждого Стиль Из Стили Цикл
		...
		Если Не ЗначениеЗаполнено(Стиль) Тогда
			Продолжить;
		КонецЕсли;
		
		ЗначенияСтиля = СтрРазделить(Стиль, ":");
		ЗначенияСтиля = СтрРазделить(стиль, ":");
		Если ЗначенияСтиля.Количество() < 2 Тогда
			Продолжить;
		КонецЕсли;
		ИмяСтиля      = СокрЛП(ЗначенияСтиля[0]);
		ЗначениеСтиля = СокрЛП(ЗначенияСтиля[ЗначенияСтиля.ВГраница()]);
