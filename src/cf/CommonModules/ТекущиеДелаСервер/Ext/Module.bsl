...
// Краткая сводка по текущим делам пользователя.
// Может выполняться длительное время, рекомендуется вызывать в фоновом задании.
//
// Возвращаемое значение:
//   Структура:
//      * ВсегоДел - Число
//      * КоличествоВажныхДел - Число
//
Функция СводкаПоТекущимДелам() Экспорт
	
	ТаблицаДел = ТекущиеДелаСлужебный.ТаблицаДел();
	
	Фильтр = Новый Структура;
	Фильтр.Вставить("ЕстьДела", Истина);
	Фильтр.Вставить("ЭтоРаздел", Истина);
	НайденоДел = ТаблицаДел.НайтиСтроки(Фильтр);
	ВсегоДел = НайденоДел.Количество();
	
	Фильтр = Новый Структура;
	Фильтр.Вставить("ЕстьДела", Истина);
	Фильтр.Вставить("ЭтоРаздел", Истина);
	Фильтр.Вставить("Важное", Истина);
	НайденоДел = ТаблицаДел.НайтиСтроки(Фильтр);
	ВажныхДел = НайденоДел.Количество();
	
	Результат = Новый Структура;
	Результат.Вставить("ВсегоДел", ВсегоДел);
	Результат.Вставить("КоличествоВажныхДел", ВажныхДел);
