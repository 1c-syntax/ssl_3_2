...
// Проверяет корректность адресов по данным адресного классификатора (ГАР).
// Для некорректных адресов возвращает подходящие варианты, содержащие актуальные сведения об адресе.
// 
// Важно: проверка адресов через веб‑сервис может выполняться продолжительное время (до 120 секунд).
// Чтобы избежать зависания интерфейса пользователя (например, при открытии форм
// или массовой обработке), необходимо вызывать функцию через фоновое задание.
// 
// Параметры:
//   Адреса - Массив из Структура - проверяемые адреса. Содержит структуры с полями: 
...
//          - Строка - адрес в формате JSON или XML контактной информации.
//
// Возвращаемое значение:
//   Массив из Структура - результаты проверки. Каждый элемент массива содержит структуру с полями:
//       * АдресПроверен - Булево - признак завершения проверки адреса.
//       * АдресКорректный - Булево - признак соответствия адреса классификатору (если АдресПроверен = Истина).
//       * АдресУстарел - Булево - признак устаревшего адреса.
//       * Ошибки   - Массив из Структура - описание ошибок поиска в классификаторе:
...
//       * Варианты - Массив из Структура - содержит описание найденных вариантов в виде массива структур с полями:
//           ** Идентификатор    - УникальныйИдентификатор - уникальный идентификационный код адресного объекта.
...
//
// Пример:
//    АдресаДляПроверки = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("Адрес", АдресВJSON));
//    РезультатПроверки = АдресныйКлассификатор.ПроверитьАдреса(АдресаДляПроверки);
//    
//    Если РезультатПроверки.Количество() > 0 Тогда
//        ДанныеАдреса = РезультатПроверки[0];
//        
//        Если Не ДанныеАдреса.АдресКорректный И ДанныеАдреса.Ошибки.Количество() > 0 Тогда
//            Ошибка = ДанныеАдреса.Ошибки[0];
//            ТекстОшибки = Ошибка.Текст;
//            Если ЗначениеЗаполнено(Ошибка.Подсказка) Тогда
//                ТекстОшибки = ТекстОшибки + " " + Ошибка.Подсказка;
//            КонецЕсли;
//        КонецЕсли;
//    КонецЕсли;
//
