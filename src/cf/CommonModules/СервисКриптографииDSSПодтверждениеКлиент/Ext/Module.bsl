...
Процедура ПодключитьОжидание(КонтекстФормы, ВстроенноеПодтверждение = Ложь) Экспорт
	...
	Если ПроверитьЗапущенныйЦиклПодтверждения(КонтекстФормы, ВстроенноеПодтверждение)
		...
		КонтекстФормы.ПодключитьОбработчикОжидания("Подключаемый_ПодтверждениеОбработчикОжидания", 5, Истина);
	КонецЕсли;
...
//
Процедура ПодтверждениеПриОткрытии(КонтекстФормы, ПарольУстановлен = Ложь, ОписаниеДанных = Неопределено) Экспорт
...
Функция ПолучитьПредставлениеСпособа(ВторичнаяАвторизация, ДанныеПодтверждения, НастройкиПользователя)
	...
	ИначеЕсли ТекущийВариант = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_МобильноеПриложение") Тогда
		...
		ШаблонСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ИмяШрифта);
	...
	ИначеЕсли ТекущийВариант = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
		ШаблонСтроки = Результат = НСтр("ru = 'Офлайн-подтверждение через приложение'", КодЯзыка);
	...
	Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ШаблонСтроки);
...
Процедура ВывестиИнформациюСервера(КонтекстФормы, ИмяЭлементаФормы = "ПодтверждениеИнформацияСервера") Экспорт
	...
	УстановитьСвойствоЭлемента(КонтекстФормы.Элементы, ИмяЭлементаФормы, "Видимость",
		ЗначениеЗаполнено(КонтекстФормы[РеестрИмен.ИнформацияСервера]), ИменаЭлементов);
...
Процедура УстановитьСвойствоЭлемента(ЭлементыФормы, ИмяЭлемента, СвойствоЭлемента, ЗначениеЭлемента, ИменаЭлементов = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() И (СвойствоЭлемента = "КнопкаПоУмолчанию" Или СвойствоЭлемента = "КнопкаДополнительная") Тогда
		Если СвойствоЭлемента = "КнопкаПоУмолчанию" Тогда
			СвойствоЭлемента = "Важность";
			ЗначениеЭлемента = ?(ЗначениеЭлемента = Истина, ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
		Иначе
			СвойствоЭлемента = "Важность";
			ЗначениеЭлемента = ВажностьКнопки.Дополнительная;
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеЭлементамиОбщее(КонтекстФормы, ОткудаВызов)
	...
	Если НЕ ОблачныйСертификат Тогда
		...
		УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеОК", "Видимость", Ложь, ИменаЭлементов);
		Возврат;
	КонецЕсли;
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеОК", "Видимость", НЕ НеНачалось И НЕ ОжидатьВыполнения, ИменаЭлементов);
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеПовторить", "КнопкаДополнительная", НЕ НеНачалось, ИменаЭлементов);
	КонецЕсли;
