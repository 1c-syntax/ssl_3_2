	...
	Для Каждого СтрокаКонтактнойИнформации Из Результат.История Цикл
		...
		Если НЕ СтрокаКонтактнойИнформации.ЭтоИсторическаяКонтактнаяИнформация Тогда
			Если ПустаяСтрока(СтрокаКонтактнойИнформации.Представление)
			   И Форма.Элементы.Найти(ДополнительныеПараметры.ИмяЭлемента) <> Неопределено
			   И Форма.Элементы[ДополнительныеПараметры.ИмяЭлемента].Вид = ВидПоляФормы.ПолеНадписи Тогда
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ИначеЕсли ЭлементФормы <> Неопределено Тогда
			ЭлементФормы.РасширеннаяПодсказка.Заголовок = Комментарий;
		КонецЕсли;
	...
	Если ЭлементФормы <> Неопределено И ЭлементФормы.Вид = ВидПоляФормы.ПолеНадписи
...
Процедура КонтактнаяИнформацияДобавитьПолеВводаЗавершение(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВыбранныйЭлемент.Значение.Ссылка) Тогда
		...
		ОткрытьФорму("Справочник.ВидыКонтактнойИнформации.Форма.ФормаСписка",ПараметрыОткрытияФормы,
			ДополнительныеПараметры.Форма,,,,Оповещение,РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
		Возврат;
	КонецЕсли;
...
Процедура ПриВыполненииКоманды(Знач Форма, Знач ИмяКоманды, АсинхронныйВызов)
	...
	ИначеЕсли СтрНайти(ИмяКоманды, "Команда") <> 0 Тогда
		НачалоСтроки = СтрНайти(ИмяКоманды, "Команда");
		Команда = Лев(ИмяКоманды, НачалоСтроки - 1);
		ИмяКоманды = Сред(ИмяКоманды, НачалоСтроки);
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() И ЗначениеЗаполнено(Команда) Тогда
			ВыполнитьКомандуКонтактнойИнформации(КомандыДляВывода[Команда].Действие, Параметры);
		Иначе
...
Процедура ПриИзмененииКонтактнойИнформации(Форма, Элемент, ЭтоТабличнаяЧасть, ОбновитьФорму, АсинхронныйВызов)
	...
	Если ПустаяСтрока(Текст) Тогда
		...
		Если ОбновитьФорму Тогда
			ОбновитьСпискиВыбораПолейАдресов(Форма, ДанныеСтроки.ИмяЭлементаДляРазмещения);
		КонецЕсли;      
