...
Процедура ВойтиВОбластьДанных(ОбластьДанных) Экспорт
...
Процедура ВыйтиИзОбластиДанных(ОбластьДанных) Экспорт
	
	МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
	Если НЕ МодульРаботаВМоделиСервиса.СеансЗапущенБезРазделителей() Тогда
		
		Возврат; // Разрешено выходить из области данных только неразделенному пользователю
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ОбменДаннымиСервер.ЭтоРазделенныйПланОбменаБСП(ИмяПланаОбмена)
		И Метаданные.ПланыОбмена[ИмяПланаОбмена].РаспределеннаяИнформационнаяБаза
		И ОбменДаннымиПовтИсп.ПланОбменаИспользуетсяВМоделиСервиса(ИмяПланаОбмена) Тогда
		
		ПараметрыВыполнения.БезРасширений = Истина;
		ПараметрыПроцедуры.Вставить("ПроверенаВозможностьВыполненияОбменов", Истина);
