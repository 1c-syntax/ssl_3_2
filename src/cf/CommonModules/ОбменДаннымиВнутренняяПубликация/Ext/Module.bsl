...
Процедура ВыполнитьОбменДаннымиПоСценарию(КодСценарияОбмена) Экспорт
	...
	Если Не ОчередьЗадачВыполнена(Сценарий, , , ПерваяЗадача) Тогда
		...
		Если ЗначениеЗаполнено(ПерваяЗадача) Тогда
			ТекущаяЗадачаСценария = ЗадачаПоИдентификатору(ПерваяЗадача);
			
			Если ТекущаяЗадачаСценария.Действие = Перечисления.ДействияПриОбменеВнутренняяПубликация.ЗагрузкаДанных Тогда
				Если ЗадачаВыполняется(ТекущаяЗадачаСценария.ИдентификаторЗадачи) Тогда
					Возврат;
				КонецЕсли;
			ИначеЕсли ТекущаяЗадачаСценария.Действие = Перечисления.ДействияПриОбменеВнутренняяПубликация.ВыгрузкаДанных Тогда
				Если ЗадачаВыполняется(ТекущаяЗадачаСценария.ИдентификаторЗадачи) Тогда
					Возврат;
				КонецЕсли;
				
				ПредыдущаяЗадачаСценария = ПредыдущаяЗадача(ТекущаяЗадачаСценария.ИдентификаторЗадачи);
				
				Если ПредыдущаяЗадачаСценария <> Неопределено Тогда
					ИдентификаторОчередиЗадачи = ПредыдущаяЗадачаСценария.ИдентификаторЗадачи;
					
					Если ЗадачаВыполняется(ИдентификаторОчередиЗадачи) Тогда
						Возврат;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			ВыполнитьОчередьЗадач(ТекущаяЗадачаСценария);
		КонецЕсли;
...
Функция ПредыдущаяЗадача(ИдентификаторЗадачи)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПредЗадача.ИдентификаторЗадачи КАК ИдентификаторЗадачи,
	|	ПредЗадача.Действие КАК Действие
	|ИЗ
	|	РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК Задача
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация КАК ПредЗадача
	|		ПО Задача.ИдентификаторОбмена = ПредЗадача.ИдентификаторОбмена
	|		И Задача.НомерЗадачи - 1 = ПредЗадача.НомерЗадачи
	|ГДЕ
	|	Задача.ИдентификаторЗадачи = &ИдентификаторЗадачи
	|	И Задача.Действие = &Действие";
	
	Запрос.УстановитьПараметр("ИдентификаторЗадачи", ИдентификаторЗадачи);
	Запрос.УстановитьПараметр("Действие", Перечисления.ДействияПриОбменеВнутренняяПубликация.ЗагрузкаДанных);
...
Функция ЗадачаВыполняется(ИдентификаторОчередиЗадачи)
	
	СтатусЗадачи = ОбменДаннымиОперацииВебСервисов.СтатусЗадачи(ИдентификаторОчередиЗадачи);
	
	Если СтатусЗадачи = "Запланировано" Или СтатусЗадачи = "Выполняется" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
