...
//			       Такие ссылки открываются при помощи процедуры ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку.
...
Процедура ОбновитьЭлементыФормыНастроек(Форма, УзелИерархииЭлементов, ПараметрыОбновления = Неопределено) Экспорт 
	...
	Если Форма.ТипФормыОтчета = ТипФормыОтчета.Настройка Тогда
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			СоздатьЭлементыФормыПользовательскихНастроекДляФормыНастроек(Форма, СвойстваЭлементов.Поля, ВременнаяГруппа);
		Иначе
...
Процедура СоздатьЭлементыФормыПользовательскихНастроекДляФормыНастроек(Форма, СвойстваПолей, Группа)
		
	ГруппаОсновное = Форма.Элементы.Добавить("КомпоновщикНастроекПользовательскиеНастройкиОсновные", Тип("ГруппаФормы"), Группа);
	ГруппаОсновное.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаОсновное.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаОсновное.ОтображатьЗаголовок = Ложь;
	ГруппаОсновное.СквозноеВыравнивание = СквозноеВыравнивание.Использовать;
	
	ЭлементыПользовательскихНастроек = Форма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
	...
	Для Каждого ЭлементНастройки Из ЭлементыПользовательскихНастроек Цикл
		...
		СоздатьПолеИспользование = Истина;
		СоздатьПолеВидСравнения = Истина;
		СоздатьПоле = Истина;

		ИндексЭлементаНастройки = ЭлементыПользовательскихНастроек.Индекс(ЭлементНастройки);
		Заголовок = "";
			НомерЭлемента = ИндексЭлементаНастройки;

		Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			ЗначениеПараметраНастроек = Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти(ЭлементНастройки.Параметр);
			ЗначениеПараметраНастроек = ?(ЗначениеПараметраНастроек <> Неопределено, ЗначениеПараметраНастроек,
				Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти(ЭлементНастройки.Параметр));
			Если ЗначениеПараметраНастроек = Неопределено Тогда
				ГруппаТипДиаграммы = Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("ТипДиаграммы");
				Если ГруппаТипДиаграммы <> Неопределено Тогда
					ЗначениеПараметраНастроек = ГруппаТипДиаграммы.ЗначенияВложенныхПараметров.Найти(ЭлементНастройки.Параметр);
					ДоступныйПараметрВыводаТипДиаграммы = Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.ДоступныеПараметры.Элементы.Найти(ГруппаТипДиаграммы.Параметр);
					ДоступныйПараметрВывода = ?(ДоступныйПараметрВыводаТипДиаграммы <> Неопределено, ДоступныйПараметрВыводаТипДиаграммы.ВложенныеПараметры.Найти(ЭлементНастройки.Параметр), Неопределено);
					Заголовок = ?(ДоступныйПараметрВывода <> Неопределено, ДоступныйПараметрВывода.Заголовок, "");
				КонецЕсли;
			КонецЕсли;
			Если ЗначениеПараметраНастроек = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Заголовок = ?(ЗначениеЗаполнено(ЗначениеПараметраНастроек.ПредставлениеПользовательскойНастройки), ЗначениеПараметраНастроек.ПредставлениеПользовательскойНастройки, Заголовок);
			Если Не ЗначениеЗаполнено(Заголовок) Тогда
				ДоступныйПараметрКомпоновкиДанных = Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы.Найти(ЭлементНастройки.Параметр);
				Заголовок = ?(ДоступныйПараметрКомпоновкиДанных <> Неопределено, ДоступныйПараметрКомпоновкиДанных.Заголовок, "");
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Заголовок) Тогда
				ДоступныйПараметрВывода = Форма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.ДоступныеПараметры.Элементы.Найти(ЭлементНастройки.Параметр);
				Заголовок = ?(ДоступныйПараметрВывода <> Неопределено, ДоступныйПараметрВывода.Заголовок, "");
			КонецЕсли;

			СоздатьПолеИспользование = ЗначениеПараметраНастроек.Использование <> ИспользованиеПараметраКомпоновкиДанных.Всегда;
			СоздатьПолеВидСравнения = ТипЗнч(ЭлементНастройки.Значение) <> Тип("СтандартныйПериод");
			СоздатьГруппу = Не СоздатьПолеИспользование И Не СоздатьПолеВидСравнения;
			
		ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			СвойстваЭлемента = СвойстваПолей.Найти(ЭлементНастройки.ИдентификаторПользовательскойНастройки, "ИдентификаторНастройки");
			Если СвойстваЭлемента <> Неопределено Тогда
				Заголовок = ?(ЗначениеЗаполнено(СвойстваЭлемента.ЭлементНастройки.ПредставлениеПользовательскойНастройки),
					СвойстваЭлемента.ЭлементНастройки.ПредставлениеПользовательскойНастройки, СвойстваЭлемента.ЭлементНастройки.Представление);
				Заголовок = ?(ЗначениеЗаполнено(Заголовок), Заголовок, СвойстваЭлемента.ОписаниеНастройки.Заголовок);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ЭлементНастройки) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда 
			СоздатьГруппу = Ложь;
			СоздатьПолеВидСравнения = Ложь;
			СоздатьПоле = Ложь;
			СвойстваЭлемента = СвойстваПолей.Найти(ЭлементНастройки.ИдентификаторПользовательскойНастройки, "ИдентификаторНастройки");
			Если СвойстваЭлемента <> Неопределено Тогда
				Заголовок = ?(ЗначениеЗаполнено(СвойстваЭлемента.ЭлементНастройки.ПредставлениеПользовательскойНастройки),
					СвойстваЭлемента.ЭлементНастройки.ПредставлениеПользовательскойНастройки, СвойстваЭлемента.ЭлементНастройки.Представление);
				Заголовок = ?(ЗначениеЗаполнено(Заголовок), Заголовок, Строка(СвойстваЭлемента.ЭлементНастройки.ТипГруппы));
			КонецЕсли;
		...
		Если СоздатьГруппу Тогда
			ГруппаОтбора = Форма.Элементы.Добавить("КомпоновщикНастроекПользовательскиеНастройкиЭлемент"+ИндексЭлементаНастройки, Тип("ГруппаФормы"), ГруппаОсновное);
			ГруппаОтбора.Заголовок = Заголовок;
			...
			ГруппаОтбора.СквозноеВыравнивание = СквозноеВыравнивание.Использовать;
		Иначе
			ГруппаОтбора = ГруппаОсновное;
		КонецЕсли;
		
		Если СоздатьПолеИспользование Тогда
			ПолеИспользование = Форма.Элементы.Добавить("КомпоновщикНастроекПользовательскиеНастройкиЭлемент"+ИндексЭлементаНастройки+"Использование", Тип("ПолеФормы"), ГруппаОтбора);
			ПолеИспользование.Заголовок = Заголовок;
			ПолеИспользование.Вид = ВидПоляФормы.ПолеФлажка;
			ПолеИспользование.ПутьКДанным = "Отчет.КомпоновщикНастроек.ПользовательскиеНастройки["+ИндексЭлементаНастройки+"].Использование";
		КонецЕсли;

		Если СоздатьПолеВидСравнения Тогда
			ПолеВидСравнения = Форма.Элементы.Добавить("КомпоновщикНастроекПользовательскиеНастройкиЭлемент"+ИндексЭлементаНастройки+"ВидСравненияЭлементаОтбора", Тип("ПолеФормы"), ГруппаОтбора);
			ПолеВидСравнения.Заголовок = Заголовок;
			ПолеВидСравнения.Вид = ВидПоляФормы.ПолеВвода;
			ПолеВидСравнения.ПутьКДанным = "Отчет.КомпоновщикНастроек.ПользовательскиеНастройки["+ИндексЭлементаНастройки+"].ВидСравнения";
		КонецЕсли;
		
		Если СоздатьПоле Тогда
			ПолеЗначение = Форма.Элементы.Добавить("КомпоновщикНастроекПользовательскиеНастройкиЭлемент"+ИндексЭлементаНастройки+"Значение", Тип("ПолеФормы"), ГруппаОтбора);
			ПолеЗначение.Заголовок = Заголовок;
			ПолеЗначение.Вид = ВидПоляФормы.ПолеВвода;
			ПолеЗначение.ВыбиратьТип = Ложь;
			ПолеЗначение.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
			ПолеЗначение.ПутьКДанным = "Отчет.КомпоновщикНастроек.ПользовательскиеНастройки["+ИндексЭлементаНастройки+"].Значение";
		КонецЕсли;
...
Процедура ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл 
		...
		Если Группа = Неопределено Тогда 
			...
			Группа.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Полуторный;
		КонецЕсли;
		...
		РаспределятьПоКолонкам = ?(ИдентификаторГруппы = "Дополнительно", Ложь, Истина);
		Группа.Группировка = ?(РаспределятьПоКолонкам, ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда,
			ГруппировкаПодчиненныхЭлементовФормы.Вертикальная);
		
		ПодготовитьЭлементыФормыНастроекКРаспределению(
			ЭлементыНастроекГруппы, СвойстваГруппы.Группировка, ЭтоОсновнаяФорма(Форма), ДополнительныеСвойства, РаспределятьПоКолонкам);
...
Функция ЭлементыФормыНастроек(Форма, ОписаниеНастроек, ИменаРеквизитов)
	...
	Для Каждого Запись Из НеполныеЭлементы Цикл 
		...
		ОписаниеНастройки = ОписаниеНастроек.Найти(Элемент.ИдентификаторНастройки, "ИдентификаторНастройки"); 
		Если ОписаниеНастройки <> Неопределено И ТипЗнч(ОписаниеНастройки.ЭлементНастройки) = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
...
#Область УстановкаСвойствЭлементовИспользование
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл 
		...
		Если ТипЗнч(Поле) = Тип("ПолеФормы") Тогда 
			...
			Если Поле.Вид = ВидПоляФормы.ПолеФлажка Тогда
		...
		Если СтрДлина(Поле.Заголовок) > 40 И Форма.ТипФормыОтчета <> ТипФормыОтчета.Настройка Тогда
...
#Область УстановкаСвойствЭлементовЗначение
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл 
		...
		Иначе // Поле ввода.
			...
			Иначе
				Поле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
			КонецЕсли;
			...
			Иначе
				...
				СвязанноеПоле.ВидФлажка = ВидФлажка.Флажок;
			КонецЕсли;
			...
			Поле.АвтоМаксимальнаяШирина = Истина;
...
Процедура ПодготовитьЭлементыФормыНастроекКРаспределению(ЭлементыНастроек, Группировка, ЭтоОсновнаяФорма, ДополнительныеСвойства, РаспределятьПоКолонкам)
#Область ПередПодготовкой
	...
	КоличествоКолонок = 1;
	Если РаспределятьПоКолонкам Тогда
...
Процедура РаспределитьЭлементыФормыНастроек(Форма, Знач Группа, ЭлементыНастроек)
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл 
		...
		Если Колонка = Неопределено Тогда 
			...
			Колонка.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
...
Функция ИерархияЭлементовФормыНастроек(Форма, Родитель, Признаки, НомерСтроки, НомерКолонки)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Строка.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;
	Иначе
		Строка.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Одинарный;
	КонецЕсли;
	Строка.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
...
Процедура ВывестиПериодыНастроек(Форма, ЭлементыНастроек, ИменаРеквизитов)
	...
	ВыводитьЗаголовкиПериодов = НайденныеЭлементы.Количество() >= 2;
...
Процедура ДобавитьЭлементыСписка(Форма, ЭлементНастройки, ОписаниеЭлементаНастройки, ИмяСписка, ИменаРеквизитов)
	...
	ЗаголовокСписка = Форма.Элементы.Добавить(ИмяСписка + "Заголовок", Тип("ДекорацияФормы"), Поле.Родитель);
	ЗаголовокСписка.Заголовок = Поле.Заголовок;
	ЗаголовокСписка.АвтоМаксимальнаяВысота = Ложь;
	...
	Если ИменаРеквизитовПредопределенных.Найти(ИмяСписка) = Неопределено Тогда 
		...
		Список.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ПоСодержимому;
		Список.ВысотаВСтрокахТаблицы = 3;
		Список.АвтоМаксимальнаяВысота = Ложь;
		Список.АвтоМаксимальнаяВысотаВСтрокахТаблицы = Ложь;
		Список.МаксимальнаяВысотаВСтрокахТаблицы = 7;
		Список.РежимВыделения = РежимВыделенияТаблицы.Одиночный;
		...
		ПолеПометки.ВидФлажка = ВидФлажка.Флажок;
	...
	Список.ПоложениеЗаголовка=ПоложениеЗаголовкаЭлементаФормы.Нет;
...
Процедура ДобавитьКомандыСписка(Форма, ЭлементНастройки, ЭлементыНастроек, ИмяСписка)
	...
	ГруппаСписка.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	ГруппаСписка.РастягиватьПоГоризонтали = Ложь;
	
	Список = Элементы[ИмяСписка];
	Если Не Список.ИзменятьСоставСтрок Тогда 
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ПолеЗаголовка) = Тип("ПолеФормы") Тогда
		ПолеЗаголовка.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	КонецЕсли;
	
	ЗаголовокСписка = Форма.Элементы.Найти(ИмяСписка + "Заголовок");
	Если ЗаголовокСписка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ГруппаЗначение = Список.Родитель;
	ГруппаЗаголовка = Форма.Элементы.Добавить("ГруппаЗаголовок" + ИмяСписка, Тип("ГруппаФормы"), ГруппаЗначение);
	ГруппаЗаголовка.Заголовок = НСтр("ru = 'Заголовок'");
	ГруппаЗаголовка.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаЗаголовка.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	ГруппаЗаголовка.ОтображатьЗаголовок = Ложь;
	ГруппаЗаголовка.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	Форма.Элементы.Переместить(ЗаголовокСписка, ГруппаЗаголовка);
	Форма.Элементы.Переместить(ГруппаЗаголовка, ГруппаЗначение, Список);
	...
	Если ЕстьСсылочныйТип Тогда
		...
		ДобавитьКомандуСписка(Форма, Список.КонтекстноеМеню, ИмяКоманды, ЗаголовокКоманды,
			"Подключаемый_Список_ВставитьИзБуфера", БиблиотекаКартинок.ВставитьИзБуфераОбмена);
		
		ДекорацияВставитьИзБуфера = Форма.Элементы.Добавить(ИмяКоманды + "Картинка", Тип("ДекорацияФормы"), ГруппаЗаголовка);
		ДекорацияВставитьИзБуфера.Вид = ВидДекорацииФормы.Картинка;
		ДекорацияВставитьИзБуфера.Картинка = БиблиотекаКартинок.ВставитьИзБуфераОбмена;
		ДекорацияВставитьИзБуфера.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Центр;
		ДекорацияВставитьИзБуфера.Гиперссылка = Истина;
		ДекорацияВставитьИзБуфера.УстановитьДействие("Нажатие", "Подключаемый_Список_ВставитьИзБуфераКартинка");
	КонецЕсли;
...
Процедура ДобавитьКомандуСписка(Форма, Родитель, ИмяКоманды, Заголовок, Действие, Картинка = Неопределено)
	...
	Кнопка.ВажностьПриОтображении = ВажностьПриОтображении.Обычная;
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Кнопка.Важность = ВажностьКнопки.Дополнительная;
	Иначе
...
Процедура ВывестиЗначенияКакПоляФлажков(Форма, ЭлементыНастроек, ИменаРеквизитов)
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл 
		...
		ПолеФлажка.ВидФлажка = ВидФлажка.Флажок;
