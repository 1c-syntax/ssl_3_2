...
Функция ПолучитьФайлИзХранилищаВСервисе(Прокси, Знач ИдентификаторФайла, Знач УзелИнформационнойБазы = Неопределено,
	...
	ПротоколироватьНачалоЗагрузки(УзелИнформационнойБазы, КоличествоЧастей, СтруктураНастроекОбмена);
	
	ПолучитьЧастиФайла(Прокси, ИдентификаторСессии, КоличествоЧастей, КаталогСборки, ИменаФайлов, ОбластьДанных);
	
	Прокси.ReleaseFile(ИдентификаторСессии);
	
	ИмяАрхива = СобратьАрхивИзЧастей(КаталогСборки, ИменаФайлов);
		
	ПроверитьВалидностьАрхива(ИмяАрхива, КаталогСборки);
	
	ПротоколироватьОкончаниеЗагрузки(УзелИнформационнойБазы, ИмяАрхива, СтруктураНастроекОбмена);
	
	ИмяФайла = РаспаковатьАрхив(ИмяАрхива, КаталогСборки);
	ИмяФайлаРезультата = ПереместитьФайлВХранилище(ИмяФайла);
	
	УдалитьВременныеФайлыБезОшибок(КаталогСборки);
...
Функция ПоместитьФайлВХранилищеВСервисе(Прокси, Знач ИмяФайла, 
	...
	Архиватор = Новый ЗаписьФайлаАрхива(ИмяНеразделенногоФайла,,,,,УровеньСжатияФайлаАрхива.Максимальный);
...
Процедура ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов(ПараметрыПроцедуры, АдресХранилища) Экспорт
	...
	ПроверенаВозможностьВыполненияОбменов = ПараметрыПроцедуры.Свойство("ПроверенаВозможностьВыполненияОбменов")
		И ПараметрыПроцедуры["ПроверенаВозможностьВыполненияОбменов"];
	...
	ПараметрыОбменаДанными.ПроверенаВозможностьВыполненияОбменов = ПроверенаВозможностьВыполненияОбменов;
	...
	Если ИспользоватьСжатие Тогда
		...
		Архиватор = Новый ЗаписьФайлаАрхива(ИмяФайлаДляПомещенияВХранилище,,,,,УровеньСжатияФайлаАрхива.Максимальный);
...
// Создает временный каталог для сборки файла из частей в каталоге временного хранилища файлов
//
// Возвращаемое значение:
...
// Протоколирует начало загрузки файла из сервиса передачи файлов в журнал регистрации
//
// Параметры:
//   УзелИнформационнойБазы - ПланОбменаСсылка - узел плана обмена, для которого выполняется загрузка.
//                                             Если Неопределено, протоколирование не выполняется
//   КоличествоЧастей - Число - количество частей, на которые разбит файл
//
Процедура ПротоколироватьНачалоЗагрузки(УзелИнформационнойБазы, КоличествоЧастей, СтруктураНастроекОбмена)
...
// Получает все части файла из сервиса передачи файлов и сохраняет их в указанный каталог
//
// Параметры:
//   Прокси                  - WSПрокси              - прокси для работы с сервисом передачи файлов
//   ИдентификаторСессии     - УникальныйИдентификатор - идентификатор сессии получения файла
//   КоличествоЧастей        - Число                 - количество частей файла для получения
//   КаталогСборки           - Строка                - каталог для сохранения полученных частей
//   ИменаФайлов             - Массив из Строка      - массив для добавления имен полученных файлов (заполняется)
//   ОбластьДанных           - Число                 - область данных
//
Процедура ПолучитьЧастиФайла(Прокси, ИдентификаторСессии, КоличествоЧастей, КаталогСборки, ИменаФайлов, ОбластьДанных)
	...
	Для НомерЧасти = 1 По КоличествоЧастей Цикл
		...
		ИмяФайлаЧасти = ПолучитьИмяФайлаЧасти(КаталогСборки, ШаблонИмениФайла, НомерЧасти);
...
// Формирует полное имя файла части по шаблону и номеру части
//
// Параметры:
//   КаталогСборки       - Строка - каталог для размещения файла
//   ШаблонИмениФайла    - Строка - шаблон имени файла с плейсхолдером [n]
//   НомерЧасти          - Число  - номер части файла
//
// Возвращаемое значение:
//   Строка - полное имя файла части
//
Функция ПолучитьИмяФайлаЧасти(КаталогСборки, ШаблонИмениФайла, НомерЧасти)
	...
	Возврат ИмяФайлаЧасти;
	
КонецФункции

// Объединяет части файлов в единый архив в указанном каталоге
//
// Параметры:
//   КаталогСборки - Строка         - каталог, содержащий части файлов
//   ИменаФайлов   - Массив из Строка - массив полных имен файлов частей для объединения
//
// Возвращаемое значение:
//   Строка - полное имя созданного архива
//
Функция СобратьАрхивИзЧастей(КаталогСборки, ИменаФайлов)
...
// Проверяет валидность архива и выбрасывает исключение, если архив пуст
//
// Параметры:
//   ИмяАрхива   - Строка - полное имя проверяемого архива
//   КаталогСборки - Строка - каталог для удаления при ошибке
//
Процедура ПроверитьВалидностьАрхива(ИмяАрхива, КаталогСборки)
...
// Протоколирует окончание загрузки файла из сервиса передачи файлов в журнал регистрации
//
// Параметры:
//   УзелИнформационнойБазы - ПланОбменаСсылка - узел плана обмена, для которого выполняется загрузка.
//                                             Если Неопределено, протоколирование не выполняется
//   ИмяАрхива            - Строка             - полное имя архива для определения размера
//
Процедура ПротоколироватьОкончаниеЗагрузки(УзелИнформационнойБазы, ИмяАрхива, СтруктураНастроекОбмена)
...
// Распаковывает архив и возвращает имя первого извлеченного файла
//
// Параметры:
//   ИмяАрхива   - Строка - полное имя архива для распаковки
//   КаталогСборки - Строка - каталог для распаковки
//
// Возвращаемое значение:
//   Строка - полное имя распакованного файла
//
Функция РаспаковатьАрхив(ИмяАрхива, КаталогСборки)
...
// Перемещает файл в каталог временного хранилища файлов и возвращает новое имя
//
// Параметры:
//   ИмяФайла - Строка - полное имя исходного файла для перемещения
//
// Возвращаемое значение:
//   Строка - полное имя файла в временном хранилище
//
Функция ПереместитьФайлВХранилище(ИмяФайла)
...
// Безопасно удаляет временные файлы и каталоги с обработкой ошибок
//
// Параметры:
//   Каталог - Строка - полный путь к удаляемому каталогу
//
Процедура УдалитьВременныеФайлыБезОшибок(Каталог)
