...
Функция ПараметрыПоиска() Экспорт 
	...
	Параметры.Вставить("ВидОтображения", ВидОтображенияПолнотекстовогоПоиска.HTMLТекст);
...
Функция ВыполнитьПолнотекстовыйПоиск(ПараметрыПоиска) Экспорт
	...
	ВидОтображения = ПараметрыПоиска.ВидОтображения;
	...
	Если ПустаяСтрока(КодОшибки) Тогда 
		РезультатыПоиска = РезультатыПолнотекстовогоПоиска(СписокПоиска, ВидОтображения);
	Иначе 
...
Функция РезультатыПолнотекстовогоПоиска(СписокПоиска, ВидОтображения)
	
	Результат = Новый Массив;
	
	Если ВидОтображения = ВидОтображенияПолнотекстовогоПоиска.XML Тогда
		СтрокиПоиска = СтрокиРезультатаПоискаXML(СписокПоиска);
	Иначе
		СтрокиПоиска = СтрокиРезультатаПоискаHTML(СписокПоиска);
	КонецЕсли;
	
	Для Индекс = 0 По СписокПоиска.Количество() - 1 Цикл
		
		Описание		= СтрокиПоиска.Описания.Получить(Индекс);
		Представление	= СтрокиПоиска.Представления.Получить(Индекс);
		...
		СтрокаРезультата.Вставить("Описание",      Описание);
...
Функция СтрокиРезультатаПоискаXML(СписокПоиска)
	...
	НайденныеСлова = Новый Массив;
	ИзвлеченныйТекстМетаданные = "";
	ИзвлеченныйТекстОписания = "";
	ИзвлеченныйТекстПредставления = "";
	ЗаписьТекстаМетаданных = Ложь;
	ЗаписьТекстаПредставления = Ложь;
	ЗаписьТекстаОписания = Ложь;
	ЗаписьНайденныхСлов = Ложь;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.Имя = "item" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Для Каждого Слово Из НайденныеСлова Цикл
				ИзвлеченныйТекстОписания = СтрЗаменить(ИзвлеченныйТекстОписания, Слово,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<span style=""color: %1"">%2</span>'"), "ПолнотекстовыйПоискЦветИскомогоТекста", Слово));
			КонецЦикла;
			СтрокиОписания.Добавить(Отступ + ИзвлеченныйТекстОписания);
			СтрокиПредставления.Добавить(ИзвлеченныйТекстМетаданные + ": " + ИзвлеченныйТекстПредставления);
			ИзвлеченныйТекстМетаданные = "";
			ИзвлеченныйТекстОписания = "";
			ИзвлеченныйТекстПредставления = "";
			НайденныеСлова = Новый Массив;
		...
		Если ЧтениеXML.Имя = "metadata" Тогда
			ЗаписьТекстаМетаданных = ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента;
		КонецЕсли;
		Если ЧтениеXML.Имя = "presentation" Тогда
			ЗаписьТекстаПредставления = ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента;
		КонецЕсли;
		Если ЧтениеXML.Имя = "textPortion" Тогда
			ЗаписьТекстаОписания = ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента;
			// Если это не первый тег foundWord в узле, то добавляем перенос строки.
			Если ЗначениеЗаполнено(ИзвлеченныйТекстОписания) И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				ИзвлеченныйТекстОписания = ИзвлеченныйТекстОписания + Символы.ПС + Отступ;
			КонецЕсли;
		КонецЕсли;
		Если ЧтениеXML.Имя = "foundWord" Тогда
			ЗаписьНайденныхСлов = ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента;
		КонецЕсли;
		
		Если ЗаписьТекстаМетаданных Тогда
			ИзвлеченныйТекстМетаданные = ИзвлеченныйТекстМетаданные + ЧтениеXML.Значение;
		ИначеЕсли ЗаписьТекстаПредставления Тогда
			ИзвлеченныйТекстПредставления = ИзвлеченныйТекстПредставления + СтрЗаменить(ЧтениеXML.Значение, Символы.ПС, " ");
		ИначеЕсли ЗаписьТекстаОписания Тогда
			ИзвлеченныйТекстОписания = ИзвлеченныйТекстОписания + СтрЗаменить(ЧтениеXML.Значение, Символы.ПС, " ");
			Если ЗаписьНайденныхСлов Тогда
				НайденноеСлово = СокрЛП(ЧтениеXML.Значение);
				Если ЗначениеЗаполнено(НайденноеСлово) Тогда
					НайденныеСлова.Добавить(НайденноеСлово);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Продолжить;
	...
	Результат = Новый Структура;
	Результат.Вставить("Описания", СтрокиОписания);
...
Функция СтрокиРезультатаПоискаHTML(СписокПоиска)
	...
	Результат = Новый Структура;
	Результат.Вставить("Описания", СтрокиОписанияHTML);
	Результат.Вставить("Представления", СтрокиПредставления);
