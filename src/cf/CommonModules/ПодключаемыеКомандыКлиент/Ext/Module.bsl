...
//  Строка  - ЛюбаяСсылка, Число - используется для вызова из обработчика события ПриНаведении
//
Процедура НачатьОбновлениеКоманд(Форма, Элемент = Неопределено, Строка = Неопределено) Экспорт
	
	Если Элемент <> Неопределено Тогда
		Форма.ПараметрыПодключаемыхКоманд.ИмяТекущегоИсточника = Элемент.Имя;
	КонецЕсли;
	
	Если Строка <> Неопределено Тогда
		Форма.ПараметрыПодключаемыхКоманд.НаведеннаяСтрока = Строка;
	КонецЕсли;
	
	Форма.ОтключитьОбработчикОжидания("Подключаемый_ОбновитьКоманды");
	Форма.ПодключитьОбработчикОжидания("Подключаемый_ОбновитьКоманды", 0.1, Истина);
КонецПроцедуры
...
Процедура ПродолжитьВыполнениеКоманды(ПараметрыВыполнения)
	...
	ИначеЕсли ПараметрыВыполнения.МассивСсылок.Количество() = 0 Тогда
		ПустыеСсылки = ПараметрыВыполнения.Форма.ПараметрыПодключаемыхКоманд.ПустыеСсылкиВладельцевКоманд;
		Если ПустыеСсылки.Количество() > 0 Тогда
			ПараметрКоманды = ?(Источник <> Неопределено,
				ПустыеСсылки.Получить(Источник.Имя), Неопределено);
		Иначе
...
Функция ВыбранныеОбъекты(Источник, ОписаниеКоманды)
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("Массив") Тогда
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли Не ОписаниеКоманды.МножественныйВыбор 
		Или (Источник.ПолучитьСтрокиДляОбработки().Количество() = 0 И Источник.ТекущаяСтрока <> Неопределено) Тогда
		...
		ИначеЕсли ОписаниеКоманды.ИспользованиеТекущейСтроки = "НеИспользует" И Источник.ТекущиеДанные = Неопределено Тогда
			Возврат Результат;
		Иначе
			ДобавитьСсылкуВСписок(Источник.ТекущиеДанные, Результат, ДопустимыеТипы);
		КонецЕсли;
	Иначе
		Для Каждого Идентификатор Из Источник.ПолучитьСтрокиДляОбработки() Цикл
