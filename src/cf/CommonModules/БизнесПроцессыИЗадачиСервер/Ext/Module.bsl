...
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект, 
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Задача выполнена %1 пользователем %2.'"),
		...
		ПараметрыСообщения.Идентификатор        = "ЗадачаВыполнена";
		ПараметрыСообщения.Скрываемый           = Ложь;
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ФормаЗадачи, ПараметрыСообщения, Родитель);
	...
	Если ЭтоВедущаяЗадача(ЗадачаОбъект.Ссылка) Тогда
		...
		КонецЕсли;
		Родитель = ?(ЭлементГруппаСостояние <> Неопределено, ЭлементГруппаСостояние, ФормаЗадачи);
		ТекстСообщения = НСтр("ru = 'Это ведущая задача для вложенных бизнес-процессов. Она будет выполнена автоматически при их завершении.'");
		
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.ТипСообщения         = "Информация";
		ПараметрыСообщения.Сообщение            = ТекстСообщения;
		ПараметрыСообщения.Идентификатор        = "ВедущаяЗадача";
		ПараметрыСообщения.Скрываемый           = Ложь;
	...
	ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ФормаЗадачи);
...
Процедура УстановитьОформлениеЗадач(Знач СписокЗадачИлиЕгоУсловноеОформление) Экспорт
	...
	ЭлементЦветаОформления.Значение = ЦветаПалитры.Красный;
