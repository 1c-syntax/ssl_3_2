...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Если Настройки.КоличествоПроблем = 0 Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(Форма, "ПроблемыПроверкиУчета");
		Возврат;
	КонецЕсли;
	...
	Если Настройки.ВажностьПроблемы = Перечисления.ВажностьПроблемыУчета.Ошибка Тогда
		ПараметрыВажногоСообщения.ТипСообщения = "Ошибка";
	ИначеЕсли Настройки.ВажностьПроблемы = Перечисления.ВажностьПроблемыУчета.Предупреждение
		Или Настройки.ВажностьПроблемы = Неопределено Тогда
		ПараметрыВажногоСообщения.ТипСообщения = "Предупреждение";
	Иначе
		ПараметрыВажногоСообщения.ТипСообщения = "Информация";
	КонецЕсли;
	ПараметрыВажногоСообщения.Идентификатор = "ПроблемыПроверкиУчета";
	ПараметрыВажногоСообщения.Сообщение = КонтрольВеденияУчетаСлужебный.СформироватьОбщуюСтрокуИндикатор(Форма, СсылкаНаОбъект, Настройки);
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Проблемы проверки учета'");
	ПараметрыВажногоСообщения.Обработчик = "КонтрольВеденияУчетаСлужебныйКлиент.ОбработатьДействияВИнформационномСообщении";
	
	ОбщегоНазначения.ДобавитьВажноеСообщение(Форма, ПараметрыВажногоСообщения);
