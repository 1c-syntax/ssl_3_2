...
Процедура УстановитьНачальныеНастройки(Знач ИмяПользователя, ЭтоВнешнийПользователь = Ложь) Экспорт
	...
	Если Метаданные.РежимСовместимостиИнтерфейса = Метаданные.СвойстваОбъектов.РежимСовместимостиИнтерфейса.Версия8_5
		Или Метаданные.РежимСовместимостиИнтерфейса = Метаданные.СвойстваОбъектов.РежимСовместимостиИнтерфейса.Версия8_5РазрешитьТакси Тогда
		НастройкиКлиента.ВариантИнтерфейсаКлиентскогоПриложения = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5;
	Иначе
...
//                               которые должны отображаться в ранее выбранных
//                               для возможности их удобного удаления из выбранных.
//                           - Неопределено - без отображения ранее выбранных и соответственно,
//                               без отображения добавленных, удаленных и неизмененных.
...
//            *** РолиВключить                - КнопкаФормы
//            *** РолиИсключить               - КнопкаФормы
//            *** РолиПоказатьТолькоВыбранные - КнопкаФормы
//            *** РолиПоказатьПодсистемы      - КнопкаФормы
//
...
Процедура ОбновитьНазначениеПриСозданииНаСервере(Форма, ДобавлятьПользователей = Истина, ТолькоВнешниеПользователи = Ложь) Экспорт
	...
	Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей()
	   И (    Назначение.Количество() = 1
		...
		Форма.Элементы.ВидПользователей.Видимость = Ложь;
	КонецЕсли;
	...
	Если Назначение.Количество() <> 0 Тогда
		...
		Пока Индекс >= 0 Цикл
			...
			Иначе
				Форма.ВидПользователей.Добавить(ТипПользователей, ТипПользователей.Метаданные().Синоним);
			КонецЕсли;
			Индекс = Индекс - 1;
		КонецЦикла;
	КонецЕсли;
...
Функция ЦветТекстаНедействительногоПользователя() Экспорт
	Возврат Метаданные.ЭлементыСтиля.ТекстЗапрещеннойЯчейкиЦвет.Значение;
КонецФункции
...
// Возвращаемое значение:
//  Структура:
//   * ТипСообщения - Строка - возможные значения: Ошибка, Предупреждение, Информация.
//   * Заголовок    - Строка, ФорматированнаяСтрока
//   * Сообщение    - Строка, ФорматированнаяСтрока
//   * Группа       - ГруппаФормы
//                  - Неопределено
//
Функция ПараметрыСообщения() Экспорт
	...
	Параметры.Вставить("Группа",      Неопределено);
	
	Возврат Параметры;
	
КонецФункции

// Выводит сообщение на форму-сообщение.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ПараметрыСообщения - см. ПараметрыСообщения
//
Процедура ВывестиСообщениеНаФорму(Форма, ПараметрыСообщения) Экспорт
	
	ОсновнаяГруппа = Форма.Элементы.Добавить("БФ_Сообщение", Тип("ГруппаФормы"), ПараметрыСообщения.Группа);
	ОсновнаяГруппа.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
	ОсновнаяГруппа.ОтображатьЗаголовок = Ложь;
	ОсновнаяГруппа.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	
	ДекорацияКартинка  = Форма.Элементы.Добавить(ОсновнаяГруппа.Имя + "_" + "Картинка",
		Тип("ДекорацияФормы"), ОсновнаяГруппа);
	ДекорацияКартинка.Вид      = ВидДекорацииФормы.Картинка;
	ДекорацияКартинка.Картинка = КартинкиСообщений()[ПараметрыСообщения.ТипСообщения];
	
	ГруппаНадписи = Форма.Элементы.Добавить(ОсновнаяГруппа.Имя + "_" + "Надписи",
		Тип("ГруппаФормы"), ОсновнаяГруппа);
	ГруппаНадписи.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаНадписи.ОтображатьЗаголовок = Ложь;
	ГруппаНадписи.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	Если ЗначениеЗаполнено(ПараметрыСообщения.Заголовок) Тогда
		ДекорацияЗаголовок = Форма.Элементы.Добавить(ГруппаНадписи.Имя + "_" + "Заголовок",
			Тип("ДекорацияФормы"), ГруппаНадписи);
		...
		ДекорацияЗаголовок.Шрифт = ?(ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85(),
			ШрифтыСтиля.ЗаголовокВторогоУровня, ШрифтыСтиля.ТекстВторогоУровня);
		ДекорацияЗаголовок.УстановитьДействие("ОбработкаНавигационнойСсылки",
			"Подключаемый_ОбработатьДействиеСообщения");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыСообщения.Сообщение) Тогда
		ДекорацияСообщение = Форма.Элементы.Добавить(ГруппаНадписи.Имя + "_" + "Сообщение",
			Тип("ДекорацияФормы"), ГруппаНадписи);
		ДекорацияСообщение.Вид                    = ВидДекорацииФормы.Надпись;
		ДекорацияСообщение.Заголовок              = ПараметрыСообщения.Сообщение;
		ДекорацияСообщение.АвтоМаксимальнаяШирина = Ложь;
		ДекорацияСообщение.УстановитьДействие("ОбработкаНавигационнойСсылки",
			"Подключаемый_ОбработатьДействиеСообщения");
	КонецЕсли;
...
Процедура ПриОпределенииОбъектовСКомандойГрупповогоИзмененияОбъектов(Объекты) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ГрупповоеИзменениеОбъектов")
...
Процедура ПриОпределенииОбъектовСРедактируемымиРеквизитами(Объекты) Экспорт
	...
	Объекты.Вставить(Метаданные.Справочники.ВнешниеПользователи.ПолноеИмя(),
		"РеквизитыНеРедактируемыеВГрупповойОбработке");
	
	Объекты.Вставить(Метаданные.Справочники.ГруппыВнешнихПользователей.ПолноеИмя(),
		"РеквизитыНеРедактируемыеВГрупповойОбработке");
	
	Объекты.Вставить(Метаданные.Справочники.Пользователи.ПолноеИмя(),
		"РеквизитыНеРедактируемыеВГрупповойОбработке");
...
Процедура ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка) Экспорт
	...
	Отчеты.СведенияОПользователях.ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка);
...
Процедура ЗаполнитьОбщиеНастройкиИзОбщейПолитикиПаролей(Настройки)
	...
	Настройки.СрокБездействияДоЗавершенияСеанса =
		Окр(ПолучитьВремяЗавершенияСеансаПриБездействии() / 60);
	
	Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии =
		Окр(ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии() / 60);
	...
	НастройкиПроверки =
		ДополнительныеНастройкиАутентификации.ПолучитьНастройкиПроверкиРаскрытияПароля();
...
Процедура ОбновитьОбщуюПолитикуПаролей(Настройки) Экспорт
	...
	Если ПолучитьВремяЗавершенияСеансаПриБездействии()
	  <> Настройки.СрокБездействияДоЗавершенияСеанса * 60
	   И ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии()
	  <> Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии * 60 Тогда
		
		УстановитьВремяПредупрежденияОЗавершенииСеансаПриБездействии(0);
	КонецЕсли;
	
	Если ПолучитьВремяЗавершенияСеансаПриБездействии()
	  <> Настройки.СрокБездействияДоЗавершенияСеанса * 60 Тогда
		
		УстановитьВремяЗавершенияСеансаПриБездействии(
			Настройки.СрокБездействияДоЗавершенияСеанса * 60);
	КонецЕсли;
	
	Если ПолучитьВремяПредупрежденияОЗавершенииСеансаПриБездействии()
	  <> Настройки.ПредупреждатьОЗавершенииСеансаПриБездействии * 60 Тогда
	...
	Если Записать Тогда
		ДополнительныеНастройкиАутентификации.УстановитьНастройкиАвтоматическогоСохраненияАутентификации(
			НастройкиСохранения);
	КонецЕсли;
	...
	Если Записать Тогда
		ДополнительныеНастройкиАутентификации.УстановитьНастройкиПроверкиРаскрытияПароля(
			НастройкиПроверки);
	КонецЕсли;
...
Процедура ОбновитьПолитикуПаролейПользователей(Настройки) Экспорт
	...
	Если ПолучитьПроверкуРаскрытияПаролейПользователей()
	...
	Если ПолучитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации()
	  <> НовоеДействие Тогда
		
		УстановитьДействиеПриНесоответствииПаролейПользователейТребованиямПриАутентификации(НовоеДействие);
	КонецЕсли;
...
Функция ТекущееДействиеПриВходеЕслиТребованиеНеВыполнено(ПолитикаПаролей = Неопределено)
	...
	Если ТекущееДействие = ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПотребоватьСменуПароля Тогда
		Возврат "ПотребоватьСменуПароля";
		
	ИначеЕсли ТекущееДействие = ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПредложитьСменуПароля Тогда
...
Функция ЗначениеДействияПриВходеЕслиТребованиеНеВыполнено(ИмяДействия)
	...
	Если ИмяДействия = "ПотребоватьСменуПароля" Тогда
		Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПотребоватьСменуПароля;
		
	ИначеЕсли ИмяДействия = "ПредложитьСменуПароля" Тогда
		Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации.ПредложитьСменуПароля;
	КонецЕсли;
	
	Возврат ДействиеПриНесоответствииПароляТребованиямПриАутентификации.Нет;
...
// Сравнивает старый пароль с ранее сохраненным паролем для пользователя ИБ.
//
// Параметры:
...
// Параметры:
//  ГруппаОбъект - СправочникОбъект.ГруппыПользователей, СправочникОбъект.ГруппыВнешнихПользователей
//  ЭтоНовый - Булево
//  ИзмененияСоставов - см. НовыеИзмененияСоставовГрупп
//
Процедура ПослеЗаписиГруппыПользователей(ГруппаОбъект, ЭтоНовый, ИзмененияСоставов) Экспорт
	
	ПослеОбновленияСоставовГруппПользователей(ИзмененияСоставов);
	
	ИнтеграцияПодсистемБСП.ПослеДобавленияИзмененияПользователяИлиГруппы(ГруппаОбъект.Ссылка, ЭтоНовый);
	
	Если ТипЗнч(ГруппаОбъект) = Тип("СправочникОбъект.ГруппыПользователей")
	   И ОбщегоНазначения.ПодсистемаСуществует(
		...
		МодульУправлениеДоступомСлужебныйВМоделиСервиса.ОтправитьСообщениеИзмененияГруппыПользователей(ГруппаОбъект);
	КонецЕсли;
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		...
		Для Каждого СтрокаНазначения Из НазначениеГруппыВнешнихПользователей Цикл
			ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(СтрокаНазначения.ТипПользователей));
			Если ОбъектМетаданных <> Неопределено Тогда
				МассивПредставлений.Добавить(ОбъектМетаданных.Представление());
			КонецЕсли;
		КонецЦикла;
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					Предмет, НаименованиеГруппы);
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Пользователь ""%1"" не может быть включен в группу ""%2"",
					           |т.к. в состав ее участников входят только %3.'"),
					Предмет, НаименованиеГруппы, ПредставлениеТипаОбъектаАвторизации);
			КонецЕсли;
		Иначе
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				СтрСоединить(МассивНеПеремещенныхПользователей, Символы.ПС));
		КонецЕсли;
...
Процедура ПриСозданииФормыСпискаНаСервере(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		Иначе
			ПараметрыРазмещения.КоманднаяПанель = Форма.КоманднаяПанель;
		КонецЕсли;
		МодульПодключаемыеКоманды.ПриСозданииНаСервере(Форма, ПараметрыРазмещения);
		
		ПараметрыРазмещения = МодульПодключаемыеКоманды.ПараметрыРазмещения();
		ПараметрыРазмещения.ВладелецКоманд = Элементы.Группы;
		ПараметрыРазмещения.ПрефиксГрупп = "Группы";
		ПараметрыРазмещения.КоманднаяПанель = Элементы.Группы.КоманднаяПанель;
		МодульПодключаемыеКоманды.ПриСозданииНаСервере(Форма, ПараметрыРазмещения);
		
		ЗаполнитьСкрытыеЭлементы(Форма);
	КонецЕсли;
	// Конец СтандартныеПодсистемы.ПодключаемыеКоманды
	
	// Начальное значение настройки до загрузки данных из настроек.
	Форма.ПоказыватьПользователейНижестоящихГрупп = Истина;
	
	Если ХранимыеПараметры.РежимВыбора Тогда
		СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(Форма, "ВыборПодбор");
		Форма.РежимБлокировкиПриОткрытииОкна = РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца;
	...
	ПользователиСлужебныйКлиентСервер.ОбновитьЗначениеПараметраКомпоновкиДанных(Группы,
		"Выбранные", Новый Массив);
	ПользователиСлужебныйКлиентСервер.ОбновитьЗначениеПараметраКомпоновкиДанных(Список,
		"Выбранные", Новый Массив);
	
	ОформитьИСкрытьНедействительныхПользователей(Список);
	НастроитьСписокПользователейДляКомандыУстановитьПароль(Список);
	
	РазделениеВключено = ОбщегоНазначения.РазделениеВключено();
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		Элементы.СписокПользователиИнформационнойБазы.Видимость = Ложь;
	КонецЕсли;
	
	Если Не ХранимыеПараметры.РежимВыбора Тогда
		Элементы.Группы.РежимВыбора = Ложь;
		Элементы.Список.РежимВыбора = Ложь;
	КонецЕсли;
	
	Элементы.ФормаВставитьИзБуфераОбмена.Видимость = ХранимыеПараметры.РасширенныйПодбор;
	Элементы.ПоказыватьЭлементыСписков.Видимость = ХранимыеПараметры.РасширенныйПодбор;
	...
	Если ХранимыеПараметры.РежимВыбора Тогда
		...
		Если ТипЗнч(Параметры.СкрываемыеПользователи) = Тип("СписокЗначений") Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,
				"Ссылка", Параметры.СкрываемыеПользователи, ВидСравненияКомпоновкиДанных.НеВСписке);
		КонецЕсли;
		
		// Отключение перетаскивания пользователя в формах выбора и подбора пользователей.
		Элементы.Группы.РазрешитьПеретаскивание = Ложь;
		Элементы.Список.РазрешитьНачалоПеретаскивания = Ложь;
		...
		Если Не ХранимыеПараметры.ЗакрыватьПриВыборе Тогда
			// Режим подбора.
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85()
			 Или ХранимыеПараметры.РасширенныйПодбор Тогда
				Элементы.ФормаВыбратьИЗакрыть.Видимость = Истина;
				Элементы.ФормаВыбратьИЗакрыть.Важность = ВажностьКнопки.Главная;
			КонецЕсли;
			
			Элементы.Список.МножественныйВыбор = Истина;
			Если ХранимыеПараметры.ВыборГрупп Тогда
				Элементы.Группы.МножественныйВыбор = Истина;
			КонецЕсли;
		ИначеЕсли Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			Элементы.Группы.РежимВыделения = РежимВыделенияТаблицы.Одиночный;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СписокСоздать", "Видимость", Истина);
		
		Если ХранимыеПараметры.РасширенныйПодбор Тогда
			СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(Форма, "РасширенныйПодбор");
			Элементы.Список.ОтображениеГиперссылокЯчеек = ОтображениеГиперссылокЯчеек.НеОтображать;
			Элементы.Группы.РежимВыбора = Ложь;
			Элементы.Список.РежимВыбора = Ложь;
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				ВыборГрупп = ХранимыеПараметры.ВыборГрупп И ХранимыеПараметры.ИспользоватьГруппы;
				Если ВыборГрупп
				 Или Не ХранимыеПараметры.ЗакрыватьПриВыборе Тогда
					
					Элементы.ФормаВыбратьВыделенные.Видимость = Истина;
					Если ХранимыеПараметры.ЗакрыватьПриВыборе Тогда
						Элементы.ФормаВыбратьВыделенные.Важность = ВажностьКнопки.Главная;
						Элементы.ФормаВыбратьВыделенные.РасширеннаяПодсказка.Заголовок =
							НСтр("ru = 'Выбрать значение'");
					Иначе
						Элементы.ФормаВыбратьВыделенные.РасширеннаяПодсказка.Заголовок =
							?(ВыборГрупп,
								НСтр("ru = 'Выбрать выделенные группы и выделенных пользователей'"),
								НСтр("ru = 'Выбрать выделенных пользователей'"));
					КонецЕсли;
				КонецЕсли;
			Иначе
				Элементы.Группы.РежимВыбора = Ложь;
	...
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыВыбратьТакси", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыВключитьВВыбранные", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыИсключитьИзВыбранных", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыСкопировать", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыИзменить", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СписокВыбратьТакси", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СписокВключитьВВыбранные", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СписокИсключитьИзВыбранных", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СписокСкопировать", "ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели);
	КонецЕсли;
	
	ОписаниеОбъекта = Новый Структура;
	ОписаниеОбъекта.Вставить("Ссылка", Справочники.Пользователи.ПустаяСсылка());
	ОписаниеОбъекта.Вставить("ИдентификаторПользователяИБ",
		ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	УровеньДоступа = УровеньДоступаКСвойствамПользователя(ОписаниеОбъекта);
	
	Если Не УровеньДоступа.УправлениеСписком Тогда
		Элементы.СписокУстановитьПароль.Видимость = Ложь;
	КонецЕсли;
	
	Если ОбщегоНазначения.ЭтоАвтономноеРабочееМесто()
	 Или ХранимыеПараметры.СкрытьКомандыПриВыбореПодборе Тогда
		Форма.ТолькоПросмотр = Истина;
		Элементы.Список.ТолькоПросмотр = Истина;
		Элементы.Группы.ТолькоПросмотр = Истина;
	КонецЕсли;
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗагрузкаДанныхИзФайла") Тогда
		Элементы.ФормаВставитьИзБуфераОбмена.Видимость = Ложь;
	КонецЕсли;
	
	ОграничитьИспользованиеЗаполняемогоПоляДинамическогоСписка(Список, "НомерКартинки");
	
	НастроитьКнопкиПриВыбореИПодборе(Форма);
...
//  Список - ДинамическийСписок
//
Процедура ОформитьИСкрытьНедействительныхПользователей(Список)
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста",
		ЦветТекстаНедействительногоПользователя());
...
//  Список - ДинамическийСписок
//
Процедура НастроитьСписокПользователейДляКомандыУстановитьПароль(Список)
	...
	ПользователиСлужебныйКлиентСервер.ОбновитьЗначениеПараметраКомпоновкиДанных(Список,
		"ВозможноСменитьТолькоСвойПароль",
...
//  Форма - ФормаКлиентскогоПриложения
//
Процедура ЗаполнитьСкрытыеЭлементы(Форма)
	
	СкрываемыеЭлементы =
	"ГруппыКонтекстноеМенюСоздатьНаОсновании
	|ГруппыСоздатьНаОсновании
	|СписокКонтекстноеМенюСоздатьНаОсновании
	|СписокСоздатьНаОсновании";
	
	ИменаЭлементов = СтрРазделить(СкрываемыеЭлементы, Символы.ПС);
	СкрытыеЭлементы = Новый Массив;
	...
	Для Каждого ИмяЭлемента Из ИменаЭлементов Цикл
		...
		Если НайденныйЭлемент <> Неопределено
		   И Не НайденныйЭлемент.Видимость Тогда
			СкрытыеЭлементы.Добавить(ИмяЭлемента);
		КонецЕсли;
	КонецЦикла;
	
	Форма.ХранимыеПараметры.СкрытыеЭлементы = СтрСоединить(СкрытыеЭлементы, Символы.ПС);
...
//  Форма - ФормаКлиентскогоПриложения
//
Процедура НастроитьКнопкиПриВыбореИПодборе(Форма)
	
	// При обычном выборе пользователей (без выбора групп) используется стандартная кнопка "Выбрать".
	// При обычном выборе/подборе и пользователей, и групп пользователей
	// - в интерфейсе Такси используются стандартные кнопки выбора, но только в командных панелях списков, так как их две;
	// - в интерфейсе 8.5 используется одна кнопка "Выбрать", так как есть флажки выделения строк,
	//   которые однозначно отделяют выделенные строки от активной строки.
	// Кроме того, при обычном подборе добавляется кнопка "Выбрать и закрыть".
	// При расширенном подборе:
	// - используется нестандартная кнопка "Завершить подбор",
	// - команды текущей строки и выделенных строк переносятся в дополнительное подменю,
	//   кроме своих команд включения/исключения из выбранных.
	
	Элементы          = Форма.Элементы;
	ХранимыеПараметры = Форма.ХранимыеПараметры;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"СписокКонтекстноеМенюВыбрать", "ВажностьПриОтображении", ВажностьПриОтображении.ОченьВысокая);
	
	Если Не ХранимыеПараметры.ВыборГрупп Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыВыбрать", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыКонтекстноеМенюВыбрать", "Видимость", Ложь);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппыКонтекстноеМенюВыбрать", "ВажностьПриОтображении", ВажностьПриОтображении.ОченьВысокая);
	КонецЕсли;
	...
	Если Не ХранимыеПараметры.РасширенныйПодбор Тогда
		...
		Если ВыборГрупп
		 Или Не ХранимыеПараметры.ЗакрыватьПриВыборе Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ФормаВыбрать", "Видимость", Ложь);
				
			ИначеЕсли ВыборГрупп Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ФормаВыбрать", "Видимость", Ложь);
				Элементы.ГруппыВыбратьТакси.Видимость = Истина;
				Элементы.СписокВыбратьТакси.Видимость = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Элемент Из Элементы Цикл
		Если ТипЗнч(Элемент) <> Тип("КнопкаФормы") Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНачинаетсяС(Элемент.Имя, Элементы.Группы.Имя)
		   И Элемент.Имя <> "ГруппыВыбратьТакси"
		   И Элемент.Имя <> "ГруппыВключитьВВыбранные"
		   И Элемент.Имя <> "ГруппыИсключитьИзВыбранных"
		   И Элемент.Имя <> "ГруппыПанельДействийВыделенныхСтрокКнопкаЯрлыка"
		   И Элемент.Имя <> "ГруппыПанельДействийВыделенныхСтрокСнятьВыделение"
		   И Элемент.Имя <> "ГруппыСоздать"
		 Или СтрНачинаетсяС(Элемент.Имя, Элементы.Список.Имя)
		   И Элемент.Имя <> "СписокВыбратьТакси"
		   И Элемент.Имя <> "СписокВключитьВВыбранные"
		   И Элемент.Имя <> "СписокИсключитьИзВыбранных"
		   И Элемент.Имя <> "СписокПоказатьПользователейНижестоящихГрупп"
		   И Элемент.Имя <> "СписокПоказатьНедействительныхПользователей"
		   И Элемент.Имя <> "СписокСоздать"
		   И Элемент.Имя <> "СписокПанельДействийВыделенныхСтрокКнопкаЯрлыка"
		   И Элемент.Имя <> "СписокПанельДействийВыделенныхСтрокСнятьВыделение" Тогда
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ВыбранныеПользователи - Неопределено, Массив - возвращаемое значение.
//
Процедура НастроитьПараметрыРасширеннойФормыПодбора(Форма, ВыбранныеПользователи) Экспорт
	...
	ПараметрыПодбора = НовыеПараметрыРасширеннойФормыПодбора();
	ПараметрыПодбора.ВыбранныеПользователи = Неопределено;
	...
	Если ТипЗнч(ПараметрыПодбора.ВыбранныеПользователи) = Тип("Массив") Тогда
		ВыбранныеПользователи = ПараметрыПодбора.ВыбранныеПользователи;
	Иначе
		Элементы.ПоказыватьЭлементыСписков.СписокВыбора.Удалить(4);
		Элементы.ПоказыватьЭлементыСписков.СписокВыбора.Удалить(3);
		Элементы.ПоказыватьЭлементыСписков.СписокВыбора.Удалить(2);
	КонецЕсли;
	...
	Элементы.ФормаВыбратьИЗакрыть.Доступность = Истина;
	Элементы.ФормаВыбратьИЗакрыть.Заголовок =
		?(ЗначениеЗаполнено(ПараметрыПодбора.ЗаголовокКнопкиЗавершенияПодбора),
			ПараметрыПодбора.ЗаголовокКнопкиЗавершенияПодбора,
			НСтр("ru = 'Завершить подбор'"));
	Элементы.ФормаВыбратьИЗакрыть.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Принять изменение выбранных'");
	...
	Элементы.Группы.РежимВыделения = ?(ХранимыеПараметры.ВыборГрупп,
		РежимВыделенияТаблицы.Множественный, РежимВыделенияТаблицы.Одиночный);
	Элементы.ГруппыВыбран.Видимость = ХранимыеПараметры.ВыборГрупп;
	Элементы.СписокВыбран.Видимость = Истина;
	
	Если Не ХранимыеПараметры.ВыборГрупп Тогда
		Элементы.ГруппыВключитьВВыбранные.РасширеннаяПодсказка.Заголовок =
			НСтр("ru = 'Включить пользователей группы в выбранные'");
		Элементы.ГруппыИсключитьИзВыбранных.РасширеннаяПодсказка.Заголовок =
			НСтр("ru = 'Исключить пользователей группы из выбранных'");
	КонецЕсли;
...
//  Форма - ФормаКлиентскогоПриложения
//
Процедура НастроитьРаботуВТакси(Форма) Экспорт
	
	ХранимыеПараметры = Форма.ХранимыеПараметры;
	Параметры         = Форма.Параметры;
	Элементы          = Форма.Элементы;
	
	// Расширение командной панели списка пользователей.
	Элементы.ЛеваяКолонка.Ширина = 40;
	Элементы.ПраваяКолонка.Ширина = 65;
	
	// Корректировка порядка команд в командной панели групп пользователей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Группы.КоманднаяПанель,
		"ГруппыВыбратьТакси
	...
	// Дополнение команд в контекстном меню групп пользователей.
	СтандартныеПодсистемыСервер.СкопироватьНедостающиеКнопки(Элементы.Группы.КонтекстноеМеню,
		"ГруппыВыбратьТакси*ГруппыКонтекстноеМенюВыбратьТакси
		|ГруппыВключитьВВыбранные*ГруппыКонтекстноеМенюВключитьВВыбранные
		|ГруппыИсключитьИзВыбранных*ГруппыКонтекстноеМенюИсключитьИзВыбранных");
	
	// Корректировка порядка команд в контекстном меню групп пользователей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Группы.КонтекстноеМеню,
		"ГруппыКонтекстноеМенюВыбратьТакси
		|ГруппыКонтекстноеМенюВключитьВВыбранные
		|ГруппыКонтекстноеМенюИсключитьИзВыбранных");
	
	
	// Замена командной панели формы на командную панель списка пользователей.
	Элементы.Список.ОтображатьКоманднуюПанель = Истина;
	
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(Форма.КоманднаяПанель,
		Элементы.Список.КоманднаяПанель,
		"ФормаВыбратьИЗакрыть
		|ФормаВыбратьВыделенные
		|ФормаВставитьИзБуфераОбмена");
	
	Если ХранимыеПараметры.РасширенныйПодбор Тогда
		Элементы.Переместить(Элементы.ФормаВыбратьИЗакрыть,
			Элементы.ГоризонтальнаяГруппаТакси, Элементы.ПоказыватьЭлементыСписков);
		Элементы.ФормаВыбратьИЗакрыть.КнопкаПоУмолчанию = Истина;
		Элементы.Переместить(Элементы.ФормаВставитьИзБуфераОбмена,
			Элементы.ГоризонтальнаяГруппаТакси, Элементы.ПоказыватьЭлементыСписков);
	КонецЕсли;
	
	// Корректировка порядка команд в командной панели списка пользователей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КоманднаяПанель,
		"СписокВыбратьТакси
		|ФормаВыбрать
		...
		|СписокСоздать
		|ФормаСоздать
		|СписокСкопировать:ФормаСкопировать
		|СписокИзменить:ФормаИзменить
		|СписокУстановитьПароль
		|СписокНазначитьГруппы
		|СписокПользователиИнформационнойБазы>.
		|СписокЗаблокироватьНедействительныхПользователейВСистемеВзаимодействий
		|ФормаИзменитьФорму
		|ФормаСправка");
	
	// Корректировка положения команд в командной панели списка пользователей.
	Элементы.СписокУстановитьПароль.ПоложениеВКоманднойПанели =
		ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	
	Элементы.СписокНазначитьГруппы.ПоложениеВКоманднойПанели =
		ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	
	// Дополнение команд в контекстном меню списка пользователей.
	СтандартныеПодсистемыСервер.СкопироватьНедостающиеКнопки(Элементы.Список.КонтекстноеМеню,
		"СписокВключитьВВыбранные*СписокКонтекстноеМенюВключитьВВыбранные
		|СписокИсключитьИзВыбранных*СписокКонтекстноеМенюИсключитьИзВыбранных");
	
	// Корректировка порядка команд в контекстном меню списка пользователей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КонтекстноеМеню,
		"СписокКонтекстноеМенюВключитьВВыбранные
		|СписокКонтекстноеМенюИсключитьИзВыбранных
		|СписокКонтекстноеМенюУстановитьПароль<СписокКонтекстноеМенюИзменить
		|СписокКонтекстноеМенюНазначитьГруппы");
	
	Если ХранимыеПараметры.СкрытьКомандыПриВыбореПодборе Тогда
		ПользователиСлужебныйКлиентСервер.СкрытьКомандыПриВыбореПодборе(Форма);
	КонецЕсли;
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ВыбранныеСсылки - Строка, ЛюбаяСсылка, Массив, Неопределено
//  Добавить - Неопределено, Булево
//  ВыборГруппы - Булево
//
Процедура ОбновитьСписокВыбранных(Форма, ВыбранныеСсылки, Добавить, ВыборГруппы) Экспорт
	
	ХранимыеПараметры = Форма.ХранимыеПараметры;
	Элементы          = Форма.Элементы;
	Выбранные         = Форма.Выбранные;
	
	Если ВыбранныеСсылки = Неопределено Тогда
		Возврат;
		
	ИначеЕсли ТипЗнч(ВыбранныеСсылки) = Тип("Строка") Тогда
		МассивСсылок = Элементы[ВыбранныеСсылки].ПолучитьСтрокиДляОбработки();
		
	ИначеЕсли ТипЗнч(ВыбранныеСсылки) = Тип("Массив") Тогда
		МассивСсылок = ВыбранныеСсылки;
	Иначе
		МассивСсылок = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыбранныеСсылки);
	КонецЕсли;
	
	Если Не ХранимыеПараметры.ВыборГрупп И ВыборГруппы Тогда
		МассивСсылок = ПользователиТекущейГруппы(Элементы.Список);
	КонецЕсли;
	
	ТипГрупп = Новый ОписаниеТипов("СправочникСсылка.ГруппыПользователей,
		|СправочникСсылка.ГруппыВнешнихПользователей");
	
	Для Каждого ВыбраннаяСсылка Из МассивСсылок Цикл
		Отбор = Новый Структура("Пользователь", ВыбраннаяСсылка);
		НайденныеСтроки = Выбранные.НайтиСтроки(Отбор);
		Если Добавить = Ложь И НайденныеСтроки.Количество() > 0 Тогда
			Если НайденныеСтроки[0].Изменение = "+" Тогда
				Выбранные.Удалить(НайденныеСтроки[0]);
			Иначе
				НайденныеСтроки[0].Изменение = "-";
			КонецЕсли;
		ИначеЕсли Добавить <> Ложь И НайденныеСтроки.Количество() = 0 Тогда
			СтрокаТаблицы = Выбранные.Добавить();
			СтрокаТаблицы.Пользователь = ВыбраннаяСсылка;
			СтрокаТаблицы.ЭтоГруппа = ТипГрупп.СодержитТип(ТипЗнч(ВыбраннаяСсылка));
			СтрокаТаблицы.Изменение = ?(Добавить = Истина, "+", "*");
		ИначеЕсли Добавить <> Ложь И НайденныеСтроки.Количество() > 0
		        И НайденныеСтроки[0].Изменение = "-" Тогда
			НайденныеСтроки[0].Изменение = "*";
		КонецЕсли;
	КонецЦикла;
...
// Параметры:
//  Список - ДинамическийСписок
//  Выбранные - ДанныеФормыКоллекция:
//   * Пользователь - ЛюбаяСсылка
//   * Изменение    - Строка
//  ТипСсылки - Тип
//
Процедура ОбновитьВыбранныеСсылкиДляСписка(Список, Выбранные, ТипСсылки) Экспорт
	
	ТекущиеСсылки = Новый Массив;
	Для Каждого Выбранный Из Выбранные Цикл
		Если Выбранный.Изменение = "-"
		 Или ТипЗнч(Выбранный.Пользователь) <> ТипСсылки Тогда
			Продолжить;
		КонецЕсли;
		ТекущиеСсылки.Добавить(Выбранный.Пользователь);
	КонецЦикла;
	
	ПользователиСлужебныйКлиентСервер.ОбновитьЗначениеПараметраКомпоновкиДанных(Список,
		"Выбранные", ТекущиеСсылки);
...
//   * Пользователь - ЛюбаяСсылка
//   * Изменение    - Строка
//  ПоказыватьЭлементыСписков - Строка
//  ТипСсылки - Тип
//
Процедура ОбновитьОтборСсылокДляСписка(Список, ПоказыватьЭлементыСписков, Выбранные, ТипСсылки) Экспорт
	
	Если ПоказыватьЭлементыСписков = "" Тогда
		СсылкиДляПросмотра = Null;
	Иначе
		СсылкиДляПросмотра = Новый Массив;
		Для Каждого Выбранный Из Выбранные Цикл
			Если ПоказыватьЭлементыСписков = "Добавленные"  И Выбранный.Изменение <> "+"
			 Или ПоказыватьЭлементыСписков = "Удаленные"    И Выбранный.Изменение <> "-"
			 Или ПоказыватьЭлементыСписков = "БезИзменений" И Выбранный.Изменение <> "*"
			 Или ПоказыватьЭлементыСписков = "Выбранные"    И Выбранный.Изменение =  "-"
			 Или ТипЗнч(Выбранный.Пользователь) <> ТипСсылки Тогда
				Продолжить;
			КонецЕсли;
			СсылкиДляПросмотра.Добавить(Выбранный.Пользователь);
		КонецЦикла;
	КонецЕсли;
	
	ПользователиСлужебныйКлиентСервер.ОбновитьЗначениеПараметраКомпоновкиДанных(Список,
		"СсылкиДляПросмотра", СсылкиДляПросмотра);
...
//  Форма - ФормаКлиентскогоПриложения
//
Процедура ОбновитьКоличествоВОтбореПриПодборе(Форма) Экспорт
	
	Элементы  = Форма.Элементы;
	Выбранные = Форма.Выбранные;
	
	Добавленные  = Выбранные.НайтиСтроки(Новый Структура("Изменение", "+"));
	Удаленные    = Выбранные.НайтиСтроки(Новый Структура("Изменение", "-"));
	БезИзменений = Выбранные.НайтиСтроки(Новый Структура("Изменение", "*"));
	
	СписокВыбора = Элементы.ПоказыватьЭлементыСписков.СписокВыбора;
	КоличествоВыбранных = Выбранные.Количество() - Удаленные.Количество();
	
	СписокВыбора[1].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выбранные: %1'"), Формат(КоличествоВыбранных, "ЧН=0; ЧГ="));
	
	Если СписокВыбора.Количество() < 3 Тогда
		Возврат;
	КонецЕсли;
	
	СписокВыбора[2].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Добавленные: %1'"), Формат(Добавленные.Количество(), "ЧН=0; ЧГ="));
	
	СписокВыбора[3].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удаленные: %1'"), Формат(Удаленные.Количество(), "ЧН=0; ЧГ="));
	
	СписокВыбора[4].Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Без изменений: %1'"), Формат(БезИзменений.Количество(), "ЧН=0; ЧГ="));
	
КонецПроцедуры

// Для процедуры ОбновитьСписокВыбранных.
Функция ПользователиТекущейГруппы(ПолеСписка)
	...
	Таблица = ПроцессорВывода.Вывести(ПроцессорКомпоновки);
	
	Возврат Таблица.ВыгрузитьКолонку("Ссылка");
	
КонецФункции

// Для функции ПользователиТекущейГруппы.
Процедура ДобавитьПоляПользователейГруппы(Настройки)
	
	ДобавляемыеПоля = СтрРазделить("Ссылка", ",", Ложь);
...
//  Список - ДинамическийСписок
//  ИмяПоля - Строка
//  Позиция - Число - если больше -1, тогда сдвинуть элемент в указанную позицию.
//  Направление - Строка - два значения "Возр", "Убыв".
//
Процедура УстановитьЭлементУпорядочения(Список, ИмяПоля, Позиция = -1, Направление = "Возр") Экспорт
	...
	Если ЭлементПорядка.Поле <> Поле Тогда
		ЭлементПорядка = ЭлементНастройки.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
	...
	Если Позиция >= 0 Тогда
		ЭлементНастройки.Элементы.Сдвинуть(ЭлементПорядка,
			Позиция - ЭлементНастройки.Элементы.Индекс(ЭлементПорядка));
	КонецЕсли;
	
КонецПроцедуры

// Для формы элемента справочников Пользователи и ВнешниеПользователи.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения:
//   * Объект - ДанныеФормыСтруктура
//   * Элементы - ВсеЭлементыФормы:
//      ** ГруппаВажныеСообщения - ГруппаФормы
//   * ПользовательИБСуществует - Булево
//   * ЕстьСопоставлениеСНесуществующимПользователемИБ - Булево
//   * УровеньДоступа - см. УровеньДоступаКСвойствамПользователя
//   * ДействияВФорме - Структура:
//      ** СвойстваЭлемента - Строка
//
//  ПараметрыЗаписи - Структура
//
Процедура ЗаполнитьНесоответствиеПользователяСНесуществующимПользователемИБ(Форма, ПараметрыЗаписи) Экспорт
	
	Объект   = Форма.Объект;
	Элементы = Форма.Элементы;
	
	ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(Форма, "УчетнаяЗаписьНеСуществует");
	...
	ЕстьНовоеСопоставлениеСНесуществующимПользователемИБ =
		Не Форма.ПользовательИБСуществует
		И ЗначениеЗаполнено(Объект.ИдентификаторПользователяИБ);
	
	Если ПараметрыЗаписи <> Неопределено
	   И Форма.ЕстьСопоставлениеСНесуществующимПользователемИБ
	...
	КонецЕсли;
	Форма.ЕстьСопоставлениеСНесуществующимПользователемИБ = ЕстьНовоеСопоставлениеСНесуществующимПользователемИБ;
	
	Если Не Форма.УровеньДоступа.УправлениеСписком
	 Или Не Форма.ЕстьСопоставлениеСНесуществующимПользователемИБ Тогда
		Возврат;
	КонецЕсли;
	
	Если Форма.ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru = 'Нажмите <b>Записать</b>, чтобы устранить проблему и не выводить это предупреждение.'");
	...
	ПараметрыСообщения.Идентификатор = "УчетнаяЗаписьНеСуществует";
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Учетная запись не существует'");
	...
	ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Некорректная ссылка на пользователя информационной базы. <a href=%1>Подробнее</a>'") + Рекомендация,
		"ПоказатьПояснениеКогдаНекорректнаяСсылкаНаПользователяИБ");
	ПараметрыСообщения.Обработчик = "Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки";
	ОбщегоНазначения.ДобавитьВажноеСообщение(Форма, ПараметрыСообщения);
	ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(Форма);
	
КонецПроцедуры

// Для формы элемента справочников Пользователи и ВнешниеПользователи.
// Возвращает начальные значения аутентификации при создании пользователя или копировании.
//
// Параметры:
//  ДляСервиса - Булево - вариант, когда разделение включено.
//
// Возвращаемое значение:
//  Структура:
//   * ПользовательИБАутентификацияСтандартная    - Булево
//   * ПользовательИБАутентификацияOpenID         - Булево
//   * ПользовательИБАутентификацияOpenIDConnect  - Булево
//   * ПользовательИБАутентификацияТокеномДоступа - Булево
//   * ПользовательИБАутентификацияОС             - Булево
//
Функция АутентификацияДляНовогоПользователя(ДляСервиса = Ложь) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПользовательИБАутентификацияСтандартная",    Истина);
	Результат.Вставить("ПользовательИБАутентификацияOpenID",         ДляСервиса);
	Результат.Вставить("ПользовательИБАутентификацияOpenIDConnect",  ДляСервиса);
	Результат.Вставить("ПользовательИБАутентификацияТокеномДоступа", ДляСервиса);
	Результат.Вставить("ПользовательИБАутентификацияОС",             Ложь);
...
//   * Значение - Картинка
//
Функция КартинкиСообщений()
	
	Соответствие = Новый Соответствие;
	Соответствие.Вставить("Информация",     БиблиотекаКартинок.ДиалогИнформация);
	Соответствие.Вставить("Предупреждение", БиблиотекаКартинок.ДиалогВосклицание);
	Соответствие.Вставить("Ошибка",         БиблиотекаКартинок.ДиалогСтоп);
...
Процедура НастроитьИнтерфейсРолейПриСозданииФормы(Параметры)
	...
	НайденныйЭлемент = Форма.Элементы.Найти("РолиВключить"); // КнопкаФормы
	Если НайденныйЭлемент <> Неопределено Тогда
		НайденныйЭлемент.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Включить выделенные роли'");
		НайденныйЭлемент.ПоложениеВКоманднойПанели = Положение;
	КонецЕсли;
	
	НайденныйЭлемент = Форма.Элементы.Найти("РолиИсключить"); // КнопкаФормы
	Если НайденныйЭлемент <> Неопределено Тогда
		НайденныйЭлемент.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Исключить выделенные роли'");
		НайденныйЭлемент.ПоложениеВКоманднойПанели = Положение;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы,
		"РолиПоказатьТолькоВыбранные", "ПоложениеВКоманднойПанели", Положение);
	
	// Корректировка порядка команд в командной панели ролей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Форма.Элементы.Роли.КоманднаяПанель,
		"РолиВключить
		|РолиИсключить
		|РолиПоказатьТолькоВыбранные");
	
	// Дополнение команд в контекстном меню ролей.
	СтандартныеПодсистемыСервер.СкопироватьНедостающиеКнопки(Форма.Элементы.Роли.КонтекстноеМеню,
		"РолиВключить*РолиКонтекстноеМенюВключить
		|РолиИсключить*РолиКонтекстноеМенюИсключить
		|РолиПоказатьТолькоВыбранные*РолиКонтекстноеМенюПоказатьТолькоВыбранные");
	
	// Корректировка порядка команд в контекстном меню ролей.
	СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Форма.Элементы.Роли.КонтекстноеМеню,
		"РолиКонтекстноеМенюВключить
		|РолиКонтекстноеМенюИсключить
		|РолиКонтекстноеМенюПоказатьТолькоВыбранные");
...
Процедура НастроитьИнтерфейсРолейПриЧтенииНаСервере(Параметры)
	...
	Элементы.РолиПоказатьПодсистемы.Пометка = Ложь;
	
	// Для нового элемента показать все роли, для существующего - только выбранные роли.
	Если Элементы.Найти("РолиПоказатьТолькоВыбранные") <> Неопределено Тогда
		Элементы.РолиПоказатьТолькоВыбранные.Пометка = Параметры.ОсновнойПараметр;
	КонецЕсли;
...
Процедура НастроитьИнтерфейсРолейПриЗагрузкеНастроек(Параметры)
	...
	Иначе
		...
		Элементы.РолиПоказатьПодсистемы.Пометка = Истина;
	КонецЕсли;
...
Процедура УстановитьТолькоПросмотрРолей(Параметры)
	...
	Если ТолькоПросмотрРолей <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"РолиВключить", "Доступность", Не ТолькоПросмотрРолей);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"РолиИсключить", "Доступность", Не ТолькоПросмотрРолей);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"РолиКонтекстноеМенюВключить", "Доступность", Не ТолькоПросмотрРолей);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"РолиКонтекстноеМенюИсключить", "Доступность", Не ТолькоПросмотрРолей);
	КонецЕсли;
...
Процедура ТолькоВыбранныеРоли(Параметры)
	...
	Элементы.РолиПоказатьТолькоВыбранные.Пометка =
		Не Элементы.РолиПоказатьТолькоВыбранные.Пометка;
...
Процедура ГруппировкаПоПодсистемам(Параметры)
	...
	Форма.ПоказатьПодсистемыРолей = Не Форма.ПоказатьПодсистемыРолей;
	Форма.Элементы.РолиПоказатьПодсистемы.Пометка = Форма.ПоказатьПодсистемыРолей;
...
Процедура ОбновитьДеревоРолей(Параметры)
	...
	Если Элементы.Найти("РолиПоказатьТолькоВыбранные") <> Неопределено Тогда
		Если НЕ Элементы.РолиПоказатьТолькоВыбранные.Доступность Тогда
			Элементы.РолиПоказатьТолькоВыбранные.Пометка = Истина;
		КонецЕсли;
		ПоказатьТолькоВыбранныеРоли = Элементы.РолиПоказатьТолькоВыбранные.Пометка;
	Иначе
...
Процедура ОбновитьСоставРолей(Параметры)
	...
	Роли                        = Форма.Роли;
	ПоказатьТолькоВыбранныеРоли = Форма.Элементы.РолиПоказатьТолькоВыбранные.Пометка;
	НазначениеРолей             = Параметры.НазначениеРолей;
