...
//     * ВерсияПодсистемыНаНачалоОбновления - Строка - номер сборки подсистемы (к которой относится обработчик)
//          на момент начала текущего этапа обновления.
//     * ПараметрыВыборки - см. ДополнительныеПараметрыВыборкиДанныхДляМногопоточнойОбработки
//     * АктуальныеДанные - см. ПараметрыВыборкиАктуальныхДанных
//
//     // Служебные свойства, для внутреннего использования.
//     * ЗарегистрированныеТаблицыРегистраторов - Массив - список таблиц регистратора, по которым
//          были зарегистрированы данные на плане обмена
//     * ПовторнаяРегистрация - Булево - признак выполнения повторной регистрации изменений при
//          получении сообщения обмена из подчиненных узлов.
//     * ИмяФайлаСИзменениями - Строка - полное имя файла, в который записываются изменения,
//          зарегистрированные в текущем обработчике.
...
Функция ОбработчикиОбновления(Отбор = Неопределено) Экспорт
	...
	ИначеЕсли ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ОбластиДанных = МодульРаботаВМоделиСервиса.ИспользуемыеОбластиДанных();
	Иначе
...
Функция ПрогрессОбновленияОбластейДанных(РежимОбновления) Экспорт
	...
	МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
	ИспользуемыеОбластиДанных = МодульРаботаВМоделиСервиса.ИспользуемыеОбластиДанных();
	ТаблицаИспользуемыеОбласти = Новый ТаблицаЗначений;
	ТаблицаИспользуемыеОбласти.Колонки.Добавить("ОбластьДанных", Новый ОписаниеТипов("Число"));
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(ТаблицаИспользуемыеОбласти, ИспользуемыеОбластиДанных, "ОбластьДанных");
	Запрос.УстановитьПараметр("ИспользуемыеОбласти", ТаблицаИспользуемыеОбласти);
