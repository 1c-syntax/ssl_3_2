...
Функция ОбрабатыватьОповещения(Форма, ИмяСобытия, Параметр) Экспорт
	...
	ИначеЕсли ИмяСобытия = "Запись_ДополнительныеРеквизитыИСведения" Тогда
		...
		Иначе
			...
			Если Форма.Свойства_ОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор).Количество() > 0
				Или Форма.Свойства_УстановленныеМетки.НайтиСтроки(Отбор) <> Неопределено Тогда
				Возврат Истина;
			Иначе
				Возврат Ложь;
			КонецЕсли;
	...
	ИначеЕсли ИмяСобытия = "Запись_ИзменениеМеток" И Форма = Параметр.Владелец Тогда
		ИзменитьСоставМеток(Форма, Параметр.УстановленныеМетки);
...
Процедура ВыполнитьКоманду(Форма,
	...
	ИначеЕсли ТипЗнч(Элемент) = Тип("ДекорацияФормы") Тогда
		ИмяКоманды = Элемент.Имя;
	ИначеЕсли Элемент.Имя = "Свойства_УстановленныеМетки" Тогда
		СтандартнаяОбработка = Ложь;
		ИмяКоманды = "РедактироватьМетки";
	Иначе
		ИмяЭлемента = СтрЗаменить(Элемент.Имя, "СсылочныйДополнительныйРеквизитЗначение", "ДополнительныйРеквизитЗначение");
		
		ЗначениеРеквизита = Форма[ИмяЭлемента];
		Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			РедактироватьГиперссылкуРеквизита(Форма, Истина, ИмяЭлемента);
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьГиперссылкуРеквизита(Форма, ПереходПоГиперссылке = Ложь, ИмяРеквизита = Неопределено)
...
Процедура РедактироватьГиперссылкуРеквизитаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Форма.Элементы[Результат.ИмяРеквизита].Видимость = ЗначениеЗаполнено(Результат.Значение);
		Форма.Элементы[Результат.ИмяРеквизитаСсылки].Видимость = Не ЗначениеЗаполнено(Результат.Значение);
	КонецЕсли;
	Форма.Модифицированность = Истина;
КонецПроцедуры

Процедура ИзменитьСоставМеток(Форма, Метки)
	
	Форма.Свойства_УстановленныеМетки.Очистить();
	Для Каждого Метка Из Метки Цикл
		ЗаполнитьЗначенияСвойств(Форма.Свойства_УстановленныеМетки.Добавить(), Метка);
	КонецЦикла;
