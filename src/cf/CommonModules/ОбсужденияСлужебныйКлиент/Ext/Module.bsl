...
Процедура НачатьПодборУчастниковОбсуждения(Форма, ПараметрыРасширеннойФормыПодбора) Экспорт
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗавершитьПодборУчастниковОбсуждения", ЭтотОбъект, Форма);
	
	ОткрытьФорму("Справочник.Пользователи.ФормаВыбора", ПараметрыФормы, Форма,,,, ОповещениеОЗакрытии,
		РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	
КонецПроцедуры

Процедура ЗавершитьПодборУчастниковОбсуждения(Результат, Форма) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		Для Каждого Значение Из Результат Цикл
			Участники.Добавить().Пользователь = Значение;
		КонецЦикла;
	Иначе
		Участники.Добавить().Пользователь = Результат;
	КонецЕсли;
	
	Форма.КоличествоУчастников = Участники.Количество();
...
Процедура ПоказатьИнформациюОбИнтеграции(Форма, ОписаниеИнтеграции, ОповещениеОбИзмененииИнтеграции) Экспорт
	...
	ОткрытьФорму(ИмяФормы, ОписаниеИнтеграции, Форма,,,, Оповещение,
		РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
