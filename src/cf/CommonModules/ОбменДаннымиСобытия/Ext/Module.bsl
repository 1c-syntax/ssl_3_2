...
Процедура ОтключитьИспользованиеПланаОбмена(Источник, Отказ) Экспорт
	...
	// Проверка значения свойства ОбменДанными.Загрузка добавлена для исключения избыточных блокировок.
	Если Источник.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
...
#КонецЕсли
				...
				Исключение
					...
					СтрокаСообщения = НСтр("ru = 'Ошибка алгоритма вычисления значения константы:
						...
						|%4 %5
						|%6
						|%4 %7'", ОбщегоНазначения.КодОсновногоЯзыка());
						
					СтрокаСообщения = СтрШаблон(СтрокаСообщения,
						ПРО.ИмяПланаОбмена,
						ПРО.ОбъектМетаданныхИмя,
						ИнформацияОбОшибке().Описание,
						"//",
						НСтр("ru = '{Начало алгоритма}'", ОбщегоНазначения.КодОсновногоЯзыка()),
						Строка(СтрокаДерева.ЗначениеКонстанты),
						НСтр("ru = '{Окончание алгоритма}'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Задано недопустимое значение параметра [1] для метода ОбменДаннымиСобытия.ДанныеРазличаются.'");
