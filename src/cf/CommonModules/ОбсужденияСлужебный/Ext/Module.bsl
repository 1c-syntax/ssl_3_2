...
Функция ПараметрыРасширеннойФормыПодбора(Форма) Экспорт
	
	ПараметрыПодбора = ПользователиСлужебный.НовыеПараметрыРасширеннойФормыПодбора();
	ПараметрыПодбора.ЗаголовокФормыПодбора = НСтр("ru = 'Участники обсуждения'");
	
	Для Каждого Элемент Из Форма.Участники Цикл
		Если ЗначениеЗаполнено(Элемент.Пользователь) Тогда
			ПараметрыПодбора.ВыбранныеПользователи.Добавить(Элемент.Пользователь);
		КонецЕсли;
	КонецЦикла;
	
	Возврат ПоместитьВоВременноеХранилище(ПараметрыПодбора, Форма.УникальныйИдентификатор);
