...
//    Кроме того, свойство ПарольУстановлен устанавливается Ложь (смотри СлужебныйПользовательИБ).
//
// Возвращаемое значение:
//  Структура:
...
Процедура УстановитьСвойстваПользователяИБ(Знач ИмяИлиИдентификатор, Знач ОбновляемыеСвойства,
	...
	Если УстановитьПароль Тогда
		...
		ПользовательИБ.СохраняемоеЗначениеПароля =
			ВычислитьСохраняемоеЗначениеПароляПользователя(ОбновляемыеСвойства.Пароль);
	КонецЕсли;
	...
	Если Свойства.Пароль = Неопределено
	   ...
	   И Свойства.ПарольУстановлен = Ложь Тогда
		...
		Если ПользовательИБ = Неопределено Или Не ПользовательИБ.ПарольУстановлен Тогда
			...
			Свойства.СохраняемоеЗначениеПароля = ВычислитьСохраняемоеЗначениеПароляПользователя(НовыйПароль);
		КонецЕсли;
	КонецЕсли;
...
// Используется ГенераторСлучайныхПаролей (параметр ЗапоминающийсяПароль = Истина).
//
// Параметры:
...
//  Строка - новый пароль.
//
Функция СоздатьПароль(СвойстваПароля) Экспорт
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("Пользователи.СоздатьПароль",
		"СвойстваПароля", СвойстваПароля, Тип("Структура"));
