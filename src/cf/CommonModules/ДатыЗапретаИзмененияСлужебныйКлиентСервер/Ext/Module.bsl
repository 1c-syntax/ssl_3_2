...
Процедура УточнитьНастройкуДатыЗапретаПриИзменении(Контекст, РассчитатьДатуЗапрета = Истина) Экспорт
	...
	Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
		...
		Если Контекст.НачалоДня > СрокРазрешения Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Запрещен ввод и редактирование данных за все прошлые периоды по %1 включительно (%2).
				           |Отсрочка, разрешавшая ввод и редактирование данных за период с %3 по %4, истекла %5.'"),
		...
		Иначе
			...
			КонецЕсли;
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '• По %1 включительно запрещен ввод и редактирование данных за все прошлые периоды по %2 и действует отсрочка, разрешающая ввод и редактирование данных за период с %4 по %5.
				           |• С %6 начнет действовать запрет на ввод и редактирование данных за все прошлые периоды по %5 (%3).'"),
	...
	Иначе 
		Контекст.КоличествоДнейРазрешения = 0;
		ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрещен ввод и редактирование данных за все прошлые периоды по %1 (%2)'"),
			Формат(Контекст.ДатаЗапрета, "ДЛФ=Д"), НРег(ОписанияДатЗапрета()[Контекст.ОписаниеДатыЗапрета]));
	КонецЕсли;
...
//       ** ГруппаКоличествоДнейРазрешения - ГруппаФормы
// 
Процедура ОбновитьОтображениеДатыЗапретаПриИзменении(Контекст) Экспорт
	...
	Если Контекст.ОписаниеДатыЗапрета = "" Или Контекст.ОписаниеДатыЗапрета = "ПроизвольнаяДата" Тогда
		Контекст.Элементы.ДатаЗапрета.Видимость = Контекст.ОписаниеДатыЗапрета <> "";
	...
	Контекст.Элементы.ДатаЗапрета.Видимость = Ложь;
	...
	Контекст.Элементы.ГруппаКоличествоДнейРазрешения.РасширеннаяПодсказка.Заголовок =
		Контекст.ТекстНадписиОтносительнойДатыЗапрета;
