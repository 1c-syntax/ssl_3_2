...
Функция ПредставлениеАдресногоЭлемента(Знач Наименование, Знач ТипАдресногоЭлемента, ИмяУровня = "", ВариантНаписанияТипа = "") Экспорт
	...
	Если Результат.ТипВКонец Тогда
		Возврат Наименование + " " + НРег(СокрЛП(Результат.НаименованиеТипа));
	КонецЕсли;
	
	Возврат НРег(СокрЛП(Результат.НаименованиеТипа)) + " " + Наименование;
...
Функция ИзвлечьФайлыВКаталог(ОписаниеФайлов, Знач КаталогФайлов = Неопределено, ПроверятьНаличиеФайлаСПомещениями = Истина) Экспорт
	...
	Для Каждого ОписаниеФайла Из ОписаниеФайлов Цикл
		...
		Если ФайлАрхива.Существует() И ВРег(Прав(ПолноеИмяФайла, 4)) = ".ZIP" Тогда
			...
			Архив = Новый ЧтениеФайлаАрхива(ПолноеИмяФайла);
...
Процедура ПереопределитьМуниципальныйУровеньГородаКакНаселенныйПунктПоГАР(ИмяУровня, Выборка, Суффикс)
		...
		Если ИмяУровня = "city" Тогда
			ИмяУровня = "locality";
		ИначеЕсли ИмяУровня = "district"
			И (ВРег(Выборка["ТипОбъекта" + Суффикс]) = "Г" 
			ИЛИ ВРег(Выборка["ТипОбъекта" + Суффикс]) = "Г.") Тогда
			ИмяУровня = "locality";
		КонецЕсли;
...
Процедура ОбработатьАдресныйКлассификатор(Знач Дескриптор, Знач ПутьКФайлу)
	...
	Попытка
		ЧтениеАрхива = Новый ЧтениеФайлаАрхива(ПутьКФайлу);
		ЧтениеАрхива.ИзвлечьВсе(КаталогФайлов, РежимВосстановленияПутейФайлаАрхива.НеВосстанавливать);
...
Функция ДоступныеВерсииАдресныхСведений(ПараметрыВыгрузки = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Архив = Новый ЧтениеФайлаАрхива (РезультатПолученияФайла.Путь);
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если Файл.Существует Тогда
		...
		ЧтениеАрхива = Новый ЧтениеФайлаАрхива(Файл.ПолноеИмя);
		Элемент = ЧтениеАрхива.Элементы[0];
