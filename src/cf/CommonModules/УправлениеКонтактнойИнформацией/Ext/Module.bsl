...
//                                       национальной спецификой), см. УправлениеКонтактнойИнформацией.СведенияОТелефоне
//                                       (для получения структуры телефона или факса).
...
//
Процедура ПриСозданииНаСервере(Форма, Объект, ДополнительныеПараметры = Неопределено) Экспорт
	
	ДополнительныеПараметрыКИ = ПараметрыКонтактнойИнформации();
	Если ДополнительныеПараметры <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("УправлениеКонтактнойИнформацией.ПриСозданииНаСервере",
			"ДополнительныеПараметры", ДополнительныеПараметры, Тип("Структура"));
		ЗаполнитьЗначенияСвойств(ДополнительныеПараметрыКИ, ДополнительныеПараметры);
	КонецЕсли;
	
	ТипПомещения             = ДополнительныеПараметрыКИ.ТипПомещения;
	ОтложеннаяИнициализация  = ДополнительныеПараметрыКИ.ОтложеннаяИнициализация;
	ОтложеннаяИнициализация  = ДополнительныеПараметрыКИ.ОтложеннаяИнициализация;
	ПоложениеЗаголовкаКИ     = ДополнительныеПараметрыКИ.ПоложениеЗаголовкаКИ;
	ИсключаемыеВиды          = ДополнительныеПараметрыКИ.ИсключаемыеВиды;
	СкрываемыеВиды           = ДополнительныеПараметрыКИ.СкрываемыеВиды;
	ИмяЭлементаДляРазмещения = ДополнительныеПараметрыКИ.ИмяЭлементаДляРазмещения;
	РазрешитьДобавлениеПолей = ДополнительныеПараметрыКИ.РазрешитьДобавлениеПолей;
	РазмещеныНаФорме         = ДополнительныеПараметрыКИ.РазмещеныНаФорме;
	ОбработкаНавигационнойСсылки = ДополнительныеПараметрыКИ.ОбработкаНавигационнойСсылки;
	...
	ДополнительныеПараметрыВывода = ДополнительныеПараметрыВыводаКонтактнойИнформации(НастройкиКИ.ОписаниеКоманд,
	   РазмещеныНаФорме, РазрешитьДобавлениеПолей, ИсключаемыеВиды, СкрываемыеВиды);
	
	ПараметрыКонтактнойИнформации = ПараметрыВыводаКонтактнойИнформации(Форма, ИмяЭлементаДляРазмещения,
		ПоложениеЗаголовкаКИ, ОтложеннаяИнициализация, ДополнительныеПараметрыВывода);
	...
	ГруппаДляРазмещения = Родитель(Форма, ИмяЭлементаДляРазмещения);
	...
	Если ГруппаЗначенияКИ = Неопределено Тогда
		...
		ГруппаЗначенияКИ = Форма.Элементы.Добавить(ИмяГруппы, Тип("ГруппаФормы"), ГруппаДляРазмещения);
		...
		ГруппаЗначенияКИ.РастягиватьПоГоризонтали = Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
		...
		Если ОтложеннаяИнициализация Тогда
			ДобавитьРеквизитВОписание(Форма, СтрокаКИ, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ, , СтатическийЭлемент, ИмяЭлементаДляРазмещения);
			Если СтатическийЭлемент Тогда
				ПодготовитьСтатическийЭлемент(Форма, СтрокаКИ, СозданныеЭлементы, СозданныйЭлемент, ИмяЭлементаДляРазмещения);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
	...
	ОбновитьСпискиВыбораПолейАдресов(Форма, ИмяЭлементаДляРазмещения);
...
Процедура ПриЧтенииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	ПоложениеЗаголовкаКИ = ?(ЗначениеЗаполнено(Параметры.ПоложениеЗаголовка), ПредопределенноеЗначение(Параметры.ПоложениеЗаголовка), ПоложениеЗаголовкаЭлементаФормы.Авто);
			...
			ИначеЕсли НЕ РеквизитФормы.Свойство("ЭтоИсторическаяКонтактнаяИнформация")
				...
				Если СтатическийРеквизит = Неопределено Тогда // Реквизит создается динамически.
					Если Не ОтложеннаяИнициализация И ЗначениеЗаполнено(РеквизитФормы.ИмяРеквизита) Тогда
						...
						ВидПоляДругое = ?(РеквизитФормы.Тип = Перечисления.ТипыКонтактнойИнформации.Другое,
							ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитФормы.Вид, "ВидПоляДругое"), "");
						Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(РеквизитФормы.Тип, Параметры.ОбработкаНавигационнойСсылки, ВидПоляДругое) Тогда
...
Функция ОбновитьКонтактнуюИнформацию(Форма, Объект, Результат = Неопределено) Экспорт
	...
	ИначеЕсли Результат.Свойство("ПоменятьМестамиЭлементы") Тогда
		...
		ВидПоляДругое = ?(ПервыйЭлемент.Тип = Перечисления.ТипыКонтактнойИнформации.Другое,
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПервыйЭлемент.Вид, "ВидПоляДругое"), "");
		
		Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(ПервыйЭлемент.Тип, ОбработкаНавигационнойСсылки, ВидПоляДругое) Тогда
	...
	Если Результат.Свойство("ОбновитьКонтекстноеМеню") Тогда
		Если Результат.Свойство("ИмяЭлементаДляРазмещения") Тогда
			ОбновитьСпискиВыбораПолейАдресов(Форма, Результат.ИмяЭлементаДляРазмещения);
		...
		Иначе
			Для каждого ИмяЭлементаРазмещения Из Форма.ПараметрыКонтактнойИнформации Цикл
				ОбновитьСпискиВыбораПолейАдресов(Форма, ИмяЭлементаРазмещения.Ключ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Если Результат.Свойство("Перечитать") И Результат.Свойство("ИмяЭлементаДляРазмещения") Тогда
		...
		Для Каждого СтрокаТаблицы Из ОписаниеКонтактнойИнформации Цикл
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Если СтрокаТаблицы.Комментарий <> НайденнаяСтрока.Комментарий Тогда
					Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(СтрокаТаблицы.Тип, 
					   ПараметрыКонтактнойИнформации.ОбработкаНавигационнойСсылки, СтрокаТаблицы.ВидПоляДругое) Тогда
...
Процедура ВыполнитьОтложеннуюИнициализацию(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	ПоложениеЗаголовкаКИ = ?(ЗначениеЗаполнено(ПараметрыКонтактнойИнформации.ПоложениеЗаголовка), 
		ПредопределенноеЗначение(ПараметрыКонтактнойИнформации.ПоложениеЗаголовка), ПоложениеЗаголовкаЭлементаФормы.Авто);
...
Функция ПараметрыКонтактнойИнформации() Экспорт
	...
	Результат.Вставить("ИсключаемыеВиды", Новый Массив);
...
Функция ПредставлениеАдреса(Адрес, Знач ПараметрыПредставленияАдреса = Неопределено) Экспорт
	...
	Для Каждого ИмяПоля Из ПорядокПолей Цикл
		...
		Если ИмяПоля = "Дом" Тогда
			...
			ПоляПредставления.Добавить(НаименованиеАдресногоОбъекта(АдресПоПолям.Здание, "Номер", "ТипЗдания", Истина));
			
			Для Каждого Корпус Из АдресПоПолям.Корпуса Цикл
				ПоляПредставления.Добавить(НаименованиеАдресногоОбъекта(Корпус, "Номер", "ТипЗдания", Истина));
			КонецЦикла;
			
		ИначеЕсли ИмяПоля = "Помещения" Тогда
			Для Каждого Помещение Из АдресПоПолям.Помещения Цикл
				ПоляПредставления.Добавить(НаименованиеАдресногоОбъекта(Помещение, "Номер", "ТипПомещения", Истина));
			КонецЦикла;
			
		ИначеЕсли АдресПоПолям.Свойство(ИмяПоля) И ЗначениеЗаполнено(АдресПоПолям[ИмяПоля]) Тогда
			ПоляПредставления.Добавить(НаименованиеАдресногоОбъекта(АдресПоПолям, ИмяПоля));
		КонецЕсли;
	КонецЦикла;
...
//                                                  Значение по умолчанию - Ложь.
//       ** УказыватьОКТМО - Булево - для типа Адрес. признак возможности ручного ввода кода ОКТМО в форме ввода адреса.
...
Функция ОпределитьЭлементСПочтойДляВосстановленияПароля(Форма, АдресЭлектроннойПочты, ДоступноРедактирование, ЭтоВнешнийПользователь = Ложь) Экспорт
	...
	ЗаголовокРеквизита = НСтр("ru = 'Электронная почта'");
	ИмяРеквизита = ИмяРеквизитыДляВосстановленияПароля(Форма,
		АдресЭлектроннойПочты, ТипИлиВидEmailПользователя, ЗаголовокРеквизита);
	...
	Если ЗначениеЗаполнено(ИмяРеквизита)
	   И Форма.Элементы.Найти(ИмяРеквизита) <> Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			ЗаголовокЭлемента = ?(ЗначениеЗаполнено(Форма.Элементы[ИмяРеквизита].Заголовок),
				Форма.Элементы[ИмяРеквизита].Заголовок, ЗаголовокРеквизита);
			Форма.Элементы[ИмяРеквизита].Заголовок = ЗаголовокЭлемента
...
Процедура СоздатьРеквизитыКонтактнойИнформации(Знач Форма, Знач ДобавляемыеРеквизиты, Знач ИмяОбъекта, Знач РазмещеныНаФорме,
	...
	Для Каждого ОбъектКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		Иначе
			...
			ЕстьПолеКомментарий = ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(ОбъектКонтактнойИнформации.Тип,
			   ОбработкаНавигационнойСсылки, ОбъектКонтактнойИнформации.ВидПоляДругое);
...
Процедура СкрытьКонтактнуюИнформацию(Знач Форма, Знач ДобавляемыеРеквизиты, ПараметрыВывода)
	...
	ДополнительныеПараметры = ДополнительныеПараметрыВыводаКонтактнойИнформации(ПараметрыВывода.ОписаниеКоманд,
		ПараметрыВывода.РазмещеныНаФорме, ПараметрыВывода.РазрешитьДобавлениеПолей,
...
Процедура ДобавитьКнопкуДополнительногоПоляКонтактнойИнформации(Знач Форма, Знач ИмяЭлементаДляРазмещения)
	...
	Кнопка             = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Форма.Элементы[ИмяЭлементаДляРазмещения]);
	...
	Кнопка.Важность    = ВажностьКнопки.Дополнительная;
...
Процедура ИзменитьКомментарий(Форма, ИмяРеквизита, ИмяЭлементаДляРазмещения, ТипКонтактнойИнформации)
	...
	Если Форма.Элементы.Найти("Комментарий" + ИмяРеквизита) = Неопределено Тогда
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено)
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		Если ТипЗнч(ДобавляемыйВид)= Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			ИнформацияОВидеКИ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДобавляемыйВид, "Тип, Наименование, ВидРедактирования, 
				|ВидПоляДругое, ВводитьНомерПоМаске, МаскаНомераТелефона, ХранитьИсториюИзменений, ПометкаУдаления");
			ИнформацияОВидеКИ.Вставить("Вид", ДобавляемыйВид);
		Иначе
	...
	ПараметрыКонтактнойИнформации = ПараметрыКонтактнойИнформацииФормы(Форма.ПараметрыКонтактнойИнформации, ИмяЭлементаДляРазмещения);
	ОбработкаНавигационнойСсылки = ПараметрыКонтактнойИнформации.ОбработкаНавигационнойСсылки;
	ЕстьПолеКомментарий = ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(ИнформацияОВидеКИ.Тип, ОбработкаНавигационнойСсылки, ИнформацияОВидеКИ.ВидПоляДругое);
	...
	Если ЕстьПолеКомментарий Тогда
		ПолеКомментарий = Форма.Элементы.Добавить(ИмяРеквизитаКомментарий, Тип("ПолеФормы"), ГруппаСтроки);
		...
		ПолеКомментарий.АвтоМаксимальнаяШирина = Истина;
	...
	ИначеЕсли ЕстьКомментарий Тогда
		...
		ПолеВвода.РасширеннаяПодсказка.АвтоМаксимальнаяШирина = ПолеВвода.АвтоМаксимальнаяШирина;
		ПолеВвода.РасширеннаяПодсказка.Ширина                 = ПолеВвода.Ширина;
		ПолеВвода.РасширеннаяПодсказка.МаксимальнаяШирина     = ПолеВвода.МаксимальнаяШирина;
	КонецЕсли;
...
Функция СоздатьПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница И ОбработкаНавигационнойСсылки Тогда
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес
	   Или (ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Другое
	   И ИнформацияОВидеКИ.ВидПоляДругое <> "ОднострочноеУзкое") Тогда
		Элемент.АвтоМаксимальнаяШирина = Ложь;
		Элемент.МаксимальнаяШирина = 85;
	КонецЕсли;
	
	УстановитьСвойстваПолейВвода(ИнформацияОВидеКИ, Элемент, Форма, ИмяРеквизита, ОбработкаНавигационнойСсылки);
	
	Элемент.ПоложениеЗаголовка = ?(ЗначениеЗаполнено(ПараметрыКонтактнойИнформации.ПоложениеЗаголовка),
	   ПредопределенноеЗначение(ПараметрыКонтактнойИнформации.ПоложениеЗаголовка), ПоложениеЗаголовкаЭлементаФормы.Авто);
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Другое Тогда
			Элемент.ПоложениеЗаголовка = ?(Элемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто,
				ПоложениеЗаголовкаЭлементаФормы.Лево, Элемент.ПоложениеЗаголовка);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодготовитьСтатическийЭлемент(Форма, СтрокаКИ, СозданныеЭлементы, СозданныйЭлемент, ИмяЭлементаДляРазмещения)
	...
	ГруппаЗначенияКИ = Форма.Элементы["ГруппаЗначенияКИ"+ИмяЭлементаДляРазмещения];
	Если Форма.Элементы.Найти(ИмяЭлементаГруппаСтроки) = Неопределено Тогда
		Элемент = Форма.Элементы.Найти(СтрокаКИ.ИмяРеквизита);
		Если Элемент <> Неопределено И (Элемент.Родитель = Форма.Элементы[ИмяЭлементаДляРазмещения]
		   ИЛИ Элемент.Родитель = ГруппаЗначенияКИ) Тогда
			Форма.Элементы.Переместить(Элемент, ГруппаЗначенияКИ);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ГруппаСтроки.Родитель = Форма.Элементы[ИмяЭлементаДляРазмещения]
		ИЛИ ГруппаСтроки.Родитель = ГруппаЗначенияКИ Тогда
		Форма.Элементы.Переместить(ГруппаСтроки, ГруппаЗначенияКИ);
	КонецЕсли;
	ПараметрыКонтактнойИнформации = ПараметрыКонтактнойИнформацииФормы(Форма.ПараметрыКонтактнойИнформации, ИмяЭлементаДляРазмещения);
	...
	СоздатьДействие(Форма, СтрокаКИ, СтрокаКИ.ИмяРеквизита, ГруппаСтроки, ИмяЭлементаДляРазмещения);
	...
	Иначе
		Если Форма.Элементы.Найти(СтрокаКИ.ИмяРеквизита) <> Неопределено Тогда
			...
			Если СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес
				Или (СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Другое
				И СтрокаКИ.ВидПоляДругое <> "ОднострочноеУзкое") Тогда
				ПолеВвода.АвтоМаксимальнаяШирина = Ложь;
				ПолеВвода.МаксимальнаяШирина = 85;
			КонецЕсли;
		КонецЕсли;
	...
	КонецЕсли;
	Если ПолеВвода.Вид = ВидПоляФормы.ПолеВвода Тогда
		ПолеВвода.Картинка = КартинкаТипаКонтактнойИнформации(СтрокаКИ.Тип);
	КонецЕсли;
...
Процедура УстановитьСвойстваПолейВвода(ИнформацияОВидеКИ, Элемент, Форма, ИмяРеквизита, ОбработкаНавигационнойСсылки)
	...
	Если Элемент.Вид = ВидПоляФормы.ПолеВвода Тогда
		Элемент.Картинка = КартинкаТипаКонтактнойИнформации(ИнформацияОВидеКИ.Тип);
	КонецЕсли;
...
Функция Группа(ИмяГруппы, Форма, Заголовок, ИмяЭлементаДляРазмещения, ИмяГруппыРодителя = "", ПорядокУдаления = 5)
	...
	Если Группа = Неопределено Тогда
		...
		Группа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.АвтоматическиУчитыватьВидЭкрана;
		ПараметрыКонтактнойИнформации = ПараметрыКонтактнойИнформацииФормы(Форма.ПараметрыКонтактнойИнформации, ИмяЭлементаДляРазмещения);
...
Процедура ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов)
	...
	Если СоздатьТаблицуКонтактнойИнформации Тогда
		...
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ПроверятьКорректность", Новый ОписаниеТипов("Булево"), ИмяОписания));
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ВидПоляДругое", Строка500, ИмяОписания));
	Иначе
		...
		РеквизитыДляСоздания.Вставить("ПроверятьКорректность",               Истина);
		РеквизитыДляСоздания.Вставить("ВидПоляДругое",                       Истина);
		...
		Если РеквизитыДляСоздания["ПроверятьКорректность"] Тогда
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ПроверятьКорректность", Новый ОписаниеТипов("Булево"), ИмяОписания));
		КонецЕсли;
		...
		Если РеквизитыДляСоздания["ВидПоляДругое"] Тогда
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ВидПоляДругое", Строка500, ИмяОписания));
		КонецЕсли;
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКонтактнойИнформации, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,
	...
	НоваяСтрока.ВидПоляДругое = СтрокаКонтактнойИнформации.ВидПоляДругое;
	...
	Если НоваяСтрока.Свойство("ПроверятьКорректность") Тогда
		НоваяСтрока.ПроверятьКорректность = СтрокаКонтактнойИнформации.ПроверятьКорректность;
	КонецЕсли;
	...
	Если ЗаполнятьЗначениеРеквизита И Не ЭтоРеквизитТабличнойЧасти Тогда
		...
		Если СтрокаКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес
			И ЭтоПолеНадписи И ПустаяСтрока(СтрокаКонтактнойИнформации.Представление) Тогда
		...
		ОбработкаНавигационнойСсылки = ПараметрыКонтактнойИнформации.ОбработкаНавигационнойСсылки;
		Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(СтрокаКонтактнойИнформации.Тип, ОбработкаНавигационнойСсылки, СтрокаКонтактнойИнформации.ВидПоляДругое) Тогда
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	КомандыДляВывода = УправлениеКонтактнойИнформациейКлиентСервер.КомандыДляВыводаНаФорму(ПараметрыКИ, Тип, 
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			...
			Для Каждого КомандаВывода Из КомандыДляВывода Цикл
				ИмяКоманды = КомандаВывода.Ключ + "Команда" + ИмяРеквизита;
				...
				Если Форма.Элементы.Найти(ИмяКоманды) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Элемент = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"),Форма.Элементы[ИмяРеквизита].КонтекстноеМеню);
				Элемент.ИмяКоманды = ИмяКоманды;
				ЗаполнитьЗначенияСвойств(Команда, КомандаВывода.Значение, , "Действие");
				ПараметрыКИ.ДобавленныеЭлементы.Добавить(ИмяКоманды, 1, Ложь);
			КонецЦикла;
...
Процедура ОбновитьСпискиВыбораПолейАдресов(Форма, ИмяЭлементаДляРазмещения)
	...
	Для Каждого СтрокаКИ Из ВсеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаКИ.ИмяРеквизита) Или Форма.Элементы.Найти(СтрокаКИ.ИмяРеквизита) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		КоличествоАдресовВСпискеВыбора = 0;
		СписокАдресовДляВыбора = Новый Соответствие();
		ДанныеАдреса = Новый Структура("Представление, Адрес", СтрокаКИ.Представление, СтрокаКИ.Значение);
		СписокАдресовДляВыбора.Вставить(ВРег(СтрокаКИ.Представление), ДанныеАдреса);
		
		Для Каждого Адрес Из НайденныеСтроки Цикл
			
			Если КоличествоАдресовВСпискеВыбора > 10 Тогда // Ограничение для большого количества адресов на форме
				Прервать;
			КонецЕсли;
			...
			Если СтрокаКИ.ПроверятьКорректность И Не Адрес.ПроверятьКорректность Тогда
				Продолжить;
			КонецЕсли;
			...
			Если СписокАдресовДляВыбора[ВРег(Адрес.Представление)] <> Неопределено Тогда
				ПредставлениеАдреса = "";
			Иначе
				...
				КонецЕсли;
				СписокАдресовДляВыбора.Вставить(ВРег(Адрес.Представление), ДанныеАдреса);
			КонецЕсли;
			
			КоличествоАдресовВСпискеВыбора = КоличествоАдресовВСпискеВыбора + 1;
		КонецЦикла;
		...
		Если Поле.Вид = ВидПоляФормы.ПолеВвода Тогда
			...
			Для Каждого ДанныеАдреса Из СписокАдресовДляВыбора Цикл
...
//
Функция ИмяРеквизитыДляВосстановленияПароля(Форма, АдресЭлектроннойПочты, ТипИлиВидEmailПользователя, ЗаголовокРеквизита)
	...
	Если ОписаниеПочты <> Неопределено Тогда
		...
		ЗаголовокРеквизита = Строка(ОписаниеПочты.Вид);
	КонецЕсли;
...
Процедура УстановитьДействияДляСтатическихЭлементов(Форма, СтрокаКИ, ИмяЭлементаДляРазмещения)
	...
	Если ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(Тип, ОбработкаНавигационнойСсылки, СтрокаКИ.ВидПоляДругое) Тогда
		ИмяЭлементаКомментарий = "Комментарий" + СтрокаКИ.ИмяРеквизита;
		Если Форма.Элементы.Найти(ИмяЭлементаКомментарий) <> Неопределено Тогда
...
//                       - Неопределено
//
Функция ДополнительныеПараметрыВыводаКонтактнойИнформации(ОписаниеКоманд, РазмещеныНаФорме,
...
Функция НастройкиПодсистемы(ВладелецКонтактнойИнформации)
	...
	Настройки.Вставить("ШиринаПоляКомментарий", 0);
	Настройки.Вставить("ОтображатьИконки", Ложь);
	Настройки.Вставить("ПоложениеКнопкиДобавить", ГоризонтальноеПоложениеЭлемента.Авто);
...
//
Функция ДляТипаКонтактнойИнформацииЕстьПолеКомментарий(ТипКонтактнойИнформации, ОбработкаНавигационнойСсылки, ВидПоляДругое)
	
	Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Адрес 
		ИЛИ (ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Другое И ВидПоляДругое <> "ОднострочноеУзкое") 
