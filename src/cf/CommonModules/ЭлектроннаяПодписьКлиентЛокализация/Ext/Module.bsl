...
//  ПараметрыВажногоСообщения - Структура
//
Процедура ПриПроверкеКонфликтаПрограммКриптографии(Форма, РезультатПроверки, ПараметрыВажногоСообщения) Экспорт
	...
	ПараметрыВажногоСообщения.Сообщение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru = 'На вашем компьютере установлено несколько приложений с алгоритмами ГОСТ: %1.
	...
	ПараметрыВажногоСообщения.КраткоеПредставление = НСтр("ru = 'Возможна некорректная работа'");
	// Конец Локализация
...
//  ПараметрыВажногоСообщения - Структура
//
Процедура ПриПроверкеУстановленныхПрограммКриптографии(Форма, РезультатПроверки, ЕстьПроверяемыеПрограммы, ПараметрыВажногоСообщения) Экспорт
	...
	Если Не ЕстьПроверяемыеПрограммы Тогда
		Если РезультатПроверки.Токены.Количество() > 0 Тогда
			ПараметрыВажногоСообщения.Сообщение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Для работы с усиленной квалифицированной электронной подписью на компьютере должно быть установлено сертифицированное приложение (средство криптографической защиты информации). <a href = ""%1"">Установить</a>.
					|<a href = ""%2"">Подробнее о работе с токенами без установки приложения</a>.'"),
						"ПроверитьУстановкуПриложенийКриптографии",
						ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеУстановитьБиблиотекиДляТокенов());
			ПараметрыВажногоСообщения.КраткоеПредставление = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Не установлено приложение'"),
				"ПроверитьУстановкуПриложенийКриптографии");
		Иначе
			ПараметрыВажногоСообщения.Сообщение = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				...
				|<a href = ""%1"">Установить.</a>'"), ЭлектроннаяПодписьСлужебныйКлиентСервер.ДействиеПроверитьУстановкуПриложенийКриптографии());
			ПараметрыВажногоСообщения.КраткоеПредставление = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Не установлено приложение'"),
...
Процедура ПриОткрытииФормыУстановкиПрограммКриптопровайдеров(Параметры, Владелец, Оповещение, СтандартнаяОбработка) Экспорт
	...
	ОткрытьФорму("Обработка.ПрограммыЭлектроннойПодписиИШифрования.Форма.УстановкаПриложенийДляРаботыСЭлектроннойПодписью",
		Параметры, Владелец,,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
#Если ВебКлиент Тогда
		...
		Если ТребуетсяУстановкаДополнительныхКомпонент Тогда
			...
			ОткрытьФорму("ОбщаяФорма.НачалоРаботыСЭлектроннойПодписью", ПараметрыФормы,
				Контекст.Форма, , , , Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
			Возврат Ложь;
		КонецЕсли;
#КонецЕсли
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	...
	НачатьПодписьНаТокенеВМобильномКлиенте(Оповещение, ПараметрыПодписи, Данные);
	Возврат;
#КонецЕсли
	
	Результат = Ждать ПодписьНаТокене(ПараметрыПодписи, Данные);
...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	...
	Оповещение = Новый ОписаниеОповещения("ПослеОперацииНаТокенеВМобильномКлиенте", 
		ЭтотОбъект, Контекст);
	НачатьРасшифровкуНаТокенеВМобильномКлиенте(Оповещение, ПараметрыРасшифровки, Данные);
	Возврат;
#КонецЕсли
	
	Результат = Ждать РасшифровкаНаТокене(ПараметрыРасшифровки, Данные);
	ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, Результат);
...
Процедура ДобавитьПриложениеЭлектроннойПодписи()
	...
	НоваяФорма = ОткрытьФорму("Обработка.ПрограммыЭлектроннойПодписиИШифрования.Форма.ДобавлениеПриложения",,,,,,
		Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
