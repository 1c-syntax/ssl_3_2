...
Процедура ПередДобавлениемКомандОтчетов(КомандыОтчетов, Параметры, СтандартнаяОбработка) Экспорт
	...
	ДобавитьКомандуПраваПользователей(КомандыОтчетов, Параметры);
...
Процедура ДобавитьКомандуПраваПользователей(КомандыОтчетов, Параметры)
	
	Если СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.Пользователи")
	 Или СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.ГруппыПользователей") Тогда
		
		ТипПользователей = Новый ОписаниеТипов("СправочникСсылка.Пользователи");
		ТипГрупп         = Новый ОписаниеТипов("СправочникСсылка.ГруппыПользователей");
		
	ИначеЕсли СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.ВнешниеПользователи")
	      Или СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.ГруппыВнешнихПользователей") Тогда
		
		ТипПользователей = Новый ОписаниеТипов("СправочникСсылка.ВнешниеПользователи");
		ТипГрупп         = Новый ОписаниеТипов("СправочникСсылка.ГруппыВнешнихПользователей");
	Иначе
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ПредставлениеВарианта = ?(ТипПользователей = Неопределено
			И Не СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.ПрофилиГруппДоступа"),
			НСтр("ru = 'Права пользователя'"), НСтр("ru = 'Права пользователя (на таблицу)'"));
	Иначе
		КлючВарианта = "ПраваПользователейНаТаблицу";
		ПредставлениеВарианта = ?(ТипПользователей = Неопределено
			И Не СтрНачинаетсяС(Параметры.ИмяФормы, "Справочник.ПрофилиГруппДоступа"),
			НСтр("ru = 'Права пользователей'"), НСтр("ru = 'Права пользователей (на таблицу)'"));
	КонецЕсли;
	...
	Если Параметры.ИмяФормы = "Справочник.Пользователи.Форма.ФормаСписка"
	 Или Параметры.ИмяФормы = "Справочник.ГруппыПользователей.Форма.ФормаСписка"
	 Или Параметры.ИмяФормы = "Справочник.ВнешниеПользователи.Форма.ФормаСписка"
	 Или Параметры.ИмяФормы = "Справочник.ГруппыВнешнихПользователей.Форма.ФормаСписка" Тогда
		
		Если Параметры.ИмяФормы = "Справочник.Пользователи.Форма.ФормаСписка"
		 Или Параметры.ИмяФормы = "Справочник.ВнешниеПользователи.Форма.ФормаСписка" Тогда
		
			ДобавитьКоманду(КомандыОтчетов, Истина, "НеИспользует", "ПраваПользователейНаТаблицы",
				НСтр("ru = 'Права пользователей'"), ТипПользователей);
			
			ДобавитьКоманду(КомандыОтчетов, Ложь, "Использует", "ПраваПользователяНаТаблицы",
				НСтр("ru = 'Права пользователя'"), ТипПользователей);
		КонецЕсли;
		
		ДобавитьКоманду(КомандыОтчетов, Истина, "НеИспользует", "ПраваПользователейНаТаблицы",
			НСтр("ru = 'Права пользователей групп'"), ТипГрупп);
		
		ДобавитьКоманду(КомандыОтчетов, Ложь, "Использует", "ПраваПользователейНаТаблицы",
			НСтр("ru = 'Права пользователей группы'"), ТипГрупп);
	...
	ИначеЕсли Параметры.ИмяФормы = "Справочник.Пользователи.Форма.ФормаЭлемента"
		...
		ДобавитьКоманду(КомандыОтчетов, Ложь, "", "ПраваПользователяНаТаблицы",
			НСтр("ru = 'Права пользователя'"), ТипПользователей);
		
	ИначеЕсли Параметры.ИмяФормы = "Справочник.ГруппыПользователей.Форма.ФормаЭлемента" 
	      Или Параметры.ИмяФормы = "Справочник.ГруппыВнешнихПользователей.Форма.ФормаЭлемента" Тогда
		
		ДобавитьКоманду(КомандыОтчетов, Ложь, "", "ПраваПользователейНаТаблицы",
			НСтр("ru = 'Права пользователей группы'"), ТипГрупп);
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ДобавитьКомандуПраваПользователей.
Процедура ДобавитьКоманду(КомандыОтчетов, МножественныйВыбор, ИспользованиеТекущейСтроки,
			КлючВарианта, Представление, ТипПараметра)
	...
	Если ТипПараметра.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей")) Тогда
		ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьГруппыПользователей.Имя;
		
	ИначеЕсли ТипПараметра.СодержитТип(Тип("СправочникСсылка.ГруппыВнешнихПользователей")) Тогда
		ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьГруппыВнешнихПользователей.Имя;
	Иначе
		ФункциональнаяОпция = "";
	КонецЕсли;
	...
	Команда.ФункциональныеОпции = ФункциональнаяОпция;
	Команда.МножественныйВыбор = МножественныйВыбор;
	Если ЗначениеЗаполнено(ИспользованиеТекущейСтроки) Тогда
