...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		Элементы.Переместить(Элементы.ГруппаСоздать, Элементы.Список.КоманднаяПанель, Элементы.ГруппаКомандыВзаимодействий);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.Пользователи.Форма.ФормаСписка") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			УстановитьОтветственного(ВыделенныеСтроки, ВыбранноеЗначение, МассивИзмененныхДокументов);
	...
	ИначеЕсли КонтекстВыбора = "ПредметВыполнить" Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			ВыполненаЗамена = Ложь;
			ВыделенныеСтроки = ВыделенныеСтроки();
			УстановитьПредмет(ВыделенныеСтроки, ВыбранноеЗначение, ВыполненаЗамена);
...
Процедура ДеревоВзаимодействийПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	Если Элементы.ДеревоВзаимодействий.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
		Возврат;	
	КонецЕсли;

	ЕстьПомеченныеНаУдаление = Ложь;
	Для каждого ВыделеннаяСтрока Из Элементы.ДеревоВзаимодействий.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура РассмотреноВыполнить(Команда)
	...
	УстановитьФлагРассмотрено(ВыделенныеСтроки, ВыполненаЗамена, ФлагРассмотрено, МассивВзаимодействий);
...
&НаСервере
Процедура УстановитьФлагРассмотрено(ВыделенныеСтроки, ВыполненаЗамена, ФлагРассмотрено, МассивВзаимодействий)
...
&НаСервере
Процедура УстановитьОтветственного(ВыделенныеСтроки, Ответственный, МассивИзмененныхДокументов)
...
&НаСервере
Процедура УстановитьПредмет(ВыделенныеСтроки, Предмет,ВыполненаЗамена)
...
&НаСервере
Процедура ОтложитьРассмотрение(ВыделенныеСтроки, ДатаРассмотрения, ВыполненаЗамена = Ложь)
...
Функция ВыборКорректен()
	...
	Если Элементы.СтраницыСписокДерево.ТекущаяСтраница = Элементы.СтраницаСписок Тогда
		...
		Если Элементы.Список.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
		
		Для Каждого Элемент Из Элементы.Список.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ВопросОПометкеНаУдалениеПослеЗавершения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ОбработатьИзменениеПометкиУдаленияВДереве(Элементы.ДеревоВзаимодействий.ПолучитьСтрокиДляОбработки(), 
...
Процедура ВводДатыОтложитьРассмотрениеПослеЗавершения(ВведеннаяДата, ДополнительныеПараметры) Экспорт
	...
	ВыполненаЗамена = Ложь;
	ВыделенныеСтроки = ВыделенныеСтроки();
	ОтложитьРассмотрение(ВыделенныеСтроки, ВведеннаяДата,ВыполненаЗамена);
...
Функция ВыделенныеСтроки()
	...
	Иначе
		ВыделенныеСтроки = Элементы.ДеревоВзаимодействий.ПолучитьСтрокиДляОбработки();
	КонецЕсли;
