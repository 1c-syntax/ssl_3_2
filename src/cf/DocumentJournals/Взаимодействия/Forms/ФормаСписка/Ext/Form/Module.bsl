...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыРазмещения.Вставить("КоманднаяПанель", Элементы.Список.КоманднаяПанель);
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.СписокТипВзаимодействия.Видимость = Ложь;
		Элементы.СписокСтатус.Видимость = Ложь;
	Иначе
		ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Нет;
		Элементы.КомандыФормы.Видимость = Истина;
		Элементы.ГруппаСвернутаяНавигация.ЦветТекстаЗаголовка = ЦветаПалитры.Синий;
		Элементы.Переместить(Элементы.ГруппаСоздать, Элементы.Список.КоманднаяПанель, Элементы.ГруппаКоманднаяПанельСписок);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли КонтекстВыбора = "ОтветственныйСписок" Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			УстановитьОтветственного(ВыбранноеЗначение, Элементы.Список.ПолучитьСтрокиДляОбработки());
		КонецЕсли;
	...
	ИначеЕсли КонтекстВыбора = "ПредметСписок" Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			УстановитьПредмет(ВыбранноеЗначение, Элементы.Список.ПолучитьСтрокиДляОбработки());
		КонецЕсли;
	...
	ИначеЕсли КонтекстВыбора = "ПеренестиВПапку" Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			Если СтрНачинаетсяС(ИмяТекущегоЭлемента, "Список") Тогда
				ВыполнитьПереносВПапкуМассиваПисем(Элементы[ИмяТекущегоЭлемента].ПолучитьСтрокиДляОбработки(), ВыбранноеЗначение);
			Иначе
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		Если ВыборКорректен(Элемент.Имя,Истина) 
			...
			Если Элементы.Список.ПолучитьСтрокиДляОбработки().Количество() = 1 Тогда
...
Процедура СтрокаПоискаПриИзменении(Элемент)
	...
	Иначе 
		...
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "УстарелИндексПоиска");
...
Процедура ПредупреждениеОНеотправленныхПисьмахНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПолучениеИОтправкаПриостановлена");
КонецПроцедуры
...
&НаКлиенте
Процедура ПоложениеНавигацииПриИзменении(Элемент)
...
Процедура РассмотреноВыполнитьСписок(ЗначенияПризнака)
	...
	УстановитьФлагРассмотрено(Элементы[ИмяТекущегоЭлемента].ПолучитьСтрокиДляОбработки(), ЗначенияПризнака);
	ВосстановитьРаскрытыеУзлыДерева();
...
Процедура ДобавитьВЗакладки(Команда)
	...
	Если СтрНачинаетсяС(ИмяТекущегоЭлемента, "Список") Тогда
		ЭлементДляДобавления = Элементы[ИмяТекущегоЭлемента].ПолучитьСтрокиДляОбработки();
...
Процедура УправлениеВидимостьюПриПереключенииПанелиНавигации()
	...
	Если Не ЗначениеЗаполнено(ПоложениеНавигации) Тогда
		ПоложениеНавигации = "Слева";
	КонецЕсли;
	...
	Элементы.СписокТипВзаимодействия.Видимость = Не ЭтоПапки И Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85();
	Элементы.ТипВзаимодействия.Видимость = Не ЭтоПапки;
	Если ЭтоПапки Тогда
		ТипВзаимодействия = "Все";
		ПриИзмененииТипаСервер(Ложь);
	КонецЕсли;
	
	Если ПоложениеНавигации = "Слева" И Элементы.ГруппаПанельНавигацииСлева.ПодчиненныеЭлементы.Количество() = 0 Тогда
		Элементы.Переместить(Элементы.ГруппаНавигация, Элементы.ГруппаПанельНавигацииСлева);
		НастроитьСпискиПанелиНавигации(0, Истина);
	ИначеЕсли ПоложениеНавигации = "Окно" И Элементы.ГруппаСвернутаяНавигация.ПодчиненныеЭлементы.Количество() = 0 Тогда
		Элементы.Переместить(Элементы.ГруппаНавигация, Элементы.ГруппаСвернутаяНавигация);
		НастроитьСпискиПанелиНавигации(60, Ложь);
	КонецЕсли;
...
&НаСервере
Процедура НастроитьСпискиПанелиНавигации(Ширина, Растягивание)
	
	Элементы.КоманднаяПанельНавигации.Ширина = Ширина;
	Элементы.КонтактыПанелиНавигации.Ширина = Ширина;
	Элементы.ПредметыПанельНавигации.Ширина = Ширина;
	Элементы.Закладки.Ширина                = Ширина;
	Элементы.Свойства.Ширина                = Ширина;
	Элементы.Папки.Ширина                   = Ширина;
	Элементы.Категории.Ширина               = Ширина; 
	Элементы.КоманднаяПанельНавигации.РастягиватьПоГоризонтали = Растягивание;
	Элементы.КонтактыПанелиНавигации.РастягиватьПоГоризонтали = Растягивание;
	Элементы.ПредметыПанельНавигации.РастягиватьПоГоризонтали = Растягивание;
	Элементы.Закладки.РастягиватьПоГоризонтали                = Растягивание;
	Элементы.Свойства.РастягиватьПоГоризонтали                = Растягивание;
	Элементы.Папки.РастягиватьПоГоризонтали                   = Растягивание;
	Элементы.Категории.РастягиватьПоГоризонтали               = Растягивание;
...
Процедура УстановитьЗаголовокВидаПанелиНавигации(ЗначениеОтбора = Неопределено)
	...
	ШаблонЗаголовка = НСтр("ru = 'Навигация / %1 / %2'");
	Элементы.ГруппаСвернутаяНавигация.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонЗаголовка,
		Элементы.ВыборВариантаНавигации.Заголовок,
		ЗаголовокПанелиНавигации);
...
Функция ВыборКорректен(ИмяСписка, ПоТекущейСтроке = Ложь)
	...
	Иначе
		...
		Для Каждого Элемент Из Элементы[ИмяСписка].ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ВыборДатыОтработкиПриЗавершении(ВыбраннаяДата, ДополнительныеПараметры) Экспорт
	...
	Если ВыбраннаяДата <> Неопределено Тогда
		ОтложитьРассмотрение(ВыбраннаяДата, ?(ИмяТекущегоЭлемента = Неопределено, Неопределено, Элементы[ИмяТекущегоЭлемента].ПолучитьСтрокиДляОбработки()));
		ВосстановитьРаскрытыеУзлыДерева();
	КонецЕсли;
...
Процедура ПроверитьСостояниеОтправкиПисем()
	...
	ПроверитьСостояниеОтправкиПисемНаСервере();
	Интервал = ?(ОтправкаПисемПриостановлена, 60, 600);
...
Процедура ПроверитьСостояниеОтправкиПисемНаСервере()
	...
	ОтправкаПисемПриостановлена = Взаимодействия.ОтправкаПисемПриостановлена();
	Если ОтправкаПисемПриостановлена Тогда
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Сообщение = Взаимодействия.ТекстПредупрежденияОПриостановкеОтправкиПисем();
		ПараметрыСообщения.Обработчик = "ПредупреждениеОНеотправленныхПисьмахНадписьОбработкаНавигационнойСсылки";
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Проблема с отправкой писем'");
		ПараметрыСообщения.Идентификатор = "ПолучениеИОтправкаПриостановлена";
...
Процедура ПроверитьИндексПолнотекстовогоПоиска()
	...
	Если Не ИндексПолнотекстовогоПоискаАктуален Тогда 
		...
		Иначе
			...
			ВывестиСообщениеНеактуальностиИндекса();
...
&НаСервере
Процедура ВывестиСообщениеНеактуальностиИндекса()
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.Сообщение = НСтр("ru = 'Результаты поиска могут быть неточными, повторите поиск позднее.'");
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Поиск может быть неточен'");
	ПараметрыСообщения.Идентификатор = "УстарелИндексПоиска";
	ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
