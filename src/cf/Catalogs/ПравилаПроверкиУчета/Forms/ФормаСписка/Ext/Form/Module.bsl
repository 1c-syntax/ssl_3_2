...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдминистраторСистемы Тогда
		...
		Если ОбщееРегламентноеЗадание <> Неопределено Тогда
			Если Не ОбщегоНазначения.РазделениеВключено() Тогда
				...
				КонецЕсли;
				Элементы.Расписание.Подсказка = ТекстПодсказки;
			Иначе
				Если ЭтоАдминистраторСистемы Тогда
		...
		ПредставлениеРасписания = СформироватьСтрокуСРасписанием();
	Иначе
		Элементы.Расписание.Видимость    = Ложь;
		Элементы.РегламентноеЗаданиеПредставление.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.Расписание.Доступность = ОбновлениеИнформационнойБазы.ОбъектОбработан(
		Метаданные.Справочники.ПравилаПроверкиУчета.ПолноеИмя()).Обработан;
	
	Элементы.Расписание.Гиперссылка = Не ОбщегоНазначения.РазделениеВключено();
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		ПорядокКоманд = "ФормаВыполнитьПроверку
			|ФормаВыполнитьВсеПроверки";
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(КоманднаяПанель, ПорядокКоманд);
	КонецЕсли;
...
Процедура РасписаниеНажатие(Элемент, СтандартнаяОбработка)
	...
	Диалог     = Новый ДиалогРасписанияРегламентногоЗадания(ОбщееРасписание);
...
Функция ВыделенныеПроверки(ЕстьГруппы)
	...
	Для Каждого Проверка Из Элементы.Список.ПолучитьСтрокиДляОбработки() Цикл
...
Функция СформироватьСтрокуСРасписанием()
	...
	Элементы.Расписание.Заголовок  = НСтр("ru = 'Общее расписание выполнения проверок'");
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат Новый ФорматированнаяСтрока(ОбщееРасписаниеПредставление, , , , "Ссылка");
	Иначе
		Возврат Новый ФорматированнаяСтрока(ОбщееРасписаниеПредставление, , ЦветаСтиля.ГиперссылкаЦвет);
	КонецЕсли;
...
Процедура ПредставлениеОбщегоРасписанияНажатиеНаСервереЗавершение(Расписание, ДополнительныеПараметры)
	...
	Расписание = СформироватьСтрокуСРасписанием();
