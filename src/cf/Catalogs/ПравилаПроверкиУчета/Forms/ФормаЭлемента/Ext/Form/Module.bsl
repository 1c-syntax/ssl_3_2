...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПредставлениеРасписания.Доступность          = Не ТолькоПросмотр;
...
Процедура СформироватьСтрокуСИндивидуальнымРасписанием()
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОтдельноеРегламентноеЗадание = Неопределено Тогда
			ПредставлениеИндивидуальногоРасписания = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить расписание'"),
				, , , ПоместитьВоВременноеХранилище("ДобавитьЗадание", УникальныйИдентификатор));
		Иначе
			ПредставлениеИндивидуальногоРасписания = Новый ФорматированнаяСтрока(ОтдельноеРасписаниеПредставление,
				, , , ПоместитьВоВременноеХранилище(ОтдельноеРегламентноеЗадание.Расписание, УникальныйИдентификатор));
		КонецЕсли;
		
	Иначе
		
		Если ОтдельноеРегламентноеЗадание = Неопределено Тогда
			ПредставлениеИндивидуальногоРасписания = Новый ФорматированнаяСтрока(ОтдельноеРасписаниеПредставление + ". ", , ЦветаСтиля.ГиперссылкаЦвет);
		Иначе
			ПредставлениеИндивидуальногоРасписания = Новый ФорматированнаяСтрока(ОтдельноеРасписаниеПредставление, , ЦветаСтиля.ГиперссылкаЦвет);
		КонецЕсли;
...
Процедура СформироватьСтрокуСОбщимРасписанием()
	...
	ПредставлениеОбщегоРасписания = Новый ФорматированнаяСтрока(ОбщееРасписаниеПредставление, , ЦветаСтиля.ГиперссылкаЦвет);
...
Процедура ИзменитьЗаданиеНаСервереЗавершение(Расписание, ДополнительныеПараметры)
	...
	Иначе
		...
		ПредставлениеИндивидуальногоРасписания = Новый ФорматированнаяСтрока(Строка(Расписание), , , , ПоместитьВоВременноеХранилище(Расписание, УникальныйИдентификатор));
...
Процедура УстановитьНастройкиРасписанияНаСервере()
	...
	Элементы.ПредставлениеРасписания.Видимость = Истина;
	Если СелекторРасписания = 0 Тогда
		...
		Если Объект.СпособВыполнения = Перечисления.СпособыВыполненияПроверки.ПоОбщемуРасписанию Тогда
			...
			Элементы.ПредставлениеРасписания.Заголовок = ПредставлениеОбщегоРасписания;
		...
		ИначеЕсли Объект.СпособВыполнения = Перечисления.СпособыВыполненияПроверки.ПоОтдельномуРасписанию Тогда
			...
			Элементы.СелекторРасписания.Доступность    = Истина;
			Элементы.ПредставлениеРасписания.Заголовок = ПредставлениеИндивидуальногоРасписания;
