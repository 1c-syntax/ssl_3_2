...
Процедура ПрочитатьНерабочиеДаты(ТекущийОбъект = Неопределено)
	...
	Если НерабочиеДаты.Количество() > 0 Тогда
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Заголовок = НСтр("ru = 'Нерабочие периоды'");
		ПараметрыСообщения.Сообщение = Пояснение;
		ПараметрыСообщения.Идентификатор = "НерабочиеПериоды";
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаОсновное, Элементы.Календарь);
		ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "НерабочиеПериоды");
	КонецЕсли;
