...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(Элементы.Свойства.КоманднаяПанель, КоманднаяПанель);
	КонецЕсли;
...
Процедура СвойстваНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	НомерСтрокиПриНачалеПеретаскивания = Свойства.Индекс(Элемент.ТекущиеДанные);
...
&НаКлиенте
Процедура СвойстваОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) <> Тип("ДанныеФормыЭлементКоллекции") Тогда
		Возврат;
	КонецЕсли;
	ВыполнитьКомандуНаСервере("Перетаскивание");
КонецПроцедуры
...
&НаКлиенте
Процедура Неиспользуемые(Команда)
	НовоеЗначение = Не Элементы.ФормаНеиспользуемые.Пометка;
	ПоказатьНеиспользуемыеРеквизиты = НовоеЗначение;
	Элементы.ФормаНеиспользуемые.Пометка = НовоеЗначение;
...
Процедура НастроитьОтображениеНаборов()
	...
	КнопкаНеиспользуемые     = Элементы.ФормаНеиспользуемые;
	...
	Если ВидСвойств = Перечисления.ВидыСвойств.ДополнительныеСведения Тогда
		...
		КнопкаНеиспользуемые.Заголовок   = НСтр("ru = 'Неиспользуемые сведения'");
	...
	ИначеЕсли ВидСвойств = Перечисления.ВидыСвойств.Метки Тогда
		...
		КнопкаНеиспользуемые.Заголовок   = НСтр("ru = 'Неиспользуемые метки'");
	...
	Иначе
		...
		КнопкаНеиспользуемые.Заголовок    = НСтр("ru = 'Неиспользуемые реквизиты'");
		...
		Попытка
			...
			Иначе
				...
				Если Строка <> Неопределено Тогда
					...
					ИначеЕсли Команда = "Перетаскивание" Тогда
						СтрокаТабличнойЧасти = ТабличнаяЧасть.Найти(Строка.Свойство, "Свойство");
						
						Сдвиг = Свойства.Индекс(Строка) - НомерСтрокиПриНачалеПеретаскивания;
						ТабличнаяЧасть.Сдвинуть(ТабличнаяЧасть.Индекс(СтрокаТабличнойЧасти), Сдвиг);
