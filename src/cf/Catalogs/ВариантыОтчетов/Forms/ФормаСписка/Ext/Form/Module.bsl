...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПоказатьВариантыНижестоящихРазделов.Пометка = ВключаяПодчиненные;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КоманднаяПанель, "Выбрать");
	КонецЕсли;
...
Процедура СохранитьВариантОтчетаВФайл(Команда)
	ВыбранныеВариантыОтчетов = Элементы.Список.ПолучитьСтрокиДляОбработки();
...
Процедура ВключатьПодчиненные(Команда)
	ВключаяПодчиненные = Не ВключаяПодчиненные;
	Элементы.ПоказатьВариантыНижестоящихРазделов.Пометка = ВключаяПодчиненные;
	ДеревоПодсистемТекущаяСтрока = -1;
	ПодключитьОбработчикОжидания("ДеревоПодсистемОбработчикАктивизацииСтроки", 0.1, Истина);
КонецПроцедуры
