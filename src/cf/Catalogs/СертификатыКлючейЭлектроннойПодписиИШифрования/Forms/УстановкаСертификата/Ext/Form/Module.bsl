...
Процедура ВариантУстановкиПриИзменении(Элемент)
	...
	Если ВРег(ВариантУстановки) = "ROOT" Тогда
		Сообщение = НСтр("ru = 'Установка сертификата в доверенные корневые сертификаты означает автоматическое доверие любому сертификату, выданному с использованием этого сертификата.'");
		КраткоеПредставление = НСтр("ru = 'Доверие всем сертификатам с использованием корневого'");
		ПоказатьВажноеСообщение(Сообщение, КраткоеПредставление);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "УстановкаСертификатаВДоверенные");
	КонецЕсли;
...
&НаСервере
Процедура ПоказатьВажноеСообщение(Сообщение, КраткоеПредставление)
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.Идентификатор = "УстановкаСертификатаВДоверенные";
	ПараметрыСообщения.Сообщение = Сообщение;
	ПараметрыСообщения.КраткоеПредставление = КраткоеПредставление;
	
	ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВариантУстановки);
