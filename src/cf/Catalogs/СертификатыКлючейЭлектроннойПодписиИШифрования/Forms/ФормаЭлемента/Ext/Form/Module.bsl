...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьОтображениеСпискаИспользуют();
...
&НаКлиенте
Процедура СписокПользователейОткрытие(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура УстановитьЗначениеИспользованияПодписания(Команда)
	
	ПодписаниеРазрешено = Не ПодписаниеРазрешено;
...
&НаКлиенте
Процедура ИспользуюТолькоЯ(Команда)
	
	СписокПользователей.Очистить();
	СписокПользователей.Добавить(ПользователиКлиент.ТекущийПользователь());
...
Процедура ПриСозданииНаСервереПриЧтенииНаСервере()
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СертификатыКлючейЭлектроннойПодписиИШифрования");
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		Иначе
			...
			Если НЕ ЭтоАвтор Тогда
				...
				Элементы.СписокПользователей.КнопкаОткрытия = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьОтображениеСпискаИспользуют()
	
	Массив = Объект.Пользователи.Выгрузить().ВыгрузитьКолонку("Пользователь");
	Если Массив.Количество() = 0 И ЗначениеЗаполнено(Объект.Пользователь) Тогда
		СписокПользователей.Очистить();
...
Процедура ОбновитьВидимостьПредупреждения(Знач СертификатКриптографии = Неопределено)
	...
	Если Объект.Отозван Тогда
		...
		КраткоеПредставление = НСтр("ru='Сертификат отозван'");
		Сообщение = НСтр("ru='Сертификат помечен в приложении как отозванный. Подписи, сделанные таким сертификатом верны, если содержат метку доверенного времени, добавленную ранее даты отзыва сертификата. Чтобы узнать причину и дату отзыва, обратитесь в удостоверяющий центр, выдавший сертификат.'");
		ПоказатьВажноеСообщение(КраткоеПредставление, Сообщение, Ложь);
	...
	ИначеЕсли Объект.ДействителенДо > ТекущаяДатаСеанса() Тогда
		...
		Если ЗначениеЗаполнено(АдресСертификата) Тогда
			...
			Если РезультатПроверкиУдостоверяющегоЦентраСертификата.Действует
				...
				Если ЗначениеЗаполнено(ДанныеПредупреждения.ДополнительныеСведения) Тогда
					КраткоеПредставление = ДанныеПредупреждения.ДополнительныеСведения;
					Сообщение = ДанныеПредупреждения.ДополнительныеСведения;
					ПоказатьВажноеСообщение(КраткоеПредставление, Сообщение, Ложь);
				Иначе
					ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СертификатыКлючейЭлектроннойПодписиИШифрования");
				КонецЕсли;
			...
			Иначе
				...
				КраткоеПредставление = ДанныеПредупреждения.ТекстОшибки;
				...
				Если ДанныеПредупреждения.РазрешитьПодписание
					...
					ПоказатьВажноеСообщение(КраткоеПредставление, Сообщение);
				Иначе
					ПоказатьВажноеСообщение(КраткоеПредставление, Сообщение, Ложь);
				КонецЕсли;
			КонецЕсли;
...
&НаСервере
Процедура ПоказатьВажноеСообщение(КраткоеПредставление, Сообщение, ОтображатьДействие = Истина)
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.Идентификатор = "СертификатыКлючейЭлектроннойПодписиИШифрования";
	...
	ПараметрыСообщения.Обработчик = "ОбработкаНавигационнойСсылкиВажногоСообщения";
	
	Если ОтображатьДействие Тогда
		ПараметрыСообщения.ПараметрыДействия = ОбщегоНазначения.ПараметрыДействияВажногоСообщения();
		ПараметрыСообщения.ПараметрыДействия.ИмяКоманды = "УстановитьЗначениеИспользованияПодписания";
		ПараметрыСообщения.ПараметрыДействия.Заголовок = ?(ПодписаниеРазрешено,
			НСтр("ru = 'Запретить подписание'"), НСтр("ru = 'Разрешить подписание'"));
		ПараметрыСообщения.ПараметрыДействия.Обработчик = "УстановитьЗначениеИспользованияПодписания";
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаНавигационнойСсылкиВажногоСообщения(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
