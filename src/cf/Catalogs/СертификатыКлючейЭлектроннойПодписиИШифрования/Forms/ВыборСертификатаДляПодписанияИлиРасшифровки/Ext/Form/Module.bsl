...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ru = 'Добавить'");
		
		Элементы.ПояснениеУсиленногоПароля.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Нажмите <b>Добавить</b>, чтобы перейти к вводу пароля.'"));
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостьКомандЗавершения();
	...
	Если Сертификаты.Количество() = 0
		...
		И Не ЗначениеЗаполнено(ОшибкаПолученияСертификатовНаСервере)
		И Не СписокУчетныхЗаписей.Количество() > 0 Тогда
...
&НаКлиенте
Процедура ТребуетсяАутентификацияОблачнойПодписиНажатие(ЭлементИлиКоманда,
	НавигационнаяСсылка = Неопределено, СтандартнаяОбработка = Неопределено)
...
Процедура Далее(Команда)
	...
	ТекущаяСтраницаВыборСертификата = Элементы.ГлавныеСтраницы.ТекущаяСтраница = Элементы.СтраницаВыборСертификата;
	
	Если РежимВыбораСсылки Тогда
		ВыбратьСсылку();
	ИначеЕсли ТекущаяСтраницаВыборСертификата Тогда
		ДействиеДалее();
	Иначе
		Выбрать();
	КонецЕсли;
...
Процедура ОбновитьСписокСертификатов(Оповещение = Неопределено)
	...
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.ПолучитьСвойстваСертификатовНаКлиенте(Новый ОписаниеОповещения(
			"ОбновитьСписокСертификатовПродолжение", ЭтотОбъект, Контекст), Истина, Элементы.СертификатыПоказатьВсе.Пометка);
	КонецЕсли;
...
Процедура ОбновитьСписокСертификатовНаСервере(Знач СвойстваСертификатовНаКлиенте)
	...
	ПоказыватьВсе = Элементы.СертификатыПоказатьВсе.Пометка = Истина;
	...
	ЭлектроннаяПодписьСлужебный.ОбновитьСписокСертификатов(Сертификаты, СвойстваСертификатовНаКлиенте,
		ДобавлениеВСписок, Истина, ОшибкаПолученияСертификатовНаСервере, Элементы.СертификатыПоказатьВсе.Пометка, ДополнительныеПараметры);
	...
	Если ЗначениеЗаполнено(ОшибкаПолученияСертификатовНаКлиенте) Тогда
	...
	Если ЗначениеЗаполнено(ОшибкаПолученияСертификатовНаСервере) Тогда
	...
	Если СписокУчетныхЗаписей.Количество() > 0 Тогда
		...
		ПараметрыСообщения.Идентификатор = "ТребуетсяАутентификацияОблачнойПодписи";
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Чтобы отобразить сертификаты DSS необходимо авторизоваться на сервере DSS</a>'"),
			"ТребуетсяАутентификацияОблачнойПодписиНажатие");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Необходима авторизация DSS'");
		ПараметрыСообщения.Обработчик = "ТребуетсяАутентификацияОблачнойПодписиНажатие";
...
&НаКлиенте
Процедура УстановитьВидимостьКомандЗавершения()
	...
	Иначе
		Если РежимВыбораСсылки Тогда
			Элементы.Далее.Заголовок = НСтр("ru = 'Выбрать ссылку'");
...
Процедура ДействиеДалее()
	...
	Оповещение = Новый ОписаниеОповещения("ДалееПослеПереходаКВыборуТекущегоСертификата", ЭтотОбъект);
	ПерейтиКВыборуТекущегоСертификата(Оповещение);
...
Процедура УдалитьВажныеСообщенияПриСменеСтраницы();
	...
	Если Элементы.ГлавныеСтраницы.ТекущаяСтраница = Элементы.СтраницаВыборСертификата Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СертификатыНедоступныНаКлиенте");
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СертификатыНедоступныНаСервере");
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ТребуетсяАутентификацияОблачнойПодписи");
	КонецЕсли;
