...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьВидимостьВажныхСообщений();
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭлектроннаяПодписьСлужебныйКлиент.ИспользоватьСервисОблачнойПодписи() И Не Отказ Тогда
		ПодключитьОбработчикОжидания("ПодтверждениеПриОткрытии", 0.1, Истина);
	КонецЕсли;
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("ПодтверждениеВыполнитьОсновнуюОперацию") И Источник = УникальныйИдентификатор Тогда
		...
		Иначе
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
				Элементы.Подписать.Важность = ВажностьКнопки.Главная;
			Иначе
...
Процедура СертификатПриИзмененииЗавершение(ОтпечаткиСертификатовНаКлиенте, Контекст) Экспорт
	...
	Если ЭлектроннаяПодписьСлужебныйКлиент.ИспользоватьСервисОблачнойПодписи() Тогда
		...
		ПодключитьОбработчикОжидания("ПодтверждениеПриИзменении", 0.1, Истина);
	КонецЕсли;
...
Процедура СертификатОбработкаВыбораЗавершение(ОтпечаткиСертификатовНаКлиенте, ВыбранноеЗначение) Экспорт
	...
	Если ЭлектроннаяПодписьСлужебныйКлиент.ИспользоватьСервисОблачнойПодписи() Тогда
		ПодключитьОбработчикОжидания("ПодтверждениеОбработкаВыбора", 0.1, Истина);
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьВидимостьВажныхСообщений()
	...
	Если СертификатОтозван Тогда
		...
		ПараметрыСообщения.Идентификатор = "СертификатОтозван";
		ПараметрыСообщения.Сообщение = НСтр("ru = 'Сертификат помечен в приложении как отозванный. Выберите другой сертификат.'");
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Сертификат отозван'");
		ПараметрыСообщения.Скрываемый = Ложь;
		
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщенияСертификат);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "СертификатОтозван");
		Если ЗначениеЗаполнено(РезультатПроверкиУдостоверяющегоЦентра)
			И ЗначениеЗаполнено(РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ДополнительныеСведения) Тогда
			ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
			ПараметрыСообщения.ТипСообщения = "Предупреждение";
			ПараметрыСообщения.Идентификатор = "ОшибкаПроверкиСертификата";
			ПараметрыСообщения.Сообщение = РезультатПроверкиУдостоверяющегоЦентра.Предупреждение.ДополнительныеСведения;
			ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Ошибка проверки сертификата'");
			ПараметрыСообщения.Скрываемый = Ложь;
			
			ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения, Элементы.ГруппаВажныеСообщенияСертификат);
		Иначе
			ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ОшибкаПроверкиСертификата");
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПодтверждениеПриОткрытии()
	
	МодульСервисКриптографииDSSПодтверждениеКлиент =
		ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSПодтверждениеКлиент");
	МодульСервисКриптографииDSSПодтверждениеКлиент.ПодтверждениеПриОткрытии(
		ЭтотОбъект, ЗначениеЗаполнено(Пароль) И ЗапомнитьПароль, ОписаниеДанных);
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтверждениеПриИзменении()
	
	МодульСервисКриптографииDSSПодтверждениеКлиент =
		ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSПодтверждениеКлиент");
	МодульСервисКриптографииDSSПодтверждениеКлиент.ФильтроватьСписокСпособов(ЭтотОбъект);
	МодульСервисКриптографииDSSПодтверждениеКлиент.ПодтверждениеПриИзменении(
		ЭтотОбъект, Элементы.Сертификат, ОписаниеДанных, СвойстваПароля.Значение);
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтверждениеОбработкаВыбора()
