...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СформироватьПоляВводаНаРазныхЯзыках(Ложь, ТолькоПросмотр);
	...
	Если ТолькоПросмотр Тогда
		Элементы.ГруппаЯзыки.ТолькоПросмотр = Истина;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ПрименитьИЗакрыть", ЭтотОбъект);
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, Ложь,
		НСтр("ru = 'Применить изменения параметров прописи валюты?'"));
...
&НаКлиенте
Процедура ПрименитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
