...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РазделениеВключено Тогда
		...
		Элементы.ФормаВыполнитьСценарийРежимОтладки.Видимость = Ложь;
		
	ИначеЕсли ТекущийВариантИнтерфейса = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5 Тогда
	...
	ЗаполнитьСписокУзловИОграничитьТипы();
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСоставРасписания
...
Процедура ВыполнитьСценарийВФоне(Команда)
	...
	ВыполнитьСценарийВФонеНаСервере(РезультатЗапуска);
...
Функция НастроенныеТипыТранспорта(Знач УзелИнформационнойБазы)
	...
	ИсключаемыйТранспорт = Новый Массив;
	ИсключаемыйТранспорт.Добавить(Обработки.ТранспортСообщенийОбменаПассивныйРежим.ПараметрыТранспорта().ИдентификаторТранспорта);
	...
	Для Каждого Строка Из ТаблицаТранспорта Цикл
		...
		Если ИсключаемыйТранспорт.Найти(Строка.ИдентификаторТранспорта) <> Неопределено Тогда
...
&НаСервере
Процедура ЗаполнитьСписокУзловИОграничитьТипы()
	...
	Для Каждого СтрокаТаблицыМонитора Из ТаблицаМонитора Цикл
		
		ТипПланОбмена = Тип("ПланОбменаСсылка." + СтрокаТаблицыМонитора.ИмяПланаОбмена);
		СписокУзловОбмена.Добавить(ТипПланОбмена);
		
	КонецЦикла;
	
	Элементы.НастройкиОбменаУзелИнформационнойБазы.ОграничениеТипа = Новый ОписаниеТипов(СписокУзловОбмена.ВыгрузитьЗначения());
	
КонецПроцедуры

&НаСервере
Процедура ВыполнитьСценарийВФонеНаСервере(РезультатЗапуска)
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		РезультатЗапуска.СообщениеОбОшибке = НСтр("ru ='Для запуска сценария необходимо записать элемент'");
		Возврат;
	...
	Иначе
		...
		ЗапуститьСценарийСинхронизации(РезультатЗапуска);
...
&НаСервере
Процедура ЗапуститьСценарийСинхронизации(РезультатЗапуска)
	
	СценарийСсылка = Объект.Ссылка;
	...
	ИмяЗадания = РегЗаданиеСценария.Метаданные.Имя;
	
	ФоновоеЗадание = ФоновыеЗадания.Выполнить(Метаданные.РегламентныеЗадания[ИмяЗадания].ИмяМетода, РегЗаданиеСценария.Параметры, Строка(РегЗаданиеСценария.УникальныйИдентификатор), НаименованиеФоновогоЗадания);
