...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		КонецЕсли;
		ОтключитьДоступность = Пользователи.ЭтоПолноправныйПользователь(, Истина);
		ОтключитьЭлемент(Элементы.ПользовательИБАутентификацияOpenID, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБАутентификацияOpenIDConnect, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБАутентификацияТокеномДоступа, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБАутентификацияСтандартная, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПотребоватьСменуПароляПриВходе, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБЗапрещеноИзменятьПароль, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБЗапрещеноВосстанавливатьПароль, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.СвойстваАутентификацииОС, ОтключитьДоступность);
		ОтключитьЭлемент(Элементы.ПользовательИБРежимЗапуска, ОтключитьДоступность);
	КонецЕсли;
	...
	АутентификацияДляНовогоПользователя =
		ПользователиСлужебный.АутентификацияДляНовогоПользователя(РазделениеВключено);
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Параметры.ИдентификаторПользователяИБ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		ДополнительныеПараметры.ИмяЭлементаДляРазмещения = "СтраницаКонтактнаяИнформация";
		...
		Если ПользователиСлужебный.ДоступнаНастройкаВосстановленияПароля(УровеньДоступа) Тогда
			...
			ИмяРеквизитаСПочтойДляВосстановленияПароля =
				МодульУправлениеКонтактнойИнформацией.ОпределитьЭлементСПочтойДляВосстановленияПароля(ЭтотОбъект,
					ПользовательИБАдресЭлектроннойПочты,
	...
	Если ОбщегоНазначения.ЭтоАвтономноеРабочееМесто() Тогда
		...
		Элементы.СтраницаКонтактнаяИнформация.ТолькоПросмотр = Истина;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		Если ЗначениеЗаполнено(ИмяРеквизитаСПочтойДляВосстановленияПароля)
			...
			Если ПарольДляПодтвержденияИзмененияПочты = Неопределено Тогда
				...
				ОткрытьФорму("Справочник.Пользователи.Форма.ВводПароля",, ЭтотОбъект,,,, Оповещение,
...
Процедура НедействителенПриИзменении(Элемент)
	...
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНастройкиВхода;
	...
	Если Объект.Недействителен Тогда
		ВходВПрограммуРазрешен = Ложь;
		ОтключитьНачальнуюАутентификацию();
		
	ИначеЕсли ЗначенияПрямогоИзменения.ВходВПрограммуРазрешен Тогда
		ВключитьНачальнуюАутентификацию();
...
Процедура ПользовательИБАутентификацияOpenIDПриИзменении(Элемент)
	...
	АутентификацияПриИзменении();
	ЗначенияПрямогоИзменения.АутентификацияOpenID = ПользовательИБАутентификацияOpenID;
...
Процедура ПользовательИБАутентификацияOpenIDConnectПриИзменении(Элемент)
	...
	АутентификацияПриИзменении();
	ЗначенияПрямогоИзменения.АутентификацияOpenIDConnect = ПользовательИБАутентификацияOpenIDConnect;
...
Процедура ПользовательИБАутентификацияТокеномДоступаПриИзменении(Элемент)
	...
	АутентификацияПриИзменении();
	ЗначенияПрямогоИзменения.АутентификацияТокеномДоступа = ПользовательИБАутентификацияТокеномДоступа;
...
Процедура ПользовательИБАутентификацияОСПриИзменении(Элемент)
	...
	АутентификацияПриИзменении();
	ЗначенияПрямогоИзменения.АутентификацияОС = ПользовательИБАутентификацияОС;
...
&НаСервере
Процедура ОтключитьЭлемент(Элемент, ОтключитьДоступность)
	
	Если ОтключитьДоступность Тогда
...
Процедура НастроитьФорму(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.СтраницаКонтактнаяИнформация.Видимость = ЗначениеЗаполнено(ДействияВФорме.КонтактнаяИнформация);
	...
	Если ТекущийОбъект.Служебный Тогда
		...
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Используется для автоматизации некоторых действий в приложении. <a href=%1>Подробнее</a>'"),
			"ПоказатьПояснениеКогдаСлужебныйПользователь");
...
Процедура Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки(Элемент,
	...
	Если НавигационнаяСсылка = "ПоказатьПояснениеКогдаСлужебныйПользователь" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(,
			НСтр("ru = 'Служебный пользователь создается и изменяется автоматически, скрывается при выборе и подборе, а также в списке пользователей.'"));
		
	ИначеЕсли НавигационнаяСсылка = "ЗаполнитьСвойстваВФормеИзПользователяИБ" Тогда
	...
	ИначеЕсли НавигационнаяСсылка = "ПоказатьПояснениеКогдаЕстьНесоответствиеУчетнойЗаписи" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(,
			НСтр("ru = 'Некоторые свойства пользователя информационной базы (ИБ) сохраняются в карточке пользователя.
			           |Между значениями этих свойств найдены различия.
			           |Обычно это происходит при редактировании пользователя ИБ в списке пользователей в конфигураторе (не рекомендуется).'"));
		
	ИначеЕсли НавигационнаяСсылка = "ПоказатьПояснениеКогдаНекорректнаяСсылкаНаПользователяИБ" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(,
			НСтр("ru = 'Пользователь в справочнике имеет ссылку на пользователя ИБ (из списка пользователей в конфигураторе), однако пользователь ИБ не найден в этом списке по имеющейся ссылке (или ссылка некорректная или пользователь ИБ был удален).'"));
	КонецЕсли;
	...
	Если ИмяРеквизитаКИ = ИмяРеквизитаПервойКИ Тогда
		...
		Если ВидКИ = ВидКонтактнойИнформацииEmailПользователя Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Элементы.СтраницаГлавное.Заголовок,
				Элементы.СтраницаКонтактнаяИнформация.Заголовок));
		Иначе
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
&НаКлиенте
Процедура ВключитьНачальнуюАутентификацию()
	...
	Если Не ПользовательИБАутентификацияСтандартная
	   ...
	   И Не ПользовательИБАутентификацияОС Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, АутентификацияДляНовогоПользователя);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтключитьНачальнуюАутентификацию()
	
	Если (Не ПользовательИБАутентификацияСтандартная
	      Или Не ЗначенияПрямогоИзменения.АутентификацияСтандартная)
	   И (Не ПользовательИБАутентификацияOpenID
	      Или Не ЗначенияПрямогоИзменения.АутентификацияOpenID)
	   И (Не ПользовательИБАутентификацияOpenIDConnect
	      Или Не ЗначенияПрямогоИзменения.АутентификацияOpenIDConnect)
	   И (Не ПользовательИБАутентификацияТокеномДоступа
	      Или Не ЗначенияПрямогоИзменения.АутентификацияТокеномДоступа)
	   И (Не ПользовательИБАутентификацияОС
	      Или Не ЗначенияПрямогоИзменения.АутентификацияОС) Тогда
		
		ПользовательИБАутентификацияСтандартная    = Ложь;
		ПользовательИБАутентификацияOpenID         = Ложь;
		ПользовательИБАутентификацияOpenIDConnect  = Ложь;
		ПользовательИБАутентификацияТокеномДоступа = Ложь;
...
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь, ПриСозданииНаСервере = Истина)
	...
	ИначеЕсли Не ПользовательИБОсновной И ВходВПрограммуРазрешен Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, АутентификацияДляНовогоПользователя);
	КонецЕсли;
...
Процедура ЗаполнитьЗначенияПрямогоИзменения()
	...
	ЗначенияПрямогоИзменения.Вставить("АутентификацияOpenID",           ПользовательИБАутентификацияOpenID);
	ЗначенияПрямогоИзменения.Вставить("АутентификацияOpenIDConnect",    ПользовательИБАутентификацияOpenIDConnect);
	ЗначенияПрямогоИзменения.Вставить("АутентификацияТокеномДоступа",   ПользовательИБАутентификацияТокеномДоступа);
	ЗначенияПрямогоИзменения.Вставить("АутентификацияОС",               ПользовательИБАутентификацияОС);
	...
	Если Не ПользовательИБСуществует Тогда
		...
		ЗначенияПрямогоИзменения.АутентификацияOpenID         = Ложь;
		ЗначенияПрямогоИзменения.АутентификацияOpenIDConnect  = Ложь;
		ЗначенияПрямогоИзменения.АутентификацияТокеномДоступа = Ложь;
		ЗначенияПрямогоИзменения.АутентификацияОС             = Ложь;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ПользователиСлужебный.ЗаполнитьНесоответствиеПользователяСНесуществующимПользователемИБ(
		ЭтотОбъект, ПараметрыЗаписи);
	
	ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "НесоответствиеУчетнойЗаписи");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Возврат;
	ИначеЕсли Не УровеньДоступа.УправлениеСписком Тогда
		Возврат;
	КонецЕсли;
	
	// Проверка соответствия свойства ПолноеИмя пользователя ИБ и
	...
	Если ПользовательИБПолноеИмя <> Объект.Наименование Тогда
		...
		УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Полное имя: ""<b>%1</b>""'"), ПользовательИБПолноеИмя));
	КонецЕсли;
	...
	Если ПриЧтенииВходВПрограммуРазрешен И Объект.Недействителен Тогда
		...
		УточнениеСвойств.Вставить(0, НСтр("ru = 'Вход в приложение разрешен: <b>Да</b>'"));
	КонецЕсли;
	...
	Если УровеньДоступа.ИзменениеТекущего Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
			...
			Если СтрокаОписания = Неопределено Тогда
				...
				УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Почта для восстановления пароля: ""<b>%1</b>""'"),
					ПользовательИБАдресЭлектроннойПочты));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если УточнениеСвойств.Количество() = 0
	   И Не ОтличаютсяСохраненныеСвойстваПользователяИБ Тогда
		Возврат;
	КонецЕсли;
	
	// Отображение различий свойств.
	Если УточнениеСвойств.Количество() > 0 Тогда
		СтрокаУточненияСвойств = "- " + СтрСоединить(УточнениеСвойств, "," + Символы.ПС + "- ");
	Иначе
	...
	Если ПоказатьКомментарийУстраненияРазличий Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru = 'Нажмите <b>Записать</b>, чтобы устранить различия и не выводить это предупреждение.'");
	...
	Если ПоказатьКомандыУстраненияРазличий Тогда
		УстранениеРазличий = Символы.ПС
			+ НСтр("ru = 'Можно <a href=""ЗаполнитьСвойстваВФормеИзПользователяИБ"">заполнить</a> свойства в форме из пользователя информационной базы.'");
	Иначе
		УстранениеРазличий = "";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаУточненияСвойств) Тогда
		ПояснениеНесоответствия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Следующие свойства пользователя информационной базы <a href=%1>отличаются</a> от указанных в этой форме:
			           |%2.'"),
			"ПоказатьПояснениеКогдаЕстьНесоответствиеУчетнойЗаписи",
			СтрокаУточненияСвойств) + УстранениеРазличий + ?(Не ОтличаютсяСохраненныеСвойстваПользователяИБ, "", "
			|" + НСтр("ru = 'Картинка в списке пользователей может показывать устаревшее состояние, так как отличаются и некоторые другие сохраненные свойства.'"));
	Иначе
		ПояснениеНесоответствия = УстранениеРазличий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Картинка в списке пользователей может показывать устаревшее состояние, так как некоторые свойства пользователя информационной базы <a href=%1>отличаются</a> от сохраненных.'"),
			"ПоказатьПояснениеКогдаЕстьНесоответствиеУчетнойЗаписи");
	КонецЕсли;
	
	ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
	ПараметрыСообщения.Идентификатор = "НесоответствиеУчетнойЗаписи";
	ПараметрыСообщения.ТипСообщения = "Предупреждение";
	ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Несоответствие учетной записи'");
	ПараметрыСообщения.Заголовок = ПараметрыСообщения.КраткоеПредставление;
	ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(ПояснениеНесоответствия + Рекомендация);
	ПараметрыСообщения.Обработчик = "Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки";
	ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
	ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ПользовательИБЗапрещеноВосстанавливатьПароль.Доступность =
		...
		И Элементы.ПользовательИБАутентификацияСтандартная.Доступность;
