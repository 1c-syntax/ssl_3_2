...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АутентификацияДляНовогоПользователя =
		ПользователиСлужебный.АутентификацияДляНовогоПользователя();
	АутентификацияДляНовогоПользователя.Удалить("ПользовательИБАутентификацияОС");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если ЗначениеЗаполнено(Объект.ОбъектАвторизации)
		   И МодульУправлениеКонтактнойИнформацией.СодержитКонтактнуюИнформацию(Объект.ОбъектАвторизации) Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		Если МодульУправлениеКонтактнойИнформацией.СодержитКонтактнуюИнформацию(Объект.ОбъектАвторизации) Тогда
			...
			Если ТипЗнч(УровеньДоступа) = Тип("Структура")
			   И ПользователиСлужебный.ДоступнаНастройкаВосстановленияПароля(УровеньДоступа) Тогда
...
Процедура ОтключитьНачальнуюАутентификацию()
	...
	Если (Не ПользовательИБАутентификацияСтандартная
	   ...
	   И (Не ПользовательИБАутентификацияТокеномДоступа
	      Или Не ЗначенияПрямогоИзменения.АутентификацияТокеномДоступа) Тогда
...
Процедура Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки(Элемент,
	...
	Если НавигационнаяСсылка = "ПоказатьПояснениеКогдаЕстьНесоответствиеУчетнойЗаписи" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(,
			НСтр("ru = 'Некоторые свойства пользователя информационной базы (ИБ) сохраняются в карточке внешнего пользователя.
	...
	ИначеЕсли НавигационнаяСсылка = "ПоказатьПояснениеКогдаНекорректнаяСсылкаНаПользователяИБ" Тогда
		СтандартныеПодсистемыКлиент.ПоказатьПодробнуюИнформацию(,
			НСтр("ru = 'Внешний пользователь в справочнике имеет ссылку на пользователя ИБ (из списка пользователей в конфигураторе), однако пользователь ИБ не найден в этом списке по имеющейся ссылке (или ссылка некорректная или пользователь ИБ был удален).'"));
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	Если ПользовательИБАутентификацияОС <> Ложь Тогда
		УточнениеСвойств.Добавить(НСтр("ru = 'Аутентификация ОС: <b>включена</b>'"));
	КонецЕсли;
	...
	Если ПользовательИБПользовательОС <> "" Тогда
		УточнениеСвойств.Добавить(НСтр("ru = 'Пользователь ОС: <b>указан</b>'"));
	КонецЕсли;
	
	Если ПользовательИБПоказыватьВСпискеВыбора Тогда
		ЕстьРазличияУстранимыеБезАдминистратора = Истина;
		УточнениеСвойств.Добавить(НСтр("ru = 'Показывать в списке выбора: <b>включено</b>'"));
	КонецЕсли;
	
	Если ПользовательИБРежимЗапуска <> "Авто" Тогда
		ЕстьРазличияУстранимыеБезАдминистратора = Истина;
		УточнениеСвойств.Добавить(НСтр("ru = 'Режим запуска: <b>не Авто</b>'"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаУточненияСвойств) Тогда
		ПояснениеНесоответствия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|" + НСтр("ru = 'Картинка в списке внешних пользователей и объектов авторизации может показывать устаревшее состояние, так как отличаются и некоторые другие сохраненные свойства.'"));
	Иначе
		ПояснениеНесоответствия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Картинка в списке внешних пользователей и объектов авторизации может показывать устаревшее состояние, так как некоторые свойства пользователя информационной базы <a href=%1>отличаются</a> от сохраненных.'"),
			"ПоказатьПояснениеКогдаЕстьНесоответствиеУчетнойЗаписи");
	КонецЕсли;
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.СтраницаРоли.Доступность                   = Не Объект.Недействителен;
