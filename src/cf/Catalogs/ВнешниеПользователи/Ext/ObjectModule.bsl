...
Перем ЭтоНовый, СтарыйОбъектАвторизации, ГруппаНовогоВнешнегоПользователя;
...
Процедура ПередЗаписью(Отказ)
	...
	// АПК:75-выкл блокировка группы должна быть до начала обработки пользователя ИБ.
	Если Не ОбменДанными.Загрузка
	   И ДополнительныеСвойства.Свойство("ГруппаНовогоВнешнегоПользователя")
	   И ЗначениеЗаполнено(ДополнительныеСвойства.ГруппаНовогоВнешнегоПользователя) Тогда
		
		ГруппаНовогоВнешнегоПользователя = ДополнительныеСвойства.ГруппаНовогоВнешнегоПользователя;
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ГруппыВнешнихПользователей");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ГруппаНовогоВнешнегоПользователя);
		Блокировка.Заблокировать();
		// Завершение в обработчике события ПриЗаписи.
	Иначе
...
Процедура ПриЗаписи(Отказ)
	...
	Если ГруппаНовогоВнешнегоПользователя <> Неопределено Тогда
		// Завершение (начало в обработчике события ПередЗаписью).
		// АПК:1327-выкл Блокировка установлена в обработчике события ПередЗаписью.
		...
		ОбъектГруппы.ДополнительныеСвойства.Вставить("ИзмененияСоставов", ИзмененияСоставов);
	...
	Если ГруппаНовогоВнешнегоПользователя <> Неопределено Тогда
		ПользователиСлужебный.ПослеЗаписиГруппыПользователей(ОбъектГруппы, Ложь, ИзмененияСоставов);
	Иначе
