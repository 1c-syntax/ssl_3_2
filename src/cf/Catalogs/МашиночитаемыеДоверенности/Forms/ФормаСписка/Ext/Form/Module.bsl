...
Процедура ВыгрузитьВФайл(Команда)
	...
	МашиночитаемыеДоверенностиФНССлужебныйКлиент.ВыгрузитьДоверенностиВФайлы(
		Элементы.Список.ПолучитьСтрокиДляОбработки(), ПроверятьДоступностьПерсональныхДанных);
...
Процедура ОбновитьСтатусИзРеестраФНС(Команда)
	...
	Контекст = Новый Структура("ВыделенныеСтроки", Элементы.Список.ПолучитьСтрокиДляОбработки());
...
Процедура ПослеЗагрузкиДоверенности(Результат, Контекст) Экспорт
	...
	Элементы.Список.ПолучитьСтрокиДляОбработки().Очистить();

	Для Каждого Ссылка Из Результат.ЦепочкаДоверенностей Цикл
		Элементы.Список.ПолучитьСтрокиДляОбработки().Добавить(Ссылка);
	КонецЦикла;
...
Функция ПолучитьСтатусНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"МашиночитаемыеДоверенностиФНССлужебный.ОбновитьИПрочитатьСтатусДоверенностей", Элементы.Список.ВыделенныеСтроки);
...
Процедура ОткрытьСписокСобытий(Контекст) Экспорт
	ОткрытьФорму("РегистрСведений.МашиночитаемыеДоверенностиСтатусы.Форма.СписокРезультатов",
		Контекст,ЭтотОбъект,,,,,РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
КонецПроцедуры
...
Процедура НастроитьРаботуВТакси()
	...
	ГруппаКомандыЗагрузки = Элементы.Добавить("ГруппаЗагрузкаИзВешнихИсточников",
		Тип("ГруппаФормы"), КоманднаяПанель);
	ГруппаКомандыЗагрузки.Вид = ВидГруппыФормы.Подменю;
	ГруппаКомандыЗагрузки.Заголовок = НСтр("ru='Загрузить'");
	
	Элементы.Переместить(Элементы.СписокЗагрузитьИзФайла, ГруппаКомандыЗагрузки);
	Элементы.Переместить(Элементы.СписокЗагрузитьИзРеестраФНС, ГруппаКомандыЗагрузки);
