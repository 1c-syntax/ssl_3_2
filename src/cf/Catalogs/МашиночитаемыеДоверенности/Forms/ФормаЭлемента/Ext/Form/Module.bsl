...
Процедура ДекорацияНалоговыеОрганыДействияНажатие(Элемент)
	...
	ОткрытьФорму("Справочник.МашиночитаемыеДоверенности.Форма.ФормаВводаНалоговыхОрганов",
		ПараметрыФормы,
		ЭтотОбъект,,,,
		ОписаниеОповещения,
...
&НаКлиенте
Процедура ЛицоДоверителяТипСтрокаПриИзменении(Элемент)
	
	ВыбранныйЭлемент = Элемент.СписокВыбора.НайтиПоЗначению(ЛицоДоверителяТипСтрока);
	Контекст = Новый Структура;
	Контекст.Вставить("ИмяЭлементаОрганизация", "ЛицоДоверителя");
	Контекст.Вставить("ДобавитьВТаблицуПредставителя", Ложь);
	
	ВыбратьВидОрганизацииПослеВыбора(ВыбранныйЭлемент, Контекст);
	
КонецПроцедуры

&НаКлиенте
Процедура ДоверительТипСтрокаПриИзменении(Элемент)
	
	ВыбранныйЭлемент = Элемент.СписокВыбора.НайтиПоЗначению(ДоверительТипСтрока);
	Контекст = Новый Структура;
	Контекст.Вставить("ИмяЭлементаОрганизация", "Доверитель");
	Контекст.Вставить("ДобавитьВТаблицуПредставителя", Ложь);
...
&НаКлиенте
Процедура ПредставительТипСтрокаПриИзменении(Элемент)
	
	ВыбранныйЭлемент = Элемент.СписокВыбора.НайтиПоЗначению(ПредставительТипСтрока);
	Контекст = Новый Структура;
	Контекст.Вставить("ИмяЭлементаОрганизация", "Представитель");
	Контекст.Вставить("ДобавитьВТаблицуПредставителя", Ложь);
...
Процедура ДобавитьЛицаПредставителя(Команда)
	...
	ДобавитьЛицоПредставителя();
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьПредставителяТипОрганизация(Команда)
	
	ДобавитьПредставителяВТаблицу("Организация");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьПредставителяТипФизическоеЛицо(Команда)
	
	ДобавитьПредставителяВТаблицу("ФизическоеЛицо");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьПредставителяТипКонтрагент(Команда)
	
	ДобавитьПредставителяВТаблицу("Контрагент");
...
Процедура ПроверитьПодпись(Команда)
	...
	ВыделенныеСтроки = Элементы.ЭлектронныеПодписи.ПолучитьСтрокиДляОбработки();
	...
	РаботаСФайламиСлужебныйКлиент.ПроверитьПодписи(ЭтотОбъект,
		ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, ВыделенныеСтроки, ДанныеФайла);
...
// 
// Параметры:
//  ИмяЭлементаОрганизация - Строка - имя элемента организация
//  ИмяЭлементаДляДобавления - Неопределено, Строка - имя элемента для добавления
//
&НаКлиенте
Процедура Добавить(ИмяЭлементаОрганизация, ИмяЭлементаДляДобавления = Неопределено)
...
Процедура УстановитьВидимостьИДоступность(Форма, Условие = Неопределено)
	...
	Если Условие = Неопределено Или Условие = "Доверитель" Тогда
		...
		РазрешенВводДоверенностиОснования = Форма.Элементы.ГруппаЛицоДоверителя.Видимость
		...
		Форма.Элементы.ГруппаВозможностьПередоверия.ТолькоПросмотр = РеквизитыНедоступныДляРедактирования;
...
Процедура ВидимостьПриИзмененииКоличестваПодписей(Форма)
	...
	Если ЭтоВариантИнтерфейса85 Тогда
		Форма.Элементы.ФормаДобавитьПодпись.Важность = ВажностьКнопки.Обычная;
	Иначе
	...
	Если РеквизитыНедоступныДляРедактирования Тогда
		...
		Если Форма.Подписана Тогда
			...
			Если Форма.ЭлектронныеПодписи.Количество() = 0 Тогда
				...
				Если ЭтоВариантИнтерфейса85 Тогда
					ПереопределитьНазначениеКомандыДобавитьПодпись(Форма, НазначениеДействияКомандыФормы.Завершение);
					Форма.Элементы.ФормаДобавитьПодпись.Важность = ВажностьКнопки.Главная;
				Иначе
		...
		Иначе
			...
			Иначе
				...
				Форма.Элементы.ФормаОтправитьНаПодписание.Видимость = Не ПросроченаИлиОтменена И Форма.ИспользоватьСервисМобильнойПодписи;
				...
				Иначе
					Если ЭтоВариантИнтерфейса85 Тогда
						Форма.Элементы.ФормаПодписать.Важность = ?(ПросроченаИлиОтменена,
							ВажностьКнопки.Обычная, ВажностьКнопки.Главная);
					Иначе
	...
	Форма.Элементы.ГруппаИныеСведения.ТолькоПросмотр = РеквизитыНедоступныДляРедактирования;
	ЛицоДоверителяОформление(Форма);

	Форма.Элементы.ПредставительВыборТипа.Видимость = Не РеквизитыНедоступныДляРедактирования;
	
	Если РеквизитыНедоступныДляРедактирования Тогда
		Форма.Элементы.Представитель.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
		Если Форма.ПредставительРеквизиты = Неопределено Тогда
			Форма.Элементы.Представитель.Заголовок = НСтр("ru='Физическое лицо'");
		Иначе
			Форма.Элементы.Представитель.Заголовок = ?(Форма.ПредставительРеквизиты.ЭтоФизическоеЛицо, НСтр(
				"ru='Физическое лицо'"), НСтр("ru='Организация'"));
		КонецЕсли;
	КонецЕсли;
	
	Если Форма.Элементы.ФормаДобавитьПодпись.Важность = ВажностьКнопки.Обычная
		И Форма.Команды.Найти("ДобавитьПодпись").НазначениеДействия = НазначениеДействияКомандыФормы.Завершение Тогда
		Форма.Элементы.ФормаДобавитьПодпись.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
		ПереопределитьНазначениеКомандыДобавитьПодпись(Форма, НазначениеДействияКомандыФормы.Обычное);
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ПереопределитьНазначениеКомандыДобавитьПодпись(Форма, НовоеНазначениеДействия)
	
	Форма.Команды.Найти("ДобавитьПодпись").НазначениеДействия = НовоеНазначениеДействия;
...
#КонецЕсли
	...
	Форма.Элементы.РегистрироватьВРеестре.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(ЗаголовокРегистрироватьВРеестре, Форма.Объект.НомерДоверенности);
	...
	Если Не Форма.Просрочена И (Форма.Подписана Или Форма.ТехническийСтатус = ПредопределенноеЗначение("Перечисление.ТехническиеСтатусыМЧД.Подписана")) Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			Форма.Элементы.ФормаОтправить.Важность = ?(Форма.Элементы.ФормаОтправить.Видимость,
				ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
			Форма.Элементы.ФормаЗаписатьИЗакрыть.Важность = ?(Не Форма.Элементы.ФормаОтправить.Видимость И Форма.Элементы.ФормаДобавитьПодпись.КнопкаПоУмолчанию = Ложь,
				ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
		Иначе
	...
	Иначе
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			Форма.Элементы.ФормаЗаписатьИЗакрыть.Важность = ?(Не Форма.Элементы.ФормаПодписать.Видимость И Форма.Элементы.ФормаДобавитьПодпись.КнопкаПоУмолчанию = Ложь,
				ВажностьКнопки.Главная, ВажностьКнопки.Обычная);
		Иначе
			Форма.Элементы.ФормаЗаписатьИЗакрыть.КнопкаПоУмолчанию = Не Форма.Элементы.ФормаПодписать.Видимость И Форма.Элементы.ФормаДобавитьПодпись.КнопкаПоУмолчанию = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВидимостьПредставителей(Форма)
	...
	Если Форма.ПредставительНесколько Или Форма.ЛицоПредставителяНесколько Тогда
		Форма.Элементы.ТаблицаПредставительНесколькоКоманднаяПанель.Доступность = Не Форма.РеквизитыНедоступныДляРедактирования;
	...
	Иначе
		Форма.Элементы.ТаблицаПредставительНесколькоКоманднаяПанель.Доступность = Ложь;
...
// Выбрать вид организации после выбора.
// 
// Параметры:
//  ВыбранныйЭлемент - Структура, ЭлементСпискаЗначений, Неопределено:
//     * Представление - Строка
//     * Значение -  Строка
//  Контекст - Структура:
//     * ИмяЭлементаОрганизация - Строка 
//     * ДобавитьВТаблицуПредставителя - Булево 
// 
&НаКлиенте
...
Процедура СформироватьЗаголовокДекорации(ИмяЭлементаОрганизация)
	...
	Если Реквизиты = Неопределено Или Реквизиты.ЭтоФизическоеЛицо = Неопределено Тогда
		Элементы[ИмяЭлементаОрганизация + "РасширеннаяПодсказка"].Заголовок = "";
		Возврат;
	КонецЕсли;
	
	Элементы[ИмяЭлементаОрганизация + "РасширеннаяПодсказка"].Заголовок = ЗаголовокДекорации(Реквизиты, ИмяЭлементаОрганизация);
...
Процедура ЛицоДоверителяОформление(Форма)
	...
	Если Форма.ДоверительРеквизиты <> Неопределено И (Форма.ДоверительРеквизиты.ЭтоФизическоеЛицо
		...
		Форма.Элементы.ГруппаЛицоДоверителяРеквизиты.ОтображатьЗаголовок = Ложь;
		Форма.Элементы.ЛицоДоверителяВыборТипа.Видимость = Ложь;
	Иначе
		Форма.Элементы.ГруппаЛицоДоверителяРеквизиты.ОтображатьЗаголовок = Не ЗначениеЗаполнено(Форма.Объект.РодительскаяДоверенностьНомер);
	...
	Если ЗначениеЗаполнено(Объект.ФайлДоверенности) Тогда
		...
		Если ТипЗнч(Доверители) = Тип("Массив") И Доверители.Количество() > 0 Тогда
			...
			Если Не РеквизитыДоверителя.ЭтоФизическоеЛицо Тогда
				Элементы.ДоверительПервоначальнойДоверенностиНаименование.Заголовок = НСтр("ru = 'Первоначальный доверитель - организация'");
				ДоверительПервоначальнойДоверенностиНаименование = РеквизитыДоверителя.НаименованиеПолное;
			Иначе
				Элементы.ДоверительПервоначальнойДоверенностиНаименование.Заголовок = НСтр("ru = 'Первоначальный доверитель - физическое лицо'");
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьУсловноеОформлениеТаблицыПредставительНесколько(
		Новый ПолеКомпоновкиДанных("ТаблицаПредставительНесколькоНеЗаполненыРеквизиты"),
		Новый ПолеКомпоновкиДанных("ТаблицаПредставительНесколько.НеЗаполненыРеквизиты"));
	УстановитьУсловноеОформлениеТаблицыПредставительНесколько(
		Новый ПолеКомпоновкиДанных("ТаблицаПредставительНесколькоНеЗаполненыРеквизитыФЛ"),
		Новый ПолеКомпоновкиДанных("ТаблицаПредставительНесколько.НеЗаполненыРеквизитыФЛ"));
	
КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформлениеТаблицыПредставительНесколько(ПолеОформления, ЛевоеЗначениеОтбора)
	
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	
	ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле = ПолеОформления;
	
	ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = ЛевоеЗначениеОтбора;
...
Процедура ОткрытиеФормыВводаПолномочий()
	...
	Если СтандартнаяОбработкаВыбора = Истина Тогда
		...
		ОткрытьФорму("Справочник.МашиночитаемыеДоверенности.Форма.ФормаВводаПолномочий",
			ПараметрыФормы,
			ЭтотОбъект,,,,
			ОписаниеОповещения,
			РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
&НаКлиенте
Процедура ДобавитьПредставителяВТаблицу(ТипПредставителя)
	
	Если ПредставительНесколько Тогда
		ВыбранныйЭлемент = Новый Структура;
		ВыбранныйЭлемент.Вставить("Представление", ТипПредставителя);
		ВыбранныйЭлемент.Вставить("Значение", ТипПредставителя);
		Контекст = Новый Структура;
		Контекст.Вставить("ИмяЭлементаОрганизация", ТипПредставителя);
		Контекст.Вставить("ДобавитьВТаблицуПредставителя", Истина);
		ВыбратьВидОрганизацииПослеВыбора(ВыбранныйЭлемент, Контекст);
	Иначе
		ДобавитьЛицоПредставителя();
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьЛицоПредставителя()
	...
	МашиночитаемыеДоверенностиФНССлужебныйКлиент.ОткрытьФормуВводаРеквизитов(ЭтотОбъект,
		ПараметрыФормы, Новый ОписаниеОповещения("ДобавитьВТаблицуПредставителя", ЭтотОбъект,
		Новый Структура("ИмяЭлементаОрганизация, ИдентификаторРодителя, Идентификатор",
			"ЛицоПредставителя", ПредставительИдентификатор, Неопределено)));
...
Процедура ЗаполнитьСпискиВыбораТипов()
	...
	// Доверитель
	Элементы.ДоверительВыборТипа.СписокВыбора.Добавить("Организация", НСтр("ru = 'Организация'"));
	Элементы.ДоверительВыборТипа.СписокВыбора.Добавить("ФизическоеЛицо", НСтр("ru = 'Физическое лицо'"));
	
	// Лицо доверителя
	Элементы.ЛицоДоверителяВыборТипа.СписокВыбора.Добавить("Организация", НСтр("ru = 'Организация'"));
	Элементы.ЛицоДоверителяВыборТипа.СписокВыбора.Добавить("Контрагент",     НСтр("ru = 'Контрагент'"));
	Элементы.ЛицоДоверителяВыборТипа.СписокВыбора.Добавить("ФизическоеЛицо", НСтр("ru = 'Физическое лицо'"));
	
	// Представитель
	Элементы.ПредставительВыборТипа.СписокВыбора.Добавить("Организация", НСтр("ru = 'Организация'"));
	Элементы.ПредставительВыборТипа.СписокВыбора.Добавить("Контрагент",     НСтр("ru = 'Контрагент'"));
	Элементы.ПредставительВыборТипа.СписокВыбора.Добавить("ФизическоеЛицо", НСтр("ru = 'Физическое лицо'"));
