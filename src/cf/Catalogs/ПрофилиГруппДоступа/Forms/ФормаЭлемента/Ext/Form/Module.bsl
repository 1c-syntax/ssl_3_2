...
&НаКлиенте
Перем ПараметрыЗаписиПередЗаписьюПродолжение, ВидыДоступаТекущаяСтрокаПриАктивизации;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не УправлениеДоступом.ОграничиватьДоступНаУровнеЗаписей() Тогда
		Элементы.СтраницаОграниченияДоступа.Видимость = Ложь;
	КонецЕсли;
	...
	Элементы.ВидПользователей.ТолькоПросмотр = БезРедактированияПоставляемыхЗначений;
	...
	Элементы.ФормаЗаписать.Доступность = Элементы.ФормаЗаписатьИЗакрыть.Доступность;
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект,
		НСтр("ru = 'Выбор назначения профиля групп доступа'"),,, ОписаниеОповещения);
...
Процедура УстановитьУсловноеОформление()
	...
	// Оформление предустановленного вида доступа.
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ВидыДоступаПредустановленный.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВидыДоступа.Предустановленный");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Картинка",
		БиблиотекаКартинок.ВидДоступаПредустановленный);
	
	// Оформление обычного вида доступа.
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Картинка",
		БиблиотекаКартинок.ВидДоступаОбычный);
...
&НаКлиенте
Процедура ВосстановитьПоНачальномуЗаполнению()
	
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ВосстановитьПоНачальномуЗаполнениюПродолжение", ЭтотОбъект),
		НСтр("ru = 'Операция необратима. Восстановить профиль по содержимому начального заполнения?'"),
...
Процедура УстановитьДоступностьОписанияИВосстановленияПоставляемогоПрофиля(ТекущийОбъект = Неопределено)
	...
	Если Не ПоставляемыйПрофильИзменен Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПоставляемыйПрофильИзменен");
	Иначе
		ПараметрыСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыСообщения.Идентификатор = "ПоставляемыйПрофильИзменен";
		ПараметрыСообщения.ТипСообщения = "Предупреждение";
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Поставляемый профиль изменен'");
		...
		ПараметрыСообщения.Сообщение = СтроковыеФункции.ФорматированнаяСтрока(
			?(Пользователи.ЭтоПолноправныйПользователь(,, Ложь),
				НСтр("ru = 'Настройки профиля отличаются от поставляемых (содержание вкладки <b>Описание</b> может не соответствовать текущим настройкам).
				           |<a href=""%1"">Восстановить по начальному заполнению</a>'"),
				НСтр("ru = 'Настройки профиля отличаются от поставляемых (содержание вкладки <b>Описание</b> может не соответствовать текущим настройкам).'")),
...
Процедура Подключаемый_ВажноеСообщениеОбработкаНавигационнойСсылки(Элемент,
	...
	Если НавигационнаяСсылка = "ВосстановитьПоНачальномуЗаполнению" Тогда
		ВосстановитьПоНачальномуЗаполнению();
	КонецЕсли;
...
Функция ТекстВопросаОбновитьГруппыДоступаПрофиля()
	...
	Возврат
		НСтр("ru = 'Обновить группы доступа, использующие этот профиль?
		           |
		           |Будут удалены лишние виды доступа с заданными для них значениями доступа и добавлены недостающие виды доступа.'");
