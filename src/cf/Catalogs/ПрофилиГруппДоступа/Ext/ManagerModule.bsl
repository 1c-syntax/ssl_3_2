...
Процедура ОбновитьОписаниеПоставляемыхПрофилей(ЕстьИзменения = Неопределено) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы(
			...
			Кэш.ХешСумма, ЕстьТекущиеИзменения);
...
// Для форм списка справочников Профили групп доступа и Группы доступа.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//
Процедура ЗаменитьПредставлениеКомандыСоздатьГруппуНаСоздатьПапку(Форма) Экспорт
	
	Элементы = Форма.Элементы;
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		ЭлементСоздатьГруппу = Элементы.Найти("СписокСоздатьГруппу");
	Иначе
		ЭлементСоздатьГруппу = Элементы.Найти("ФормаСоздатьГруппу");
	КонецЕсли;
	
	Если ЭлементСоздатьГруппу <> Неопределено Тогда
		ЭлементСоздатьГруппу.Заголовок = НСтр("ru = 'Создать папку'");
		ЭлементСоздатьГруппу.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Создать новую папку'");
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		ЭлементСоздатьГруппу = Элементы.Найти("СписокКонтекстноеМенюСоздатьГруппу");
		Если ЭлементСоздатьГруппу <> Неопределено Тогда
			ЭлементСоздатьГруппу.Заголовок = НСтр("ru = 'Создать папку'");
			ЭлементСоздатьГруппу.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Создать новую папку'");
		КонецЕсли;
