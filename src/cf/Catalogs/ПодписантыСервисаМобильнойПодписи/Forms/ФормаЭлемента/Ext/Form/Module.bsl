...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если ЕстьОтправленныеДокументы Тогда
			...
			ПараметрыСообщения.Сообщение = НСтр("ru = 'Была отправка в сервис 1С:Мобильная подпись'");
			ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Была отправка в 1С:Мобильная подпись'");
			
			ОбщегоНазначения.ДобавитьВажноеСообщение(ЭтотОбъект, ПараметрыСообщения);
		КонецЕсли;
		
	Иначе
		Элементы.Добавил.Видимость = Ложь;
	КонецЕсли;
