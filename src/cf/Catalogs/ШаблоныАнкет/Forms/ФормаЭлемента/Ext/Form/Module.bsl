...
Процедура ФормаДереваАнкетыПриАктивизацииСтроки(Элемент)
	...
	ТекущиеДанные = Элементы.ФормаДереваАнкеты.ТекущиеДанные;
	ЗаголовокПоля = НСтр("ru = 'Заметки к выбранному вопросу'");
	Если ТекущиеДанные = Неопределено Тогда
		Элементы.ДеревоАнкетыЗаметки.Заголовок = ЗаголовокПоля;
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ТипВопроса) Тогда
		ЗаголовокПоля = НСтр("ru = 'Заметки к выбранному разделу'")
	КонецЕсли;
	Элементы.ДеревоАнкетыЗаметки.Заголовок = ЗаголовокПоля;
