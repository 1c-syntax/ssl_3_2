...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПолучитьФункциональнуюОпцию("ХранитьИсториюРассылкиОтчетов") И НЕ Объект.Личная И Объект.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если СостояниеРассылки.СОшибками Тогда
			...
			ГиперСсылкаПовторнаяРассылка = Новый ФорматированнаяСтрока(НСтр("ru = 'Повторить рассылку'"), , , ,"ПовторитьРассылку");
			ПараметрыВажногоСообщения.Сообщение = Новый ФорматированнаяСтрока(ТекстСообщения, " ", ГиперСсылкаПовторнаяРассылка);
			...
			ПараметрыВажногоСообщения.Обработчик = "Подключаемый_ПовторнаяРассылкаОбработкаНавигационнойСсылки";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ГруппаУстановитьСнятьПометкиЕслиПустой, Элементы.КоманднаяПанельФорматыОтчетов);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПаролиШифрованиеЗакрытие" Тогда
		Если Объект.УстановитьПаролиЗашифровать <>  Параметр.УстановитьПаролиЗашифровать Тогда
			Объект.УстановитьПаролиЗашифровать =  Параметр.УстановитьПаролиЗашифровать;
			Модифицированность = Истина;
			УстановитьЗаголовокПаролиШифрование(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПовторнаяРассылкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылкаФорматированнойСтроки <> "ПовторитьРассылку" Тогда
		Возврат;
	КонецЕсли;
...
Процедура МесяцыПриИзменении(Элемент)
	...
	МесяцыЧислами = Новый Массив;
	Для Каждого Месяц Из Месяцы Цикл
		МесяцыЧислами.Добавить(Кэш.Соответствия.Месяцы[Месяц.Значение]);
	КонецЦикла;
	Расписание.Месяцы = МесяцыЧислами;
...
Процедура ДниНеделиПриИзменении(Элемент)
	...
	ДниНеделиЧислами = Новый Массив;
	Для Каждого ДеньНедели Из ДниНедели Цикл
		ДниНеделиЧислами.Добавить(Кэш.Соответствия.ДниНедели[ДеньНедели.Значение]);
	КонецЦикла;
	Расписание.ДниНедели = ДниНеделиЧислами;
...
&НаКлиенте
Процедура ВыбратьВсеМесяцы(Команда)

	МесяцыЧислами = Новый Массив;
	Для Каждого Месяц Из Элементы.Месяцы.СписокВыбора Цикл
...
Процедура ПрикреплятьОтчетыВоВложенияПриИзменении(Элемент)
	...
	Если Объект.ПрикреплятьОтчетыВоВложения Тогда
		Объект.ТолькоУведомить = Ложь;
	Иначе
		Если Не Объект.ВставлятьОтчетыВТекстПисьма Тогда
			ПроверкаШифрованияПередВключениемОтчетовВТелеПисьма();
...
Процедура ОткрытьФормуПаролиШифрование()
	...
	ПараметрыФормы.Вставить("Ссылка", Объект.Ссылка);
	ПараметрыФормы.Вставить("НаименованиеРассылки", Объект.Наименование);
	ПараметрыФормы.Вставить("УстановитьПаролиЗашифровать", Объект.УстановитьПаролиЗашифровать);
...
Процедура УстановитьВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Иначе
			...
			Если Не РассылкаОбщая Тогда
				Элементы.ПолучателиРассылки.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
			Иначе
	...
	Если Изменения = "" Или Изменения = "ИспользоватьЭлектроннуюПочту" Или Изменения = "ВидРассылки" Тогда
		...
		Элементы.ВставлятьОтчетыВТекстПисьма.Доступность = Объект.ИспользоватьЭлектроннуюПочту;
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ИспользоватьЭлектроннуюПочту" Или Изменения = "ВидРассылки"
		...
		УстановитьЗаголовокПаролиШифрование(Форма);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Еженедельно" Тогда
			Форма.ВремяНачала = Форма.Расписание.ВремяНачала;
			Форма.ДниНедели.Очистить();
			Для Каждого КлючИЗначение Из Форма.Кэш.Соответствия.ДниНедели Цикл
				Если Форма.Расписание.ДниНедели.Найти(КлючИЗначение.Значение) <> Неопределено Тогда
					ДеньНедели = Форма.Элементы.ДниНедели.СписокВыбора.НайтиПоЗначению(КлючИЗначение.Ключ);
					Если ДеньНедели <> Неопределено Тогда
						Форма.ДниНедели.Добавить(ДеньНедели);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ru = 'с начала месяца'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ru = 'с конца месяца'");
			КонецЕсли;
			Форма.Месяцы.Очистить();

			Для Каждого КлючИЗначение Из Форма.Кэш.Соответствия.Месяцы Цикл
				Если Форма.Расписание.Месяцы.Найти(КлючИЗначение.Значение) <> Неопределено Тогда
					Месяц = Форма.Элементы.Месяцы.СписокВыбора.НайтиПоЗначению(КлючИЗначение.Ключ);
					Если Месяц <> Неопределено Тогда
						Форма.Месяцы.Добавить(Месяц);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодПовтораВТечениеДня" Или Изменения = "ПериодичностьРасписания" Тогда
		ИспользоватьПериодПовтораВТечениеДня = Число(Форма.ПериодПовтораВТечениеДня) <> 0;
		Элементы.ВремяОкончания.Доступность = ИспользоватьПериодПовтораВТечениеДня;
		Форма.ВремяОкончания = ?(ИспользоватьПериодПовтораВТечениеДня, Форма.ВремяОкончания, '00010101');
		Форма.Расписание.ВремяКонца = Форма.ВремяОкончания;
	КонецЕсли;
	
	Если Изменения = "" Или Изменения = "НачалоКонецМесяца" Тогда
		Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = ?(Форма.Расписание.ДеньВМесяце >= 0, НСтр("ru = 'с начала месяца'"), НСтр("ru = 'с конца месяца'"));
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьПаролейСертификатов(Форма)
	...
	Если Форма.ДоступноШифрованиеВложений Тогда
		Если Объект.Персонализирована Тогда
			...
			Элементы.ПарольАрхива.Видимость = Ложь;
			Элементы.ПаролиШифрование.Доступность = Истина;
		...
		Иначе
			...
			Элементы.ПарольАрхива.Видимость = Истина;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокПаролиШифрование(Форма)
	Если Форма.ДоступноШифрованиеВложений Тогда
		Если Форма.Объект.УстановитьПаролиЗашифровать Тогда
			Форма.Элементы.ПаролиШифрование.Заголовок = ?(Форма.Объект.Архивировать,
				НСтр("ru = 'Изменить пароли и шифрование'"), НСтр("ru = 'Изменить шифрование'"));
		Иначе
			Форма.Элементы.ПаролиШифрование.Заголовок = ?(Форма.Объект.Архивировать,
				НСтр("ru = 'Установить пароли и зашифровать'"), НСтр("ru = 'Зашифровать'"));
		КонецЕсли;
	Иначе
		Форма.Элементы.ПаролиШифрование.Заголовок = ?(Форма.Объект.УстановитьПаролиЗашифровать,
			НСтр("ru = 'Изменить пароли'"), НСтр("ru = 'Установить пароли'"));
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьКэш()
	...
	СписокДнейНедели.Вставить("Понедельник", 1);
	СписокДнейНедели.Вставить("Вторник",     2);
	СписокДнейНедели.Вставить("Среда",       3);
	СписокДнейНедели.Вставить("Четверг",     4);
	СписокДнейНедели.Вставить("Пятница",     5);
	СписокДнейНедели.Вставить("Суббота",     6);
	СписокДнейНедели.Вставить("Воскресенье", 7);
	СписокДнейНедели = Новый ФиксированноеСоответствие(СписокДнейНедели);
	
	СписокМесяцев = Новый Соответствие;
	СписокМесяцев.Вставить("Январь",   1);
	СписокМесяцев.Вставить("Февраль",  2);
	СписокМесяцев.Вставить("Март",     3);
	СписокМесяцев.Вставить("Апрель",   4);
	СписокМесяцев.Вставить("Май",      5);
	СписокМесяцев.Вставить("Июнь",     6);
	СписокМесяцев.Вставить("Июль",     7);
	СписокМесяцев.Вставить("Август",   8);
	СписокМесяцев.Вставить("Сентябрь", 9);
	СписокМесяцев.Вставить("Октябрь",  10);
	СписокМесяцев.Вставить("Ноябрь",   11);
	СписокМесяцев.Вставить("Декабрь",  12);
	СписокМесяцев = Новый ФиксированноеСоответствие(СписокМесяцев);
	...
	Кэш.Вставить("Соответствия", Новый ФиксированнаяСтруктура("ДниНедели, Месяцы", СписокДнейНедели, СписокМесяцев));
...
Процедура СоздатьРеквизитЭлементСертификатДляШифрования()
	...
	Элементы.Переместить(Группа, Элементы.НастройкиФайловОтчетов, Элементы.ТранслитерироватьИменаФайлов);
...
Процедура ЗаполнитьСпискиВыбора()
	...
	СписокДниНедели = Элементы.ДниНедели.СписокВыбора;
	СписокДниНедели.Очистить();
	СписокДниНедели.Добавить("Понедельник", НСтр("ru = 'Понедельник'"));
	СписокДниНедели.Добавить("Вторник",     НСтр("ru = 'Вторник'"));
	СписокДниНедели.Добавить("Среда",       НСтр("ru = 'Среда'"));
	СписокДниНедели.Добавить("Четверг",     НСтр("ru = 'Четверг'"));
	СписокДниНедели.Добавить("Пятница",     НСтр("ru = 'Пятница'"));
	СписокДниНедели.Добавить("Суббота",     НСтр("ru = 'Суббота'"));
	СписокДниНедели.Добавить("Воскресенье", НСтр("ru = 'Воскресенье'"));
	
	СписокМесяцы = Элементы.Месяцы.СписокВыбора;
	СписокМесяцы.Очистить();
	СписокМесяцы.Добавить("Январь",   НСтр("ru = 'Январь'"));
	СписокМесяцы.Добавить("Февраль",  НСтр("ru = 'Февраль'"));
	СписокМесяцы.Добавить("Март",     НСтр("ru = 'Март'"));
	СписокМесяцы.Добавить("Апрель",   НСтр("ru = 'Апрель'"));
	СписокМесяцы.Добавить("Май",      НСтр("ru = 'Май'"));
	СписокМесяцы.Добавить("Июнь",     НСтр("ru = 'Июнь'"));
	СписокМесяцы.Добавить("Июль",     НСтр("ru = 'Июль'"));
	СписокМесяцы.Добавить("Август",   НСтр("ru = 'Август'"));
	СписокМесяцы.Добавить("Сентябрь", НСтр("ru = 'Сентябрь'"));
	СписокМесяцы.Добавить("Октябрь",  НСтр("ru = 'Октябрь'"));
	СписокМесяцы.Добавить("Ноябрь",   НСтр("ru = 'Ноябрь'"));
	СписокМесяцы.Добавить("Декабрь",  НСтр("ru = 'Декабрь'")); 
