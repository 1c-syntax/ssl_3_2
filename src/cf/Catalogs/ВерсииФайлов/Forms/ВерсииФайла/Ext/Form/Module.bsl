...
Процедура Сравнить(Команда)
	...
	ЧислоВыделенныхСтрок = СтрокиДляОбработки.Количество();
	...
	Если ЧислоВыделенныхСтрок = 2 Тогда
		ПервыйФайл = СтрокиДляОбработки[0];
		ВторойФайл = СтрокиДляОбработки[1];
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ПервыйФайл);
		Расширение = НРег(ДанныеСтроки.Расширение);
	ИначеЕсли ЧислоВыделенныхСтрок = 1 Тогда
		ПервыйФайл = СтрокиДляОбработки[0];
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ПервыйФайл);
		ВторойФайл = ДанныеСтроки.РодительскаяВерсия;
		Расширение = НРег(ДанныеСтроки.Расширение);
	КонецЕсли;
...
Процедура ИзменитьДоступностьКоманд()
	...
	Если Элементы.Список.ТекущиеДанные = Неопределено
	 Или Не Элементы.Список.ТекущиеДанные.Свойство("Автор") Тогда
		Возврат;
	КонецЕсли;
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаВерсийФайлов)
	...
	ТекстЗапроса = 
		...
		|	ВЫРАЗИТЬ(ВерсииФайлов.Комментарий КАК СТРОКА(64)) КАК Комментарий,
