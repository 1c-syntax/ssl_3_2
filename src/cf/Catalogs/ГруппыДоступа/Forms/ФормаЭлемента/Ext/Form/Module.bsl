...
&НаКлиенте
Перем ВыборИПодборВнешнихПользователей, ВидыДоступаТекущаяСтрокаПриАктивизации;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Корректировка порядка команд в командной панели формы.
	...
	Если Элемент.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		Для Каждого ВыделеннаяСтрока Из Элемент.ВыделенныеСтроки Цикл
			...
			Если ДанныеРодителя <> Неопределено Тогда
				Идентификатор = ДанныеРодителя.ПолучитьИдентификатор();
				Если Элемент.ВыделенныеСтроки.Найти(Идентификатор) = Неопределено Тогда
					СтрокаРодителя = ДанныеРодителя;
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ВидыДоступаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ТолькоПросмотр
	   И Не Элементы.ВидыДоступа.ТолькоПросмотр Тогда
...
Процедура НачальнаяНастройкаПриЧтенииИСоздании(ТекущийОбъект)
	...
	Иначе
		...
		Элементы.Пользователи.Видимость               = Не ПользовательЗаполнен;
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			...
			Элементы.Профиль.ТолькоПросмотр = Истина;
			Элементы.ВидыДоступа.ТолькоПросмотр = Истина;
...
Процедура ОбновитьСоставВидовДоступа(Знач ПриЧтенииНаСервере = Ложь)
	...
	Элементы.СтраницаОграниченияДоступа.Доступность = Объект.ВидыДоступа.Количество() > 0;
...
Процедура ПерейтиКЗначениюДоступа()
	...
	Если ПерейтиКЗначениюДоступа <> Неопределено Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаОграниченияДоступа;
		ТекущийЭлемент = Элементы.СтраницаОграниченияДоступа;
