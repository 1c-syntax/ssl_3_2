...
// См. УправлениеДоступом.ИзменитьСоставАдминистраторов
//
// Параметры:
//  Пользователь - см. УправлениеДоступом.ИзменитьСоставАдминистраторов.Пользователь
//  Добавить     - см. УправлениеДоступом.ИзменитьСоставАдминистраторов.Добавить
//  Уточнение    - см. УправлениеДоступом.ИзменитьСоставАдминистраторов.Уточнение
//  ПриСозданииИлиАвторизацииАдминистратора - Булево - требуется запись в режиме
//    загрузки данных (без обработчиков событий).
//
Процедура ИзменитьСоставАдминистраторов(Пользователь, Добавить, Уточнение,
		ПриСозданииИлиАвторизацииАдминистратора = Ложь) Экспорт
	...
	ПользовательУжеДобавлен  = Не РезультатыЗапроса[0].Пустой();
	ЭтоСлужебныйПользователь = Не РезультатыЗапроса[1].Пустой();
	
	Если Добавить = ПользовательУжеДобавлен
	 Или ЭтоСлужебныйПользователь И Добавить Тогда
		Возврат;
	КонецЕсли;
	...
	Если Добавить Тогда
		КомментарийДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь ""%1""
			           |%2
			           |добавлен в группу доступа Администраторы по причине:
			           |%3'"), Пользователь, ПолучитьНавигационнуюСсылку(Пользователь), Уточнение);
	Иначе
		КомментарийДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь ""%1""
			           |%2
			           |удален из группы доступа Администраторы по причине:
			           |%3'"), Пользователь, ПолучитьНавигационнуюСсылку(Пользователь), Уточнение);
	КонецЕсли;
	...
	Попытка
		...
		СтрокаТЧ = Объект.Пользователи.Найти(Пользователь, "Пользователь");
		
		Если Добавить И СтрокаТЧ = Неопределено Тогда
			Объект.Пользователи.Добавить().Пользователь = Пользователь;
			
		ИначеЕсли Не Добавить И СтрокаТЧ <> Неопределено Тогда
			Объект.Пользователи.Удалить(СтрокаТЧ);
		КонецЕсли;
		
		Если Объект.Модифицированность() Тогда
			Если ПриСозданииИлиАвторизацииАдминистратора Тогда
