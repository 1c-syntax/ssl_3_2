...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ПараметрыСообщения.Сообщение  = НСтр("ru = 'Правила добавления новых и использования существующих дополнительных обработок устанавливает провайдер сервиса.
			|За консультацией обращайтесь в службу поддержки сервиса или в компанию, которая предоставила доступ.'");
		ПараметрыСообщения.ТипСообщения = "Информация";
		ПараметрыСообщения.КраткоеПредставление = НСтр("ru = 'Составом обработок управляет провайдер'");
		ПараметрыСообщения.Идентификатор = "ДополнительныеОтчетыИОбработкиПояснениеСервис";
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппе(Элементы.Список.КоманднаяПанель, "Создать:СписокСоздать");
	КонецЕсли;
...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	ИзменениеПубликации(ВариантПубликации, ВыделенныеСтроки);
...
&НаСервере
Процедура ИзменениеПубликации(ВариантПубликации, ВыделенныеСтроки)
