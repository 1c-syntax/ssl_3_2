...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Отображение
	Элементы.Отображение.СписокВыбора.Добавить("ИспользоватьДляФормыСписка", НСтр("ru = 'В списках'"));
	Элементы.Отображение.СписокВыбора.Добавить("ИспользоватьДляФормыОбъекта", НСтр("ru = 'В формах элементов справочников и документов'"));
	Элементы.Отображение.СписокВыбора.Добавить("Везде", НСтр("ru = 'В списках и формах элементов'"));
	
	Если Объект.ИспользоватьДляФормыСписка И Объект.ИспользоватьДляФормыОбъекта Тогда
		Отображение = "Везде";
	ИначеЕсли Объект.ИспользоватьДляФормыСписка Тогда
		Отображение = "ИспользоватьДляФормыСписка";
	ИначеЕсли Объект.ИспользоватьДляФормыОбъекта Тогда
		Отображение = "ИспользоватьДляФормыОбъекта";
	КонецЕсли;
...
&НаКлиенте
Процедура РазмещениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура РазмещениеУдалениеМножественныхЗначений(Элемент, Идентификаторы, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура РазместитьВРазделах(Команда)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.ВариантыДополнительногоОтчета.ПолучитьСтрокиДляОбработки() Цикл
...
&НаКлиенте
Процедура ОткрытьРедактированиеРазмещенияВРазделах()
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если КоличествоРазрешений = 0 Тогда
		...
		Если БезопасныйРежим Тогда
			Элементы.РежимРаботы.Заголовок = НСтр("ru = 'Подключается безопасно'");
			Элементы.РежимРаботы.ЦветТекста = ЦветаПалитры.Зеленый;
			Элементы.РежимРаботы.РасширеннаяПодсказка.Заголовок = 
				НСтр("ru = 'Этот отчет или обработка будет подключаться к приложению в безопасном режиме.
				|Подробно о режимах подключения дополнительных отчетов и обработок см. информацию в справке к форме.'");
		Иначе
			Элементы.РежимРаботы.Заголовок = НСтр("ru = 'Подключается не безопасно'");
			Элементы.РежимРаботы.ЦветТекста = ЦветаПалитры.Красный;
			Элементы.РежимРаботы.РасширеннаяПодсказка.Заголовок = 
				НСтр("ru = 'Этот отчет или обработка будет подключаться к приложению в небезопасном режиме.
				|Подробно о режимах подключения дополнительных отчетов и обработок к приложению см. информацию в справке к форме.'");
		КонецЕсли;
	...
	ЗаполнитьРазмещение(ЭтоГлобальнаяОбработка);
	...
	Элементы.Отображение.Видимость = Не ЭтоГлобальнаяОбработка И Не ЭтоПечатнаяФорма;
	Если Не Элементы.Отображение.Видимость Тогда
		Объект.ИспользоватьДляФормыОбъекта = Истина;
		Объект.ИспользоватьДляФормыСписка = Истина;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьРазмещение(Знач ЭтоГлобальнаяОбработка)
	Размещение.Очистить();
	Если ЭтоГлобальнаяОбработка Тогда
		Для Каждого СтрокаРаздел Из Объект.Разделы Цикл
			...
			Размещение.Добавить(СтрокаРаздел.Раздел, ПредставлениеРаздела);
		КонецЦикла;
	Иначе
		Для Каждого СтрокаНазначение Из Объект.Назначение Цикл
			...
			Размещение.Добавить(СтрокаНазначение.ОбъектНазначения, ПредставлениеОбъекта);
		КонецЦикла;
	КонецЕсли;
	Если Размещение.Количество() =0 Тогда
		ПустоеЗначение = НСтр("ru = 'Не определено'");
		Размещение.Добавить(ПустоеЗначение, ПустоеЗначение);
	КонецЕсли;
КонецПроцедуры
