...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		СтандартныеПодсистемыСервер.ПереместитьЭлементыВГруппу(Элементы.Шаблоны.КоманднаяПанель, КоманднаяПанель);
	КонецЕсли;
...
&НаКлиенте
Процедура ШаблоныПриНаведении(Элемент, Строка, Поле)
	
	ВидимостьКоманд = Ложь;
	Если ТипЗнч(Строка) = Тип("Число") Тогда
		ДанныеСтроки = Шаблоны.НайтиПоИдентификатору(Строка);
		ВидимостьКоманд = ЗначениеЗаполнено(ДанныеСтроки.Ссылка);
	КонецЕсли;
	Элементы.ШаблоныИзменитьШаблон.Видимость = ВидимостьКоманд;
	Элементы.ШаблоныСкопировать.Видимость    = ВидимостьКоманд;
...
Функция ПоместитьФайлыВАрхив(ПечатныеФормыДокументов, ПереданныеНастройки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		ЗаписьZipФайла = Новый ЗаписьФайлаАрхива(ИмяАрхива);
