...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.БлокировкаОкнаВладельца <> Неопределено Тогда
		ЭтотОбъект.РежимБлокировкиПриОткрытииОкна = Параметры.БлокировкаОкнаВладельца;
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Переместить(Элементы.ОтменитьПринятиеКИсполнению, КоманднаяПанель);
		Элементы.Переместить(Элементы.ПринятьКИсполнению, КоманднаяПанель);
		Элементы.Переместить(Элементы.ОткрытьБизнесПроцесс, КоманднаяПанель);
		Элементы.Переместить(Элементы.ОткрытьПредметЗадачи, КоманднаяПанель);
	КонецЕсли;
...
Процедура СписокПриНаведении(Элемент, Строка, Поле) 
	...
	ПереключитьПринятиеКИсполнению(ДанныеСтроки);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ВыделенныеСтроки.Количество() > 0 Тогда
		Элементы.ПринятьКИсполнению.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанелиИВДополнительномПодменю;
		Элементы.ОтменитьПринятиеКИсполнению.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанелиИВДополнительномПодменю;
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			ПереключитьПринятиеКИсполнению(Элемент.ТекущиеДанные);
		КонецЕсли;
	Иначе
		Элементы.ПринятьКИсполнению.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
		Элементы.ОтменитьПринятиеКИсполнению.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	КонецЕсли;
...
Процедура ПринятьКИсполнению(Команда)
	...
	БизнесПроцессыИЗадачиКлиент.ПринятьЗадачиКИсполнению(Элементы.Список.ПолучитьСтрокиДляОбработки());
...
Процедура ОтменитьПринятиеКИсполнению(Команда)
	...
	БизнесПроцессыИЗадачиКлиент.ОтменитьПринятиеЗадачКИсполнению(Элементы.Список.ПолучитьСтрокиДляОбработки());
...
&НаКлиенте
Процедура ПереключитьПринятиеКИсполнению(ДанныеСтроки)
	
	Если ДанныеСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеСтроки.Свойство("ПринятаКИсполнению") Тогда
		ПринятаКИсполнению = Не ДанныеСтроки.ПринятаКИсполнению;
	Иначе
...
Процедура УстановитьДоступностьПринятьКИсполнению(ЗначениеФлага)
	...
	Результат = БизнесПроцессыИЗадачиКлиент.ПризнакиПринятьКИсполнению(Элементы.Список, ЗначениеФлага);
	
	Элементы.ПринятьКИсполнению.Видимость                               = Результат.ПризнакПринятьКИсполнению;
	Элементы.СписокКонтекстноеМенюПринятьКИсполнению.Видимость          = Результат.ПризнакПринятьКИсполнению;
	Элементы.ОтменитьПринятиеКИсполнению.Видимость                      = Результат.ПризнакОтменитьПринятиеКИсполнению;
	Элементы.СписокКонтекстноеМенюОтменитьПринятиеКИсполнению.Видимость = Результат.ПризнакОтменитьПринятиеКИсполнению;
