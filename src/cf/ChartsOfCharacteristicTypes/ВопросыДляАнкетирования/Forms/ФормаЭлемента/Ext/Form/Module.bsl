...
Процедура ДлинаПриИзменении(Элемент)
	...
	УстановитьЗаголовокГруппыПараметровЧисла();
...
Процедура УправлениеВидимостью()
	...
	Если Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.ОдинВариантИз") Тогда
		Элементы.ГруппаВидПереключателяОтображать.Видимость     = Истина;
		Элементы.ГруппаВидОтображенияБулевоОтображать.Видимость = Ложь;
	ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Булево") Тогда
		Элементы.ГруппаВидПереключателяОтображать.Видимость     = Ложь;
		Элементы.ГруппаВидОтображенияБулевоОтображать.Видимость = Истина;
	Иначе
		Элементы.ГруппаВидПереключателяОтображать.Видимость     = Ложь;
		Элементы.ГруппаВидОтображенияБулевоОтображать.Видимость = Ложь;
	КонецЕсли;
...
Процедура ДоступностьТаблицыВариантыОтветов(Форма)
	...
	Иначе
		Форма.Элементы.ВариантыОтветовИнформация.Видимость   = Ложь;
...
&НаКлиенте
Процедура УстановитьЗаголовокГруппыПараметровЧисла()
	
	ЗаголовокМассив = Новый Массив;
	ЗаголовокМассив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Длина: %1'"), Объект.Длина));
	ЗаголовокМассив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Точность: %1'"), Объект.Точность));
	
	Если Объект.ИспользоватьМинимальноеЗначение Тогда
		ЗаголовокМассив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Минимальное значение: %1'"), Объект.МинимальноеЗначение));
	КонецЕсли;
	
	Если Объект.ИспользоватьМаксимальноеЗначение Тогда
		ЗаголовокМассив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Максимальное значение: %1'"), Объект.МаксимальноеЗначение));
	КонецЕсли;
	
	Если Объект.ПоказыватьПолосуРегулирования Тогда
		ЗаголовокМассив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Полоса регулирования с шагом: %1'"), Объект.ШагПолосыРегулирования));
	КонецЕсли;
	
	Элементы.РеквизитыЧисла.Заголовок = СтрСоединить(ЗаголовокМассив, ", ");
