...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимПомощника И Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		...
		Элементы.НеиспользуемыеРеквизиты.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	КонецЕсли;
	...
	Элементы.МногострочноеПолеВводаЧисло.Доступность = (ПредставлениеРеквизита <> "ОднострочноеПолеВвода");
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
		Элементы.Комментарий.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	КонецЕсли;
	
	ОбновитьОтображениеВажногоСообщения();
...
&НаКлиенте
Процедура ПолеПереключательПриИзменении(Элемент)
	Элементы.МногострочноеПолеВводаЧисло.Доступность = (ПредставлениеРеквизита <> "ОднострочноеПолеВвода");
	Элементы.ВыводитьВВидеГиперссылки.Доступность    = (ПредставлениеРеквизита = "ОднострочноеПолеВвода");
КонецПроцедуры
...
Процедура Изменить(Команда)
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		ОткрытьФорму("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ФормаОбъекта",
			ПараметрыФормы, Элементы.Свойства,,,,, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ВидСвойства = Перечисления.ВидыСвойств.ДополнительныеСведения Тогда
		Элементы.НеиспользуемыеРеквизиты.Заголовок = НСтр("ru = 'Неиспользуемые сведения'");
	...
	Иначе
		Элементы.НеиспользуемыеРеквизиты.Заголовок = НСтр("ru = 'Неиспользуемые реквизиты'");
	КонецЕсли;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
	...
	Если РежимПомощника Тогда
		Элементы.КомандыОбъекта.Видимость = Ложь;
		Если ОбщегоНазначенияКлиентСервер.ЭтоВариантИнтерфейса85() Тогда
			Элементы.КомандаДалее.Важность = ВажностьКнопки.Главная;
		Иначе
			Элементы.КомандаДалее.КнопкаПоУмолчанию = Истина;
		КонецЕсли;
		Элементы.КнопкиЗаписи.Видимость = Ложь;
	Иначе
		Элементы.ВыборРеквизита.Видимость = Ложь;
	...
	Если Объект.ЭтоДополнительноеСведение
		...
		Элементы.ГруппаТипЗначения.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Иначе
		Элементы.ГруппаТипЗначения.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	КонецЕсли;
	...
	Если Объект.ВидСвойства = Перечисления.ВидыСвойств.Метки Тогда
		...
		Элементы.МногострочноеПолеВводаЧисло.Видимость = Ложь;
	...
	Элементы.НеиспользуемыеРеквизиты.Видимость = РежимПомощника;
	Элементы.ЗаполнитьИдентификаторДляФормул.Доступность = НЕ Элементы.ИдентификаторДляФормул.ТолькоПросмотр;
...
Процедура ПереключитьНастройкиВыводаРеквизита(ТипЗначения)
	...
	Элементы.ПолеПереключатель.Видимость           = РазрешитьВыборМногострочногоПоля;
	Элементы.МногострочноеПолеВводаЧисло.Видимость = РазрешитьВыборМногострочногоПоля;
	Элементы.ВыводитьВВидеГиперссылки.Видимость    = РазрешитьВыводВВидеГиперссылки;
	
	Если Не РазрешитьВыборМногострочногоПоля Тогда
		Объект.ВыводитьВВидеГиперссылки = Ложь;
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Объект.ВидСвойства = Перечисления.ВидыСвойств.ДополнительныеСведения
			Или Объект.ЭтоДополнительноеСведение Тогда
			Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Сведение)'");
		ИначеЕсли Объект.ВидСвойства = Перечисления.ВидыСвойств.Метки Тогда
			Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Метка)'");
		Иначе
			Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Реквизит)'");
		КонецЕсли;
	Иначе
		Если Объект.ВидСвойства = Перечисления.ВидыСвойств.ДополнительныеСведения
			Или Объект.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru = 'Сведение (создание)'");
		ИначеЕсли Объект.ВидСвойства = Перечисления.ВидыСвойств.Метки Тогда
			Заголовок = НСтр("ru = 'Метка (создание)'");
		Иначе
			Заголовок = НСтр("ru = 'Реквизит (создание)'");
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьОтображениеВажногоСообщения()
	
	Если Объект.ТипЗначения.СодержитТип(Тип("Строка"))
		И Объект.ТипЗначения.Типы().Количество() = 1
		И Объект.ТипЗначения.КвалификаторыСтроки.Длина = 0
		И Объект.ВыводитьВВидеГиперссылки Тогда
		
		ПараметрыВажногоСообщения = ОбщегоНазначения.ПараметрыВажногоСообщения();
		ПараметрыВажногоСообщения.Идентификатор = "ПроблемыВыводаГиперссылки";
		ПараметрыВажногоСообщения.Сообщение = НСтр("ru = 'Не поддерживается вывод в виде гиперссылки для строк неограниченной длины, реквизит будет отображаться в виде обычного поля ввода.
			|Установите длину строки 1024 символа или меньше.'");
		ОбщегоНазначения.ВставитьВажноеСообщение(ЭтотОбъект, ПараметрыВажногоСообщения, Элементы.СтраницаОсновное, Элементы.ГруппаНаименованиеРеквизита);
		
		ОбщегоНазначения.ВывестиСтатусыИВажныеСообщенияНаФорму(ЭтотОбъект);
	Иначе
		ОбщегоНазначенияКлиентСервер.УдалитьСтатусИлиВажноеСообщение(ЭтотОбъект, "ПроблемыВыводаГиперссылки");
	КонецЕсли;
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ru = 'Установить формат поля'");
	Иначе
		ТекстЗаголовка = НСтр("ru = 'Изменить формат поля'");
	КонецЕсли;
